...
static vx_status VX_CALLBACK ownCopyImagePtrKernel(
  ...
  if (NULL != node && NULL != parameters &&
    ...
    for (p = 0; p < planes && status == VX_SUCCESS; p++) {
      ...
      status |= vxMapImagePatch(output, &rect, p, &map_id, &dst_addr, &dst,
      ...
      for (y = 0; y < height && status == VX_SUCCESS; y += dst_addr.step_y) {
        ...
        len = (dst_addr.stride_x * dst_addr.dim_x * dst_addr.scale_x) /
              VX_SCALE_UNITY;
 ...
 * \ingroup group_debug_ext
 */
static vx_param_description_t copy_image_kernel_params[] = {
    {VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownCopyImageKernel(vx_node node,
                                                const vx_reference parameters[],
                                                vx_uint32 num) {
  if (NULL != node && NULL != parameters &&
      num == dimof(copy_image_kernel_params)) {
...
own_copy_image_validator(vx_node node, const vx_reference parameters[],
                         vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(copy_image_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param)) {
      ...
      if (VX_SUCCESS == status) {
        ...
        if (VX_SUCCESS == status) {
          status |= vxSetMetaFormatAttribute(metas[1], VX_IMAGE_WIDTH, &width,
                                             sizeof(width));
          status |= vxSetMetaFormatAttribute(metas[1], VX_IMAGE_HEIGHT, &height,
                                             sizeof(height));
          status |= vxSetMetaFormatAttribute(metas[1], VX_IMAGE_FORMAT, &format,
                                             sizeof(format));
        } else
          status = VX_ERROR_INVALID_PARAMETERS;
      }

      if (NULL != input) vxReleaseImage(&input);
    }
 ...
 * \ingroup group_debug_ext
 */
static vx_param_description_t copy_array_kernel_params[] = {
    {VX_INPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownCopyArrayKernel(vx_node node,
                                                const vx_reference parameters[],
                                                vx_uint32 num) {
  if (NULL != node && NULL != parameters &&
      num == dimof(copy_array_kernel_params)) {
...
static vx_status VX_CALLBACK
own_copy_array_validator(vx_node node, const vx_reference parameters[],
                         vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  ...
  if (NULL != node && NULL != parameters &&
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param)) {
      ...
      if (VX_SUCCESS == status) {
        ...
        status |= vxQueryArray(input, VX_ARRAY_ITEMTYPE, &item_type,
                               sizeof(item_type));
        status |=
            vxQueryArray(input, VX_ARRAY_CAPACITY, &capacity, sizeof(capacity));

        if (VX_SUCCESS == status) {
          status |= vxSetMetaFormatAttribute(metas[1], VX_ARRAY_ITEMTYPE,
                                             &item_type, sizeof(item_type));
          status |= vxSetMetaFormatAttribute(metas[1], VX_ARRAY_CAPACITY,
                                             &capacity, sizeof(capacity));
        } else
          status = VX_ERROR_INVALID_PARAMETERS;
      }

      if (NULL != input) vxReleaseArray(&input);
    } else
      status = VX_ERROR_INVALID_PARAMETERS;
...
vx_kernel_description_t copy_image_ptr_kernel = {
    ...
    ownCopyImagePtrKernel,
    copy_image_ptr_kernel_params,
    dimof(copy_image_ptr_kernel_params),
    own_all_pass_validator,
    NULL,
    NULL,
    NULL,
...
vx_kernel_description_t copy_image_kernel = {
    ...
    ownCopyImageKernel,
    copy_image_kernel_params,
    dimof(copy_image_kernel_params),
    own_copy_image_validator,
    NULL,
    NULL,
    NULL,
...
vx_kernel_description_t copy_array_kernel = {
    ...
    ownCopyArrayKernel,
    copy_array_kernel_params,
    dimof(copy_array_kernel_params),
    own_copy_array_validator,
    NULL,
    NULL,
    NULL,
