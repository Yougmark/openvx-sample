  ...
  if (NULL != node && NULL != parameters &&
    ...
    if (VX_SUCCESS == status) {
      for (p = 0u; (p < planes); p++) {
        ...
        status |=
            vxMapImagePatch(image, &rect, p, &map_id, &addr, &ptr, VX_READ_ONLY,
        ...
        if ((status == VX_SUCCESS) && (ptr)) {
          ...
          if (errors > 0) {
            vxAddLogEntry(
                (vx_reference)node, VX_FAILURE,
                "Checked %p of %u sub-pixels with 0x%08x with %u errors\n", ptr,
                count, value.dword, errors);
          }
...
own_check_image_validator(vx_node node, const vx_reference parameters[],
                          vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(check_image_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param)) {
      ...
      if (NULL != scalar) vxReleaseScalar(&scalar);
    }
...
static vx_param_description_t check_array_kernel_params[] = {
    {VX_INPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
    {VX_INPUT, VX_TYPE_SCALAR, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_SCALAR, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownCheckArrayKernel(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(check_array_kernel_params)) {
    ...
    status |= vxMapArrayRange(arr, 0, num_items, &map_id, &stride, &ptr,
    ...
    if (status == VX_SUCCESS) {
      ...
      if (errors > 0) {
        vxAddLogEntry((vx_reference)node, VX_FAILURE,
                      "Check array %p of " VX_FMT_SIZE
                      " items with 0x%02x, found %u errors\n",
...
static vx_status VX_CALLBACK
own_check_array_validator(vx_node node, const vx_reference parameters[],
                          vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(check_array_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param1) &&
      ...
      if (NULL != arr) vxReleaseArray(&arr);
...
vx_kernel_description_t check_image_kernel = {
    ...
    ownCheckImageKernel,
    check_image_kernel_params,
    dimof(check_image_kernel_params),
    own_check_image_validator,
    NULL,
    NULL,
    NULL,
...
vx_kernel_description_t check_array_kernel = {
    ...
    ownCheckArrayKernel,
    check_array_kernel_params,
    dimof(check_array_kernel_params),
    own_check_array_validator,
    NULL,
    NULL,
    NULL,
