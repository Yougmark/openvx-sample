...
static vx_param_description_t fill_image_kernel_params[] = {
    {VX_INPUT, VX_TYPE_SCALAR, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownFillImageKernel(vx_node node,
                                                const vx_reference parameters[],
                                                vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(fill_image_kernel_params)) {
    vx_int32 i = 0;
    vx_uint32 x = 0u;
    vx_uint32 y = 0u;
    vx_uint32 p = 0u;
    ...
    for (p = 0u; p < planes; p++) {
      ...
      status |= vxMapImagePatch(image, &rect, p, &map_id, &addr, &ptr,
...
own_fill_image_validator(vx_node node, const vx_reference parameters[],
                         vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(fill_image_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param1) &&
      ...
      status |=
          vxQueryParameter(param2, VX_PARAMETER_REF, &image, sizeof(image));
...
vx_kernel_description_t fill_image_kernel = {
    ...
    ownFillImageKernel,
    fill_image_kernel_params,
    dimof(fill_image_kernel_params),
    own_fill_image_validator,
    NULL,
    NULL,
    NULL,
