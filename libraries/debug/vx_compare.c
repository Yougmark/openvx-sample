...
static vx_param_description_t compare_images_kernel_params[] = {
    {VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
    {VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_SCALAR, VX_PARAMETER_STATE_REQUIRED},
};

vx_status VX_CALLBACK ownCompareImagesKernel(vx_node node,
                                             const vx_reference parameters[],
                                             vx_uint32 num) {
  vx_status status = VX_FAILURE;

  if (NULL != node && NULL != parameters &&
      num == dimof(compare_images_kernel_params)) {
    ...
    if (VX_SUCCESS == status && (a_planes == b_planes) &&
        (a_format == b_format) && (a_width == b_width) &&
        (a_height == b_height)) {
      vx_int32 i;
      vx_uint32 x;
      vx_uint32 y;
      ...
      vx_imagepatch_addressing_t a_addrs[4] = {VX_IMAGEPATCH_ADDR_INIT,
                                               VX_IMAGEPATCH_ADDR_INIT,
                                               VX_IMAGEPATCH_ADDR_INIT};
      vx_imagepatch_addressing_t b_addrs[4] = {VX_IMAGEPATCH_ADDR_INIT,
                                               VX_IMAGEPATCH_ADDR_INIT,
                                               VX_IMAGEPATCH_ADDR_INIT};
      ...
      if (VX_SUCCESS == status &&
          vxFindOverlapRectangle(&rect_a, &rect_b, &rect) == vx_true_e) {
        ...
        for (p = 0; p < a_planes; p++) {
          status |= vxMapImagePatch(a, &rect, p, &a_map_id, &a_addrs[p],
                                    &a_base_ptrs[p], VX_READ_ONLY,
                                    VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
          status |= vxMapImagePatch(b, &rect, p, &b_map_id, &b_addrs[p],
                                    &b_base_ptrs[p], VX_READ_ONLY,
                                    VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
        }

        if (status != VX_SUCCESS) {
          vxAddLogEntry((vx_reference)node, VX_FAILURE,
                        "Failed to get patch on a and/or b\n");
        }

        for (p = 0; p < a_planes; p++) {
          for (y = 0; y < a_addrs[p].dim_y; y += a_addrs[p].step_y) {
            for (x = 0; x < a_addrs[p].dim_x; x += a_addrs[p].step_x) {
              uint8_t* a_ptr = vxFormatImagePatchAddress2d(a_base_ptrs[p], x, y,
                                                           &a_addrs[p]);
              uint8_t* b_ptr = vxFormatImagePatchAddress2d(b_base_ptrs[p], x, y,
                                                           &b_addrs[p]);
...
own_compare_images_validator(vx_node node, const vx_reference parameters[],
                             vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(compare_images_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param1) &&
      ...
      if (VX_SUCCESS == status) {
        ...
        status |=
            vxQueryImage(a, VX_IMAGE_HEIGHT, &height[0], sizeof(height[0]));
        status |=
            vxQueryImage(a, VX_IMAGE_FORMAT, &format[0], sizeof(format[0]));

        status |= vxQueryImage(b, VX_IMAGE_WIDTH, &width[1], sizeof(width[1]));
        status |=
            vxQueryImage(b, VX_IMAGE_HEIGHT, &height[1], sizeof(height[1]));
        status |=
            vxQueryImage(b, VX_IMAGE_FORMAT, &format[1], sizeof(format[1]));

        if (VX_SUCCESS == status && width[0] == width[1] &&
            height[0] == height[1] && format[0] == format[1]) {
          status = VX_SUCCESS;
        } else
        ...
        if (VX_SUCCESS == status) {
          ...
          status |= vxSetMetaFormatAttribute(metas[2], VX_SCALAR_TYPE,
                                             &dst_type, sizeof(dst_type));
        }
      }

      if (NULL != a) vxReleaseImage(&a);

      if (NULL != b) vxReleaseImage(&b);
    }
...
vx_kernel_description_t compare_images_kernel = {
    ...
    ownCompareImagesKernel,
    compare_images_kernel_params,
    dimof(compare_images_kernel_params),
    own_compare_images_validator,
    NULL,
    NULL,
    NULL,
