...
vx_node vxCopyImageNode(vx_graph graph, vx_image input, vx_image output) {
  ...
  return vxCreateNodeByStructure(graph, VX_KERNEL_DEBUG_COPY_IMAGE, params,
                                 dimof(params));
}
...
vx_node vxCopyArrayNode(vx_graph graph, vx_array input, vx_array output) {
  ...
  return vxCreateNodeByStructure(graph, VX_KERNEL_DEBUG_COPY_ARRAY, params,
                                 dimof(params));
}
...
vx_node vxFWriteImageNode(vx_graph graph, vx_image image,
  ...
  if (vxGetStatus((vx_reference)filepath) == VX_SUCCESS) {
    status =
        vxAddArrayItems(filepath, VX_MAX_FILE_NAME, &name[0], sizeof(name[0]));
    if (status == VX_SUCCESS) {
      vx_reference params[] = {(vx_reference)image, (vx_reference)filepath};

      node = vxCreateNodeByStructure(graph, VX_KERNEL_DEBUG_FWRITE_IMAGE,
                                     params, dimof(params));
      vxReleaseArray(&filepath); /* the graph should add a reference to this, so
                                    we don't need it. */
    }
  }
  return node;
...
vx_node vxFWriteArrayNode(vx_graph graph, vx_array arr,
                          vx_char name[VX_MAX_FILE_NAME]) {
  vx_status status = VX_SUCCESS;
  vx_node node = 0;
  vx_context context = vxGetContext((vx_reference)graph);
  ...
  if (vxGetStatus((vx_reference)filepath) == VX_SUCCESS) {
    ...
    if (status == VX_SUCCESS) {
      ...
      node = vxCreateNodeByStructure(graph, VX_KERNEL_DEBUG_FWRITE_ARRAY,
                                     params, dimof(params));
      vxReleaseArray(&filepath);  // the graph should add a reference to this,
                                  // so we don't need it.
    }
  }
  return node;
...
vx_node vxFReadImageNode(vx_graph graph, vx_char name[VX_MAX_FILE_NAME],
  ...
  if (vxGetStatus((vx_reference)filepath) == VX_SUCCESS) {
    ...
    if (status == VX_SUCCESS) {
      ...
      node = vxCreateNodeByStructure(graph, VX_KERNEL_DEBUG_FREAD_IMAGE, params,
                                     dimof(params));
      vxReleaseArray(&filepath);  // the graph should add a reference to this,
                                  // so we don't need it.
    }
...
vx_node vxFReadArrayNode(vx_graph graph, vx_char name[VX_MAX_FILE_NAME],
  ...
  if (vxGetStatus((vx_reference)filepath) == VX_SUCCESS) {
    ...
    if (status == VX_SUCCESS) {
      ...
      node = vxCreateNodeByStructure(graph, VX_KERNEL_DEBUG_FREAD_ARRAY, params,
                                     dimof(params));
      vxReleaseArray(&filepath);  // the graph should add a reference to this,
                                  // so we don't need it.
    }
...
vx_node vxFillImageNode(vx_graph graph, vx_uint32 value, vx_image output) {
  ...
  vx_node node = vxCreateNodeByStructure(graph, VX_KERNEL_FILL_IMAGE, params,
                                         dimof(params));
...
vx_node vxCheckImageNode(vx_graph graph, vx_image input, vx_uint32 value,
  ...
  vx_node node = vxCreateNodeByStructure(graph, VX_KERNEL_CHECK_IMAGE, params,
                                         dimof(params));
  vxReleaseScalar(&fill); /* the graph will keep a reference */
  return node;
}
...
vx_node vxCheckArrayNode(vx_graph graph, vx_array input, vx_uint8 value,
  ...
  vx_node node = vxCreateNodeByStructure(graph, VX_KERNEL_CHECK_ARRAY, params,
                                         dimof(params));
  vxReleaseScalar(&fill); /* the graph will keep a reference */
  return node;
}
...
vx_node vxCompareImagesNode(vx_graph graph, vx_image a, vx_image b,
  ...
  vx_node node = vxCreateNodeByStructure(graph, VX_KERNEL_COMPARE_IMAGE, params,
                                         dimof(params));
  return node;
}
...
vx_node vxCopyImageFromPtrNode(vx_graph graph, void *ptr, vx_image output) {
  ...
  vx_node node = vxCreateNodeByStructure(graph, VX_KERNEL_COPY_IMAGE_FROM_PTR,
                                         params, dimof(params));
...
vx_status vxuFWriteArray(vx_context context, vx_array arr,
                         vx_char name[VX_MAX_FILE_NAME]) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
