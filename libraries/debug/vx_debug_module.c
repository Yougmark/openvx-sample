...
static vx_kernel_description_t* kernels[] = {
    &fwrite_image_kernel,   &fread_image_kernel, &fwrite_array_kernel,
    &fread_array_kernel,    &check_image_kernel, &check_array_kernel,
    &copy_image_kernel,     &copy_array_kernel,  &fill_image_kernel,
...
/*VX_API_ENTRY*/ vx_status VX_API_CALL vxPublishKernels(vx_context context) {
  ...
  for (k = 0; k < num_kernels; k++) {
    vx_kernel kernel = vxAddUserKernel(
        context, kernels[k]->name, kernels[k]->enumeration,
        kernels[k]->function, kernels[k]->numParams, kernels[k]->validate,
        kernels[k]->initialize, kernels[k]->deinitialize);
