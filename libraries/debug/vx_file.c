...
own_fwrite_image_validator(vx_node node, const vx_reference parameters[],
                           vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(fwrite_image_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param1) &&
      ...
      status |=
          vxQueryParameter(param1, VX_PARAMETER_REF, &image, sizeof(image));
      status |=
          vxQueryParameter(param2, VX_PARAMETER_REF, &fname, sizeof(fname));
      ...
      if (VX_SUCCESS == status &&
        ...
        if (VX_SUCCESS == status &&
            ((VX_DF_IMAGE_U8 == format) || (VX_DF_IMAGE_S16 == format) ||
             (VX_DF_IMAGE_U16 == format) || (VX_DF_IMAGE_U32 == format) ||
             (VX_DF_IMAGE_UYVY == format) || (VX_DF_IMAGE_YUYV == format) ||
             (VX_DF_IMAGE_IYUV == format) || (VX_DF_IMAGE_RGB == format))) {
          status = VX_SUCCESS;
        } else
        ...
        if (VX_SUCCESS == status) {
          ...
          status |= vxMapArrayRange(
              fname, 0, VX_MAX_FILE_NAME, &map_id, &filename_stride,
              (void**)&filename, VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
      ...
      if (NULL != image) vxReleaseImage(&image);

      if (NULL != fname) vxReleaseArray(&fname);
    } else
      status = VX_ERROR_INVALID_PARAMETERS;
 ...
 * \ingroup group_debug_ext
 */
static vx_param_description_t fwrite_array_kernel_params[] = {
    {VX_INPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
    {VX_INPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownFWriteArrayKernel(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_FAILURE;

  if (NULL != node && NULL != parameters &&
      num == dimof(fwrite_array_kernel_params)) {
    ...
    status = vxMapArrayRange(file, 0, VX_MAX_FILE_NAME, &fname_map_id,
                             &filename_stride, (void**)&filename, VX_READ_ONLY,
                             VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
    if (status != VX_SUCCESS || filename_stride != sizeof(vx_char)) {
      vxUnmapArrayRange(file, fname_map_id);
      vxAddLogEntry((vx_reference)file, VX_FAILURE,
                    "Incorrect array " VX_FMT_REF "\n", file);
      return VX_FAILURE;
    }
    ...
    status |= vxMapArrayRange(arr, 0, num_items, &arr_map_id, &stride, &arrptr,
    ...
    if (status == VX_SUCCESS) {
      if (fwrite(&num_items, sizeof(num_items), 1, fp) == sizeof(num_items) &&
        ...
        for (i = 0; i < num_items; ++i) {
          if (fwrite(vxFormatArrayPointer(arrptr, i, stride), item_size, 1,
                     fp) != item_size) {
            status = VX_FAILURE;
            break;
          }
 ...
 * \ingroup group_debug_ext
 */
static vx_param_description_t fread_image_kernel_params[] = {
    {VX_INPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownFReadImageKernel(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_FAILURE;

  if (NULL != node && NULL != parameters &&
      num == dimof(fread_image_kernel_params)) {
...
static vx_status VX_CALLBACK
own_fread_image_validator(vx_node node, const vx_reference parameters[],
                          vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  ...
  if (NULL != node && NULL != parameters &&
    ...
    if (VX_SUCCESS == status) {
      ...
      status |=
          vxQueryParameter(param, VX_PARAMETER_REF, &fname, sizeof(fname));
      ...
      if (VX_SUCCESS == status &&
        ...
        if (VX_SUCCESS == status && sizeof(vx_char) == filename_stride) {
          if (strncmp(filename, "", VX_MAX_FILE_NAME) == 0) {
            ...
            if (VX_SUCCESS == status) {
              vxSetMetaFormatAttribute(metas[1], VX_IMAGE_WIDTH, &width,
                                       sizeof(width));
              vxSetMetaFormatAttribute(metas[1], VX_IMAGE_HEIGHT, &height,
                                       sizeof(height));
              vxSetMetaFormatAttribute(metas[1], VX_IMAGE_FORMAT, &format,
                                       sizeof(format));
            }
 ...
 * \ingroup group_debug_ext
 */
static vx_param_description_t fread_array_kernel_params[] = {
    {VX_INPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_ARRAY, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownFReadArrayKernel(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_FAILURE;

  if (NULL != node && NULL != parameters &&
      num == dimof(fread_array_kernel_params)) {
    vx_array file = (vx_array)parameters[0];
    ...
    status = vxMapArrayRange(file, 0, VX_MAX_FILE_NAME, &map_id,
                             &filename_stride, (void**)&filename, VX_READ_ONLY,
                             VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
    if (status != VX_SUCCESS || filename_stride != sizeof(vx_char)) {
    ...
    if (fread(&num_items, sizeof(num_items), 1, fp) == sizeof(num_items) &&
      ...
      vxQueryArray(arr, VX_ARRAY_ITEMSIZE, &arr_item_size,
                   sizeof(arr_item_size));

      if (arr_capacity >= num_items && arr_item_size == item_size) {
        void* tmpbuf = malloc(num_items * item_size);
        if (tmpbuf) {
          if (fread(tmpbuf, item_size, num_items, fp) ==
              (num_items * item_size)) {
            status = VX_SUCCESS;
        ...
        vxAddLogEntry((vx_reference)file, VX_FAILURE,
                      "Incorrect destination array " VX_FMT_REF "\n", arr);
        status = VX_FAILURE;
      }
    } else {
      status = VX_FAILURE;
...
static vx_status VX_CALLBACK
own_all_pass_validator(vx_node node, const vx_reference parameters[],
                       vx_uint32 num, vx_meta_format metas[]) {
  (void)node;
  (void)parameters;
  (void)num;
...
vx_kernel_description_t fwrite_image_kernel = {
    ...
    ownFWriteImageKernel,
    fwrite_image_kernel_params,
    dimof(fwrite_image_kernel_params),
    own_fwrite_image_validator,
    NULL,
    NULL,
    NULL,
...
vx_kernel_description_t fwrite_array_kernel = {
    ...
    ownFWriteArrayKernel,
    fwrite_array_kernel_params,
    dimof(fwrite_array_kernel_params),
    own_all_pass_validator,
    NULL,
    NULL,
    NULL,
...
vx_kernel_description_t fread_image_kernel = {
    ...
    ownFReadImageKernel,
    fread_image_kernel_params,
    dimof(fread_image_kernel_params),
    own_fread_image_validator,
    NULL,
    NULL,
    NULL,
...
vx_kernel_description_t fread_array_kernel = {
    ...
    ownFReadArrayKernel,
    fread_array_kernel_params,
    dimof(fread_array_kernel_params),
    own_all_pass_validator,
    NULL,
    NULL,
    NULL,
