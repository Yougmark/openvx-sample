...
static vx_status VX_CALLBACK vxWarpPerspectiveKernel(
...
static vx_status VX_CALLBACK vxWarpAffineKernel(vx_node node,
  ...
  if (index == 0) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (matrix) {
        ...
        if ((data_type == VX_TYPE_FLOAT32) && (columns == mat_columns) &&
            (rows == 3)) {
          status = VX_SUCCESS;
        }
...
static vx_status VX_CALLBACK vxWarpAffineInputValidator(vx_node node,
                                                        vx_uint32 index) {
  return vxWarpInputValidator(node, index, 2);
}

static vx_status VX_CALLBACK vxWarpPerspectiveInputValidator(vx_node node,
                                                             vx_uint32 index) {
  return vxWarpInputValidator(node, index, 3);
}

static vx_status VX_CALLBACK vxWarpOutputValidator(vx_node node,
                                                   vx_uint32 index,
