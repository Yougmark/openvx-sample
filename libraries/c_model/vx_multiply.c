...
static vx_status VX_CALLBACK vxMultiplyInputValidator(vx_node node,
                                                      vx_uint32 index) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (scalar) {
        ...
        if (type == VX_TYPE_FLOAT32) {
          ...
          if ((vxCopyScalar(scalar, &scale, VX_READ_ONLY,
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (scalar) {
        ...
        if (stype == VX_TYPE_ENUM) {
          ...
          vxCopyScalar(scalar, &rouding_policy, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST);
...
static vx_status VX_CALLBACK vxMultiplyOutputValidator(vx_node node,
                                                       vx_uint32 index,
  ...
  if (index == 5) {
    ...
    if ((vxGetStatus((vx_reference)param[0]) == VX_SUCCESS) &&
      ...
      vx_image images[3];
      vxQueryParameter(param[0], VX_PARAMETER_REF, &images[0],
                       sizeof(images[0]));
      vxQueryParameter(param[1], VX_PARAMETER_REF, &images[1],
                       sizeof(images[1]));
      vxQueryParameter(param[2], VX_PARAMETER_REF, &images[2],
                       sizeof(images[2]));
      if (images[0] && images[1] && images[2]) {
        ...
        vxQueryImage(images[0], VX_IMAGE_FORMAT, &informat[0],
                     sizeof(informat[0]));
        vxQueryImage(images[1], VX_IMAGE_FORMAT, &informat[1],
                     sizeof(informat[1]));
        vxQueryImage(images[2], VX_IMAGE_FORMAT, &outformat, sizeof(outformat));

        if (informat[0] == VX_DF_IMAGE_U8 && informat[1] == VX_DF_IMAGE_U8 &&
...
static vx_status VX_CALLBACK vxMultiplyKernel(vx_node node,
  ...
  if (num == dimof(multiply_kernel_params)) {
    ...
    return vxMultiply(in0, in1, scale_param, opolicy_param, rpolicy_param,
                      output);
  }
...
vx_kernel_description_t multiply_kernel = {
    ...
    vxMultiplyKernel,
    multiply_kernel_params,
    dimof(multiply_kernel_params),
    NULL,
