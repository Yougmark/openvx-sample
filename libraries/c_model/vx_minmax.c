...
static vx_status VX_CALLBACK vxMinMaxInputValidator(vx_node node,
                                                    vx_uint32 index) {
...
static vx_status VX_CALLBACK vxMinMaxOutputValidator(vx_node node,
                                                     vx_uint32 index,
    ...
    if ((vxGetStatus((vx_reference)param[0]) == VX_SUCCESS) &&
      ...
      if (images[0] && images[1] && images[2]) {
        ...
        if (informat[0] == VX_DF_IMAGE_U8 && informat[1] == VX_DF_IMAGE_U8 &&
          ...
          status = VX_SUCCESS;
        } else if (informat[0] == VX_DF_IMAGE_S16 &&
                   informat[1] == VX_DF_IMAGE_S16 &&
...
static vx_status VX_CALLBACK vxMinKernel(vx_node node,
...
vx_kernel_description_t min_kernel = {
    ...
    vxMinKernel,
    min_max_kernel_params,
    dimof(min_max_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK vxMaxKernel(vx_node node,
                                         const vx_reference parameters[],
                                         vx_uint32 num) {
  (void)node;
