...
static VX_INLINE int validFormat(vx_enum data_type, vx_uint8 fixed_point_pos) {
  return (data_type == VX_TYPE_INT16 &&
          fixed_point_pos == Q78_FIXED_POINT_POSITION) ||     // Q78
...
static VX_INLINE enum TensorCFmt getTensorCFmt(vx_tensor tensor) {
  if (tensor->data_type == VX_TYPE_INT16 &&
      tensor->fixed_point_position == Q78_FIXED_POINT_POSITION)
...
static VX_INLINE size_t calcStride(bool downscale_roundup, size_t input_sz,
                                   size_t pad_sz, size_t weight_sz,
                                   size_t dilation_sz, size_t output_sz) {
  if (input_sz + 2 * pad_sz >= weight_sz + (weight_sz - 1) * dilation_sz &&
      output_sz > 1) {
    const size_t a =
        input_sz + 2 * pad_sz - weight_sz - (weight_sz - 1) * dilation_sz;
...
static VX_INLINE vx_status scalarCheckGet(vx_scalar sc, vx_enum type,
                                          void *ptr) {
  assert(type > VX_TYPE_INVALID && type <= VX_TYPE_BOOL);
...
vx_status VX_CALLBACK nnConvolutionKernel(vx_node node,
  ...
  vx_scalar downscale_rounding_sc =
      (vx_scalar)parameters[CONV_PARAM_DOWNSCALE_ROUNDING];
  ...
  vx_size dilation_y;
  VX_CALL(
      vxCopyScalar(padding_x_sc, &pad_x, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
  VX_CALL(
      vxCopyScalar(padding_y_sc, &pad_y, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
  VX_CALL(vxCopyScalar(overflow_policy_sc, &overflow, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST));
  VX_CALL(vxCopyScalar(rounding_policy_sc, &rounding, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST));
  VX_CALL(vxCopyScalar(downscale_rounding_sc, &downscale_rounding, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST));
  VX_CALL(vxCopyScalar(dilation_x_sc, &dilation_x, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST));
  VX_CALL(vxCopyScalar(dilation_y_sc, &dilation_y, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST));

  const bool ceil_round = downscale_rounding == VX_NN_DS_SIZE_ROUNDING_CEILING;
  const size_t stride_x =
      calcStride(ceil_round, input->dimensions[0], pad_x,
                 weights->dimensions[0], dilation_x, output->dimensions[0]);
  const size_t stride_y =
      calcStride(ceil_round, input->dimensions[1], pad_y,
                 weights->dimensions[1], dilation_y, output->dimensions[1]);
  ...
  UNLESS(output_ptr) { return VX_ERROR_NO_MEMORY; }
#else
  void *output_ptr = output->addr;
#endif

  ConvolutionKernelImpl(fmt, input->addr, input_td, weights->addr, weight_td,
                        (biases ? biases->addr : NULL), bias_td, pad_x, pad_y,
                        stride_x, stride_y, overflow == VX_CONVERT_POLICY_WRAP,
                        rounding == VX_ROUND_POLICY_TO_NEAREST_EVEN, dilation_x,
                        dilation_y, output_ptr, output_td);
  ...
  status = vxCopyTensorPatch(output, output_td.dim_num, view_start,
                             output_td.dims, output_td.strides, output_ptr,
...
static vx_status VX_CALLBACK nnConvolutionInputValidator(vx_node node,
                                                         vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  vx_tensor data;
  vx_parameter param;
  ...
  switch (index) {
    case CONV_PARAM_TENSOR_IN:
      ...
      if (data) {
        vxQueryTensor(data, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims,
                      sizeof(num_of_dims));
        vxQueryTensor(data, VX_TENSOR_DATA_TYPE, &data_type, sizeof(data_type));
        vxQueryTensor(data, VX_TENSOR_DATA_TYPE, &data_format,
                      sizeof(data_format));
        vxQueryTensor(data, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos,
                      sizeof(fixed_point_pos));
    ...
    case CONV_PARAM_PAD_Y:
      param = vxGetParameterByIndex(node, index);
      status = vxQueryParameter(param, VX_PARAMETER_REF, &padding_scalar,
                                sizeof(padding_scalar));
    ...
    case CONV_PARAM_OVERFLOW:
      param = vxGetParameterByIndex(node, index);
      vxQueryParameter(param, VX_PARAMETER_REF, &overflow_policy_sc,
                       sizeof(overflow_policy_sc));
      if (overflow_policy_sc) {
        vxCopyScalar(overflow_policy_sc, &overflow_policy, VX_READ_ONLY,
                     VX_MEMORY_TYPE_HOST);
    ...
    case CONV_PARAM_ROUNDING:
      param = vxGetParameterByIndex(node, index);
      status = vxQueryParameter(param, VX_PARAMETER_REF, &rounding_policy_sc,
                                sizeof(rounding_policy_sc));
      if (rounding_policy_sc) {
        status |= vxCopyScalar(rounding_policy_sc, &rounding_policy,
    ...
    case CONV_PARAM_DOWNSCALE_ROUNDING:
      param = vxGetParameterByIndex(node, index);
      status = vxQueryParameter(param, VX_PARAMETER_REF,
                                &down_scale_size_rounding_sc,
                                sizeof(down_scale_size_rounding_sc));
      if (down_scale_size_rounding_sc) {
        status |=
            vxCopyScalar(down_scale_size_rounding_sc, &down_scale_size_rounding,
                         VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
...
static vx_status VX_CALLBACK nnConvolutionOutputValidator(vx_node node,
                                                          vx_uint32 index,
                                                          vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  ...
  vx_size dims_in[VX_MAX_TENSOR_DIMENSIONS],
      dims_weight[VX_MAX_TENSOR_DIMENSIONS],
      dims_out[VX_MAX_TENSOR_DIMENSIONS] = {0};
  vx_enum format_in;
  ...
  if (index == CONV_PARAM_TENSOR_OUT) {
    ...
    if (param_in && param_weights && param_pad_x && param_pad_y && param_out) {
      ...
      vxQueryParameter(param_weights, VX_PARAMETER_REF, &weight,
                       sizeof(weight));
      vxQueryParameter(param_pad_x, VX_PARAMETER_REF, &pad_x_s,
                       sizeof(pad_x_s));
      vxQueryParameter(param_pad_y, VX_PARAMETER_REF, &pad_y_s,
                       sizeof(pad_y_s));
      ...
      if (input && weight && output) {
        vxQueryTensor(input, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_in,
                      sizeof(num_of_dims_in));
        vxQueryTensor(input, VX_TENSOR_DIMS, dims_in,
                      num_of_dims_in * sizeof(vx_size));
        vxQueryTensor(input, VX_TENSOR_DATA_TYPE, &format_in,
                      sizeof(format_in));
        vxQueryTensor(input, VX_TENSOR_FIXED_POINT_POSITION,
                      &fixed_point_pos_in, sizeof(fixed_point_pos_in));
        vxQueryTensor(output, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_out,
                      sizeof(num_of_dims_out));
        vxQueryTensor(output, VX_TENSOR_DIMS, dims_out,
                      num_of_dims_out * sizeof(vx_size));
        vxQueryTensor(weight, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_weight,
                      sizeof(num_of_dims_weight));
        vxQueryTensor(weight, VX_TENSOR_DIMS, dims_weight,
                      num_of_dims_weight * sizeof(vx_size));
        ...
        vxSetMetaFormatAttribute(meta, VX_TENSOR_NUMBER_OF_DIMS,
                                 &num_of_dims_out, sizeof(num_of_dims_out));
        vxSetMetaFormatAttribute(meta, VX_TENSOR_DIMS, dims_out,
                                 num_of_dims_out * sizeof(vx_size));
        vxSetMetaFormatAttribute(meta, VX_TENSOR_DATA_TYPE, &format_in,
                                 sizeof(format_in));
        vxSetMetaFormatAttribute(meta, VX_TENSOR_FIXED_POINT_POSITION,
                                 &fixed_point_pos_in,
                                 sizeof(fixed_point_pos_in));
        status = VX_SUCCESS;
...
vx_kernel_description_t nn_convolution_kernel = {
    ...
    nnConvolutionKernel,
    nn_convolution_kernel_params,
    dimof(nn_convolution_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK nnPoolingKernel(vx_node node,
                                             const vx_reference *parameters,
                                             vx_uint32 num) {
  (void)node;
  ...
  VX_CALL(vxCopyScalar(pool_type_sc, &pool_type, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST));
  ...
  const vx_size stride_x = calcStride(use_ceil, inputs->dimensions[0], pad_x,
                                      size_x, 0, outputs->dimensions[0]);
  const vx_size stride_y = calcStride(use_ceil, inputs->dimensions[1], pad_y,
                                      size_y, 0, outputs->dimensions[1]);
  ...
  tensor_desc_t input_td = {inputs->number_of_dimensions, inputs->dimensions,
                            inputs->stride};
  tensor_desc_t output_td = {outputs->number_of_dimensions, outputs->dimensions,
                             outputs->stride};
  ...
  PoolingKernelImpl(fmt, inputs->addr, input_td, pool_type == VX_NN_POOLING_MAX,
                    size_x, size_y, pad_x, pad_y, stride_x, stride_y,
                    output_ptr, output_td);

#ifdef HACK_FOR_LACK_OF_INNER_NODE_OUTPUT_MEM_ALLOC
  const vx_size view_start[VX_MAX_TENSOR_DIMENSIONS] = {0};
  vx_status status = vxCopyTensorPatch(
      outputs, output_td.dim_num, view_start, output_td.dims, output_td.strides,
      output_ptr, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
  free(output_ptr);
  return status;
#else
  return VX_SUCCESS;
...
static vx_status VX_CALLBACK nnPoolingInputValidator(vx_node node,
                                                     vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  vx_parameter param;
  ...
  switch (index) {
    case POOL_PARAM_TENSOR_IN:
      ...
      vxQueryTensor(input, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_in,
                    sizeof(num_of_dims_in));
      vxQueryTensor(input, VX_TENSOR_DATA_TYPE, &data_format,
                    sizeof(data_format));
      vxQueryTensor(input, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos,
                    sizeof(fixed_point_pos));
      if ((num_of_dims_in == 3 || num_of_dims_in == 4) &&
          validFormat(data_format, fixed_point_pos)) {
        status = VX_SUCCESS;
      }
      vxReleaseTensor(&input);
    ...
    case POOL_PARAM_TYPE:
      param = vxGetParameterByIndex(node, index);
      vxQueryParameter(param, VX_PARAMETER_REF, &pool_type_sc,
                       sizeof(pool_type_sc));
    ...
    case POOL_PARAM_ROUNDING:
      param = vxGetParameterByIndex(node, index);
      vxQueryParameter(param, VX_PARAMETER_REF, &rounding_sc,
                       sizeof(rounding_sc));
      vxCopyScalar(rounding_sc, &rounding, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
      if (rounding == VX_NN_DS_SIZE_ROUNDING_FLOOR ||
          rounding == VX_NN_DS_SIZE_ROUNDING_CEILING) {
        status = VX_SUCCESS;
      }
...
static vx_status VX_CALLBACK nnPoolingOutputValidator(vx_node node,
                                                      vx_uint32 index,
                                                      vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  ...
  if (index == POOL_PARAM_TENSOR_OUT) {
    ...
    vx_parameter param_rounding =
        vxGetParameterByIndex(node, POOL_PARAM_ROUNDING);
    vx_parameter param_out = vxGetParameterByIndex(node, POOL_PARAM_TENSOR_OUT);

    if (param_in && param_size_x && param_size_y && param_pad_x &&
        param_pad_y && param_rounding && param_out) {
      vx_tensor input;
      vx_scalar type_sc;
      vx_scalar size_x_sc;
      vx_scalar size_y_sc;
      ...
      vxQueryParameter(param_size_x, VX_PARAMETER_REF, &size_x_sc,
                       sizeof(size_x_sc));
      vxQueryParameter(param_size_y, VX_PARAMETER_REF, &size_y_sc,
                       sizeof(size_y_sc));
      vxQueryParameter(param_pad_x, VX_PARAMETER_REF, &pad_x_sc,
                       sizeof(pad_x_sc));
      vxQueryParameter(param_pad_y, VX_PARAMETER_REF, &pad_y_sc,
                       sizeof(pad_y_sc));
      vxQueryParameter(param_rounding, VX_PARAMETER_REF, &rounding_sc,
                       sizeof(rounding_sc));
      vxQueryParameter(param_out, VX_PARAMETER_REF, &output, sizeof(output));
      ...
      if (input) {
        ...
        vxQueryTensor(input, VX_TENSOR_DIMS, dims_in,
                      num_of_dims_in * sizeof(vx_size));
        vxQueryTensor(input, VX_TENSOR_DATA_TYPE, &data_type,
                      sizeof(data_type));
        vxQueryTensor(input, VX_TENSOR_FIXED_POINT_POSITION, &fixed_pos,
                      sizeof(fixed_pos));
        vxQueryTensor(output, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_out,
                      sizeof(num_of_dims_out));
        //                vxCopyScalar(sc_pad_y, &pool_pad_y, VX_READ_ONLY,
        //                VX_MEMORY_TYPE_HOST);
        vxQueryTensor(output, VX_TENSOR_DIMS, dims_out,
                      num_of_dims_out * sizeof(vx_size));
        vx_size pool_stride_x =
            calcStride(rounding, dims_in[0], pad_x, size_x, 0, dims_out[0]);
        vx_size pool_stride_y =
            calcStride(rounding, dims_in[1], pad_y, size_y, 0, dims_out[1]);
        if (pool_stride_x && pool_stride_y) {
          num_of_dims_out = num_of_dims_in;
          if (num_of_dims_out > 3)
            dims_out[3] = num_of_dims_in > 3 ? dims_in[3] : 0;
          dims_out[2] = dims_in[2];
          //                dims_out[1] = (vx_size)((double)(dims_in[1] +
          //                2*pool_pad_y - pool_size_y) / pool_stride_y + 1);
          //                dims_out[0] = (vx_size)((double)(dims_in[0] +
          //                2*pool_pad_x - pool_size_x) / pool_stride_x + 1);
          vxSetMetaFormatAttribute(meta, VX_TENSOR_NUMBER_OF_DIMS,
                                   &num_of_dims_out, sizeof(num_of_dims_out));
          vxSetMetaFormatAttribute(meta, VX_TENSOR_DIMS, dims_out,
                                   num_of_dims_out * sizeof(vx_size));
          vxSetMetaFormatAttribute(meta, VX_TENSOR_DATA_TYPE, &data_type,
                                   sizeof(data_type));
          vxSetMetaFormatAttribute(meta, VX_TENSOR_FIXED_POINT_POSITION,
                                   &fixed_pos, sizeof(fixed_pos));
          status = VX_SUCCESS;
        }
...
vx_kernel_description_t nn_pooling_kernel = {
    ...
    nnPoolingKernel,
    nn_pooling_kernel_params,
    dimof(nn_pooling_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK nnFullyConnectedKernel(
  ...
  FullyConnectedKernelImpl(fmt, input->addr, input_td, weights->addr, weight_td,
...
static vx_status VX_CALLBACK nnFullyConnectedInputValidator(vx_node node,
                                                            vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  ...
  switch (index) {
    case FC_PARAM_TENSOR_IN:
      ...
      vxQueryTensor(in, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims,
                    sizeof(num_of_dims));
      vxQueryTensor(in, VX_TENSOR_DATA_TYPE, &data_type, sizeof(data_type));
      vxQueryTensor(in, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos,
                    sizeof(fixed_point_pos));
    ...
    case FC_PARAM_WEIGHTS:
      ...
      vxQueryTensor(wt, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims,
                    sizeof(num_of_dims));
      vxQueryTensor(in, VX_TENSOR_NUMBER_OF_DIMS, &in_num_of_dims,
                    sizeof(in_num_of_dims));
      vxQueryTensor(out, VX_TENSOR_NUMBER_OF_DIMS, &out_dim_num,
                    sizeof(out_dim_num));
      ...
      vxQueryTensor(wt, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos,
                    sizeof(fixed_point_pos));
      if (validFormat(data_type, fixed_point_pos)) {
        ...
        if (core_dim_num == 1) {
          if (num_of_dims == 2 && wt_dims[0] == in_dims[0] &&
              wt_dims[1] == out_dims[0]) {
            status = VX_SUCCESS;
          }
        } else if (core_dim_num == 3) {
          if (num_of_dims == 2) {
            if ((wt_dims[0] == in_dims[0] * in_dims[1] * in_dims[2]) &&
                wt_dims[1] == out_dims[0]) {
              status = VX_SUCCESS;
            }
    ...
    case FC_PARAM_ROUNDING:
      param = vxGetParameterByIndex(node, index);
      vxQueryParameter(param, VX_PARAMETER_REF, &rounding_policy_sc,
                       sizeof(rounding_policy_sc));
      vxCopyScalar(rounding_policy_sc, &rounding_policy, VX_READ_ONLY,
                   VX_MEMORY_TYPE_HOST);
      if (rounding_policy == VX_ROUND_POLICY_TO_ZERO ||
          rounding_policy == VX_ROUND_POLICY_TO_NEAREST_EVEN) {
        status = VX_SUCCESS;
...
static vx_status VX_CALLBACK nnFullyConnectedOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  vx_parameter param0, param1, param5;
  vx_tensor in, weight, out;
  vx_size num_of_dims_in, num_of_dims_wt, num_of_dims_out = 1;
  vx_size dims_in[VX_MAX_TENSOR_DIMENSIONS] = {0},
          dims_weight[VX_MAX_TENSOR_DIMENSIONS] = {0},
          dims_out[VX_MAX_TENSOR_DIMENSIONS] = {0};
  ...
  if (index == FC_PARAM_TENSOR_OUT) {
    ...
    if (param0 && param1 && param5) {
      ...
      if (in && weight && out) {
        vxQueryTensor(in, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_in,
                      sizeof(num_of_dims_in));
        vxQueryTensor(in, VX_TENSOR_DIMS, dims_in,
                      num_of_dims_in * sizeof(vx_size));
        vxQueryTensor(weight, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_wt,
                      sizeof(num_of_dims_wt));
        vxQueryTensor(weight, VX_TENSOR_DIMS, dims_weight,
                      num_of_dims_wt * sizeof(vx_size));
        vxQueryTensor(out, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims_out,
                      sizeof(num_of_dims_out));
        vxQueryTensor(out, VX_TENSOR_DIMS, dims_out,
                      num_of_dims_out * sizeof(vx_size));
        ...
        vxSetMetaFormatAttribute(meta, VX_TENSOR_DATA_TYPE, &data_type,
                                 sizeof(data_type));
        vxSetMetaFormatAttribute(meta, VX_TENSOR_FIXED_POINT_POSITION,
                                 &fixed_point_pos, sizeof(fixed_point_pos));
        if (validFormat(data_type, fixed_point_pos)) {
          status = VX_SUCCESS;
        }
      }
...
vx_kernel_description_t nn_fully_connected_kernel = {
    ...
    nn_fully_connected_kernel_params,
    dimof(nn_fully_connected_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK nnSoftmaxKernel(vx_node node,
                                             const vx_reference *parameters,
                                             vx_uint32 num) {
  (void)node;
  ...
  // TODO: do we need to use the extenral api for the tensors once to validate
  // them, here?

  tensor_desc_t input_td = {input->number_of_dimensions, input->dimensions,
                            input->stride};
  tensor_desc_t output_td = {output->number_of_dimensions, output->dimensions,
                             output->stride};
  ...
  SoftmaxKernelImpl(fmt, input->addr, input_td, output_ptr, output_td);

#ifdef HACK_FOR_LACK_OF_INNER_NODE_OUTPUT_MEM_ALLOC
  const vx_size view_start[VX_MAX_TENSOR_DIMENSIONS] = {0};
  vx_status status = vxCopyTensorPatch(
      output, output_td.dim_num, view_start, output_td.dims, output_td.strides,
      output_ptr, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
  free(output_ptr);
  return status;
#else
...
static vx_status VX_CALLBACK nnSoftmaxInputValidator(vx_node node,
                                                     vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  vx_parameter param;
  vx_enum data_type;
...
static vx_status VX_CALLBACK nnSoftmaxOutputValidator(vx_node node,
                                                      vx_uint32 index,
                                                      vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  vx_parameter param;
    ...
    if (param) {
      ...
      if (data) {
        ...
        vxQueryTensor(data, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos,
                      sizeof(fixed_point_pos));
        if (validFormat(data_type, fixed_point_pos)) {
          vxSetMetaFormatAttribute(meta, VX_TENSOR_NUMBER_OF_DIMS, &num_of_dims,
                                   sizeof(num_of_dims));
...
vx_kernel_description_t nn_softmax_kernel = {
    ...
    nnSoftmaxKernel,
    nn_softmax_kernel_params,
    dimof(nn_softmax_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK nnNormalizationKernel(
    vx_node node, const vx_reference *parameters, vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)node;
  ...
  if (num == NORM_PARAMS_NUMBER) {
    ...
    status = NNNormalizationKernelImpl(inputs, type, normalization_size, alpha,
                                       beta, outputs);
  }
  return status;
}

static vx_status VX_CALLBACK nnNormalizationInputValidator(vx_node node,
                                                           vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  vx_parameter param;
  ...
  switch (index) {
    ...
    case NORM_PARAM_TYPE:
      param = vxGetParameterByIndex(node, index);
      vxQueryParameter(param, VX_PARAMETER_REF, &norm_type_sc,
                       sizeof(norm_type_sc));
...
static vx_status VX_CALLBACK nnNormalizationOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  vx_parameter param;
  vx_enum data_type;
...
vx_kernel_description_t nn_norm_kernel = {
    ...
    nn_norm_kernel_params,
    dimof(nn_norm_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK nnActivationKernel(vx_node node,
                                                const vx_reference *parameters,
                                                vx_uint32 num) {
  (void)node;
  UNLESS(num == ACTIVATION_PARAMS_NUMBER) {
  ...
  ActivationKernelImpl(fmt, input->addr, in_td, func, a, b, output_ptr, out_td);

#ifdef HACK_FOR_LACK_OF_INNER_NODE_OUTPUT_MEM_ALLOC
  const vx_size view_start[VX_MAX_TENSOR_DIMENSIONS] = {0};
  vx_status status = vxCopyTensorPatch(output, out_td.dim_num, view_start,
                                       out_td.dims, out_td.strides, output_ptr,
                                       VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
  free(output_ptr);
  return status;
#else
...
nnActivationValidator(vx_node node, const vx_reference parameters[],
  ...
  UNLESS(func == VX_NN_ACTIVATION_LOGISTIC ||
         ...
         func == VX_NN_ACTIVATION_RELU || func == VX_NN_ACTIVATION_BRELU ||
         func == VX_NN_ACTIVATION_SOFTRELU || func == VX_NN_ACTIVATION_ABS ||
         func == VX_NN_ACTIVATION_SQUARE || func == VX_NN_ACTIVATION_SQRT ||
         func == VX_NN_ACTIVATION_LINEAR) {
    VX_PRINT(VX_ZONE_ERROR,
             "Activation layer func param must be LOGISTIC, HYPERBOLIC_TAN, "
             "RELU, BRELU, SOFTRELU, ABS, SQUARE, SQRT or, LINEAR");
    return VX_ERROR_INVALID_PARAMETERS;
  }
  ...
  UNLESS(func == VX_NN_ACTIVATION_LOGISTIC ||
    ...
    VX_PRINT(VX_ZONE_ERROR,
             "Activation layer only supports LOGISTIC, HYPERBOLIC_TAN and "
             "RELU, atm");
    return VX_ERROR_NOT_SUPPORTED;
  }

  // TODO: this is really awkward, the description vx_khr_nn.h says both must be
  // >=0, but why??
  UNLESS(a >= 0 && b >= 0) {
    VX_PRINT(VX_ZONE_ERROR,
             "Activation layer expects both a and b to be >= 0 (Even when not "
             "used, atm)");
    return VX_ERROR_INVALID_PARAMETERS;
  }
  ...
  VX_CALL(vxQueryTensor(input, VX_TENSOR_FIXED_POINT_POSITION, &i_fpp,
                        sizeof(i_fpp)));
  VX_CALL(vxQueryTensor(output, VX_TENSOR_FIXED_POINT_POSITION, &o_fpp,
                        sizeof(o_fpp)));
  ...
  UNLESS((o_dt == VX_TYPE_INT16 && o_fpp == Q78_FIXED_POINT_POSITION) ||
    ...
    VX_PRINT(VX_ZONE_ERROR,
             "Activation layer only supports Q78, U8 and S8 formats");
    return VX_ERROR_INVALID_FORMAT;
  }

  UNLESS(i_dt == o_dt) {
    VX_PRINT(VX_ZONE_ERROR,
             "Activation layer requires matching tensor data_types");
    return VX_ERROR_INVALID_FORMAT;
  }

  UNLESS(i_fpp == o_fpp) {
    VX_PRINT(VX_ZONE_ERROR,
             "Activation layer requires matching tensor fixed_point_position");
    return VX_ERROR_INVALID_FORMAT;
  }
  ...
  UNLESS(in_td.dim_num == out_td.dim_num && in_td.dim_num <= 4) {
    VX_PRINT(
        VX_ZONE_ERROR,
        "Activation layer requires (1 <= input.dim_num = output.dim_num <= 4)");
  ...
  UNLESS(in_td.dims[0] == out_td.dims[0] &&
    ...
    VX_PRINT(
        VX_ZONE_ERROR,
        "Activation layers input and output tensor shape must be identical");
    return VX_ERROR_INVALID_DIMENSION;
  }

  vx_meta_format *meta = &metas[ACTIVATION_PARAM_TENSOR_OUT];
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_NUMBER_OF_DIMS,
                                   &out_td.dim_num, sizeof(out_td.dim_num)));
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_DIMS, out_td.dims,
                                   sizeof(*out_td.dims) * out_td.dim_num));
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_DATA_TYPE, &o_dt,
                                   sizeof(o_dt)));
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_FIXED_POINT_POSITION,
                                   &o_fpp, sizeof(o_fpp)));
  return VX_SUCCESS;
}
...
vx_kernel_description_t nn_activation_kernel = {
    ...
    nnActivationKernel,
    nn_activation_kernel_params,
    dimof(nn_activation_kernel_params),
...
static vx_status VX_CALLBACK nnROIPoolingKernel(vx_node node,
                                                const vx_reference *parameters,
                                                vx_uint32 num) {
  (void)node;
  ...
  vx_uint8 in1_fpp;
  vx_uint8 out_fpp;
  VX_CALL(vxQueryTensor(in0, VX_TENSOR_FIXED_POINT_POSITION, &in0_fpp,
                        sizeof(in0_fpp)));
  VX_CALL(vxQueryTensor(in1, VX_TENSOR_FIXED_POINT_POSITION, &in1_fpp,
                        sizeof(in1_fpp)));
  VX_CALL(vxQueryTensor(out, VX_TENSOR_FIXED_POINT_POSITION, &out_fpp,
                        sizeof(out_fpp)));
  ...
  ROIPoolingKernelImpl(fmt, in0->addr, in0_td, in1->addr, in1_td, output_ptr,
                       out_td);

#ifdef HACK_FOR_LACK_OF_INNER_NODE_OUTPUT_MEM_ALLOC
  const vx_size view_start[VX_MAX_TENSOR_DIMENSIONS] = {0};
  vx_status status = vxCopyTensorPatch(out, out_td.dim_num, view_start,
                                       out_td.dims, out_td.strides, output_ptr,
                                       VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
  free(output_ptr);
...
static vx_status VX_CALLBACK
nnROIPoolingValidator(vx_node node, const vx_reference parameters[],
                      vx_uint32 num, vx_meta_format metas[]) {
  (void)node;
  UNLESS(num == ROIPOOLING_PARAMS_NUMBER) return VX_ERROR_INVALID_PARAMETERS;
...

  {
    vx_enum sc_type;
    VX_CALL(
        vxQueryScalar(pool_type_sc, VX_SCALAR_TYPE, &sc_type, sizeof(sc_type)));
    ...
    UNLESS(pool_type == VX_NN_POOLING_MAX) {
      VX_PRINT(
          VX_ZONE_ERROR,
          "ROIPooling layer only supports VX_NN_POOLING_MAX (%d) atm (got %d)",
  ...
  vx_enum in0_dt, in1_dt, out_dt;
  VX_CALL(
      vxQueryTensor(input_data, VX_TENSOR_DATA_TYPE, &in0_dt, sizeof(in0_dt)));
  VX_CALL(
      vxQueryTensor(input_rois, VX_TENSOR_DATA_TYPE, &in1_dt, sizeof(in1_dt)));
  VX_CALL(
      vxQueryTensor(output_arr, VX_TENSOR_DATA_TYPE, &out_dt, sizeof(out_dt)));

  vx_uint8 in0_fpp;
  vx_uint8 in1_fpp;
  vx_uint8 out_fpp;
  VX_CALL(vxQueryTensor(input_data, VX_TENSOR_FIXED_POINT_POSITION, &in0_fpp,
                        sizeof(in0_fpp)));
  VX_CALL(vxQueryTensor(input_rois, VX_TENSOR_FIXED_POINT_POSITION, &in1_fpp,
                        sizeof(in1_fpp)));
  VX_CALL(vxQueryTensor(output_arr, VX_TENSOR_FIXED_POINT_POSITION, &out_fpp,
                        sizeof(out_fpp)));
  ...
  UNLESS(in0_dt == in1_dt && in0_dt == out_dt) {
    VX_PRINT(
        VX_ZONE_ERROR, "ROIPooling layer requires matching tensor data_types"
        /*" (got { %d, %d, %d })", in0_dt, in1_dt, out_dt*/);
    return VX_ERROR_INVALID_FORMAT;
  }
  UNLESS(in0_fpp == in1_fpp && in0_fpp == out_fpp) {
    VX_PRINT(VX_ZONE_ERROR,
             "ROIPooling layer requires matching tensor fixed_point_position"
  ...
  // TODO: should we get these values through the external API for some reason
  // here?
  ...
  UNLESS(in0_td.dim_num == 3 || (in0_td.dims[3] == in1_td.dims[2] &&
                                 in0_td.dims[3] == out_td.dims[4])) {
  ...
  UNLESS(in1_td.dims[0] == 4) {
    VX_PRINT(VX_ZONE_ERROR,
             "ROIPooling layer requires input_rois.dims[0] == 4 for 2D rois"
  ...
  UNLESS(in1_td.dims[1] == out_td.dims[3]) {
    VX_PRINT(VX_ZONE_ERROR,
             "ROIPooling layer roi # mismatch between input_rois.dims[1] and "
             "output_arr.dims[3]"
  ...
  UNLESS(in0_td.dims[2] == out_td.dims[2]) {
    VX_PRINT(VX_ZONE_ERROR,
             "ROIPooling layer channel # mismatch between input_data.dims[2] "
             "and output_arr.dims[2]"
             /*" (got { %zu, %zu })", in0_td.dim[2], out_td.dims[2]*/);
    return VX_ERROR_INVALID_DIMENSION;
  }

  VX_CALL(vxSetMetaFormatAttribute(metas[3], VX_TENSOR_NUMBER_OF_DIMS,
                                   &out_td.dim_num, sizeof(out_td.dim_num)));
  VX_CALL(vxSetMetaFormatAttribute(metas[3], VX_TENSOR_DIMS, out_td.dims,
                                   sizeof(*out_td.dims) * out_td.dim_num));
  VX_CALL(vxSetMetaFormatAttribute(metas[3], VX_TENSOR_DATA_TYPE, &out_dt,
                                   sizeof(out_dt)));
  VX_CALL(vxSetMetaFormatAttribute(metas[3], VX_TENSOR_FIXED_POINT_POSITION,
                                   &out_fpp, sizeof(out_fpp)));
...
vx_kernel_description_t nn_roipooling_kernel = {
    ...
    nnROIPoolingKernel,
    nn_roipooling_kernel_params,
    dimof(nn_roipooling_kernel_params),
...
static vx_status VX_CALLBACK nnDeconvolutionKernel(
  ...
  VX_CALL(vxCopyScalar(pad_y_sc, &pad_y, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
  VX_CALL(
      vxCopyScalar(overflow_sc, &overflow, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
  VX_CALL(
      vxCopyScalar(rounding_sc, &rounding, VX_READ_ONLY, VX_MEMORY_TYPE_HOST));
  ...
  vx_size upscale_x, upscale_y;
  {
    ...
    upscale_x =
        (output->dimensions[0] + 2 * pad_x - weights->dimensions[0] - a_x) /
        (input->dimensions[0] - 1);
    upscale_y =
        (output->dimensions[1] + 2 * pad_y - weights->dimensions[1] - a_y) /
        (input->dimensions[1] - 1);

    assert(upscale_x > 0);
    assert(upscale_y > 0);
    assert(output->dimensions[0] == (input->dimensions[0] - 1) * upscale_x +
                                        a_x + weights->dimensions[0] -
                                        2 * pad_x);
    assert(output->dimensions[1] == (input->dimensions[1] - 1) * upscale_y +
                                        a_y + weights->dimensions[1] -
                                        2 * pad_y);
  }
  ...
  DeconvolutionKernelImpl(fmt, input->addr, in_td, weights->addr, weight_td,
                          (biases ? biases->addr : NULL), bias_td, pad_x, pad_y,
                          upscale_x, upscale_y,
                          overflow == VX_CONVERT_POLICY_WRAP,
                          rounding == VX_ROUND_POLICY_TO_NEAREST_EVEN, a_x, a_y,
                          output_ptr, out_td);
...
static vx_status VX_CALLBACK
nnDeconvolutionValidator(vx_node node, const vx_reference parameters[],
                         vx_uint32 num, vx_meta_format metas[]) {
  (void)node;
  UNLESS(num == DECONV_PARAMS_NUMBER) return VX_ERROR_INVALID_PARAMETERS;
  ...
  VX_CALL(vxQueryTensor(input, VX_TENSOR_FIXED_POINT_POSITION, &i_fpp,
                        sizeof(i_fpp)));
  VX_CALL(vxQueryTensor(weights, VX_TENSOR_FIXED_POINT_POSITION, &w_fpp,
                        sizeof(w_fpp)));
  if (biases) {
    VX_CALL(vxQueryTensor(biases, VX_TENSOR_FIXED_POINT_POSITION, &b_fpp,
                          sizeof(b_fpp)));
  }
  VX_CALL(vxQueryTensor(output, VX_TENSOR_FIXED_POINT_POSITION, &o_fpp,
                        sizeof(o_fpp)));
  ...
  UNLESS((o_dt == VX_TYPE_INT16 && o_fpp == Q78_FIXED_POINT_POSITION) ||
         ...
         (o_dt == VX_TYPE_UINT8 && !o_fpp)) {
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layer only supports Q78, U8 and S8 formats");
    return VX_ERROR_INVALID_FORMAT;
  }

  UNLESS(i_dt == w_dt && (!biases || i_dt == b_dt) && i_dt == o_dt) {
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layer requires matching tensor data_types");
    return VX_ERROR_INVALID_FORMAT;
  }
  ...
  UNLESS(i_fpp == w_fpp && (!biases || i_fpp == b_fpp) && i_fpp == o_fpp) {
    VX_PRINT(
        VX_ZONE_ERROR,
        "Deconvolution layer requires matching tensor fixed_point_position");
    return VX_ERROR_INVALID_FORMAT;
  }
  ...
  UNLESS(in_td.dim_num == out_td.dim_num &&
    ...
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layer requires 3 dimensionsal inputs and outputs "
             "with an additional optional batching dim");
    return VX_ERROR_INVALID_DIMENSION;
  }

  UNLESS(in_td.dim_num == 3 || (in_td.dims[3] == out_td.dims[3])) {
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layer requires the batching dim to match between "
  ...
  UNLESS(weight_td.dim_num == 4) {
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layer requires 4 dimensional weights");
    return VX_ERROR_INVALID_DIMENSION;
  }

  UNLESS(weight_td.dims[2] == in_td.dims[2] &&
         weight_td.dims[3] == out_td.dims[2]) {
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layers weights 3rd and 4th dim must match the IFM "
             "and OFM (3rd dim of input and output resp.)");
    return VX_ERROR_INVALID_DIMENSION;
  }

  // Note effectively our transposed conv padding is weight_sz - pad_sz - 1,
  // where pad_sz is the user provided "regular" conv padding.
  UNLESS(weight_td.dims[0] >= pad_x + 1 && weight_td.dims[1] >= pad_y + 1) {
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layer should satisfy (weight_sz >= pad_sz + 1)");
    return VX_ERROR_INVALID_DIMENSION;
  }
  ...
  UNLESS(bias_td.dim_num == 0 ||
         ...
         (bias_td.dim_num == 3 && bias_td.dims[0] == out_td.dims[0] &&
          bias_td.dims[1] == out_td.dims[1] &&
          bias_td.dims[2] == out_td.dims[2])) {
    VX_PRINT(VX_ZONE_ERROR,
             "Deconvolution layer supports no bias, [OFM] and [Out W, Out H, "
             "OFM] bias layouts");
    return VX_ERROR_INVALID_DIMENSION;
  }
  ...
  {
    // TODO: verify these! we could infact allow the first two coniditons and
    // use an upsacle of 1, just no time atm
    UNLESS(out_td.dims[0] + 2 * pad_x >= weight_td.dims[0] + a_x &&
           out_td.dims[1] + 2 * pad_y >= weight_td.dims[1] + a_y) {
      VX_PRINT(VX_ZONE_ERROR,
               "Deconvolution layer requires (out_sz + 2 * pad_sz >= weight_sz "
               "+ a) for upscale to be >= 1");
      return VX_ERROR_NOT_SUPPORTED;
    }
    ...
    vx_size upscale_x = (out_td.dims[0] + 2 * pad_x - weight_td.dims[0] - a_x) /
                        (in_td.dims[0] - 1);
    vx_size upscale_y = (out_td.dims[1] + 2 * pad_y - weight_td.dims[1] - a_y) /
                        (in_td.dims[1] - 1);

    UNLESS(out_td.dims[0] == (in_td.dims[0] - 1) * upscale_x + a_x +
                                 weight_td.dims[0] - 2 * pad_x &&
           out_td.dims[1] == (in_td.dims[1] - 1) * upscale_y + a_y +
                                 weight_td.dims[1] - 2 * pad_y) {
      VX_PRINT(
          VX_ZONE_ERROR,
          "Deconvolution layer requires (out_sz == (in_sz - 1) * upscale + a + "
          "weight_sz - 2 * pad_sz) to be satisfiable for an int upscale > 0");
      return VX_ERROR_INVALID_DIMENSION;
    }

    UNLESS(a_x >= 0 && a_x < upscale_x && a_y >= 0 && a_y < upscale_y) {
...
vx_kernel_description_t nn_deconvolution_kernel = {
    ...
    nn_deconvolution_kernel_params,
    dimof(nn_deconvolution_kernel_params),
