...
static vx_status VX_CALLBACK vxLBPKernel(vx_node node,
...
static vx_status VX_CALLBACK vxLBPInputValidator(vx_node node,
                                                 vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (scalar) {
        ...
        if (stype == VX_TYPE_ENUM) {
          ...
          if ((format == VX_LBP) || (format == VX_MLBP) ||
              (format == VX_ULBP)) {
            status = VX_SUCCESS;
          } else {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (value) {
        ...
        if (stype == VX_TYPE_INT8) {
          ...
          if ((format == VX_LBP || format == VX_ULBP) && (gs == 3 || gs == 5)) {
            status = VX_SUCCESS;
...
static vx_status VX_CALLBACK vxLBPOutputValidator(vx_node node, vx_uint32 index,
