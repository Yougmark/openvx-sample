 ...
 */
#define AREA_SCALE_ENABLE                                      \
  0 /* TODO enable this again after changing implementation in \
       kernels/c_model/c_scale.c */
...
static vx_status VX_CALLBACK vxScaleImageKernel(vx_node node,
...
static vx_status VX_CALLBACK vxScaleImageInputValidator(vx_node node,
                                                        vx_uint32 index) {
  ...
  if (index == 0) {
    ...
    if (input) {
      ...
      if (format == VX_DF_IMAGE_U8) {
        ...
        /* enable internal S16 format support (needed for laplacian pyramid
         * reconstruction) */
        vx_scalar scalar = 0;
...
static vx_status VX_CALLBACK vxScaleImageOutputValidator(
...
static vx_status VX_CALLBACK vxScaleImageInitializer(
  ...
  if (num == dimof(scale_kernel_params)) {
     ...
     * gcd_h); */
    if (gcd_w != 0 && gcd_h != 0) {
      size = (w1 / gcd_w) * (w2 / gcd_w) * (h1 / gcd_h) * (h2 / gcd_h) *
             sizeof(vx_float64);
    }
    ...
    size = 1;
#endif
    vxQueryKernel(node->kernel, VX_KERNEL_LOCAL_DATA_SIZE, &kernel_data_size,
                  sizeof(kernel_data_size));
...
vx_kernel_description_t scale_image_kernel = {
    VX_KERNEL_SCALE_IMAGE,      "org.khronos.openvx.scale_image",
    vxScaleImageKernel,         scale_kernel_params,
    dimof(scale_kernel_params), NULL,
    vxScaleImageInputValidator, vxScaleImageOutputValidator,
    vxScaleImageInitializer,    NULL,
};

/* half scale gaussian kernel */
static vx_status VX_CALLBACK vxHalfscaleGaussianKernel(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
...
vxHalfscaleGaussianInputValidator(vx_node node, vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
    vx_parameter param = vxGetParameterByIndex(node, index);
...
static vx_status VX_CALLBACK vxHalfscaleGaussianOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format_t *ptr) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 1) {
    vx_parameter src_param = vxGetParameterByIndex(node, 0);
...
static vx_status VX_CALLBACK vxHalfscaleGaussianInitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  ...
  if (num == dimof(scale_kernel_params)) {
    ...
    if (status == VX_SUCCESS) {
      ...
      status |= vxCopyScalar((vx_scalar)parameters[2], &kernel_size,
                             VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
      if (kernel_size == 1) {
        vx_node nodes[] = {
            vxScaleImageNode(subgraph, input, output,
        ...
        for (i = 0; i < dimof(nodes); i++) {
          status |= vxSetNodeAttribute(nodes[i], VX_NODE_BORDER, &borders,
                                       sizeof(borders));
        }

        status |= vxAddParameterToGraphByIndex(subgraph, nodes[0],
                                               0); /* input image */
        status |= vxAddParameterToGraphByIndex(subgraph, nodes[0],
                                               1); /* output image */
        ...
        if (kernel_size == 3 || convolution) {
          ...
          vx_node nodes[] = {
              kernel_size == 3
                  ? vxGaussian3x3Node(subgraph, input, virt)
                  : vxConvolveNode(subgraph, input, convolution, virt),
              vxScaleImageNode(subgraph, virt, output,
                               VX_INTERPOLATION_NEAREST_NEIGHBOR),
          };
          ...
          status |= vxAddParameterToGraphByIndex(subgraph, nodes[0],
                                                 0); /* input image */
          status |= vxAddParameterToGraphByIndex(subgraph, nodes[1],
                                                 1); /* output image */
          status |= vxAddParameterToGraphByIndex(
              subgraph, node, 2); /* gradient size - refer to self to quiet
                                     sub-graph validator */
          ...
          if (convolution) status |= vxReleaseConvolution(&convolution);
...
static vx_status VX_CALLBACK vxHalfscaleGaussianDeinitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;
...
vx_kernel_description_t halfscale_gaussian_kernel = {
    VX_KERNEL_HALFSCALE_GAUSSIAN,      "org.khronos.openvx.halfscale_gaussian",
    vxHalfscaleGaussianKernel,         scale_kernel_params,
    dimof(scale_kernel_params),        NULL,
    vxHalfscaleGaussianInputValidator, vxHalfscaleGaussianOutputValidator,
    vxHalfscaleGaussianInitializer,    vxHalfscaleGaussianDeinitializer,
};
