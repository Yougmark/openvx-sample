...
static vx_status tensorElementwiseBinaryMathOp(
    enum ElementwiseTensorMathOp op, vx_tensor in0, vx_tensor in1,
  ...
  if (op == ELEMENTWISE_TENSOR_MUL) {
    ...
    VX_CALL(vxCopyScalar(rounding_sc, &rounding, VX_READ_ONLY,
                         VX_MEMORY_TYPE_HOST));
  } else {
  ...
  tensor_desc_t in0_td = {in0->number_of_dimensions, in0->dimensions,
                          in0->stride};
  tensor_desc_t in1_td = {in1->number_of_dimensions, in1->dimensions,
                          in1->stride};
  tensor_desc_t out_td = {out->number_of_dimensions, out->dimensions,
                          out->stride};
...
#endif
  ElementwiseTensorOpImpl(op, fmt, in0->addr, in0_td, in1->addr, in1_td, scale,
...
vx_status VX_CALLBACK tensorMultiplyKernel(vx_node node,
  ...
  if (num == MULTIPLY_PARAMS_NUMBER) {
    ...
    vx_scalar overflow_sc =
        (vx_scalar)parameters[MULTIPLY_PARAM_OVERFLOW_POLICY];
    vx_scalar rounding_sc =
        (vx_scalar)parameters[MULTIPLY_PARAM_ROUNDING_POLICY];
    vx_tensor out = (vx_tensor)parameters[MULTIPLY_PARAM_TENSOR_OUT];

    return tensorElementwiseBinaryMathOp(ELEMENTWISE_TENSOR_MUL, in0, in1,
                                         scale_sc, overflow_sc, rounding_sc,
                                         out);
  }
...
static vx_status validateFloatScalar(vx_scalar sc, /*OUT*/ vx_float32 *f) {
  if (!sc)
    return VX_ERROR_INVALID_REFERENCE;  // TODO: won't vxQueryScalar handle this
                                        // anyway??
...
static vx_status validateRoundingPolicy(vx_scalar rounding_policy_sc) {
  ...
  if (rounding_policy_sc) /* rounding_policy: truncate or saturate. */
  {
    ...
    if (stype == VX_TYPE_ENUM) {
      ...
      vxCopyScalar(rounding_policy_sc, &rouding_policy, VX_READ_ONLY,
                   VX_MEMORY_TYPE_HOST);
...
static vx_status CalculateOutputDimensions(vx_size *in1_dim, vx_size *in2_dim,
                                           vx_size *out_dim_calculated,
                                           vx_size dims_num) {
  vx_status status = VX_SUCCESS;
  ...
  for (int i = 0; i < (int)dims_num; i++) {
    if (in1_dim[i] == in2_dim[i]) {
      out_dim_calculated[i] = in1_dim[i];
               ...
               in2_dim[i] == 1)  // TODO: what do we reall want to do here?
    {
...
static vx_status SetOutputMetaFormat(vx_tensor in1, vx_tensor in2,
                                     vx_tensor out, vx_meta_format *meta) {
  vx_status status = VX_SUCCESS;
  ...
  vxQueryTensor(in1, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos1,
                sizeof(fixed_point_pos1));

  vxQueryTensor(in2, VX_TENSOR_DATA_TYPE, &format2, sizeof(format2));
  vxQueryTensor(in2, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos2,
                sizeof(fixed_point_pos2));

  vxQueryTensor(out, VX_TENSOR_DATA_TYPE, &format_out, sizeof(format_out));
  vxQueryTensor(out, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos_out,
                sizeof(fixed_point_pos_out));
  ...
  if ((format1 != format2) || (fixed_point_pos1 != fixed_point_pos2) ||
    ...
    status |= vxQueryTensor(in1, VX_TENSOR_NUMBER_OF_DIMS, &in1_num_dims,
                            sizeof(in1_num_dims));
    status |= vxQueryTensor(in2, VX_TENSOR_NUMBER_OF_DIMS, &in2_num_dims,
                            sizeof(in2_num_dims));
    status |= vxQueryTensor(out, VX_TENSOR_NUMBER_OF_DIMS, &out_num_dims,
                            sizeof(out_num_dims));
    if (in1_num_dims != in2_num_dims ||
        (out_num_dims != 0 && out_num_dims != in1_num_dims)) {
      status = VX_ERROR_INVALID_DIMENSION;
    } else {
      status = CalculateOutputDimensions(in1_dim, in2_dim, out_dim_calculated,
                                         in1_num_dims);
      ...
      if (status == VX_SUCCESS) {
        status |= vxSetMetaFormatAttribute(*meta, VX_TENSOR_DATA_TYPE, &format1,
                                           sizeof(format1));
        status |= vxSetMetaFormatAttribute(
            *meta, VX_TENSOR_FIXED_POINT_POSITION, &fixed_point_pos1,
            sizeof(fixed_point_pos1));
        status |=
            vxSetMetaFormatAttribute(*meta, VX_TENSOR_DIMS, out_dim_calculated,
                                     sizeof(out_dim_calculated));
        status |= vxSetMetaFormatAttribute(*meta, VX_TENSOR_NUMBER_OF_DIMS,
                                           &in1_num_dims, sizeof(in1_num_dims));
      }
    }
  }
...
tensorMultiplyValidator(vx_node node, const vx_reference parameters[],
                        vx_uint32 num, vx_meta_format metas[]) {
  (void)node;

  if (num != MULTIPLY_PARAMS_NUMBER) return VX_ERROR_INVALID_PARAMETERS;
  // Check input tensors
  vx_tensor in1 = (vx_tensor)parameters[MULTIPLY_PARAM_TENSOR1];
  vx_tensor in2 = (vx_tensor)parameters[MULTIPLY_PARAM_TENSOR2];
  vx_tensor out = (vx_tensor)parameters[MULTIPLY_PARAM_TENSOR_OUT];
  vx_status status = VX_SUCCESS;
  status = validateOverflowPolicy(
      (vx_scalar)parameters[MULTIPLY_PARAM_OVERFLOW_POLICY]);
  status |= validateRoundingPolicy(
      (vx_scalar)parameters[MULTIPLY_PARAM_ROUNDING_POLICY]);
  status |= validateScaleFactor((vx_scalar)parameters[MULTIPLY_PARAM_SCALE]);
  status |=
      SetOutputMetaFormat(in1, in2, out, &metas[MULTIPLY_PARAM_TENSOR_OUT]);
...
vx_kernel_description_t tensor_multiply_kernel = {
    ...
    tensor_multiply_kernel_params,
    dimof(tensor_multiply_kernel_params),
...
static vx_status VX_CALLBACK tensorAddKernel(vx_node node,
                                             const vx_reference *parameters,
                                             vx_uint32 num) {
  (void)node;
  ...
  if (num == ADDSUB_PARAMS_NUMBER) {
    ...
    return tensorElementwiseBinaryMathOp(ELEMENTWISE_TENSOR_ADD, in0, in1, NULL,
                                         overflow_sc, NULL, out);
  }
 ...
 */
static vx_status VX_CALLBACK
tensorAddSubValidator(vx_node node, const vx_reference parameters[],
                      vx_uint32 num, vx_meta_format metas[]) {
  (void)node;
  ...
  vx_status status = VX_SUCCESS;
  status = validateOverflowPolicy(
      (vx_scalar)parameters[ADDSUB_PARAM_OVERFLOW_POLICY]);
...
vx_kernel_description_t tensor_add_kernel = {
    ...
    tensorAddKernel,
    tensor_addsub_kernel_params,
    dimof(tensor_addsub_kernel_params),
...
static vx_status VX_CALLBACK tensorSubtractKernel(
  ...
  if (num == ADDSUB_PARAMS_NUMBER) {
    ...
    return tensorElementwiseBinaryMathOp(ELEMENTWISE_TENSOR_SUB, in0, in1, NULL,
                                         overflow_sc, NULL, out);
  }
...
static vx_status VX_CALLBACK tensorConvertDepthKernel(
    vx_node node, const vx_reference *parameters, vx_uint32 num) {
  (void)node;
  ...
  TensorConvertDepthKernelImpl(in->addr, in_td, src_fmt, dst_fmt,
                               overflow == VX_CONVERT_POLICY_WRAP, norm, offset,
                               output_ptr, out_td);
...
static vx_status VX_CALLBACK
tensorConvertDepthValidator(vx_node node, const vx_reference parameters[],
                            vx_uint32 num, vx_meta_format metas[]) {
  (void)node;
  ...
  VX_CALL(vxQueryTensor(in, VX_TENSOR_FIXED_POINT_POSITION, &in_fixed_point_pos,
                        sizeof(in_fixed_point_pos)));

  VX_CALL(vxQueryTensor(out, VX_TENSOR_DATA_TYPE, &out_fmt, sizeof(out_fmt)));
  VX_CALL(vxQueryTensor(out, VX_TENSOR_FIXED_POINT_POSITION,
                        &out_fixed_point_pos, sizeof(out_fixed_point_pos)));

  // TODO: use ownIsValidFormat(...)
  const bool valid_in_fmt =
      (in_fmt == VX_TYPE_INT16 &&
       in_fixed_point_pos == Q78_FIXED_POINT_POSITION) ||
      ((in_fmt == VX_TYPE_UINT8 || in_fmt == VX_TYPE_INT8) &&
       !in_fixed_point_pos);

  const bool valid_out_fmt =
      (out_fmt == VX_TYPE_INT16 &&
       out_fixed_point_pos == Q78_FIXED_POINT_POSITION) ||
      ((out_fmt == VX_TYPE_UINT8 || out_fmt == VX_TYPE_INT8) &&
       !out_fixed_point_pos);
  ...
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_DATA_TYPE, &out_fmt,
                                   sizeof(out_fmt)));
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_FIXED_POINT_POSITION,
                                   &out_fixed_point_pos,
                                   sizeof(out_fixed_point_pos)));
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_DIMS, out_dims,
                                   sizeof(*out_dims) * out_dim_num));
  VX_CALL(vxSetMetaFormatAttribute(*meta, VX_TENSOR_NUMBER_OF_DIMS,
                                   &out_dim_num, sizeof(out_dim_num)));
...
vx_kernel_description_t tensor_convert_depth_kernel = {
    ...
    tensor_convert_depth_kernel_params,
    dimof(tensor_convert_depth_kernel_params),
