...
static vx_status VX_CALLBACK vxThresholdKernel(vx_node node,
...
static vx_status VX_CALLBACK vxThresholdInputValidator(vx_node node,
                                                       vx_uint32 index) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (threshold) {
        ...
        if ((type == VX_THRESHOLD_TYPE_BINARY) ||
          ...
          vx_enum data_type = 0;
          vxQueryThreshold(threshold, VX_THRESHOLD_DATA_TYPE, &data_type,
                           sizeof(data_type));
...
static vx_status VX_CALLBACK vxThresholdOutputValidator(vx_node node,
                                                        vx_uint32 index,
...
vx_kernel_description_t threshold_kernel = {
    ...
    vxThresholdKernel,
    threshold_kernel_params,
    dimof(threshold_kernel_params),
    NULL,
