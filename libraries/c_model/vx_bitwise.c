...
/*
 * The three bitwise kernels with binary parameters have the same parameter
 * domain so let's just have one set of validators.
 */

static vx_status VX_CALLBACK vxBinaryBitwiseInputValidator(vx_node node,
                                                           vx_uint32 index) {
  ...
  if (index == 0) {
    ...
    if (input) {
      ...
      if (format == VX_DF_IMAGE_U8) status = VX_SUCCESS;
...
static vx_status VX_CALLBACK vxBinaryBitwiseOutputValidator(
  ...
  if (index == 2) {
    ...
    if (param0) {
      ...
      /*
       * When passing on the geometry to the output image, we only look at image
       * 0, as both input images are verified to match, at input validation.
       */
      if (image0) {
...
static vx_status VX_CALLBACK vxAndKernel(vx_node node,
...
static vx_status VX_CALLBACK vxOrKernel(vx_node node,
                                        const vx_reference *parameters,
                                        vx_uint32 num) {
  (void)node;
...
static vx_status VX_CALLBACK vxXorKernel(vx_node node,
                                         const vx_reference *parameters,
                                         vx_uint32 num) {
  (void)node;
...
static vx_status VX_CALLBACK vxUnaryBitwiseInputValidator(vx_node node,
                                                          vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
...
static vx_status VX_CALLBACK vxUnaryBitwiseOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format_t *ptr) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 1) {
...
static vx_status VX_CALLBACK vxNotKernel(vx_node node,
                                         const vx_reference *parameters,
                                         vx_uint32 num) {
  (void)node;
...
vx_kernel_description_t not_kernel = {
    ...
    vxNotKernel,
    unary_bitwise_kernel_params,
    dimof(unary_bitwise_kernel_params),
    NULL,
