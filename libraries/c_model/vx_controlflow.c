...
vxScalarOperationParamsValidator(vx_node node, const vx_reference parameters[],
                                 vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL == node || NULL == parameters ||
      num != dimof(scalar_operation_kernel_params) || NULL == metas) {
    return status;
  }
  ...
  if (vxGetStatus((vx_reference)param0) == VX_SUCCESS) {
    ...
    if (scalar) {
      ...
      if (stype0 == VX_TYPE_ENUM) {
        ...
        if ((operation == VX_SCALAR_OP_AND) || (operation == VX_SCALAR_OP_OR) ||
  ...
  switch (operation) {
    ...
    case VX_SCALAR_OP_NAND: {
      if ((stype1 == VX_TYPE_BOOL) && (stype2 == VX_TYPE_BOOL) &&
          (stype3 == VX_TYPE_BOOL)) {
        status = VX_SUCCESS;
      } else {
    ...
    case VX_SCALAR_OP_GREATEREQ: {
      if ((stype1 == VX_TYPE_INT8 || stype1 == VX_TYPE_UINT8 ||
           stype1 == VX_TYPE_INT16 || stype1 == VX_TYPE_UINT16 ||
           stype1 == VX_TYPE_INT32 || stype1 == VX_TYPE_UINT32 ||
           stype1 == VX_TYPE_SIZE || stype1 == VX_TYPE_FLOAT32) &&
          (stype2 == VX_TYPE_INT8 || stype2 == VX_TYPE_UINT8 ||
           stype2 == VX_TYPE_INT16 || stype2 == VX_TYPE_UINT16 ||
           stype2 == VX_TYPE_INT32 || stype2 == VX_TYPE_UINT32 ||
           stype2 == VX_TYPE_SIZE || stype2 == VX_TYPE_FLOAT32) &&
          (stype3 == VX_TYPE_BOOL)) {
        status = VX_SUCCESS;
      } else {
        status = VX_ERROR_INVALID_PARAMETERS;
    ...
    case VX_SCALAR_OP_MODULUS: {
      if ((stype1 == VX_TYPE_INT8 || stype1 == VX_TYPE_UINT8 ||
          ...
          (stype2 == VX_TYPE_INT8 || stype2 == VX_TYPE_UINT8 ||
           ...
           stype2 == VX_TYPE_INT32 || stype2 == VX_TYPE_UINT32 ||
           stype2 == VX_TYPE_SIZE) &&
          (stype3 == VX_TYPE_INT8 || stype3 == VX_TYPE_UINT8 ||
           stype3 == VX_TYPE_INT16 || stype3 == VX_TYPE_UINT16 ||
           stype3 == VX_TYPE_INT32 || stype3 == VX_TYPE_UINT32 ||
           stype3 == VX_TYPE_SIZE || stype3 == VX_TYPE_FLOAT32)) {
        status = VX_SUCCESS;
      } else {
        status = VX_ERROR_INVALID_PARAMETERS;
      }
...
static vx_status VX_CALLBACK vxScalarOperationKernel(
...
vx_kernel_description_t scalar_operation_kernel = {
    ...
    scalar_operation_kernel_params,
    dimof(scalar_operation_kernel_params),
...
static vx_status VX_CALLBACK
vxSelectParamsValidator(vx_node node, const vx_reference parameters[],
                        vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL == node || NULL == parameters ||
      num != dimof(select_kernel_params) || NULL == metas) {
    return status;
  }
  ...
  if (vxGetStatus((vx_reference)param0) == VX_SUCCESS) {
    ...
    status = vxQueryParameter(param0, VX_PARAMETER_REF, &condition_scalar,
                              sizeof(condition_scalar));
    if ((status == VX_SUCCESS) && (condition_scalar)) {
      ...
      if (type0 == VX_TYPE_BOOL) {
        ...
        status = vxCopyScalar(condition_scalar, &condition_, VX_READ_ONLY,
                              VX_MEMORY_TYPE_HOST);
        if ((status == VX_SUCCESS) &&
            ((condition_ == vx_false_e) || (condition_ == vx_true_e))) {
          status = VX_SUCCESS;
        }
      } else {
        status = VX_ERROR_INVALID_TYPE;
  ...
  if (param1) {
    ...
    if (ref1) {
      ...
      if ((type1 == VX_TYPE_IMAGE) || (type1 == VX_TYPE_ARRAY) ||
          (type1 == VX_TYPE_PYRAMID) || (type1 == VX_TYPE_SCALAR) ||
          (type1 == VX_TYPE_MATRIX) || (type1 == VX_TYPE_CONVOLUTION) ||
          (type1 == VX_TYPE_DISTRIBUTION) || (type1 == VX_TYPE_REMAP) ||
          (type1 == VX_TYPE_LUT) || (type1 == VX_TYPE_THRESHOLD) ||
          (type1 == VX_TYPE_TENSOR) || (type1 == VX_TYPE_OBJECT_ARRAY)) {
        status = VX_SUCCESS;
      } else {
        status = VX_ERROR_INVALID_TYPE;
      }
  ...
  if (param2) {
    ...
    if (ref2) {
      ...
      if ((type2 == VX_TYPE_IMAGE) || (type2 == VX_TYPE_ARRAY) ||
          (type2 == VX_TYPE_PYRAMID) || (type2 == VX_TYPE_SCALAR) ||
          (type2 == VX_TYPE_MATRIX) || (type2 == VX_TYPE_CONVOLUTION) ||
          (type2 == VX_TYPE_DISTRIBUTION) || (type2 == VX_TYPE_REMAP) ||
          (type2 == VX_TYPE_LUT) || (type2 == VX_TYPE_THRESHOLD) ||
          (type2 == VX_TYPE_TENSOR) || (type2 == VX_TYPE_OBJECT_ARRAY)) {
        status = VX_SUCCESS;
      } else {
        status = VX_ERROR_INVALID_TYPE;
      }
  ...
  if (param3) {
    ...
    if (ref3) {
      ...
      if ((type3 == VX_TYPE_IMAGE) || (type3 == VX_TYPE_ARRAY) ||
          (type3 == VX_TYPE_PYRAMID) || (type3 == VX_TYPE_SCALAR) ||
          (type3 == VX_TYPE_MATRIX) || (type3 == VX_TYPE_CONVOLUTION) ||
          (type3 == VX_TYPE_DISTRIBUTION) || (type3 == VX_TYPE_REMAP) ||
          (type3 == VX_TYPE_LUT) || (type3 == VX_TYPE_THRESHOLD) ||
          (type3 == VX_TYPE_TENSOR) || (type3 == VX_TYPE_OBJECT_ARRAY)) {
        status = VX_SUCCESS;
      } else {
        status = VX_ERROR_INVALID_TYPE;
      }
  ...
  if ((status == VX_SUCCESS) && (type1 != VX_TYPE_INVALID) &&
...
static vx_status VX_CALLBACK vxSelectKernel(vx_node node,
...
vx_kernel_description_t select_kernel = {
    ...
    vxSelectKernel,
    select_kernel_params,
    dimof(select_kernel_params),
