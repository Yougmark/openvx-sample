...
static vx_status VX_CALLBACK vxConvolveKernel(vx_node node,
...
static vx_status VX_CALLBACK vxConvolveInputValidator(vx_node node,
                                                      vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
...
static vx_status VX_CALLBACK vxConvolveOutputValidator(vx_node node,
                                                       vx_uint32 index,
  ...
  if (index == 2) {
    ...
    if ((vxGetStatus((vx_reference)params[0]) == VX_SUCCESS) &&
      ...
      if (input && output) {
        ...
        vxQueryImage(output, VX_IMAGE_FORMAT, &output_format,
                     sizeof(output_format));

        ptr->type = VX_TYPE_IMAGE;
        ptr->dim.image.format =
            output_format == VX_DF_IMAGE_U8 ? VX_DF_IMAGE_U8 : VX_DF_IMAGE_S16;
...
vx_kernel_description_t convolution_kernel = {
    ...
    vxConvolveKernel,
    convolution_kernel_params,
    dimof(convolution_kernel_params),
    NULL,
