...
static const vx_int16 gaussian5x5[5][5] = {{1, 4, 6, 4, 1},
                                           {4, 16, 24, 16, 4},
                                           {6, 24, 36, 24, 6},
                                           {4, 16, 24, 16, 4},
                                           {1, 4, 6, 4, 1}};
...
/*! \note Look at \ref vxGaussianPyramidNode to see how this pyramid
 * construction works */
...
static vx_convolution vxCreateGaussian5x5Convolution(vx_context context) {
  ...
  vx_status status = vxCopyConvolutionCoefficients(
      conv, (vx_int16 *)gaussian5x5, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
  if (status != VX_SUCCESS) {
    vxReleaseConvolution(&conv);
    return NULL;
  }

  status = vxSetConvolutionAttribute(
      conv, VX_CONVOLUTION_SCALE, (void *)&gaussian5x5scale, sizeof(vx_uint32));
  if (status != VX_SUCCESS) {
    vxReleaseConvolution(&conv);
    return NULL;
  }
...
static vx_status ownCopyImage(vx_image input, vx_image output) {
  ...
  for (p = 0; p < planes && status == VX_SUCCESS; p++) {
    ...
    status |= vxMapImagePatch(input, &src_rect, p, &map_id1, &src_addr, &src,
                              VX_READ_ONLY, VX_MEMORY_TYPE_HOST, 0);
    status |= vxMapImagePatch(output, &dst_rect, p, &map_id2, &dst_addr, &dst,
                              VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST, 0);

    for (y = 0; y < src_addr.dim_y && status == VX_SUCCESS;
         y += src_addr.step_y) {
      for (x = 0; x < src_addr.dim_x && status == VX_SUCCESS;
           x += src_addr.step_x) {
        void *srcp = vxFormatImagePatchAddress2d(src, x, y, &src_addr);
        void *dstp = vxFormatImagePatchAddress2d(dst, x, y, &dst_addr);
        vx_int32 out0 = src_format == VX_DF_IMAGE_U8 ? *(vx_uint8 *)srcp
                                                     : *(vx_int16 *)srcp;
...
static void auxReadRect(const void *base,
                        const vx_imagepatch_addressing_t *addr,
                        const vx_border_t *borders, vx_df_image type,
                        vx_uint32 center_x, vx_uint32 center_y,
                        vx_uint32 radius_x, vx_uint32 radius_y,
                        void *destination) {
        ...
        switch (type) {
          case VX_DF_IMAGE_U8:
            ((vx_uint8 *)destination)[dest_index] =
                *(vx_uint8 *)(ptr + y * stride_y + x * stride_x);
            break;
          case VX_DF_IMAGE_S16:
          case VX_DF_IMAGE_U16:
            ((vx_uint16 *)destination)[dest_index] =
                *(vx_uint16 *)(ptr + y * stride_y + x * stride_x);
            break;
          case VX_DF_IMAGE_S32:
          case VX_DF_IMAGE_U32:
            ((vx_uint32 *)destination)[dest_index] =
                *(vx_uint32 *)(ptr + y * stride_y + x * stride_x);
            break;
          default:
            abort();
        }
...
#define INSERT_ZERO_Y(slice, y) \
  for (int i = 0; i < CONV_DIM; i++) slice[CONV_DIM * (1 - y) + i] = 0;
#define INSERT_VALUES_Y(slice, y)                                   \
  for (int i = 0; i < CONV_DIM; i++)                                \
    slice[CONV_DIM * (high_y - y) + i + CONV_DIM_HALF * CONV_DIM] = \
        slice[CONV_DIM * (high_y - y) + i];
#define INSERT_ZERO_X(slice, x) \
  for (int i = 0; i < CONV_DIM; i++) slice[CONV_DIM * i + 1 - x] = 0;
#define INSERT_VALUES_X(slice, x)                        \
  for (int i = 0; i < CONV_DIM; i++)                     \
    slice[CONV_DIM * i + (high_x - x) + CONV_DIM_HALF] = \
        slice[CONV_DIM * i + (high_x - x)];

static vx_status replicateConvolve(vx_image src, vx_convolution conv,
                                   vx_image dst, vx_border_t *bordermode) {
  vx_int32 y, x, i;
  void *src_base = NULL;
  void *dst_base = NULL;
  ...
  status |= vxQueryConvolution(conv, VX_CONVOLUTION_COLUMNS, &conv_width,
                               sizeof(conv_width));
  status |= vxQueryConvolution(conv, VX_CONVOLUTION_ROWS, &conv_height,
                               sizeof(conv_height));
  status |=
      vxQueryConvolution(conv, VX_CONVOLUTION_SCALE, &scale, sizeof(scale));
  conv_radius_x = (vx_int32)conv_width / 2;
  conv_radius_y = (vx_int32)conv_height / 2;
  status |= vxCopyConvolutionCoefficients(conv, conv_mat, VX_READ_ONLY,
                                          VX_MEMORY_TYPE_HOST);
  status |= vxGetValidRegionImage(src, &rect);
  status |=
      vxAccessImagePatch(src, &rect, 0, &src_addr, &src_base, VX_READ_ONLY);
  status |=
      vxAccessImagePatch(dst, &rect, 0, &dst_addr, &dst_base, VX_WRITE_ONLY);
  ...
  for (y = low_y; y < high_y; ++y) {
    for (x = low_x; x < high_x; ++x) {
      ...
      if (src_format == VX_DF_IMAGE_U8) {
        ...
        auxReadRect(src_base, &src_addr, bordermode, src_format, x, y,
                    conv_radius_x, conv_radius_y, slice);

        // purpose of this section is to compensate extra terms caused by
        // replicate border mode (it is the only one allowed)
...
static vx_status upsampleImage(vx_context context, vx_uint32 width,
                               vx_uint32 height, vx_image filling,
                               vx_convolution conv, vx_image upsample,
                               vx_border_t *border) {
  vx_status status = VX_SUCCESS;
  ...
  status |= vxQueryImage(filling, VX_IMAGE_FORMAT, &filling_format,
                         sizeof(filling_format));
  ...
  status |= vxMapImagePatch(tmp, &tmp_rect, 0, &tmp_map_id, &tmp_addr,
                            (void **)&tmp_base, VX_READ_AND_WRITE,
                            VX_MEMORY_TYPE_HOST, 0);
  status = vxGetValidRegionImage(filling, &filling_rect);
  status |= vxMapImagePatch(filling, &filling_rect, 0, &filling_map_id,
                            &filling_addr, (void **)&filling_base,
                            VX_READ_AND_WRITE, VX_MEMORY_TYPE_HOST, 0);

  for (int ix = 0; ix < (int)width; ix++) {
    for (int iy = 0; iy < (int)height; iy++) {
      void *tmp_datap =
          vxFormatImagePatchAddress2d(tmp_base, ix, iy, &tmp_addr);
      ...
      if (iy % 2 != 0 || ix % 2 != 0) {
        ...
        void *filling_tmp = vxFormatImagePatchAddress2d(filling_base, ix / 2,
                                                        iy / 2, &filling_addr);
        vx_int32 filling_data = filling_format == VX_DF_IMAGE_U8
                                    ? *(vx_uint8 *)filling_tmp
                                    : *(vx_int16 *)filling_tmp;
        if (format == VX_DF_IMAGE_U8) {
  ...
  status |= vxQueryImage(upsample, VX_IMAGE_FORMAT, &upsample_format,
                         sizeof(upsample_format));
  status = vxGetValidRegionImage(upsample, &upsample_rect);
  status |= vxMapImagePatch(upsample, &upsample_rect, 0, &upsample_map_id,
                            &upsample_addr, (void **)&upsample_base,
                            VX_READ_AND_WRITE, VX_MEMORY_TYPE_HOST, 0);

  for (int ix = 0; ix < (int)width; ix++) {
    for (int iy = 0; iy < (int)height; iy++) {
      void *upsample_p =
          vxFormatImagePatchAddress2d(upsample_base, ix, iy, &upsample_addr);
      vx_int32 upsample_data = upsample_format == VX_DF_IMAGE_U8
                                   ? *(vx_uint8 *)upsample_p
                                   : *(vx_int16 *)upsample_p;
      upsample_data *= 4;
...
static vx_status VX_CALLBACK vxGaussianPyramidInputValidator(vx_node node,
                                                             vx_uint32 index) {
...
static vx_status VX_CALLBACK vxGaussianPyramidOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format_t *ptr) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 1) {
    vx_image src = 0;
...
static vx_status VX_CALLBACK vxGaussianPyramidInitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  ...
  if (num == dimof(gaussian_pyramid_kernel_params)) {
    ...
    if (subgraph) {
      ...
      if (VX_SUCCESS != status) return status;
    ...
    if (status == VX_SUCCESS) {
      ...
      status |=
          vxQueryPyramid(gaussian, VX_PYRAMID_LEVELS, &levels, sizeof(levels));
      for (lev = 1; lev < levels; lev++) {
        /*! \internal Scaling is already precomputed for each level by the \ref
         * vxCreatePyramid function */
        ...
        status |=
            vxSetNodeAttribute(gtmp, VX_NODE_BORDER, &border, sizeof(border));
      ...
      status |=
          vxAddParameterToGraphByIndex(subgraph, copy, 0); /* input image */
      status |= vxAddParameterToGraphByIndex(
          subgraph, node,
          1); /* output pyramid - refer to self to quiet sub-graph validator */
...
static vx_status VX_CALLBACK vxGaussianPyramidDeinitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;
...
vx_kernel_description_t gaussian_pyramid_kernel = {
    ...
    gaussian_pyramid_kernel_params,
    dimof(gaussian_pyramid_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK vxLaplacianPyramidInputValidator(vx_node node,
                                                              vx_uint32 index) {
  vx_status status = VX_SUCCESS;
  if (index == 0) {
    vx_image input = 0;
    ...
    if (status == VX_SUCCESS && input) {
      ...
      if (status == VX_SUCCESS) {
        if (format != VX_DF_IMAGE_U8) status = VX_ERROR_INVALID_PARAMETERS;
      }
...
static vx_status VX_CALLBACK vxLaplacianPyramidOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format_t *ptr) {
  vx_status status = VX_SUCCESS;
  if (index == 1) {
    vx_image input = 0;
    ...
    status |=
        vxQueryParameter(src_param, VX_PARAMETER_REF, &input, sizeof(input));
    if (status == VX_SUCCESS && input) {
      vx_uint32 width = 0;
      vx_uint32 height = 0;
      vx_df_image format = 0;
      ...
      status |= vxQueryParameter(dst_param, VX_PARAMETER_REF, &laplacian,
                                 sizeof(laplacian));
      if (status == VX_SUCCESS && laplacian) {
        ...
        status |= vxQueryPyramid(laplacian, VX_PYRAMID_WIDTH, &pyr_width,
                                 sizeof(pyr_width));
        status |= vxQueryPyramid(laplacian, VX_PYRAMID_HEIGHT, &pyr_height,
                                 sizeof(pyr_height));
        status |= vxQueryPyramid(laplacian, VX_PYRAMID_FORMAT, &pyr_format,
                                 sizeof(pyr_format));
        status |= vxQueryPyramid(laplacian, VX_PYRAMID_SCALE, &pyr_scale,
                                 sizeof(pyr_scale));
        status |= vxQueryPyramid(laplacian, VX_PYRAMID_LEVELS, &pyr_levels,
                                 sizeof(pyr_levels));

        if (status == VX_SUCCESS) {
          if (pyr_width == width && pyr_height == height &&
              pyr_format == VX_DF_IMAGE_S16 &&
              pyr_scale == VX_SCALE_PYRAMID_HALF) {
            /* fill in the meta data with the attributes so that the checker
             * will pass */
            ptr->type = VX_TYPE_PYRAMID;
  ...
  if (index == 2) {
    ...
    status |= vxQueryParameter(src_param, VX_PARAMETER_REF, &laplacian,
                               sizeof(laplacian));
    if (status == VX_SUCCESS && laplacian) {
      vx_uint32 width = 0;
      vx_uint32 height = 0;
      ...
      status |=
          vxQueryPyramid(laplacian, VX_PYRAMID_LEVELS, &levels, sizeof(levels));
      ...
      status |=
          vxQueryImage(last_level, VX_IMAGE_HEIGHT, &height, sizeof(height));
      status |=
          vxQueryImage(last_level, VX_IMAGE_FORMAT, &format, sizeof(format));

      status |= vxQueryParameter(dst_param, VX_PARAMETER_REF, &output,
                                 sizeof(output));
      if (status == VX_SUCCESS && output) {
        ...
        status |=
            vxQueryImage(output, VX_IMAGE_WIDTH, &dst_width, sizeof(dst_width));
        status |= vxQueryImage(output, VX_IMAGE_HEIGHT, &dst_height,
                               sizeof(dst_height));
        status |= vxQueryImage(output, VX_IMAGE_FORMAT, &dst_format,
                               sizeof(dst_format));
...
static vx_status VX_CALLBACK vxLaplacianPyramidInitializer(
  ...
  if (num == dimof(laplacian_pyramid_kernel_params)) {
    ...
    vxSetContextAttribute(context, VX_CONTEXT_IMMEDIATE_BORDER, &border,
                          sizeof(border));
    ...
    for (lev = 0; lev < levels; lev++) {
      pyr_gauss_curr_level_filtered =
          vxCreateImage(context, level_width, level_height, VX_DF_IMAGE_S16);
      upsampleImage(context, level_width, level_height, gauss_next, conv,
                    pyr_gauss_curr_level_filtered, &border);

      pyr_laplacian_curr_level = vxGetPyramidLevel(laplacian, (vx_uint32)lev);
      status |= vxuSubtract(context, gauss_cur, pyr_gauss_curr_level_filtered,
...
static vx_status VX_CALLBACK vxLaplacianPyramidDeinitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)node;
  (void)parameters;
...
vx_kernel_description_t laplacian_pyramid_kernel = {
    ...
    laplacian_pyramid_kernel_params,
    dimof(laplacian_pyramid_kernel_params),
    NULL,
...
static vx_status VX_CALLBACK vxLaplacianReconstructKernel(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_FAILURE;
  (void)node;
  (void)parameters;
...
vxLaplacianReconstructInputValidator(vx_node node, vx_uint32 index) {
  vx_status status = VX_SUCCESS;
  if (index == 0) {
    vx_pyramid laplacian = 0;
    vx_parameter param = vxGetParameterByIndex(node, index);

    status |= vxQueryParameter(param, VX_PARAMETER_REF, &laplacian,
                               sizeof(laplacian));
    if (status == VX_SUCCESS && laplacian) {
      vx_df_image pyr_format = 0;
      vx_float32 pyr_scale = 0;
  ...
  if (index == 1) {
    ...
    status |= vxQueryParameter(param0, VX_PARAMETER_REF, &laplacian,
                               sizeof(laplacian));
    ...
    if (status == VX_SUCCESS && laplacian && input) {
      ...
      pyr_last_level_img =
          vxGetPyramidLevel(laplacian, (vx_uint32)(pyr_levels - 1));

      status |=
          vxQueryImage(pyr_last_level_img, VX_IMAGE_WIDTH,
                       &pyr_last_level_width, sizeof(pyr_last_level_width));
      status |=
          vxQueryImage(pyr_last_level_img, VX_IMAGE_HEIGHT,
                       &pyr_last_level_height, sizeof(pyr_last_level_height));

      status |= vxQueryImage(input, VX_IMAGE_WIDTH, &lowest_res_width,
                             sizeof(lowest_res_width));
      status |= vxQueryImage(input, VX_IMAGE_HEIGHT, &lowest_res_height,
                             sizeof(lowest_res_height));
      status |= vxQueryImage(input, VX_IMAGE_FORMAT, &lowest_res_format,
                             sizeof(lowest_res_format));

      if (status == VX_SUCCESS) {
        if ((pyr_last_level_width != lowest_res_width * 2) ||
            (pyr_last_level_height != lowest_res_height * 2)) {
          status = VX_ERROR_INVALID_PARAMETERS;
        }
      }
...
static vx_status VX_CALLBACK vxLaplacianReconstructOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format_t *ptr) {
  vx_status status = VX_SUCCESS;
  if (index == 2) {
    vx_pyramid laplacian = 0;
...
static vx_status VX_CALLBACK vxLaplacianReconstructInitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_SUCCESS;
  ...
  if (num == dimof(laplacian_reconstruct_kernel_params)) {
    ...
    for (lev = 0; lev < levels; lev++) {
      ...
      upsampleImage(context, level_width, level_height, filling, conv, filter,
                    &border);
...
static vx_status VX_CALLBACK vxLaplacianReconstructDeinitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;
...
vx_kernel_description_t laplacian_reconstruct_kernel = {
    ...
    laplacian_reconstruct_kernel_params,
    dimof(laplacian_reconstruct_kernel_params),
    NULL,
