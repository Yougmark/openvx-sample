...
static vx_status VX_CALLBACK vxChannelExtractKernel(
...
static vx_status VX_CALLBACK vxChannelExtractInputValidator(vx_node node,
                                                            vx_uint32 index) {
        ...
        if (vxGetStatus((vx_reference)param0) == VX_SUCCESS) {
          ...
          if (image) {
            ...
            switch (format) {
              case VX_DF_IMAGE_RGB:
              case VX_DF_IMAGE_RGBX:
                if ((channel == VX_CHANNEL_R) || (channel == VX_CHANNEL_G) ||
                    (channel == VX_CHANNEL_B) || (channel == VX_CHANNEL_A)) {
                  status = VX_SUCCESS;
                }
                break;
              case VX_DF_IMAGE_YUV4:
              ...
              case VX_DF_IMAGE_UYVY:
              case VX_DF_IMAGE_YUYV:
                if ((channel == VX_CHANNEL_Y) || (channel == VX_CHANNEL_U) ||
...
static vx_status VX_CALLBACK vxChannelExtractOutputValidator(
  ...
  if (index == 2) {
    ...
    if ((vxGetStatus((vx_reference)param0) == VX_SUCCESS) &&
      ...
      if ((input) && (chan)) {
        ...
        if (channel != VX_CHANNEL_Y) switch (format) {
            case VX_DF_IMAGE_IYUV:
            case VX_DF_IMAGE_NV12:
            case VX_DF_IMAGE_NV21:
              width /= 2;
...
vx_kernel_description_t channelextract_kernel = {
    ...
    channel_extract_kernel_params,
    dimof(channel_extract_kernel_params),
    NULL,
