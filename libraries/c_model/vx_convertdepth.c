...
static vx_status VX_CALLBACK vxConvertDepthKernel(
...
static vx_status VX_CALLBACK vxConvertDepthInputValidator(vx_node node,
                                                          vx_uint32 index) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if ((status == VX_SUCCESS) && input) {
        ...
        if ((status != VX_SUCCESS) || (format == VX_DF_IMAGE_U8) ||
            ...
            (format == VX_DF_IMAGE_U16) || (format == VX_DF_IMAGE_U32) ||
            (format == VX_DF_IMAGE_S32) || (format == VX_DF_IMAGE_F32) ||
#endif
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (scalar) {
        ...
        if (stype == VX_TYPE_ENUM) {
          ...
          vxCopyScalar(scalar, &overflow_policy, VX_READ_ONLY,
                       VX_MEMORY_TYPE_HOST);
...
static vx_status VX_CALLBACK vxConvertDepthOutputValidator(
  ...
  if (index == 1) {
    ...
    if ((vxGetStatus((vx_reference)param[0]) == VX_SUCCESS) &&
      ...
      status = VX_SUCCESS;
      status |= vxQueryParameter(param[0], VX_PARAMETER_REF, &images[0],
                                 sizeof(images[0]));
      status |= vxQueryParameter(param[1], VX_PARAMETER_REF, &images[1],
                                 sizeof(images[1]));
      if ((status == VX_SUCCESS) && (images[0]) && (images[1])) {
        ...
        status |= vxQueryImage(images[0], VX_IMAGE_FORMAT, &format[0],
                               sizeof(format[0]));
        status |= vxQueryImage(images[1], VX_IMAGE_FORMAT, &format[1],
                               sizeof(format[1]));
        if (((format[0] == VX_DF_IMAGE_U8) && (format[1] == VX_DF_IMAGE_S16)) ||
            ...
            ((format[0] == VX_DF_IMAGE_U16) &&
             (format[1] == VX_DF_IMAGE_U32)) ||
            ((format[0] == VX_DF_IMAGE_S16) &&
             (format[1] == VX_DF_IMAGE_S32)) ||
            ((format[0] == VX_DF_IMAGE_U32) && (format[1] == VX_DF_IMAGE_U8)) ||
            ((format[0] == VX_DF_IMAGE_U32) &&
             (format[1] == VX_DF_IMAGE_U16)) ||
            ((format[0] == VX_DF_IMAGE_S32) &&
             (format[1] == VX_DF_IMAGE_S16)) ||
            ((format[0] == VX_DF_IMAGE_F32) &&
             (format[1] == VX_DF_IMAGE_U8)) || /* non-specification */
#endif
...
vx_kernel_description_t convertdepth_kernel = {
    ...
    convertdepth_kernel_params,
    dimof(convertdepth_kernel_params),
    NULL,
