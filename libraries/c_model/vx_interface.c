...
vx_status VX_CALLBACK vxTilingKernel(vx_node node,
...
vx_action vxTargetProcess(vx_target target, vx_node_t *nodes[],
  ...
  for (n = startIndex;
       (n < (startIndex + numNodes)) && (action == VX_ACTION_CONTINUE); n++) {
    vx_context context = vxGetContext((vx_reference)nodes[n]);
    VX_PRINT(VX_ZONE_GRAPH,
             "Executing Kernel %s:%d in Nodes[%u] on target %s\n",
             nodes[n]->kernel->name, nodes[n]->kernel->enumeration, n,
             nodes[n]->base.context->targets[nodes[n]->affinity].name);

    if (context->perf_enabled) ownStartCapture(&nodes[n]->perf);
    ...
    if (nodes[n]->is_replicated == vx_true_e) {
      ...
      for (param = 0; param < num_parameters; ++param) {
        if (nodes[n]->replicated_flags[param] == vx_true_e) {
          ...
          } else if ((nodes[n]->parameters[param])->scope->type ==
                     VX_TYPE_OBJECT_ARRAY) {
            vx_object_array arr =
                (vx_object_array)(nodes[n]->parameters[param])->scope;
      ...
      if (status == VX_SUCCESS) {
        ...
        for (replica = 0; replica < num_replicas; ++replica) {
          for (param = 0; param < num_parameters; ++param) {
            if (nodes[n]->replicated_flags[param] == vx_true_e) {
              if ((nodes[n]->parameters[param])->scope->type ==
                  VX_TYPE_PYRAMID) {
                vx_pyramid pyr =
                    (vx_pyramid)(nodes[n]->parameters[param])->scope;
          ...
          status = nodes[n]->kernel->function((vx_node)nodes[n], parameters,
                                              num_parameters);
        }
      }
    } else {
      status = nodes[n]->kernel->function(
          (vx_node)nodes[n], (vx_reference *)nodes[n]->parameters,
    ...
    if (context->perf_enabled) ownStopCapture(&nodes[n]->perf);
...
vx_kernel vxTargetAddKernel(vx_target target, vx_char name[VX_MAX_KERNEL_NAME],
  ...
  for (k = 0; k < VX_INT_MAX_KERNELS; k++) {
    ...
    if (kernel->enabled == vx_false_e) {
      ownInitializeKernel(target->base.context, kernel, enumeration, func_ptr,
                          name, NULL, numParams, validate, input, output,
...
vx_kernel vxTargetAddTilingKernel(
  ...
  for (k = 0; k < VX_INT_MAX_KERNELS; k++) {
    ...
    if (kernel->enabled == vx_false_e) {
      ...
      ownInitializeKernel(target->base.context, kernel, enumeration,
                          vxTilingKernel, name, NULL, numParams, NULL, input,
                          output, NULL, NULL);
      VX_PRINT(VX_ZONE_KERNEL, "Reserving %s Kernel[%u] for %s\n", target->name,
               k, kernel->name);
      target->num_kernels++;
...
static vx_status vxGetPatchToTile(vx_image image, vx_rectangle_t *rect,
                                  vx_tile_t *tile) {
  vx_status status = VX_SUCCESS;
  vx_uint32 p = 0;
  ...
  for (p = 0; p < img->planes; p++) {
    tile->base[p] = NULL;
    status = vxAccessImagePatch(image, rect, 0, &tile->addr[p],
                                (void **)&tile->base[p], VX_READ_AND_WRITE);
  }
  return status;
}

static vx_status vxSetTileToPatch(vx_image image, vx_rectangle_t *rect,
                                  vx_tile_t *tile) {
  vx_image_t *img = (vx_image_t *)image;
  vx_uint32 p = 0;
  vx_status status = VX_SUCCESS;
...
vx_status VX_CALLBACK vxTilingKernel(vx_node node,
  ...
  for (p = 0u; p < num; p++) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      vxQueryParameter(param, VX_PARAMETER_DIRECTION, &dirs[p],
                       sizeof(dirs[p]));
    ...
    if (types[p] == VX_TYPE_IMAGE) {
      vxQueryNode(node, VX_NODE_OUTPUT_TILE_BLOCK_SIZE, &tiles[p].tile_block,
                  sizeof(vx_tile_block_size_t));
      vxQueryNode(node, VX_NODE_INPUT_NEIGHBORHOOD, &tiles[p].neighborhood,
                  sizeof(vx_neighborhood_size_t));
      ownPrintImage((vx_image_t *)parameters[p]);
      images[p] = (vx_image)parameters[p];
      vxQueryImage(images[p], VX_IMAGE_WIDTH, &tiles[p].image.width,
                   sizeof(vx_uint32));
      vxQueryImage(images[p], VX_IMAGE_HEIGHT, &tiles[p].image.height,
                   sizeof(vx_uint32));
      vxQueryImage(images[p], VX_IMAGE_FORMAT, &tiles[p].image.format,
                   sizeof(vx_df_image));
      vxQueryImage(images[p], VX_IMAGE_SPACE, &tiles[p].image.space,
                   sizeof(vx_enum));
      vxQueryImage(images[p], VX_IMAGE_RANGE, &tiles[p].image.range,
                   sizeof(vx_enum));
      params[p] = &tiles[p];
      ...
      vxCopyScalar((vx_scalar)parameters[p], (void *)&scalars[p], VX_READ_ONLY,
                   VX_MEMORY_TYPE_HOST);
      params[p] = &scalars[p];
    }
  ...
  status |=
      vxQueryImage(images[index], VX_IMAGE_HEIGHT, &height, sizeof(height));
