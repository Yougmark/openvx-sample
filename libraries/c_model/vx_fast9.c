...
static vx_status VX_CALLBACK vxFast9CornersKernel(
  ...
  if (num == 5) {
    ...
    if (status == VX_SUCCESS) {
      status =
          vxFast9Corners(src, sens, nonm, points, num_corners, &bordermode);
    }
  }
  return status;
}

static vx_status VX_CALLBACK vxFast9InputValidator(vx_node node,
                                                   vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
  ...
  if (index == 2) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      status = vxQueryParameter(param, VX_PARAMETER_REF, &s_nonmax,
                                sizeof(s_nonmax));
      if ((status == VX_SUCCESS) && (s_nonmax)) {
        ...
        if (type == VX_TYPE_BOOL) {
          ...
          status = vxCopyScalar(s_nonmax, &nonmax, VX_READ_ONLY,
                                VX_MEMORY_TYPE_HOST);
          if ((status == VX_SUCCESS) &&
              ((nonmax == vx_false_e) || (nonmax == vx_true_e))) {
            status = VX_SUCCESS;
          } else {
            status = VX_ERROR_INVALID_VALUE;
          }
...
static vx_status VX_CALLBACK vxFast9OutputValidator(vx_node node,
                                                    vx_uint32 index,
