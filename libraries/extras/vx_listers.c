...
static vx_status VX_CALLBACK ownImageListerKernel(
  ...
  if (NULL != node && NULL != parameters &&
    ...
    status |= vxQueryArray(arr, VX_ARRAY_CAPACITY, &dst_capacity,
                           sizeof(dst_capacity));

    status |= vxMapImagePatch(src, &rect, 0, &src_map_id, &src_addr, &src_base,
        ...
        if (ptr) {
          ...
          if ((set == vx_true_e) && (strength > 0.0f)) {
            if (num_corners < dst_capacity) {
              ...
              status |=
                  vxAddArrayItems(arr, 1, &keypoint, sizeof(vx_keypoint_t));
            }
    ...
    if (s_num_points)
      status |= vxCopyScalar(s_num_points, &num_corners, VX_WRITE_ONLY,
                             VX_MEMORY_TYPE_HOST);
...
own_image_lister_validator(vx_node node, const vx_reference parameters[],
                           vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;

  if (NULL != node && num == dimof(image_lister_kernel_params) &&
      NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param)) {
      ...
      if (VX_SUCCESS == status) {
        ...
        status |= vxSetMetaFormatAttribute(metas[1], VX_ARRAY_ITEMTYPE, &type,
                                           sizeof(type));
      }
      ...
      if (VX_SUCCESS == status && NULL != metas[2]) {
        ...
        status |= vxSetMetaFormatAttribute(metas[2], VX_SCALAR_TYPE, &type,
                                           sizeof(type));
      }
    }
...
vx_kernel_description_t image_lister_kernel = {
    ...
    ownImageListerKernel,
    image_lister_kernel_params,
    dimof(image_lister_kernel_params),
    own_image_lister_validator,
    NULL,
    NULL,
    NULL,
