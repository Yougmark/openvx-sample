...
static vx_status VX_CALLBACK ownScharr3x3Kernel(vx_node node,
  ...
  if (NULL != node && NULL != parameters &&
    ...
    status |= vxMapImagePatch(src, &rect, 0, &src_map_id, &src_addr,
                              (void**)&src_base, VX_READ_ONLY,
                              VX_MEMORY_TYPE_HOST, VX_NOGAP_X);

    if (NULL != grad_x)
      status |= vxMapImagePatch(grad_x, &rect, 0, &grad_x_map_id, &dst_addr_x,
                                (void**)&dst_base_x, VX_WRITE_ONLY,
                                VX_MEMORY_TYPE_HOST, VX_NOGAP_X);

    if (NULL != grad_y)
      status |= vxMapImagePatch(grad_y, &rect, 0, &grad_y_map_id, &dst_addr_y,
                                (void**)&dst_base_y, VX_WRITE_ONLY,
                                VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
      ...
      if (borders.mode == VX_BORDER_UNDEFINED) {
        ...
        for (y = 1; y < (src_addr.dim_y - 1); y++) {
          for (x = 1; x < (src_addr.dim_x - 1); x++) {
            ...
            vx_int16* out_x =
                vxFormatImagePatchAddress2d(dst_base_x, x, y, &dst_addr_x);
            vx_int16* out_y =
                vxFormatImagePatchAddress2d(dst_base_y, x, y, &dst_addr_y);

            if (out_x) {
              *out_x = 0;
              for (i = 0; i < dimof(gdx); i++) *out_x += (ops[i] * gdx[i][0]);
            }

            if (out_y) {
              *out_y = 0;
              for (i = 0; i < dimof(gdy); i++) *out_y += (ops[i] * gdy[i][0]);
            }
          }  // for x
        }    // for y
      }      // VX_BORDER_UNDEFINED
    ...
    if (NULL != grad_x) status |= vxUnmapImagePatch(grad_x, grad_x_map_id);

    if (NULL != grad_y) status |= vxUnmapImagePatch(grad_y, grad_y_map_id);
  } /* if ptrs non NULL */

  return status;
} /* ownScharr3x3Kernel() */

static vx_status VX_CALLBACK set_scharr3x3_valid_rectangle(
    vx_node node, vx_uint32 index, const vx_rectangle_t* const input_valid[],
    vx_rectangle_t* const output_valid[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && index < dimof(scharr3x3_kernel_params) &&
...
own_scharr3x3_validator(vx_node node, const vx_reference parameters[],
                        vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;

  if (NULL != node && num == dimof(scharr3x3_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param)) {
      ...
      if (VX_SUCCESS == status) {
        ...
        if (VX_SUCCESS == status && (src_width >= 3 && src_height >= 3 &&
                                     src_format == VX_DF_IMAGE_U8)) {
          status = VX_SUCCESS;
        }
        ...
        if (VX_SUCCESS == status) {
          ...
          if (NULL != metas[1]) {
            ...
            status |= vxSetMetaFormatAttribute(metas[1], VX_IMAGE_WIDTH,
                                               &src_width, sizeof(src_width));
            status |= vxSetMetaFormatAttribute(metas[1], VX_IMAGE_HEIGHT,
                                               &src_height, sizeof(src_height));
            status |= vxSetMetaFormatAttribute(metas[1], VX_IMAGE_FORMAT,
                                               &dst_format, sizeof(dst_format));
          }

          if (NULL != metas[2]) {
            /* if optional parameter non NULL */
            status |= vxSetMetaFormatAttribute(metas[2], VX_IMAGE_WIDTH,
                                               &src_width, sizeof(src_width));
          ...
          if (VX_SUCCESS == status) {
            if (VX_BORDER_UNDEFINED == borders.mode) {
              ...
              if (NULL != metas[1])
                status |=
                    vxSetMetaFormatAttribute(metas[1], VX_VALID_RECT_CALLBACK,
                                             &callback, sizeof(callback));

              if (NULL != metas[2])
                status |=
                    vxSetMetaFormatAttribute(metas[2], VX_VALID_RECT_CALLBACK,
                                             &callback, sizeof(callback));
            } else
              status = VX_ERROR_NOT_IMPLEMENTED;
          }
...
vx_kernel_description_t scharr3x3_kernel = {VX_KERNEL_EXTRAS_SCHARR_3x3,
                                            "org.khronos.extras.scharr3x3",
                                            ownScharr3x3Kernel,
                                            scharr3x3_kernel_params,
                                            dimof(scharr3x3_kernel_params),
                                            own_scharr3x3_validator,
                                            NULL,
                                            NULL,
                                            NULL,
...
static vx_int16 ops7_y[7][7] = {
    {-1, -6, -15, -20, -15, -6, -1},    {-4, -24, -60, -80, -60, -24, -4},
    {-5, -30, -75, -100, -75, -30, -5}, {0, 0, 0, 0, 0, 0, 0},
    {5, 30, 75, 100, 75, 30, 5},        {4, 24, 60, 80, 60, 24, 4},
    {1, 6, 15, 20, 15, 6, 1},
};

static vx_status VX_CALLBACK ownSobelMxNKernel(vx_node node,
                                               const vx_reference parameters[],
                                               vx_uint32 num) {
  vx_status status = VX_FAILURE;
    ...
    if (VX_SUCCESS == status) {
      for (y = low_y; y < high_y; y++) {
        for (x = low_x; x < high_x; x++) {
          ...
          vx_uint8 square[7 * 7];
          vxReadRectangle(src_base, &src_addr, &borders, VX_DF_IMAGE_U8, x, y,
                          b, b, square);
          ...
          if (NULL != out_x) *out_x = (vx_float32)sum_x;

          if (NULL != out_y) *out_y = (vx_float32)sum_y;
        }  // for x
      }    // for y
    }
...
static vx_status VX_CALLBACK set_sobelMxN_valid_rectangle(
    vx_node node, vx_uint32 index, const vx_rectangle_t* const input_valid[],
    vx_rectangle_t* const output_valid[]) {
  vx_status status = VX_FAILURE;

  if (NULL != node && index < dimof(sobelMxN_kernel_params) &&
      NULL != input_valid && NULL != output_valid) {
    vx_parameter param = 0;
    vx_scalar win = 0;
    ...
    if (VX_SUCCESS == status) {
      ...
      if (VX_SUCCESS == status && VX_TYPE_INT32 == type) {
        status =
            vxCopyScalar(win, &kernel_size, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
        if (VX_SUCCESS == status &&
    ...
    if (NULL != win) vxReleaseScalar(&win);
...
static vx_status VX_CALLBACK
own_sobelMxN_validator(vx_node node, const vx_reference parameters[],
                       vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;

  if (NULL != node && num == dimof(sobelMxN_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == status) {
      ...
      if (VX_SUCCESS == status &&
        ...
        /* expected VX_DF_IMAGE_U8 format and size not less than filter kernel
         * size */
        if (VX_SUCCESS == status && src_width >= kernel_size &&
            src_height >= kernel_size && src_format == VX_DF_IMAGE_U8) {
          status = VX_SUCCESS;
        } else
          status = VX_ERROR_INVALID_PARAMETERS;
      } else
      ...
      /* validate output images */
      if (VX_SUCCESS == status) {
        /* sobel MxN use F32 as output format to maintain arithmetic precision
         * for 5x5 and 7x7 kernels */
        ...
        if (NULL != metas[3]) {
          /* if optional parameter non NULL */
          status |= vxSetMetaFormatAttribute(metas[3], VX_IMAGE_WIDTH,
                                             &src_width, sizeof(src_width));
...
vx_kernel_description_t sobelMxN_kernel = {
    ...
    ownSobelMxNKernel,
    sobelMxN_kernel_params,
    dimof(sobelMxN_kernel_params),
    own_sobelMxN_validator,
    NULL,
    NULL,
    NULL,
