...
static vx_param_description_t laplacian3x3_kernel_params[] = {
    {VX_INPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
    {VX_OUTPUT, VX_TYPE_IMAGE, VX_PARAMETER_STATE_REQUIRED},
};

static vx_status VX_CALLBACK ownLaplacian3x3Kernel(
...
static vx_status VX_CALLBACK set_laplacian3x3_valid_rectangle(
    vx_node node, vx_uint32 index, const vx_rectangle_t* const input_valid[],
    vx_rectangle_t* const output_valid[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && index < dimof(laplacian3x3_kernel_params) &&
...
own_laplacian3x3_validator(vx_node node, const vx_reference parameters[],
                           vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;

  if (NULL != node && num == dimof(laplacian3x3_kernel_params) &&
      NULL != metas) {
    ...
    if (VX_SUCCESS == status) {
      ...
      if (VX_SUCCESS == status) {
        ...
        status |= vxSetMetaFormatAttribute(metas[1], VX_IMAGE_WIDTH, &src_width,
                                           sizeof(src_width));
        ...
        if (VX_SUCCESS == status) {
          if (VX_BORDER_UNDEFINED == borders.mode) {
            ...
            status = vxSetMetaFormatAttribute(metas[1], VX_VALID_RECT_CALLBACK,
...
vx_kernel_description_t laplacian3x3_kernel = {
    ...
    ownLaplacian3x3Kernel,
    laplacian3x3_kernel_params,
    dimof(laplacian3x3_kernel_params),
    own_laplacian3x3_validator,
    NULL,
    NULL,
    NULL,
