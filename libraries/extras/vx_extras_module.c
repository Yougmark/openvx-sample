...
static vx_kernel_description_t* kernels[] = {
    &edge_trace_kernel,   &euclidean_nonmaxsuppression_harris_kernel,
    &harris_score_kernel, &laplacian3x3_kernel,
    &image_lister_kernel, &nonmaxsuppression_kernel,
    &norm_kernel,         &scharr3x3_kernel,
    &sobelMxN_kernel,
};
...
/*VX_API_ENTRY*/ vx_status VX_API_CALL vxPublishKernels(vx_context context) {
  ...
  for (k = 0; k < num_kernels; k++) {
    ...
    kernel = vxAddUserKernel(context, kernels[k]->name, kernels[k]->enumeration,
                             kernels[k]->function, kernels[k]->numParams,
                             kernels[k]->validate, kernels[k]->initialize,
