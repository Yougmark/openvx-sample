...
static vx_status ownNorm(vx_image input_x, vx_image input_y,
                         vx_scalar norm_type, vx_image output) {
  vx_uint32 x;
  vx_uint32 y;
  ...
  status |= vxMapImagePatch(input_x, &rect, 0, &src_x_map_id, &src_addr_x,
                            (void**)&src_base_x, VX_READ_ONLY,
                            VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  status |= vxMapImagePatch(input_y, &rect, 0, &src_y_map_id, &src_addr_y,
                            (void**)&src_base_y, VX_READ_ONLY,
                            VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  status |= vxMapImagePatch(output, &rect, 0, &dst_map_id, &dst_addr,
                            (void**)&dst_base, VX_WRITE_ONLY,
                            VX_MEMORY_TYPE_HOST, VX_NOGAP_X);

  status |= vxCopyScalar(norm_type, &norm_type_value, VX_READ_ONLY,
                         VX_MEMORY_TYPE_HOST);
  ...
  for (y = 0; y < src_addr_x.dim_y; y++) {
    for (x = 0; x < src_addr_x.dim_x; x++) {
      vx_float32* in_x =
          vxFormatImagePatchAddress2d(src_base_x, x, y, &src_addr_x);
      vx_float32* in_y =
          vxFormatImagePatchAddress2d(src_base_y, x, y, &src_addr_y);
...
static vx_status VX_CALLBACK ownNormKernel(vx_node node,
...
static vx_status VX_CALLBACK own_norm_validator(vx_node node,
                                                const vx_reference parameters[],
                                                vx_uint32 num,
  ...
  if (NULL != node && num == dimof(norm_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param1) &&
      ...
      status |= vxQueryParameter(param3, VX_PARAMETER_REF, &norm_type,
                                 sizeof(norm_type));
    ...
    if (NULL != norm_type) vxReleaseScalar(&norm_type);
