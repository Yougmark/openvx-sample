...
static vx_param_description_t harris_score_kernel_params[] = {
...
static vx_status VX_CALLBACK ownHarrisScoreKernel(
  ...
  if (NULL != node && NULL != parameters &&
    ...
    status |=
        vxCopyScalar(grad_s, &grad_size, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
    status |=
        vxCopyScalar(winds, &block_size, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
    ...
    if (status == VX_SUCCESS) {
      ...
      status |=
          vxMapImagePatch(grad_x, &rect, 0, &grad_x_map_id, &gx_addr, &gx_base,
                          VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
      status |=
          vxMapImagePatch(grad_y, &rect, 0, &grad_y_map_id, &gy_addr, &gy_base,
                          VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
      status |=
          vxMapImagePatch(dst, &rect, 0, &dst_map_id, &dst_addr, &dst_base,
        ...
        if (borders.mode == VX_BORDER_UNDEFINED) {
          ...
          for (y = b; (y < (vx_int32)(gx_addr.dim_y - b)); y++) {
            for (x = b; x < (vx_int32)(gx_addr.dim_x - b); x++) {
              ...
              for (j = -b2; j <= b2; j++) {
                for (i = -b2; i <= b2; i++) {
                  vx_float32* pgx = vxFormatImagePatchAddress2d(
                      gx_base, x + i, y + j, &gx_addr);
                  vx_float32* pgy = vxFormatImagePatchAddress2d(
                      gy_base, x + i, y + j, &gy_addr);
...
static vx_status VX_CALLBACK set_harris_score_valid_rectangle(
    vx_node node, vx_uint32 index, const vx_rectangle_t* const input_valid[],
    vx_rectangle_t* const output_valid[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && index < dimof(harris_score_kernel_params) &&
    ...
    if (VX_BORDER_UNDEFINED == borders.mode) {
      ...
      if (VX_SUCCESS == vxGetStatus((vx_reference)param) &&
          VX_SUCCESS == vxQueryParameter(param, VX_PARAMETER_REF, &block_size,
                                         sizeof(block_size))) {
        vx_enum type = 0;
        ...
        if (VX_SUCCESS == status && VX_TYPE_INT32 == type) {
          ...
          status = vxCopyScalar(block_size, &win_size, VX_READ_ONLY,
                                VX_MEMORY_TYPE_HOST);
          ...
          if (VX_SUCCESS == status &&
            ...
            if (input_valid[0]->start_x > input_valid[1]->end_x ||
              ...
              output_valid[0]->end_x =
                  OWN_MIN(input_valid[0]->end_x, input_valid[1]->end_x) -
                  border_size;
              output_valid[0]->end_y =
                  OWN_MIN(input_valid[0]->end_y, input_valid[1]->end_y) -
                  border_size;
              status = VX_SUCCESS;
            }
          } else
      ...
      if (NULL != block_size) vxReleaseScalar(&block_size);
...
own_harris_score_validator(vx_node node, const vx_reference parameters[],
                           vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;

  if (NULL != node && num == dimof(harris_score_kernel_params) &&
      NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param1) &&
      ...
      status |= vxQueryParameter(param3, VX_PARAMETER_REF, &sensitivity,
                                 sizeof(sensitivity));
      status |= vxQueryParameter(param4, VX_PARAMETER_REF, &gradient_size,
                                 sizeof(gradient_size));
      status |= vxQueryParameter(param5, VX_PARAMETER_REF, &block_size,
                                 sizeof(block_size));

      if (VX_SUCCESS == status && VX_SUCCESS == vxGetStatus((vx_reference)dx) &&
        ...
        status |=
            vxQueryImage(dx, VX_IMAGE_HEIGHT, &dx_height, sizeof(dx_height));
        status |=
            vxQueryImage(dx, VX_IMAGE_FORMAT, &dx_format, sizeof(dx_format));

        status |= vxQueryImage(dy, VX_IMAGE_WIDTH, &dy_width, sizeof(dy_width));
        status |=
            vxQueryImage(dy, VX_IMAGE_HEIGHT, &dy_height, sizeof(dy_height));
        status |=
            vxQueryImage(dy, VX_IMAGE_FORMAT, &dy_format, sizeof(dy_format));

        /* validate input images */
        if (VX_SUCCESS == status && dx_format == VX_DF_IMAGE_F32 &&
            dy_format == VX_DF_IMAGE_F32 && dx_width == dy_width &&
            dx_height == dy_height) {
          status = VX_SUCCESS;
        } else
        ...
        if (VX_SUCCESS == status) {
          ...
          status |=
              vxQueryScalar(sensitivity, VX_SCALAR_TYPE, &type, sizeof(type));
        ...
        if (VX_SUCCESS == status) {
          ...
          status |=
              vxQueryScalar(gradient_size, VX_SCALAR_TYPE, &type, sizeof(type));

          if (VX_SUCCESS == status && VX_TYPE_INT32 == type) {
            vx_int32 size = 0;

            status |= vxCopyScalar(gradient_size, &size, VX_READ_ONLY,
                                   VX_MEMORY_TYPE_HOST);

            if (VX_SUCCESS == status && (size == 3 || size == 5 || size == 7)) {
              status = VX_SUCCESS;
            } else
              status = VX_ERROR_INVALID_PARAMETERS;
          } else {
        ...
        if (VX_SUCCESS == status) {
          ...
          status |=
              vxQueryScalar(block_size, VX_SCALAR_TYPE, &type, sizeof(type));

          if (VX_SUCCESS == status && VX_TYPE_INT32 == type) {
            vx_int32 size = 0;

            status |= vxCopyScalar(block_size, &size, VX_READ_ONLY,
                                   VX_MEMORY_TYPE_HOST);
        ...
        if (VX_SUCCESS == status) {
          ...
          status |= vxSetMetaFormatAttribute(metas[5], VX_IMAGE_WIDTH,
                                             &dx_width, sizeof(dx_width));
          status |= vxSetMetaFormatAttribute(metas[5], VX_IMAGE_HEIGHT,
                                             &dx_height, sizeof(dx_height));
          status |= vxSetMetaFormatAttribute(metas[5], VX_IMAGE_FORMAT,
                                             &dst_format, sizeof(dst_format));

          status |= vxSetMetaFormatAttribute(metas[5], VX_VALID_RECT_CALLBACK,
                                             &callback, sizeof(callback));
        }
      }
    }

    if (NULL != dx) vxReleaseImage(&dx);

    if (NULL != dy) vxReleaseImage(&dy);

    if (NULL != sensitivity) vxReleaseScalar(&sensitivity);

    if (NULL != gradient_size) vxReleaseScalar(&gradient_size);
    ...
    if (NULL != param4) vxReleaseParameter(&param4);

    if (NULL != param5) vxReleaseParameter(&param5);
  }  // if ptrs non NULL
...
vx_kernel_description_t harris_score_kernel = {
    ...
    dimof(harris_score_kernel_params),
    own_harris_score_validator,
    NULL,
    NULL,
    NULL,
