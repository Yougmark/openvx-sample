...
static C_KERNEL_INLINE int_fast32_t loadValueAsRawInt(enum TensorCFmt fmt,
                                                      const void *ptr) {
  switch (fmt) {
    case TENSOR_C_FMT_Q78:
      return *(int16_t *)ptr;
...
static C_KERNEL_INLINE void storeRawIntValue(enum TensorCFmt fmt,
                                             int_fast32_t val,
                                             /*OUT*/ void *ptr) {
  switch (fmt) {
    case TENSOR_C_FMT_Q78:
...
static C_KERNEL_INLINE float value2Float(enum TensorCFmt fmt,
                                         int_fast32_t val) {
  switch (fmt) {
    case TENSOR_C_FMT_Q78:
      return short2float((int16_t)val);
    case TENSOR_C_FMT_U8:
      return (float)val;
    case TENSOR_C_FMT_S8:
      return (float)val;
...
static C_KERNEL_INLINE int_fast32_t wrapOrSat(enum TensorCFmt fmt,
                                              int_fast32_t val, bool wrap) {
  switch (fmt) {
    case TENSOR_C_FMT_Q78:
      return wrap ? trunc_to_int16(val) : CLAMP(val, INT16_MIN, INT16_MAX);
    case TENSOR_C_FMT_U8:
      return wrap ? (uint8_t)val : CLAMP(val, 0, UINT8_MAX);
    case TENSOR_C_FMT_S8:
      return wrap ? trunc_to_int8(val) : CLAMP(val, INT8_MIN, INT8_MAX);
    default:
      assert(0);
      return 0;
  }
...
void ConvolutionKernelImpl(
    enum TensorCFmt fmt, const void *input_ptr, tensor_desc_t input,
    const void *weight_ptr, tensor_desc_t weight, const void *bias_ptr,
    tensor_desc_t bias, size_t pad_x, size_t pad_y, size_t stride_x,
    size_t stride_y,
    bool wrap,   // true for WRAP, else SATURATE
    bool to_ne,  // true for ROUND_TO_NE, else ROUND_TO_ZERO (only used for fmt
                 // == TT_MUL)
    size_t dilation_x, size_t dilation_y, void *output_ptr,
    tensor_desc_t output) {
  assert(fmt == TENSOR_C_FMT_Q78 || fmt == TENSOR_C_FMT_U8 ||
         fmt == TENSOR_C_FMT_S8);
  ...
  for (size_t b = 0; b < output_b;
       ++b, in_b_ptr += input.strides[3], out_b_ptr += output.strides[3])
        ...
        for (size_t x = 0; x < output_w; ++x) {
          ...
          if (bias_present) {
            const size_t bias_byte_offset =
                bias_shared ? (bias.strides[0] * ofm)
                            : (bias.strides[2] * ofm + bias.strides[1] * y +
                               bias.strides[0] * x);
              ...
              for (size_t w_x = 0; w_x < weight_w; ++w_x) {
                ...
                if (tmp_x >= pad_x && tmp_x < input_w + pad_x &&
                  ...
                  const size_t weight_byte_offset =
                      weight.strides[3] * ofm + weight.strides[2] * ifm +
                      weight.strides[1] * w_y + weight.strides[0] * w_x;

                  const int_fast32_t i_val =
                      loadValueAsRawInt(fmt, in_b_ptr + input_byte_offset);
                  const int_fast32_t w_val = loadValueAsRawInt(
                      fmt, (char *)weight_ptr + weight_byte_offset);

                  // This is ok since all of them fit into int32_t
                  sum = applyWrapRoundingToAccum(fmt, i_val * w_val, wrap,
                                                 to_ne) +
                        sum;
                }
              }
          ...
          const size_t output_byte_offset = output.strides[2] * ofm +
                                            output.strides[1] * y +
                                            output.strides[0] * x;
    ...
    for (size_t b1 = 0; b1 < tmp_batch_dims[1]; ++b1)
      for (size_t b0 = 0; b0 < tmp_batch_dims[0]; ++b0)
        for (size_t ofm = 0; ofm < ofm_num; ++ofm) {
          int_fast32_t sum =
              bias_present ? loadValueAsRawInt(
                                 fmt, (char *)bias_ptr + bias.strides[0] * ofm)
                           : 0;
            ...
            for (size_t y = 0; y < tmp_input_dims[1]; ++y)
              for (size_t x = 0; x < tmp_input_dims[0]; ++x) {
                size_t weight_byte_offset =
                    weight.strides[weight.dim_num - 1] * ofm;
                if (core_dim_num == 1) {
                  weight_byte_offset += weight.strides[0] * ifm;
                } else if (weight.dim_num == 2) {
                  const size_t count =
                      x + tmp_input_dims[0] * (y + tmp_input_dims[1] * ifm);
                  weight_byte_offset += weight.strides[0] * count;
                } else {
                  weight_byte_offset += weight.strides[2] * ifm +
                                        weight.strides[1] * y +
                                        weight.strides[0] * x;
                }

                const size_t input_byte_offset =
                    (batch_dim_num > 2 ? input.strides[core_dim_num + 2] * b2
                                       : 0) +
                    (batch_dim_num > 1 ? input.strides[core_dim_num + 1] * b1
                                       : 0) +
                    (batch_dim_num > 0 ? input.strides[core_dim_num + 0] * b0
                                       : 0) +
                ...
                const int_fast32_t w_val = loadValueAsRawInt(
                    fmt, (char *)weight_ptr + weight_byte_offset);
                const int_fast32_t i_val = loadValueAsRawInt(
                    fmt, (char *)input_ptr + input_byte_offset);

                // This is ok since all of them fit into int32_t
                sum =
                    applyWrapRoundingToAccum(fmt, i_val * w_val, wrap, to_ne) +
                    sum;
              }
...
void PoolingKernelImpl(enum TensorCFmt fmt, const void *input_ptr,
                       tensor_desc_t input,
                       bool max_pooling,  // MAX vs AVG pooling
                       size_t size_x, size_t size_y, size_t pad_x, size_t pad_y,
                       size_t stride_x, size_t stride_y, void *output_ptr,
                       tensor_desc_t output) {
  assert(input.dim_num == 3 || input.dim_num == 4);
  assert(output.dim_num == input.dim_num);
  ...
  //    assert(missing_div_with_round_mode((input_w + 2 * pad_x - size_x),
  //    stride_x) + 1 == output_w); assert(missing_div_with_round_mode((input_h
  //    + 2 * pad_y - size_y), stride_y) + 1 == output_h);
  ...
  // TODO: previously there was a 1d/3d stride for ofm but there's no 1D pool,
  // right?
    ...
    for (size_t c = 0; c < output_c; ++c)
      for (size_t y = 0; y < output_h; ++y)
        for (size_t x = 0; x < output_w; ++x) {
          ...
          const size_t xx_start =
              CLAMP(x * stride_x, pad_x, input_w + pad_x) - pad_x;
          const size_t xx_after =
              CLAMP(x * stride_x + size_x, pad_x, input_w + pad_x) - pad_x;

          const size_t yy_start =
              CLAMP(y * stride_y, pad_y, input_h + pad_y) - pad_y;
          const size_t yy_after =
              CLAMP(y * stride_y + size_y, pad_y, input_h + pad_y) - pad_y;

          for (size_t yy = yy_start; yy < yy_after; ++yy)
            for (size_t xx = xx_start; xx < xx_after; ++xx) {
              const size_t input_byte_offset = input.strides[2] * c +
                                               input.strides[1] * yy +
                                               input.strides[0] * xx;
              const int32_t i_val =
                  loadValueAsRawInt(fmt, in_b_ptr + input_byte_offset);
          ...
          if (!max_pooling) {
            ...
            result = CLAMP(result / (size_x * size_y), getMinValue(fmt),
                           getMaxValue(fmt));
          }

          const size_t output_byte_offset = output.strides[2] * c +
                                            output.strides[1] * y +
                                            output.strides[0] * x;
...
void SoftmaxKernelImpl(enum TensorCFmt fmt, const void *input_ptr,
                       tensor_desc_t input, void *output_ptr,
                       tensor_desc_t output) {
// TODO: @Tomer, should we allow extra batch dims beyond 4? conv and poll have
// upto 3 of them! if not we can just discard this define and its usage
#define SOFTMAX_ALLOW_EXTRA_DIMS
  ...
  // which describe batching before the main loop for clarity, since the later
  // may be partially shifted depending on the key dim.
      ...
      for (size_t b2 = 0; b2 < batch_sz[2]; ++b2)
        for (size_t b1 = 0; b1 < batch_sz[1]; ++b1)
          for (size_t b0 = 0; b0 < batch_sz[0]; ++b0) {
            // Input and output pointers for the current batch being processed.
            const char *in_b_ptr =
                (char *)input_ptr + batch_in_strides[2] * b2 +
                batch_in_strides[1] * b1 + batch_in_strides[0] * b0;
            char *out_b_ptr = (char *)output_ptr + batch_out_strides[2] * b2 +
            ...
            for (size_t i = 0; i < key_sz; ++i) {
              const int_fast32_t in =
                  loadValueAsRawInt(fmt, in_b_ptr + key_in_stride * i);
            ...
            for (size_t i = 0; i < key_sz; ++i) {
              ...
              storeRawIntValue(fmt, quantize(fmt, expf(in_val) / sum),
                               out_b_ptr + key_in_stride * i);
            }
...
void ROIPoolingKernelImpl(enum TensorCFmt fmt, const void *in0_ptr,
                          tensor_desc_t in0, const void *in1_ptr,
                          tensor_desc_t in1, void *out_ptr, tensor_desc_t out) {
      ...
      for (size_t c = 0; c < out_c; ++c)
        for (size_t y = 0; y < out_h; ++y)
          for (size_t x = 0; x < out_w; ++x) {
            const char *roi_b_ptr =
                (char *)in1_ptr + in1.strides[1] * r + in1.strides[2] * b;

            const int roi_x0 =
                loadValueAsRawInt(fmt, roi_b_ptr + in1.strides[0] * 0);
            const int roi_y0 =
                loadValueAsRawInt(fmt, roi_b_ptr + in1.strides[0] * 1);
            const int roi_x1 =
                loadValueAsRawInt(fmt, roi_b_ptr + in1.strides[0] * 2);
            const int roi_y1 =
                loadValueAsRawInt(fmt, roi_b_ptr + in1.strides[0] * 3);
            ...
            const char *data_b_ptr =
                (char *)in0_ptr + in0.strides[3] * b + in0.strides[2] * c;
            ...
            for (int yy = y_begin; yy < y_after; ++yy)
              for (int xx = x_begin; xx < x_after; ++xx) {
                const void *val_ptr =
                    data_b_ptr + in0.strides[1] * yy + in0.strides[0] * xx;
            ...
            const size_t output_byte_offset =
                out.strides[4] * b + out.strides[3] * r + out.strides[2] * c +
...
void DeconvolutionKernelImpl(
    enum TensorCFmt fmt, const void *input_ptr, tensor_desc_t input,
    const void *weight_ptr, tensor_desc_t weight, const void *bias_ptr,
    tensor_desc_t bias, vx_size pad_x, vx_size pad_y, vx_size upscale_x,
    vx_size upscale_y,
    bool wrap,   // true for WRAP, else SATURATE
    bool to_ne,  // true for ROUND_TO_NE, else ROUND_TO_ZERO
    vx_size a_x, vx_size a_y, void *output_ptr, tensor_desc_t output) {
  assert(input.dim_num == 3 || input.dim_num == 4);
  assert(weight.dim_num == 4);
  assert(bias.dim_num == 0 || bias.dim_num == 1 || bias.dim_num == 3);
  assert(output.dim_num == input.dim_num);
  ...
  // and the stride being 1, the output width comes down to this plus 1 -
  // weight_w which ends up being
    ...
    for (size_t ofm = 0; ofm < output_c; ++ofm)
      for (size_t y = 0; y < output_h; ++y)
        for (size_t x = 0; x < output_w; ++x) {
          ...
          // if, but does the compiler fail to hoist them out???
          const size_t output_byte_offset =
              (b ? output.strides[3] * b : 0) + output.strides[2] * ofm +
              output.strides[1] * y + output.strides[0] * x;
          storeRawIntValue(fmt, sum, out_b_ptr + output_byte_offset);
        }
}
...
static vx_int16 quantize_q78(double x, int bits) {
  ...
  vx_int16 maxVal =
      (1 << (16 - 1)) - 1;  // if numBits == 8, the number should be 127,
                            // allowing range of [-127,+127]
...
// table[0-1] = myRanges, table[2-34] = mySegments, table[35-66] = myA,
// table[67-98] = myB
static pwl_table *createPwlNormLut(vx_float32 beta) {
  ...
  for (int i = 0; i <= PWL_NORM_NUM_SEGMENTS; i++)
    S[i] = (tmp_table[0] +
            i * (tmp_table[1] - tmp_table[0]) / PWL_NORM_NUM_SEGMENTS);
  ...
  for (int i = 0; i < PWL_NORM_NUM_SEGMENTS; i++) {
    ...
    A[i] =
        ((y2 - y1) / (x2 - x1));  // Simple equations of line between 2 points
...
vx_status NNNormalizationKernelImpl(vx_tensor inputs, vx_scalar type_scalar,
                                    vx_scalar norm_size_scalar,
                                    vx_scalar alpha_scalar,
                                    vx_scalar beta_scalar, vx_tensor outputs) {
  ...
  vx_size input_dimensions[MAX_NUM_OF_DIMENSIONS] = {0},
          input_stride[MAX_NUM_OF_DIMENSIONS] = {0};
  vx_size output_dimensions[MAX_NUM_OF_DIMENSIONS] = {0},
          output_stride[MAX_NUM_OF_DIMENSIONS] = {0};
  status |= AllocatePatch(inputs, &num_dims_in, input_dimensions, input_stride,
                          (void **)&pinput, VX_READ_ONLY);
  status |= AllocatePatch(outputs, &num_dims_out, output_dimensions,
                          output_stride, (void **)&poutput, VX_WRITE_ONLY);

  status |=
      vxCopyScalar(type_scalar, &norm_type, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
  status |= vxCopyScalar(norm_size_scalar, &norm_size, VX_READ_ONLY,
                         VX_MEMORY_TYPE_HOST);
  status |=
      vxCopyScalar(alpha_scalar, &alpha_f, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
  status |=
      vxCopyScalar(beta_scalar, &beta_f, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
  ...
  if (norm_type == VX_NN_NORMALIZATION_ACROSS_MAPS) {
    ...
    for (vx_uint32 batch_iter = 0; batch_iter < batch_size; batch_iter++) {
      for (vx_int32 ifm = 0; ifm < (vx_int32)num_ifms; ifm++) {
        max_ifm = ((ifm + (vx_int32)norm_size / 2) > (max_allowed_ifm - 1)
                       ? (max_allowed_ifm - 1)
                       : (ifm + norm_size / 2));
        min_ifm = ((ifm - (vx_int32)norm_size / 2 < min_allowed_ifm)
                       ? min_allowed_ifm
                       : (ifm - norm_size / 2));
        for (vx_int32 y = 0; y < (int)ih; y++) {
          for (vx_int32 x = 0; x < (int)iw; x++) {
            // Scaling factor is expected to be a small number by definition, so
            // 32 bits are more than enough
            vx_int32 pwl_in = 0;
            for (vx_int32 curr_ifm = min_ifm; curr_ifm <= max_ifm; curr_ifm++) {
              offset = (batch_iter * (vx_int32)input_stride[3] +
                        curr_ifm * (vx_int32)input_stride[2] +
                        y * (vx_int32)input_stride[1] +
                        x * (vx_int32)input_stride[0]) /
                       2;  // TODO: Assuming 16 bits elements
              vx_int16 scaled_input =
                  mul_truncate((vx_int32)pinput[offset] * scaling_factor, 8);
            ...
            offset = (batch_iter * input_stride[3] + ifm * input_stride[2] +
                      y * input_stride[1] + x * input_stride[0]) /
                     2;  // TODO: Assuming 16 bits elements
            ...
            // pwl_in, (int)tmp2, (int)poutput[offset]);
          }
        }
      }
    }
          ...
          for (vx_int32 x = 0; x < (int)iw; x++) {
            ...
            for (vx_uint32 ny = 0; ny < norm_size; ny++) {
              for (vx_uint32 nx = 0; nx < norm_size; nx++) {
                ...
                if ((currYind >= 0) && (currXind >= 0) &&
                    (currYind < (int)ih) && (currXind < (int)iw)) {
                  offset = (batch_iter * input_stride[3] +
                            ifm * input_stride[2] + currYind * input_stride[1] +
                            currXind * input_stride[0]) /
                           2;  // TODO: Assuming 16 bits elements
                  vx_int32 scaled_input = mul_truncate(
                      (vx_int32)pinput[offset] * scaling_factor, 8);
                  vx_int32 scaled_input_sq =
                      (vx_int32)scaled_input * scaled_input;
                  pwl_in += scaled_input_sq;
                }
              }
            }
  ...
  status |= ReleasePatch(inputs, num_dims_in, input_dimensions, input_stride,
                         (void **)&pinput, VX_READ_ONLY);
  status |= ReleasePatch(outputs, num_dims_out, output_dimensions,
                         output_stride, (void **)&poutput, VX_WRITE_ONLY);

  return status;
}

void ActivationKernelImpl(enum TensorCFmt fmt, const void *input_ptr,
                          tensor_desc_t input,
                          vx_enum func,  // MAX vs AVG pooling
                          float a, float b, void *output_ptr,
                          tensor_desc_t output) {
  (void)a;
  (void)b;
        ...
        for (size_t x = 0; x < output_w; ++x) {
          ...
          switch (func) { /*
            ...
            case VX_NN_ACTIVATION_HYPERBOLIC_TAN:
              ...
              result =
                  quantize(fmt, 2 / (1 + exp(-2 * value2Float(fmt, i_val))) -
                                    1);  // Use formula instead of LUT, not sure
                                         // at all it works.

              break;
              /* relu and brelu are currently redundant as only unsigned int 16
               * is supported */
            ...
            case VX_NN_ACTIVATION_LOGISTIC:
              result = quantize(
                  fmt,
                  1 / (1 + exp(-value2Float(
                               fmt, i_val))));  // TODO: only for q78, and i am
                                                // not sure it is good!
              break;
            default:
              result = 0;
