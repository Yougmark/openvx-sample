...
vx_status vxThreshold(vx_image src_image, vx_threshold threshold,
  ...
  if (type == VX_THRESHOLD_TYPE_BINARY) {
    vxQueryThreshold(threshold, VX_THRESHOLD_THRESHOLD_VALUE, &value,
                     sizeof(value));
    VX_PRINT(VX_ZONE_INFO, "threshold type binary, value = %u\n", value);
  } else if (type == VX_THRESHOLD_TYPE_RANGE) {
    vxQueryThreshold(threshold, VX_THRESHOLD_THRESHOLD_LOWER, &lower,
                     sizeof(lower));
    vxQueryThreshold(threshold, VX_THRESHOLD_THRESHOLD_UPPER, &upper,
                     sizeof(upper));
    VX_PRINT(VX_ZONE_INFO, "threshold type range, lower = %u, upper = %u\n",
             lower, upper);
  }

  vxQueryThreshold(threshold, VX_THRESHOLD_TRUE_VALUE, &true_value,
                   sizeof(true_value));
  vxQueryThreshold(threshold, VX_THRESHOLD_FALSE_VALUE, &false_value,
                   sizeof(false_value));
  VX_PRINT(VX_ZONE_INFO,
           "threshold true value = %u, threshold false value = %u\n",
  ...
  vxQueryThreshold(threshold, VX_THRESHOLD_INPUT_FORMAT, &format,
                   sizeof(format));
  ...
  for (y = 0; y < src_addr.dim_y; y++) {
    for (x = 0; x < src_addr.dim_x; x++) {
      ...
      if (format == VX_DF_IMAGE_S16) {  // case of input: VX_DF_IMAGE_S16 ->
                                        // output: VX_DF_IMAGE_U8
      ...
      } else {  // case of input: VX_DF_IMAGE_U8  -> output: VX_DF_IMAGE_U8
