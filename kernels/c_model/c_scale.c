...
#define AREA_SCALE_ENABLE                                      \
  0 /* TODO enable this again after changing implementation in \
       sample/targets/vx_scale.c */

static vx_bool read_pixel(void *base, vx_imagepatch_addressing_t *addr,
                          vx_int32 x, vx_int32 y, const vx_border_t *borders,
                          vx_uint8 *pixel) {
  vx_bool out_of_bounds = (x < 0 || y < 0 || x >= (vx_int32)addr->dim_x ||
                           y >= (vx_int32)addr->dim_y);
  vx_uint32 bx, by;
  vx_uint8 *bpixel;
  if (out_of_bounds) {
    if (borders->mode == VX_BORDER_UNDEFINED) return vx_false_e;
...
static vx_bool read_pixel_16s(void *base, vx_imagepatch_addressing_t *addr,
                              vx_int32 x, vx_int32 y,
                              const vx_border_t *borders, vx_int16 *pixel) {
  vx_uint32 bx;
  vx_uint32 by;
  vx_int16 *bpixel;
...
static vx_status vxNearestScaling(vx_image src_image, vx_image dst_image,
  ...
  for (y2 = 0; y2 < (vx_int32)h2; y2++) {
    for (x2 = 0; x2 < (vx_int32)w2; x2++) {
      if (VX_DF_IMAGE_U8 == format) {
        vx_uint8 v = 0;
        vx_uint8 *dst =
            vxFormatImagePatchAddress2d(dst_base, x2, y2, &dst_addr);
        ...
        x1 = (vx_int32)x_min;
        y1 = (vx_int32)y_min;
        if (x_src - x_min >= 0.5f) x1++;
        if (y_src - y_min >= 0.5f) y1++;
        // printf("x2,y2={%u,%u} => x1,y1={%u,%u}\n", x2,y2,x1,y1);
        if (dst &&
            vx_true_e == read_pixel(src_base, &src_addr, x1, y1, borders, &v))
          *dst = v;
      } else {
        vx_int16 v = 0;
        vx_int16 *dst =
        ...
        if (dst && vx_true_e ==
                       read_pixel_16s(src_base, &src_addr, x1, y1, borders, &v))
          *dst = v;
      }
    }
  }
...
static vx_status vxBilinearScaling(vx_image src_image, vx_image dst_image,
                                   const vx_border_t *borders) {
  vx_status status = VX_SUCCESS;
  vx_int32 x2, y2;
  void *src_base = NULL, *dst_base = NULL;
  ...
  for (y2 = 0; y2 < (vx_int32)h2; y2++) {
    for (x2 = 0; x2 < (vx_int32)w2; x2++) {
      ...
      vx_bool defined_tl =
          read_pixel(src_base, &src_addr, x1 + 0, y1 + 0, borders, &tl);
      vx_bool defined_tr =
          read_pixel(src_base, &src_addr, x1 + 1, y1 + 0, borders, &tr);
      vx_bool defined_bl =
          read_pixel(src_base, &src_addr, x1 + 0, y1 + 1, borders, &bl);
      vx_bool defined_br =
          read_pixel(src_base, &src_addr, x1 + 1, y1 + 1, borders, &br);
      ...
      if (defined == vx_false_e) {
        ...
        if (defined_any) {
          if ((defined_tl == vx_false_e || defined_tr == vx_false_e) &&
              fabs(t - 1.0) <= 0.001)
            defined_tl = defined_tr = vx_true_e;
          else if ((defined_bl == vx_false_e || defined_br == vx_false_e) &&
                   fabs(t - 0.0) <= 0.001)
            defined_bl = defined_br = vx_true_e;
          if ((defined_tl == vx_false_e || defined_bl == vx_false_e) &&
              fabs(s - 1.0) <= 0.001)
            defined_tl = defined_bl = vx_true_e;
          else if ((defined_tr == vx_false_e || defined_br == vx_false_e) &&
                   fabs(s - 0.0) <= 0.001)
      ...
      if (defined == vx_true_e) {
        vx_float32 ref = (1 - s) * (1 - t) * tl + (s) * (1 - t) * tr +
                         (1 - s) * (t)*bl + (s) * (t)*br;
        vx_uint8 ref_8u;
        if (ref > 255) ref_8u = 255;
...
static vx_status vxAreaScaling(vx_image src_image, vx_image dst_image,
                               const vx_border_t *borders, vx_float64 *interm,
                               vx_size size) {
  vx_status status = VX_SUCCESS;
  void *src_base = NULL, *dst_base = NULL;
  vx_rectangle_t src_rect, dst_rect;
  vx_imagepatch_addressing_t src_addr, dst_addr;
  vx_uint32 by, bx, y, x, yi, xi, gcd_w, gcd_h, r1w, r1h, r2w, r2h, r_w, b_w,
      b_h;
  vx_uint32 w1 = 0, h1 = 0, w2 = 0, h2 = 0;
    ...
    for (bx = 0; bx < b_w; bx++) {
      ...
      for (y = 0; y < r1h; y++) {
        for (x = 0; x < r1w; x++) {
          ...
          for (yi = 0; yi < r2h; yi++) {
            for (xi = 0; xi < r2w; xi++) {
              vx_uint32 k = ((((y * r2h) + yi) * r_w) + ((x * r2w) + xi));
      ...
      for (y = 0; y < r2h; y++) {
        for (x = 0; x < r2w; x++) {
          ...
          for (yi = 0; yi < r1h; yi++) {
            for (xi = 0; xi < r1w; xi++) {
              vx_uint32 k = ((((y * r1h) + yi) * r_w) + ((x * r1w) + xi));
              sum += interm[k];
            }
          }
