...
static vx_status vxMorphology3x3(vx_image src, vx_image dst,
  ...
  for (y = low_y; (y < high_y) && (status == VX_SUCCESS); y++) {
    for (x = low_x; x < high_x; x++) {
      ...
      vxReadRectangle(src_base, &src_addr, borders, VX_DF_IMAGE_U8, x, y, 1, 1,
                      &pixels);

      m = pixels[0];
      for (i = 1; i < dimof(pixels); i++) m = op(m, pixels[i]);
...
static vx_uint8 min_op(vx_uint8 a, vx_uint8 b) { return a < b ? a : b; }

static vx_uint8 max_op(vx_uint8 a, vx_uint8 b) { return a > b ? a : b; }
