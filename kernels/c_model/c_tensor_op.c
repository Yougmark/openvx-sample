...
#define OP_TEMPLATE(VX_TYPE_, EXPR_)                                        \
  for (vx_size i = 0; i < out_size; ++i) {                                  \
    vx_size in0_byte_offset = getFlatTensorByteOffsetWithBroadcast(         \
        i, input0.dim_num, input0.dims, input0.strides, output.dims);       \
    vx_size in1_byte_offset = getFlatTensorByteOffsetWithBroadcast(         \
        i, input1.dim_num, input1.dims, input1.strides, output.dims);       \
    vx_size out_byte_offset = ComputeGlobalPositionsFromIndex(              \
        i, output.dims, output.strides, output.dim_num, &out_byte_offset);  \
                                                                            \
  ...
  // input_byte_offset == ... + (input0.dims[3] > 1 ? i3 * input0.strides[3] :
  // 0) + ... We create an extra array of strides containning 0s if the input
  // dim is being broadcasted here, to avoid the extra contionals in the loop
          ...
          for (vx_size i1 = 0; i1 < output_dim1; ++i1)                 \
            for (vx_size i0 = 0; i0 < output_dim0; ++i0) {             \
              vx_size in0_byte_offset =                                \
                  in0_strides[5] * i5 + in0_strides[4] * i4 +          \
                  in0_strides[3] * i3 + in0_strides[2] * i2 +          \
                  in0_strides[1] * i1 + in0_strides[0] * i0;           \
              vx_size in1_byte_offset =                                \
                  in1_strides[5] * i5 + in1_strides[4] * i4 +          \
                  in1_strides[3] * i3 + in1_strides[2] * i2 +          \
                  in1_strides[1] * i1 + in1_strides[0] * i0;           \
              vx_size out_byte_offset =                                \
                  output.strides[5] * i5 + output.strides[4] * i4 +    \
                  output.strides[3] * i3 + output.strides[2] * i2 +    \
                  output.strides[1] * i1 + output.strides[0] * i0;     \
                                                                       \
              VX_TYPE_ in0_val =                                       \
                  *(VX_TYPE_ *)((char *)input0_ptr + in0_byte_offset); \
              VX_TYPE_ in1_val =                                       \
                  *(VX_TYPE_ *)((char *)input1_ptr + in1_byte_offset); \
              VX_TYPE_ *res_ptr =                                      \
                  (VX_TYPE_ *)((char *)output_ptr + out_byte_offset);  \
                                                                       \
              do {                                                     \
                EXPR_                                                  \
              } while (0);                                             \
  ...
  switch (fmt) {
    case TENSOR_C_FMT_Q78: {
      switch (op) {
        case ELEMENTWISE_TENSOR_ADD:
          if (wrap) {
            OP_TEMPLATE(int16_t, { *res_ptr = (int16_t)(in0_val + in1_val); });
          }  // TODO: cast issue?
          else {
            OP_TEMPLATE(int16_t,
                        { *res_ptr = conversion_24_8(in0_val + in1_val); });
          }
          break;
        ...
        case ELEMENTWISE_TENSOR_MUL: {
          ...
          if (wrap) {
            if (to_ne) {
              OP_TEMPLATE(vx_int16, {
                *res_ptr = (int16_t)nearbyint(in0_val * in1_val * new_scale);
              });
            }  // TODO: cast issue?
            else {
              OP_TEMPLATE(vx_int16, {
                *res_ptr = (int16_t)trunc(in0_val * in1_val * new_scale);
              });
            }  // TODO: cast issue?
          } else {
            if (to_ne) {
              OP_TEMPLATE(vx_int16, {
                *res_ptr = (vx_int16)conversion_24_8(
                    (uint32_t)nearbyint(in0_val * in1_val * new_scale));
              });
            } else {
              OP_TEMPLATE(vx_int16, {
                *res_ptr = (vx_int16)conversion_24_8(
                    (uint32_t)trunc(in0_val * in1_val * new_scale));
              });
            }
          }
        } break;
    ...
    case TENSOR_C_FMT_U8: {
      switch (op) {
        case ELEMENTWISE_TENSOR_ADD:
          if (wrap) {
            OP_TEMPLATE(uint8_t, { *res_ptr = in0_val + in1_val; });
          } else {
            OP_TEMPLATE(uint8_t,
                        { *res_ptr = CLAMP(in0_val + in1_val, 0, UINT8_MAX); });
          }
          break;
        case ELEMENTWISE_TENSOR_SUB:
          if (wrap) {
        ...
        case ELEMENTWISE_TENSOR_MUL:
          if (wrap) {
            if (to_ne) {
              OP_TEMPLATE(uint8_t, {
                *res_ptr = (uint8_t)nearbyint(in0_val * in1_val * scale);
              });
            } else {
              OP_TEMPLATE(uint8_t, {
                *res_ptr = (uint8_t)trunc(in0_val * in1_val * scale);
              });
            }
          } else {
            if (to_ne) {
              OP_TEMPLATE(uint8_t, {
                *res_ptr = (uint8_t)CLAMP(nearbyint(in0_val * in1_val * scale),
                                          0, UINT8_MAX);
              });
            } else {
              OP_TEMPLATE(uint8_t, {
                *res_ptr = (uint8_t)CLAMP(trunc(in0_val * in1_val * scale), 0,
                                          UINT8_MAX);
              });
            }
          }
    ...
    case TENSOR_C_FMT_S8: {
      switch (op) {
        case ELEMENTWISE_TENSOR_ADD:
          if (wrap) {
            OP_TEMPLATE(int8_t, { *res_ptr = (int8_t)(in0_val + in1_val); });
          }  // TODO: cast issue?
          else {
            OP_TEMPLATE(int8_t, {
              *res_ptr = CLAMP(in0_val + in1_val, INT8_MIN, INT8_MAX);
            });
          }
          break;
        case ELEMENTWISE_TENSOR_SUB:
        ...
        case ELEMENTWISE_TENSOR_MUL:
          if (wrap) {
            if (to_ne) {
              OP_TEMPLATE(int8_t, {
                *res_ptr = (int8_t)nearbyint(in0_val * in1_val * scale);
              });
            }  // TODO: cast issue?
            else {
              OP_TEMPLATE(int8_t, {
                *res_ptr = (int8_t)trunc(in0_val * in1_val * scale);
              });
            }  // TODO: cast issue?
          } else {
            if (to_ne) {
              OP_TEMPLATE(int8_t, {
                *res_ptr = (int8_t)CLAMP(nearbyint(in0_val * in1_val * scale),
                                         INT8_MIN, INT8_MAX);
              });
            } else {
              OP_TEMPLATE(int8_t, {
                *res_ptr = (int8_t)CLAMP(trunc(in0_val * in1_val * scale),
                                         INT8_MIN, INT8_MAX);
              });
            }
          }
...
static C_KERNEL_INLINE void singleValueDepthConvert(const void *in,
                                                    enum TensorCFmt src_fmt,
                                                    bool wrap, float scale,
                                                    float offset,
                                                    /*OUT*/ void *out,
  ...
  switch (dst_fmt) {
    case TENSOR_C_FMT_Q78:
      tmp *= 1 << Q78_FIXED_POINT_POSITION;
      *(int16_t *)out = wrap ? (int16_t)tmp
                             : (int16_t)CLAMP(tmp, INT16_MIN,
                                              INT16_MAX);  // TODO: cast issue?
      break;
    case TENSOR_C_FMT_U8:
      *(uint8_t *)out =
          wrap ? (uint8_t)tmp
               : (uint8_t)CLAMP(tmp, 0, UINT8_MAX);  // CLAMP not needed
      break;
    case TENSOR_C_FMT_S8:
      *(int8_t *)out =
          wrap ? (int8_t)tmp
               : (int8_t)CLAMP(tmp, INT8_MIN, INT8_MAX);  // TODO: cast issue?
      break;
    default:
      assert(0);
  }
...
void TensorConvertDepthKernelImpl(const void *input_ptr, tensor_desc_t input,
  ...
  for (size_t i = 0; i < input.dim_num; ++i)
    assert(output.dims[i] == input.dims[i]);
  ...
  for (size_t i = 0; i < count; ++i) {
    size_t input_byte_offset = ComputeGlobalPositionsFromIndex(
        i, input.dims, input.strides, input.dim_num, &input_byte_offset);
    size_t output_byte_offset = ComputeGlobalPositionsFromIndex(
        i, output.dims, output.strides, output.dim_num, &output_byte_offset);
          ...
          for (size_t i1 = 0; i1 < output_dim1; ++i1)
            for (size_t i0 = 0; i0 < output_dim0; ++i0) {
              const size_t input_byte_offset =
                  input.strides[5] * i5 + input.strides[4] * i4 +
                  input.strides[3] * i3 + input.strides[2] * i2 +
                  input.strides[1] * i1 + input.strides[0] * i0;
              ...
              singleValueDepthConvert(in, src_fmt, wrap, scale, offset, out,
                                      dst_fmt);
            }
#endif
}
