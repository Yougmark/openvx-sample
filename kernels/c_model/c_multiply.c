...
vx_status vxMultiply(vx_image in0, vx_image in1, vx_scalar scale_param,
  ...
  status |= vxAccessImagePatch(output, &rect, 0, &dst_addr, (void **)&dst_base,
                               VX_WRITE_ONLY);
  status |=
      vxCopyScalar(scale_param, &scale, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
  status |= vxCopyScalar(opolicy_param, &overflow_policy, VX_READ_ONLY,
                         VX_MEMORY_TYPE_HOST);
  status |= vxCopyScalar(rpolicy_param, &rounding_policy, VX_READ_ONLY,
                         VX_MEMORY_TYPE_HOST);
    ...
    for (x = 0; x < dst_addr.dim_x; x++) {
      ...
      } else /* Already verified to be VX_CONVERT_POLICY_WRAP. */
      {
        ...
        final_result_value = (out_format == VX_DF_IMAGE_U8)
                                 ? (vx_uint8)int_typed_result
                                 : (vx_int16)int_typed_result;
      }
