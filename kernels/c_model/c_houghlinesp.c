...
unsigned vx_rng_next() {
  state = (vx_uint64)(unsigned)state * /*CV_RNG_COEFF*/ 4164903690U +
          (unsigned)(state >> 32);
  return (unsigned)state;
}

vx_int32 vx_rng_uniform(vx_int32 a, vx_int32 b) {
  return a == b ? a : (vx_int32)(vx_rng_next() % (b - a) + a);
}
...
vx_status vxHoughLinesP(vx_image img, vx_array param_hough_lines_array,
  ...
  vxQueryArray(param_hough_lines_array, VX_ARRAY_NUMITEMS,
               &param_hough_lines_array_length,
               sizeof(param_hough_lines_array_length));
  vxMapArrayRange(param_hough_lines_array, 0, param_hough_lines_array_length,
                  &param_hough_lines_array_map_id,
                  &param_hough_lines_array_stride, &param_hough_lines_array_ptr,
                  VX_READ_AND_WRITE, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  vx_hough_lines_p_t *param_hough_lines =
      (vx_hough_lines_p_t *)param_hough_lines_array_ptr;
  ...
  vx_array accum = vxCreateArray(context_houghlines_internal, VX_TYPE_INT32,
                                 numrho * numangle);
  vx_array mask =
      vxCreateArray(context_houghlines_internal, VX_TYPE_UINT8, width * height);
  vx_array trigtab =
      vxCreateArray(context_houghlines_internal, VX_TYPE_FLOAT32, numangle * 2);
  vx_array nzloc = vxCreateArray(context_houghlines_internal,
                                 VX_TYPE_COORDINATES2D, width * height);
  ...
  vxMapArrayRange(accum, 0, numrho * numangle, &accum_map_id, &accum_stride,
                  &accum_ptr, VX_READ_AND_WRITE, VX_MEMORY_TYPE_HOST,
                  VX_NOGAP_X);
  vx_int32 *accum_p = (vx_int32 *)accum_ptr;

  for (int n = 0; n < numangle; n++) {
    vx_float32 cos_v =
        (vx_float32)(cos((vx_float64)n * (param_hough_lines->theta)) * irho);
    vx_float32 sin_v =
        (vx_float32)(sin((vx_float64)n * (param_hough_lines->theta)) * irho);
  ...
  vxMapArrayRange(trigtab, 0, numangle * 2, &trigtab_map_id, &trigtab_stride,
                  &trigtab_ptr, VX_READ_AND_WRITE, VX_MEMORY_TYPE_HOST,
                  VX_NOGAP_X);
  ...
  for (pt.y = 0; pt.y < (vx_uint32)height; pt.y++) {
    const vx_uint8 *data =
        vxFormatImagePatchAddress2d(src_base, 0, pt.y, &src_addr);
  ...
  vxMapArrayRange(mask, 0, width * height, &mask_map_id, &mask_stride,
                  &mask_ptr, VX_READ_AND_WRITE, VX_MEMORY_TYPE_HOST,
                  VX_NOGAP_X);
  vxMapArrayRange(nzloc, 0, nzcount, &nzloc_map_id, &nzloc_stride, &nzloc_ptr,
                  VX_READ_AND_WRITE, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  ...
  for (; nzcount > 0; nzcount--) {
    ...
    if (max_val < param_hough_lines->threshold) continue;
    ...
    good_line = abs((int)line_end[1].x - (int)line_end[0].x) >=
                    param_hough_lines->line_length ||
                abs((int)line_end[1].y - (int)line_end[0].y) >=
                    param_hough_lines->line_length;
    ...
    if (good_line) {
      vx_line2d_t lines;
      if ((line_end[0].x > line_end[1].x) ||
          (line_end[0].x == line_end[1].x && line_end[0].y > line_end[1].y)) {
  ...
  status |= vxUnmapArrayRange(param_hough_lines_array,
                              param_hough_lines_array_map_id);
