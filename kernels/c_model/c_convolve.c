...
vx_status vxConvolve(vx_image src, vx_convolution conv, vx_image dst,
  ...
  if (bordermode->mode == VX_BORDER_UNDEFINED) {
    low_x = conv_radius_x;
    high_x = ((src_addr.dim_x >= (vx_uint32)conv_radius_x)
                  ? src_addr.dim_x - conv_radius_x
                  : 0);
    low_y = conv_radius_y;
    high_y = ((src_addr.dim_y >= (vx_uint32)conv_radius_y)
                  ? src_addr.dim_y - conv_radius_y
                  : 0);
    vxAlterRectangle(&rect, conv_radius_x, conv_radius_y, -conv_radius_x,
                     -conv_radius_y);
  } else {
    low_x = 0;
  ...
  for (y = low_y; y < high_y; ++y) {
    for (x = low_x; x < high_x; ++x) {
      ...
      if (src_format == VX_DF_IMAGE_U8) {
        ...
        vxReadRectangle(src_base, &src_addr, bordermode, src_format, x, y,
