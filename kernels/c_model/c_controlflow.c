...
static vx_status logical_operation(vx_enum operation, vx_scalar a, vx_scalar b,
                                   vx_scalar output);
static vx_status comparison_operation(vx_enum operation, vx_scalar a,
                                      vx_scalar b, vx_scalar output);
static vx_status arithmetic_operation(vx_enum operation, vx_scalar a,
                                      vx_scalar b, vx_scalar output);
...
vx_status vxSelect(vx_scalar condition_param, vx_reference true_value,
                   vx_reference false_value, vx_reference output) {
  vx_status status = VX_SUCCESS;
  vx_bool condition = vx_false_e;
  status = vxCopyScalar(condition_param, &condition, VX_READ_ONLY,
                        VX_MEMORY_TYPE_HOST);
...
static vx_status logical_operation(vx_enum operation, vx_scalar a, vx_scalar b,
  ...
  if ((a_type != VX_TYPE_BOOL) || (b_type != VX_TYPE_BOOL) ||
      (o_type != VX_TYPE_BOOL)) {
  ...
  if (operation == VX_SCALAR_OP_AND) {
    ...
    o_value = ((a_value && !b_value) || (!a_value && b_value)) ? vx_true_e
                                                               : vx_false_e;
    status = vxWriteScalarValue(output, &o_value);
...
#define CREATE_CONVERT_AND_WRITE_FUNCTION(INPUT_TYPE)                      \
  static vx_status convert_and_write_##INPUT_TYPE(INPUT_TYPE value,        \
                                                  vx_scalar a) {           \
    vx_status status = VX_SUCCESS;                                         \
    vx_enum a_type = VX_TYPE_INVALID;                                      \
    vxQueryScalar(a, VX_SCALAR_TYPE, &a_type, sizeof(a_type));             \
    switch (a_type) {                                                      \
      case VX_TYPE_FLOAT32: {                                              \
        ...
        status =                                                           \
            vxCopyScalar(a, &a_value, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST); \
        break;                                                             \
      }                                                                    \
      case VX_TYPE_INT8: {                                                 \
...
//    vx_bool compare_operation_vx_float32(vx_enum operation, vx_float32 a,
//    vx_float32 b); vx_bool compare_operation_vx_int32(vx_enum operation,
//    vx_int32 a, vx_int32 b); vx_bool compare_operation_vx_size(vx_enum
//    operation, vx_size a, vx_size b);
//
#define CREATE_COMPARE_OPERATION(INPUT_TYPE)                       \
  static vx_bool compare_operation_##INPUT_TYPE(                   \
      vx_enum operation, INPUT_TYPE a_value, INPUT_TYPE b_value) { \
    vx_bool o_value = vx_false_e;                                  \
    switch (operation) {                                           \
      case VX_SCALAR_OP_EQUAL:                                     \
        o_value = (a_value == b_value) ? vx_true_e : vx_false_e;   \
...
//    vx_bool arithmetic_operation_vx_float32(vx_enum operation, vx_float32 a,
//    vx_float32 b); vx_bool arithmetic_operation_vx_int32(vx_enum operation,
//    vx_int32 a, vx_int32 b); vx_bool arithmetic_operation_vx_size(vx_enum
//    operation, vx_size a, vx_size b);
//
#define CREATE_ARITHMETIC_OPERATION(INPUT_TYPE)                    \
  static INPUT_TYPE arithmetic_operation_##INPUT_TYPE(             \
      vx_enum operation, INPUT_TYPE a_value, INPUT_TYPE b_value) { \
...
static vx_status comparison_operation(vx_enum operation, vx_scalar a,
  ...
  } else if (a_type == VX_TYPE_INT8 || a_type == VX_TYPE_INT16 ||
             a_type == VX_TYPE_INT32 || b_type == VX_TYPE_INT8 ||
             b_type == VX_TYPE_INT16 || b_type == VX_TYPE_INT32) {
...
static vx_status arithmetic_operation(vx_enum operation, vx_scalar a,
  ...
  // TODO: need special handling for mixing VX_SIZE and integer for 64-bit
  // hardware
  if (a_type == VX_TYPE_FLOAT32 || b_type == VX_TYPE_FLOAT32) {
    vx_float32 a_value = read_and_convert_to_vx_float32(a);
    vx_float32 b_value = read_and_convert_to_vx_float32(b);
    vx_float32 o_value =
        arithmetic_operation_vx_float32(operation, a_value, b_value);
    ...
    vx_int32 b_value = read_and_convert_to_vx_int32(b);
    vx_int32 o_value =
        arithmetic_operation_vx_int32(operation, a_value, b_value);
