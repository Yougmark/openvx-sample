  ...
  if ((format == VX_DF_IMAGE_RGB) || (format == VX_DF_IMAGE_RGBX)) {
    ...
    for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y) {
      for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x) {
        ...
        uint8_t *dst =
            vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
        dst[0] = planes[0][0];
        dst[1] = planes[1][0];
        dst[2] = planes[2][0];
        if (format == VX_DF_IMAGE_RGBX) {
          planes[3] = vxFormatImagePatchAddress2d(base_src_ptrs[3], x, y,
                                                  &src_addrs[3]);
          dst[3] = planes[3][0];
        }
      }
    ...
    for (p = 0; p < 3; p++) {
      ...
      // get the plane
      vxAccessImagePatch(inputs[p], &rect, 0, &src_addr, &base_src_ptr,
                         VX_READ_ONLY);
      vxAccessImagePatch(output, &rect, p, &dst_addr, &base_dst_ptr,
                         VX_WRITE_ONLY);
      ...
      for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y) {
        for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x) {
          ...
          uint8_t *src =
              vxFormatImagePatchAddress2d(base_src_ptr, x1, y1, &src_addr);
          uint8_t *dst =
              vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
          *dst = *src;
        }
    ...
    // plane 0
    {
      ...
      // get the plane
      vxAccessImagePatch(inputs[0], &rect, 0, &src_addr, &base_src_ptr,
                         VX_READ_ONLY);
      vxAccessImagePatch(output, &rect, 0, &dst_addr, &base_dst_ptr,
                         VX_WRITE_ONLY);
      ...
      for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y) {
        for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x) {
          uint8_t *src =
              vxFormatImagePatchAddress2d(base_src_ptr, x, y, &src_addr);
          uint8_t *dst =
              vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
          *dst = *src;
        }
    ...
    // plane 1
    {
      ...
      // get the plane
      vxAccessImagePatch(inputs[1], &rect, 0, &src0_addr, &base_src0_ptr,
                         VX_READ_ONLY);
      vxAccessImagePatch(inputs[2], &rect, 0, &src1_addr, &base_src1_ptr,
                         VX_READ_ONLY);
      vxAccessImagePatch(output, &rect, 1, &dst_addr, &base_dst_ptr,
                         VX_WRITE_ONLY);
      ...
      for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y) {
        for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x) {
          ...
          uint8_t *src0 =
              vxFormatImagePatchAddress2d(base_src0_ptr, x1, y1, &src0_addr);
          uint8_t *src1 =
              vxFormatImagePatchAddress2d(base_src1_ptr, x1, y1, &src1_addr);
          uint8_t *dst =
              vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
          dst[1 - vidx] = *src0;
          dst[vidx] = *src1;
    ...
    vxAccessImagePatch(inputs[0], &rect, 0, &src0_addr, &base_src0_ptr,
                       VX_READ_ONLY);
    vxAccessImagePatch(inputs[1], &rect, 0, &src1_addr, &base_src1_ptr,
                       VX_READ_ONLY);
    vxAccessImagePatch(inputs[2], &rect, 0, &src2_addr, &base_src2_ptr,
                       VX_READ_ONLY);
    vxAccessImagePatch(output, &rect, 0, &dst_addr, &base_dst_ptr,
                       VX_WRITE_ONLY);
    ...
    for (y = 0; y < dst_addr.dim_y; y += dst_addr.step_y) {
      for (x = 0; x < dst_addr.dim_x; x += dst_addr.step_x * 2) {
        ...
        uint8_t *srcy0 =
            vxFormatImagePatchAddress2d(base_src0_ptr, x1, y1, &src0_addr);
        uint8_t *srcy1 = vxFormatImagePatchAddress2d(
            base_src0_ptr, x1 + src0_addr.step_x, y1, &src0_addr);
        uint8_t *srcu =
            vxFormatImagePatchAddress2d(base_src1_ptr, x2, y2, &src1_addr);
        uint8_t *srcv =
            vxFormatImagePatchAddress2d(base_src2_ptr, x2, y2, &src2_addr);
        uint8_t *dst0 =
            vxFormatImagePatchAddress2d(base_dst_ptr, x, y, &dst_addr);
        uint8_t *dst1 = vxFormatImagePatchAddress2d(
            base_dst_ptr, x + dst_addr.step_x, y, &dst_addr);
        dst0[yidx] = *srcy0;
        dst1[yidx] = *srcy1;
        dst0[1 - yidx] = *srcu;
        dst1[1 - yidx] = *srcv;
...
static vx_status vxCopyPlaneToImage(vx_image src, vx_uint32 src_plane,
                                    vx_uint8 src_component,
                                    vx_uint32 x_subsampling, vx_image dst) {
  void *src_base = NULL;
  void *dst_base = NULL;
  ...
  status |= vxAccessImagePatch(src, &src_rect, src_plane, &src_addr, &src_base,
                               VX_READ_ONLY);
  if (status == VX_SUCCESS) {
    ...
    status = vxAccessImagePatch(dst, &dst_rect, 0, &dst_addr, &dst_base,
                                VX_WRITE_ONLY);
    if (status == VX_SUCCESS) {
      for (y = 0; y < dst_addr.dim_y; y++) {
        for (x = 0; x < dst_addr.dim_x; x++) {
          vx_uint8 *srcp = vxFormatImagePatchAddress2d(
              src_base, x * VX_SCALE_UNITY / src_addr.scale_x * x_subsampling,
              y * VX_SCALE_UNITY / src_addr.scale_y, &src_addr);
          vx_uint8 *dstp =
...
vx_status vxChannelExtract(vx_image src, vx_scalar channel, vx_image dst) {
  ...
  switch (format) {
    ...
    case VX_DF_IMAGE_UYVY:
      if (chan == VX_CHANNEL_Y)
        status = vxCopyPlaneToImage(src, 0, format == VX_DF_IMAGE_YUYV ? 0 : 1,
                                    1, dst);
      else
        status = vxCopyPlaneToImage(src, 0,
                                    (format == VX_DF_IMAGE_YUYV ? 1 : 0) +
                                        (chan == VX_CHANNEL_U ? 0 : 2),
                                    2, dst);
      break;
  }
  return status;
