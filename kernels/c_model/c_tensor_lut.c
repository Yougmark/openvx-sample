...
vx_status vxTensorTableLookup(void *src, vx_size *src_strides, vx_size *dims,
  ...
  for (vx_size i = 0; i < out_size; i++) {
    ...
    vx_size src_pos = ComputeGlobalPositionsFromIndex(i, dims, src_strides,
                                                      num_of_dims, &src_pos0);
    vx_size dst_pos = ComputeGlobalPositionsFromIndex(i, dims, dst_strides,
                                                      num_of_dims, &dst_pos0);
