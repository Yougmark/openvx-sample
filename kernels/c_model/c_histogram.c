...
vx_status vxHistogram(vx_image src, vx_distribution dist) {
  ...
  status |= vxMapImagePatch(src, &src_rect, 0, &src_map_id, &src_addr,
                            &src_base, VX_READ_ONLY, VX_MEMORY_TYPE_HOST, 0);
  status |= vxMapDistribution(dist, &dst_map_id, &dist_ptr, VX_WRITE_ONLY,
    ...
    for (y = 0; y < src_addr.dim_y; y++) {
      for (x = 0; x < src_addr.dim_x; x++) {
        if (format == VX_DF_IMAGE_U8) {
          ...
          if (((vx_size)offset <= (vx_size)pixel) &&
              ((vx_size)pixel < (vx_size)(offset + range))) {
...
vx_status vxEqualizeHist(vx_image src, vx_image dst) {
  ...
  if (status == VX_SUCCESS) {
/* for 16-bit support (U16 or S16), the code can be duplicated with NUM_BINS =
 * 65536 and PIXEL = vx_uint16. */
#define NUM_BINS 256

    /* allocate a fixed-size temp array to store the image histogram &
