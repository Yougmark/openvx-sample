...
static C_KERNEL_INLINE void storeSatFormatted(int val, const void* ptr,
                                              vx_enum type) {
  switch (type) {
    case VX_TYPE_INT8:
      *(vx_int8*)ptr = CLAMP(val, INT8_MIN, INT8_MAX);
      break;
    case VX_TYPE_UINT8:
      *(vx_uint8*)ptr = CLAMP(val, 0, UINT8_MAX);
      break;
    case VX_TYPE_INT16:
      *(vx_int16*)ptr = CLAMP(val / Q78_SCALE, INT16_MIN, INT16_MAX);
      break;
    default:
      assert(0);
  }
...
static C_KERNEL_INLINE int addToMulAccumFormatted(int sum, int val,
                                                  vx_enum type) {
  switch (type) {
    case VX_TYPE_INT8:
      return sum + val;
  ...
  for (size_t i = 0; i < dims1[1]; i++)
    for (size_t j = 0; j < dims2[0]; j++) {
      ...
      for (size_t k = 0; k < dims1[0]; ++k) {
        const void* src1_ptr =
            (char*)src1 + src1_strides[1] * i + src1_strides[0] * k;
        const void* src2_ptr =
            (char*)src2 + src2_strides[1] * k + src2_strides[0] * j;
