...
static vx_int32 vx_add_op(vx_int32 a, vx_int32 b) { return (a + b); }

static vx_int32 vx_sub_op(vx_int32 a, vx_int32 b) { return (a - b); }

// generic arithmetic op
static vx_status vxBinaryU8S16OverflowOp(vx_image in0, vx_image in1,
                                         vx_scalar policy_param,
                                         vx_image output, arithmeticOp op) {
  ...
  status |= vxAccessImagePatch(in0, &rect, 0, &src_addr[0],
                               (void **)&src_base[0], VX_READ_ONLY);
  status |= vxAccessImagePatch(in1, &rect, 0, &src_addr[1],
                               (void **)&src_base[1], VX_READ_ONLY);
  status |= vxAccessImagePatch(output, &rect, 0, &dst_addr, (void **)&dst_base,
                               VX_WRITE_ONLY);
  status |= vxCopyScalar(policy_param, &overflow_policy, VX_READ_ONLY,
                         VX_MEMORY_TYPE_HOST);
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      ...
      vx_int32 src0 = in0_format == VX_DF_IMAGE_U8 ? *(vx_uint8 *)src0p
                                                   : *(vx_int16 *)src0p;
      vx_int32 src1 = in1_format == VX_DF_IMAGE_U8 ? *(vx_uint8 *)src1p
                                                   : *(vx_int16 *)src1p;
      ...
      } else /* Already verified to be VX_CONVERT_POLICY_WRAP. */
      {
        ...
        final_result_value = (out_format == VX_DF_IMAGE_U8)
                                 ? (vx_uint8)overflowing_result
                                 : (vx_int16)overflowing_result;
      }
