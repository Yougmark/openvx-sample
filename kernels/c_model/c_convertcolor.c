...
static void yuv2rgb_bt601(vx_uint8 y, vx_uint8 cb, vx_uint8 cr, vx_uint8 *r,
                          vx_uint8 *g, vx_uint8 *b) {
  /*
  R'= Y' + 0.000*U' + 1.403*V'
  G'= Y' - 0.344*U' - 0.714*V'
  B'= Y' + 1.773*U' + 0.000*V'
...
static void yuv2rgb_bt709(vx_uint8 y, vx_uint8 cb, vx_uint8 cr, vx_uint8 *r,
                          vx_uint8 *g, vx_uint8 *b) {
  /*
...
static void rgb2yuv_bt709(vx_uint8 r, vx_uint8 g, vx_uint8 b, vx_uint8 *y,
                          vx_uint8 *cb, vx_uint8 *cr) {
  /*
...
vx_status vxConvertColor(vx_image src, vx_image dst) {
    ...
    if (dst_format == VX_DF_IMAGE_RGB || dst_format == VX_DF_IMAGE_RGBX) {
      for (y = 0; y < dst_addr[0].dim_y; y++) {
        for (x = 0; x < dst_addr[0].dim_x; x++) {
          ...
          dst_rgb[1] = src_rgb[1];
          dst_rgb[2] = src_rgb[2];
          if (dst_format == VX_DF_IMAGE_RGBX) dst_rgb[3] = 255;
        }
      }
      ...
      for (y = 0; y < dst_addr[0].dim_y; y += 2) {
        for (x = 0; x < dst_addr[0].dim_x; x += 2) {
          vx_uint8 *rgb[4] = {
              ...
              vxFormatImagePatchAddress2d(src_base[0], x + 1, y + 1,
                                          &src_addr[0])};
          vx_uint8 *luma[4] = {
              vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]),
              vxFormatImagePatchAddress2d(dst_base[0], x + 1, y, &dst_addr[0]),
              vxFormatImagePatchAddress2d(dst_base[0], x, y + 1, &dst_addr[0]),
              vxFormatImagePatchAddress2d(dst_base[0], x + 1, y + 1,
                                          &dst_addr[0])};
          vx_uint8 *cbcr =
              vxFormatImagePatchAddress2d(dst_base[1], x, y, &dst_addr[1]);
          rgb2yuv_bt709(rgb[0][0], rgb[0][1], rgb[0][2], &luma[0][0], &cb[0],
                        &cr[0]);
          rgb2yuv_bt709(rgb[1][0], rgb[1][1], rgb[1][2], &luma[1][0], &cb[1],
                        &cr[1]);
          rgb2yuv_bt709(rgb[2][0], rgb[2][1], rgb[2][2], &luma[2][0], &cb[2],
                        &cr[2]);
          rgb2yuv_bt709(rgb[3][0], rgb[3][1], rgb[3][2], &luma[3][0], &cb[3],
                        &cr[3]);
      ...
      for (y = 0; y < dst_addr[0].dim_y; y += 2) {
        for (x = 0; x < dst_addr[0].dim_x; x += 2) {
          ...
          cbp[0] = (uint8_t)(((vx_uint16)cb[0] + (vx_uint16)cb[1] +
                              (vx_uint16)cb[2] + (vx_uint16)cb[3]) >>
                             2);
          crp[0] = (uint8_t)(((vx_uint16)cr[0] + (vx_uint16)cr[1] +
                              (vx_uint16)cr[2] + (vx_uint16)cr[3]) >>
                             2);
        }
      }
    }
    ...
    if ((dst_format == VX_DF_IMAGE_RGB) || (dst_format == VX_DF_IMAGE_RGBX)) {
      for (y = 0; y < dst_addr[0].dim_y; y++) {
        for (x = 0; x < dst_addr[0].dim_x; x++) {
          ...
          vx_uint8 *rgb =
              vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
          if (dst_format == VX_DF_IMAGE_RGBX) rgb[3] = 255;

          if (src_space == VX_COLOR_SPACE_BT601_525 ||
              src_space == VX_COLOR_SPACE_BT601_625)
            yuv2rgb_bt601(luma[0], crcb[u_pix], crcb[v_pix], &rgb[0], &rgb[1],
                          &rgb[2]);
          else /*if (src_space == VX_COLOR_SPACE_BT709)*/
            yuv2rgb_bt709(luma[0], crcb[u_pix], crcb[v_pix], &rgb[0], &rgb[1],
                          &rgb[2]);
        }
      }
    } else if (dst_format == VX_DF_IMAGE_NV12 ||
               dst_format == VX_DF_IMAGE_NV21) {
      for (y = 0; y < dst_addr[0].dim_y; y++) {
        for (x = 0; x < dst_addr[0].dim_x; x++) {
          vx_uint8 *luma[2] = {
              vxFormatImagePatchAddress2d(src_base[0], x, y, &src_addr[0]),
          ...
          vx_uint8 *crcb =
              vxFormatImagePatchAddress2d(dst_base[1], x, y, &dst_addr[1]);
          yuv2yuv_601to709(luma[0][0], cbcr[0], cbcr[1], &luma[1][0], &crcb[1],
                           &crcb[0]);
        }
      }
    } else if (dst_format == VX_DF_IMAGE_YUV4) {
      ...
      for (y = 0; y < dst_addr[0].dim_y; y++) {
        for (x = 0; x < dst_addr[0].dim_x; x++) {
          ...
          vx_uint8 *cb =
              vxFormatImagePatchAddress2d(src_base[1], x, y, &src_addr[1]);
          vx_uint8 *cr =
              vxFormatImagePatchAddress2d(src_base[2], x, y, &src_addr[2]);
          vx_uint8 *rgb =
              vxFormatImagePatchAddress2d(dst_base[0], x, y, &dst_addr[0]);
          if (dst_format == VX_DF_IMAGE_RGBX) rgb[3] = 255;
  ...
  if (status != VX_SUCCESS) {
    VX_PRINT(VX_ZONE_ERROR,
             "Failed to set image patches on source or destination\n");
  }
