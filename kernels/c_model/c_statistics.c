...
vx_status vxMeanStdDev(vx_image input, vx_scalar mean, vx_scalar stddev) {
  vx_float32 fmean = 0.0f;
  vx_float64 sum = 0.0, sum_diff_sqrs = 0.0;
  vx_float32 fstddev = 0.0f;
  ...
  status |= vxMapImagePatch(input, &rect, 0, &map_id, &addrs, &base_ptr,
...
static void analyzeMinMaxValue(vx_uint32 x, vx_uint32 y, vx_int64 v,
  ...
  vxQueryArray(minLoc, VX_ARRAY_CAPACITY, &minLocCapacity,
               sizeof(minLocCapacity));
  vxQueryArray(maxLoc, VX_ARRAY_CAPACITY, &maxLocCapacity,
               sizeof(maxLocCapacity));
...
vx_status vxMinMaxLoc(vx_image input, vx_scalar minVal, vx_scalar maxVal,
  ...
  status |= vxGetValidRegionImage(input, &rect);
  status |=
      vxMapImagePatch(input, &rect, 0, &map_id, &src_addr, (void **)&src_base,
                      VX_READ_ONLY, VX_MEMORY_TYPE_HOST, 0);
  for (y = 0; y < src_addr.dim_y; y++) {
    for (x = 0; x < src_addr.dim_x; x++) {
      ...
      if (format == VX_DF_IMAGE_U8) {
        ...
        analyzeMinMaxValue(x, y, v, &iMinVal, &iMaxVal, &iMinCount, &iMaxCount,
                           minLoc, maxLoc);
      } else if (format == VX_DF_IMAGE_U16) {
  ...
  switch (format) {
    case VX_DF_IMAGE_U8: {
      ...
      if (minVal)
        vxCopyScalar(minVal, &min, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
      if (maxVal)
        vxCopyScalar(maxVal, &max, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
    } break;
  ...
  if (minCount)
    vxCopyScalar(minCount, &iMinCount, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
  if (maxCount)
    vxCopyScalar(maxCount, &iMaxCount, VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST);
