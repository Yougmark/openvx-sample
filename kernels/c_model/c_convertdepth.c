...
vx_status vxConvertDepth(vx_image input, vx_image output, vx_scalar spol,
  ...
  status |=
      vxQueryImage(output, VX_IMAGE_FORMAT, &format[1], sizeof(format[1]));
  status |= vxGetValidRegionImage(input, &rect);
  status |=
      vxAccessImagePatch(input, &rect, 0, &src_addr, &src_base, VX_READ_ONLY);
  status |=
      vxAccessImagePatch(output, &rect, 0, &dst_addr, &dst_base, VX_WRITE_ONLY);
  for (y = 0; y < src_addr.dim_y; y++) {
    for (x = 0; x < src_addr.dim_x; x++) {
      if ((format[0] == VX_DF_IMAGE_U8) && (format[1] == VX_DF_IMAGE_U16)) {
      ...
      } else if ((format[0] == VX_DF_IMAGE_U8) &&
      ...
      } else if ((format[0] == VX_DF_IMAGE_U16) &&
                 (format[1] == VX_DF_IMAGE_U32)) {
        vx_uint16 *src = vxFormatImagePatchAddress2d(src_base, x, y, &src_addr);
        vx_uint32 *dst = vxFormatImagePatchAddress2d(dst_base, x, y, &dst_addr);
        *dst = ((vx_uint32)(*src)) << shift;
      } else if ((format[0] == VX_DF_IMAGE_S16) &&
        ...
        *dst = ((vx_int32)(*src)) << shift;
      ...
      } else if ((format[0] == VX_DF_IMAGE_U32) &&
                 (format[1] == VX_DF_IMAGE_U8)) {
      ...
      } else if ((format[0] == VX_DF_IMAGE_S32) &&
                 (format[1] == VX_DF_IMAGE_S16)) {
      ...
      } else if ((format[0] == VX_DF_IMAGE_F32) &&
                 (format[1] == VX_DF_IMAGE_U8)) {
        vx_float32 *src =
        ...
        if (pf > 0.0f) {
          ...
          //                    this print. You get too much of those prints in
          //                    harris corner
        }
