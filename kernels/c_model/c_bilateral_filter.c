...
static vx_status bilateralFilter_8u(void* src, vx_size* src_strides,
                                    vx_size* dims, vx_size num_of_dims,
                                    vx_int32 diameter, vx_float32 sigma_space,
                                    vx_float32 sigma_color, void* dst,
                                    vx_size* dst_strides) {
  ...
  vx_float32* space_weight =
      (vx_float32*)malloc((radius * 2 + 1) * sizeof(vx_float32));
  if (!space_weight) {
  ...
  for (i = radius; i < (vx_int32)(out_size - radius); i++) {
    ...
    src_pos = ComputeGlobalPositionsFromIndex(i, dims, src_strides, num_of_dims,
                                              &src_pos0);
    dst_pos = ComputeGlobalPositionsFromIndex(i, dims, dst_strides, num_of_dims,
                                              &dst_pos0);
    ...
    for (j = -radius; j <= radius; j++) {
      vx_size nei_pos0 = 0;
      nei_pos = ComputeGlobalPositionsFromIndex(i + j, dims, src_strides,
                                                num_of_dims, &nei_pos0);
...
static vx_status bilateralFilter_16(void* src, vx_size* src_strides,
                                    vx_size* dims, vx_size num_of_dims,
                                    vx_int32 diameter, vx_float32 sigma_space,
                                    vx_float32 sigma_color, void* dst,
                                    vx_size* dst_strides) {
...
vx_status vxBilateralFilter(void* src, vx_size* src_strides, vx_size* dims,
                            ...
                            vx_float32 sigma_space, vx_float32 sigma_color,
                            void* dst, vx_size* dst_strides, vx_enum type) {
  vx_status status = VX_SUCCESS;
  if (type == VX_TYPE_UINT8) {
    status = bilateralFilter_8u(src, src_strides, dims, num_of_dims, diameter,
                                sigma_space, sigma_color, dst, dst_strides);
  } else if (type == VX_TYPE_INT16) {
    status = bilateralFilter_16(src, src_strides, dims, num_of_dims, diameter,
                                sigma_space, sigma_color, dst, dst_strides);
  }
