...
double vxInvSqrt64d(double arg) {
  double x, y;
...
vx_int64 vxCompareBlocksHamming_8u(const vx_uint8 *vec1, const vx_uint8 *vec2,
                                   int len) {
  int i, s = 0;
  vx_int64 sum = 0;
  for (i = 0; i <= len - 4; i += 4) {
...
*/
vx_int64 vxCrossCorr_8u(const vx_uint8 *vec1, const vx_uint8 *vec2,
                        vx_int32 len) {
  vx_int32 i, s = 0;
  vx_int64 sum = 0;
  for (i = 0; i <= len - 4; i += 4) {
...
vx_int64 vxCompareBlocksL1_8u(const vx_uint8 *vec1, const vx_uint8 *vec2,
                              int len) {
  int i, s = 0;
  vx_int64 sum = 0;
  for (i = 0; i <= len - 4; i += 4) {
...
vx_int64 vxCompareBlocksL2_8u(const vx_uint8 *vec1, const vx_uint8 *vec2,
                              int len) {
  int i, s = 0;
  vx_int64 sum = 0;
  for (i = 0; i <= len - 4; i += 4) {
...
vx_int64 vxCompareBlocksL2_Square_8u(const vx_uint8 *vec1, const vx_uint8 *vec2,
                                     int len) {
  int i, s = 0;
  vx_int64 sum = 0;
  for (i = 0; i <= len - 4; i += 4) {
...
int vx_align(vx_uint32 size) { return (size + 16 + 32 - 1) & -32; }
...
vx_status vxMatchTemplateGetBufSize(vx_uint32 src_width, vx_uint32 src_height,
                                    ...
                                    vx_uint32 template_height, int *bufferSize,
  ...
  if (!bufferSize) return VX_FAILURE;
  *bufferSize = 0;

  vxCalculateBufferSizes(src_width, src_height, template_width, template_height,
                         is_centered, is_normed, &imgBufSize, &templBufSize,
                         &sumBufSize, &sqsumBufSize, &resNumBufSize,
                         &resDenomBufSize);
...
vx_status vxMatchTemplateEntry(
    const vx_image source_image, const vx_uint32 src_width,
    const vx_uint32 src_height, const vx_image template_image,
    const vx_uint32 template_width, const vx_uint32 template_height,
    vx_image output, void *pBuffer, vx_int32 is_centered, vx_int32 is_normed,
    void **imgBuf, void **templBuf, void **sumBuf, void **sqsumBuf,
    void **resNum, void **resDenom) {
  vx_status status = VX_SUCCESS;
  vx_int32 templBufSize = 0, imgBufSize = 0, sumBufSize = 0, sqsumBufSize = 0,
           resNumBufSize = 0, resDenomBufSize = 0;
  vx_uint32 i;
  (void)output;
  ...
  *resNum = buffer;
  ...
  if (is_centered || is_normed) {
    if (sumBuf) *sumBuf = buffer;
    buffer += sumBufSize;

    if (sqsumBuf) *sqsumBuf = buffer;
    buffer += sqsumBufSize;

    if (resDenom) *resDenom = buffer;
    buffer += resDenomBufSize;
  }
  void *src;
  vx_imagepatch_addressing_t addr = VX_IMAGEPATCH_ADDR_INIT,
                             template_addr = VX_IMAGEPATCH_ADDR_INIT;
  ...
  src = NULL;
  status = vxMapImagePatch(source_image, &source_rect, 0, &source_map_id, &addr,
                           &src, VX_READ_ONLY, VX_MEMORY_TYPE_HOST, 0);
  if (status != VX_SUCCESS) return status;
  ...
  if (status != VX_SUCCESS) return status;
  src = NULL;
  status = vxMapImagePatch(template_image, &template_rect, 0, &template_map_id,
                           &template_addr, &src, VX_READ_ONLY,
                           VX_MEMORY_TYPE_HOST, 0);
  if (status != VX_SUCCESS) return status;
  for (i = 0; i < template_height; i++) {
    vx_uint8 *srcp =
        vxFormatImagePatchAddress1d(src, i * template_width, &template_addr);
...
vx_status matchTemplate_HAMMING(const vx_image source_image,
  ...
  vx_status result = vxMatchTemplateEntry(
      source_image, src_width, src_height, template_image, template_width,
      template_height, result_image, pBuffer, 0, 0, (void **)&imgBuf,
      (void **)&templBuf, 0, 0, (void **)&resNum, 0);
  ...
  status |=
      vxMapImagePatch(source_image, &rect, 0, &map_id, &addr, (void **)&src,
                      VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  if (status != VX_SUCCESS) return status;
  status |= vxGetValidRegionImage(result_image, &result_rect);
  status |= vxMapImagePatch(result_image, &result_rect, 0, &result_map_id,
                            &result_addr, (void **)&result_src, VX_WRITE_ONLY,
  ...
  for (x = 0; x < result_width; x++) {
    ...
    if (x > 0) {
      ...
      dst += template_width;
      for (y = 0; y < (vx_int32)src_height;
           y++, source_x += src_width, dst += template_width) {
        vx_uint8 *srcp =
            vxFormatImagePatchAddress1d(src, (vx_int32)source_x, &addr);
        dst[0] = srcp[0];
      }
    }
    ...
    for (y = 0; y < result_height; y++) {
      ...
      vx_int16 *result_srcp = vxFormatImagePatchAddress1d(
          result_src, x + y * result_width, &result_addr);
...
vx_status matchTemplate_COMPARE_L1(const vx_image source_image,
                                   vx_uint32 src_width, vx_uint32 src_height,
                                   const vx_image template_image,
                                   vx_uint32 template_width,
                                   vx_uint32 template_height,
  ...
  for (x = 0; x < result_width; x++) {
    ...
    for (y = 0; y < result_height; y++) {
      vx_float64 res = winCoeff * ((vx_float64)resNum[y]);
      vx_int16 *result_srcp =
          vxFormatImagePatchAddress2d(result_src, x, y, &result_addr);
      *result_srcp = (vx_int16)res;
    }
  }
...
vx_status matchTemplate_COMPARE_L2(const vx_image source_image,
                                   vx_uint32 src_width, vx_uint32 src_height,
                                   const vx_image template_image,
                                   vx_uint32 template_width,
                                   vx_uint32 template_height,
...
vx_status matchTemplate_CCORR(const vx_image source_image, vx_uint32 src_width,
                              vx_uint32 src_height,
                              const vx_image template_image,
                              vx_uint32 template_width,
                              vx_uint32 template_height, vx_image result_image,
                              void *pBuffer) {
  vx_status status = VX_SUCCESS;
  vx_uint8 *imgBuf = 0;
  vx_uint8 *templBuf = 0;
  ...
  for (x = 0; x < result_width; x++) {
    ...
    if (x > 0) {
      ...
      for (y = 0; y < (vx_int32)src_height;
           y++, source_x += src_width, dst += template_width) {
        vx_uint8 *srcp =
            vxFormatImagePatchAddress1d(src, (vx_uint32)source_x, &addr);
        dst[0] = srcp[0];
      }
    }
...
vx_status matchTemplate_COMPARE_L2_NORM(
    const vx_image source_image, vx_uint32 src_width, vx_uint32 src_height,
    const vx_image template_image, vx_uint32 template_width,
    vx_uint32 template_height, vx_image result_image, void *pBuffer) {
  vx_status status = VX_SUCCESS;
  vx_uint8 *imgBuf = 0;
  vx_uint8 *templBuf = 0;
  vx_int64 *sqsumBuf = 0;
  ...
  vx_status result = vxMatchTemplateEntry(
      source_image, src_width, src_height, template_image, template_width,
      template_height, result_image, pBuffer, 0, 1, (void **)&imgBuf,
      (void **)&templBuf, 0, (void **)&sqsumBuf, (void **)&resNum, 0);
  if (result != VX_SUCCESS) return result;
  vx_imagepatch_addressing_t addr = VX_IMAGEPATCH_ADDR_INIT;
  vx_imagepatch_addressing_t result_addr = VX_IMAGEPATCH_ADDR_INIT;
  vx_map_id map_id = 0;
...
vx_status matchTemplate_COMPARE_CCORR_NORM(
    const vx_image source_image, vx_uint32 src_width, vx_uint32 src_height,
    const vx_image template_image, vx_uint32 template_width,
    vx_uint32 template_height, vx_image result_image, void *pBuffer) {
  vx_status status = VX_SUCCESS;
  ...
  vx_status result = vxMatchTemplateEntry(
      source_image, src_width, src_height, template_image, template_width,
      template_height, result_image, pBuffer, 0, 1, (void **)&imgBuf,
      (void **)&templBuf, 0, (void **)&sqsumBuf, (void **)&resNum,
      (void **)&resDenom);
  if (result != VX_SUCCESS) return result;
  vx_imagepatch_addressing_t addr = VX_IMAGEPATCH_ADDR_INIT;
  vx_map_id map_id = 0;
  ...
  for (x = 0; x < result_width; x++) {
    ...
    for (y = 0; y < result_height; y++, imgPtr += template_width) {
      // compute sum_{grave{x},grave{y}}^{w,h} T(grave{x},grave{y}) *
      // I(x+grave{x},y+grave{y})
    ...
    for (y = 0; y < result_height; y++) {
      vx_float64 res =
          ((vx_float64)resNum[y]) * templCoeff *
...
vx_status vxMatchTemplate(vx_image src, vx_image template_image,
  ...
  vxQueryImage(template_image, VX_IMAGE_WIDTH, &template_width,
               sizeof(vx_uint32));
  vxQueryImage(template_image, VX_IMAGE_HEIGHT, &template_height,
               sizeof(vx_uint32));

  int bufferSize;
  vxMatchTemplateGetBufSize(src_width, src_height, template_width,
                            template_height, &bufferSize, 1, 1);
  ...
  switch (matchMethod) {
    case VX_COMPARE_HAMMING: {
      matchTemplate_HAMMING(src, src_width, src_height, template_image,
                            template_width, template_height, output, pBuffer);
      break;
    }
    case VX_COMPARE_L1: {
      matchTemplate_COMPARE_L1(src, src_width, src_height, template_image,
                               template_width, template_height, output,
                               pBuffer);
      break;
    }
    case VX_COMPARE_L2: {
      matchTemplate_COMPARE_L2(src, src_width, src_height, template_image,
                               template_width, template_height, output,
                               pBuffer);
      break;
    }
    case VX_COMPARE_CCORR: {
      matchTemplate_CCORR(src, src_width, src_height, template_image,
                          template_width, template_height, output, pBuffer);
      break;
    }
    case VX_COMPARE_L2_NORM: {
      matchTemplate_COMPARE_L2_NORM(src, src_width, src_height, template_image,
                                    template_width, template_height, output,
                                    pBuffer);
      break;
    }
    case VX_COMPARE_CCORR_NORM: {
      matchTemplate_COMPARE_CCORR_NORM(src, src_width, src_height,
                                       template_image, template_width,
                                       template_height, output, pBuffer);
      break;
    }
  }
  free(pBuffer);
