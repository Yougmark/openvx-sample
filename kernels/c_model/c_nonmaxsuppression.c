...
vx_status vxNonMaxSuppression(vx_image input, vx_image mask, vx_scalar win_size,
  ...
  status |= vxMapImagePatch(output, &rect, 0, &dst_map_id, &dst_addr, &dst_base,
                            VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST, 0);
  status |= vxGetValidRegionImage(input, &src_rect);
  status |= vxMapImagePatch(input, &src_rect, 0, &src_map_id, &src_addr,
                            &src_base, VX_READ_ONLY, VX_MEMORY_TYPE_HOST, 0);
  if (mask != NULL) {
    status |= vxGetValidRegionImage(mask, &mask_rect);
    status |= vxMapImagePatch(mask, &mask_rect, 0, &mask_map_id, &mask_addr,
                              (void **)&mask_base, VX_READ_AND_WRITE,
  ...
  for (vx_int32 x = border; x < (width - border); x++) {
    for (vx_int32 y = border; y < (height - border); y++) {
      vx_uint8 *_mask;
      if (mask != NULL) {
        _mask = (vx_uint8 *)vxFormatImagePatchAddress2d(mask_base, x, y,
                                                        &mask_addr);
      } else {
        _mask = &mask_data;
      }
      ...
      vx_int32 src_val =
          format == VX_DF_IMAGE_U8 ? *(vx_uint8 *)val_p : *(vx_int16 *)val_p;
      if (*_mask != 0) {
        ...
        for (vx_int32 i = -border; i <= border; i++) {
          for (vx_int32 j = -border; j <= border; j++) {
            void *neighbor =
                vxFormatImagePatchAddress2d(src_base, x + i, y + j, &src_addr);
            if (mask != NULL) {
              _mask = (vx_uint8 *)vxFormatImagePatchAddress2d(
                  mask_base, x + i, y + j, &mask_addr);
            } else {
              _mask = &mask_data;
            }
            vx_int32 neighbor_val = format == VX_DF_IMAGE_U8
                                        ? *(vx_uint8 *)neighbor
                                        : *(vx_int16 *)neighbor;
            if ((*_mask == 0) &&
