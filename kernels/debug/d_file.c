...
vx_status ownFWriteImage(vx_image input, vx_array file) {
  ...
  vx_imagepatch_addressing_t addr[4] = {
      VX_IMAGEPATCH_ADDR_INIT, VX_IMAGEPATCH_ADDR_INIT, VX_IMAGEPATCH_ADDR_INIT,
  ...
  for (p = 0u; p < planes; p++) {
    status |= vxMapImagePatch(input, &rect, p, &plane_map_id[p], &addr[p],
                              (void**)&src[p], VX_READ_ONLY,
                              VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  }

  for (p = 0u; (p < planes) && (status == VX_SUCCESS); p++) {
    size_t len =
        addr[p].stride_x * (addr[p].dim_x * addr[p].scale_x) / VX_SCALE_UNITY;
    ...
    for (y = 0u; y < height; y += addr[p].step_y) {
      ...
      for (i = 0; i < sx; ++i) wrote += fwrite(&value, sizeof(value), 1, fp);
    ...
    if (status == VX_FAILURE) {
      vxAddLogEntry((vx_reference)file, VX_FAILURE,
                    "Failed to write image to file correctly\n");
      break;
    }
  }
  ...
  if (vxUnmapArrayRange(file, fname_map_id) != VX_SUCCESS) {
    vxAddLogEntry((vx_reference)file, VX_FAILURE,
                  "Failed to release handle to filename array!\n");
  }
...
vx_status ownFReadImage(vx_array file, vx_image output) {
  ...
  } else if (ext && (strcmp(ext, ".yuv") == 0 || strcmp(ext, ".rgb") == 0 ||
                     strcmp(ext, ".bw") == 0)) {
  ...
  for (p = 0; p < planes; p++) {
    src = NULL;
    status |=
        vxMapImagePatch(output, &rect, p, &image_map_id, &addr, (void**)&src,
