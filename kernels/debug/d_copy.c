...
vx_status ownCopyImage(vx_image input, vx_image output) {
  ...
  for (p = 0; p < planes && status == VX_SUCCESS; p++) {
    ...
    status |= vxMapImagePatch(input, &rect, p, &src_map_id, &src_addr, &src,
                              VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
    status |= vxMapImagePatch(output, &rect, p, &dst_map_id, &dst_addr, &dst,
...
vx_status ownCopyArray(vx_array src, vx_array dst) {
  ...
  status |=
      vxQueryArray(dst, VX_ARRAY_CAPACITY, &dst_capacity, sizeof(dst_capacity));
