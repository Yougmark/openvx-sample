...
void quicksort(struct kp_elem list[], int m, int n) {
  ...
  if (m < n) {
    ...
    j = n;
    while (i <= j) {
      while ((i <= n) && (list[i].resp >= key.resp)) i++;
      while ((j >= m) && (list[j].resp < key.resp)) j--;
...
vx_status ownEuclideanNonMaxSuppressionHarris(vx_image src, vx_scalar thr,
  ...
  status |= vxMapImagePatch(dst, &rect, 0, &dst_map_id, &dst_addr, &dst_base,
  ...
  if (status == VX_SUCCESS) {
    ...
    struct kp_elem* kp_list = (struct kp_elem*)malloc(
        src_addr.dim_x * src_addr.dim_y * sizeof(struct kp_elem));
      ...
      for (y = 0; y < (vx_int32)src_addr.dim_y; y++) {
        for (x = 0; x < (vx_int32)src_addr.dim_x; x++) {
          ...
          *out = 0;
          ...
          if ((x > 0) && (x < (vx_int32)src_addr.dim_x - 1) && (y > 0) &&
              (y < (vx_int32)src_addr.dim_y - 1)) {
            vx_float32* ptr =
            ...
            if ((*ptr >= thresh) &&
                ((*ptr >= *ptr99) && (*ptr >= *ptr90) && (*ptr >= *ptr91) &&
                 (*ptr >= *ptr09) && (*ptr > *ptr01) && (*ptr > *ptr19) &&
                 (*ptr > *ptr10) && (*ptr > *ptr11))) {
              kp_list[nb_kp].x = x;
              kp_list[nb_kp].y = y;
          ...
          if ((y + j >= 0) && (y + j < (vx_int32)src_addr.dim_y)) {
            for (i = -r; i <= r; i++) {
              if ((x + i >= 0) && (x + i < (vx_int32)src_addr.dim_x)) {
                ...
                if (d < radius) {
                  vx_float32* non = vxFormatImagePatchAddress2d(
                      dst_base, x + i, y + j, &dst_addr);
                  if (*non) {
                    found = 1;
                    break;
                  }
...
vx_status ownNonMaxSuppression(vx_image i_mag, vx_image i_ang, vx_image i_edge,
  ...
  status |= vxMapImagePatch(i_mag, &rect, 0, &mag_map_id, &mag_addr, &mag_base,
                            VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  status |= vxMapImagePatch(i_ang, &rect, 0, &ang_map_id, &ang_addr, &ang_base,
                            VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  status |=
      vxMapImagePatch(i_edge, &rect, 0, &edge_map_id, &edge_addr, &edge_base,
                      VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  ...
  for (y = low_y; y < high_y; y++) {
    for (x = low_x; x < high_x; x++) {
      ...
      vx_uint8 angle =
          (vx_uint8)(127 - ang[0]);  // shift range back after vxPhase
      ...
      if (format == VX_DF_IMAGE_U8) {
        vx_uint8 mag[9];
        vx_uint8* edge =
            vxFormatImagePatchAddress2d(edge_base, x, y, &edge_addr);
