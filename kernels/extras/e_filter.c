...
vx_int32 own_convolve8with16(void* base, vx_uint32 x, vx_uint32 y,
  ...
  vx_int32 div = conv[0][0] + conv[0][1] + conv[0][2] + conv[1][0] +
                 conv[1][1] + conv[1][2] + conv[2][0] + conv[2][1] + conv[2][2];

  vx_int32 sum = (conv[0][0] * pixels[0][0]) + (conv[0][1] * pixels[0][1]) +
  ...
  if (bordermode->mode == VX_BORDER_UNDEFINED) {
    ...
    for (y = 1; y < (src_addr.dim_y - 1); y++) {
      for (x = 1; x < (src_addr.dim_x - 1); x++) {
        vx_int32 value =
            own_convolve8with16(src_base, x, y, &src_addr, laplacian);
