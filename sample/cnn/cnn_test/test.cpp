...
int16_t *MapFile(const char *fileName, size_t *filesize) {
  ...
  *filesize = fileInfo.nFileSizeLow;
  ...
  handle = ::CreateFileMapping(hFile, NULL, PAGE_READONLY, 0, 0, NULL);
  if (!handle) {
    return NULL;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (networkType == NULL) {
    printf(
        "Usage: cnn_test <network> [input file] [weights flie] [output "
        "file]\n");
    goto error;
  }
  ...
  {
    printf(
        "Unsupported CNN type. Supported networks are overfeat, alexnet, "
        "googlenet2.\n");
    goto error;
  }
  ...
  if (strcmp(networkType, "overfeat") == 0) {
    ...
    printf("Available networks are overfeat, alexnet, googlenet2, bds, test\n");
    goto error;
  }
