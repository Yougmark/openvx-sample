...
#define VX_KERNEL_FAKE_MAX \
  (VX_KERNEL_CHANNEL_EXTRACT)  // supposed to be VX_KERNEL_MAX but until all the
                               // kernels are implemented, this will be used.
 ...
 */
#define VALARM(message, ...) \
  printf("[%s:%u] " message "\n", __FUNCTION__, __LINE__, __VA_ARGS__)
#define ALARM(message) VALARM("%s", message)
#define VFAIL(label, message, ...)                 \
  do {                                             \
    VALARM(message, __VA_ARGS__);                  \
    if (status == VX_SUCCESS) status = VX_FAILURE; \
    goto label;                                    \
  } while (0)
...
/*! \brief The structure which correlates each unit test with a result and a
 * name. */
...
void vx_print_addressing(void *ptr, vx_imagepatch_addressing_t *addr) {
  printf("ptr=%p dim={%u,%u} stride={%d,%d} scale={%u,%u} step={%u,%u}\n", ptr,
         addr->dim_x, addr->dim_y, addr->stride_x, addr->stride_y,
         addr->scale_x, addr->scale_y, addr->step_x, addr->step_y);
}
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    if (status == VX_SUCCESS) {
      ...
      if (kernel) {
        ...
        status = VX_SUCCESS;
        if (vxQueryContext(context, VX_CONTEXT_UNIQUE_KERNELS, &num_kernels,
                           sizeof(vx_uint32)) != VX_SUCCESS)
          status = VX_ERROR_NOT_SUFFICIENT;
        if (vxQueryContext(context, VX_CONTEXT_MODULES, &num_modules,
                           sizeof(vx_uint32)) != VX_SUCCESS)
          status = VX_ERROR_NOT_SUFFICIENT;
        if (vxQueryContext(context, VX_CONTEXT_REFERENCES, &num_references,
                           sizeof(vx_uint32)) != VX_SUCCESS)
          status = VX_ERROR_NOT_SUFFICIENT;
        printf("[VX_TEST] Kernels:%u Modules:%u Refs:%u\n", num_kernels,
               num_modules, num_references);
    ...
    if (vxLoadKernels(context, "xyz") == VX_SUCCESS) {
      ...
      if (kernel) {
        ...
        if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
          ...
          // we're also testing the "set parameter inside the wrapper. It will
          // check for 0 refs.
          ...
          if (xyz) {
            ...
            void *ptr = NULL;
            status =
                vxQueryNode(xyz, VX_NODE_LOCAL_DATA_SIZE, &size, sizeof(size));
            status =
                vxQueryNode(xyz, VX_NODE_LOCAL_DATA_PTR, &ptr, sizeof(ptr));
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
      ...
      if (node) {
        ...
        if (vxQueryGraph(graph, VX_GRAPH_NUMNODES, &numNodes,
                         sizeof(numNodes)) != VX_SUCCESS) {
          status = VX_ERROR_NOT_SUFFICIENT;
        } else {
          if (numNodes != 1) {
            status = VX_ERROR_NOT_SUFFICIENT;
...
vx_status vx_test_framework_copy_virtual(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
      ...
      vx_uint32 numNodes = 0;
      if (vxQueryGraph(graph, VX_GRAPH_NUMNODES, &numNodes, sizeof(numNodes)) !=
          VX_SUCCESS) {
        status = VX_ERROR_NOT_SUFFICIENT;
      } else {
        if (numNodes != 2) {
...
vx_status vx_test_framework_virtualimage(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
      ...
      status =
          vxAccessImagePatch(virt, &rect, 0, &addr, &base, VX_READ_AND_WRITE);
...
vx_status vx_test_framework_kernels(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    if (status != VX_SUCCESS) {
      ...
      if (status != VX_SUCCESS) goto exit2;
    exit2:
      vxReleaseKernel(&kernel);
      vxReleaseNode(&node);
      vxReleaseGraph(&graph);
...
#define DEPTH_TEST (3)
      ...
      status |= vxLoadKernels(context, "openvx-debug");
      nodes[0] = vxCopyImageNode(graph, images[0],
                                 (vx_image)vxGetReferenceFromDelay(delay, 0));
      nodes[1] = vxCopyImageNode(
          graph, (vx_image)vxGetReferenceFromDelay(delay, -1), images[1]);
      nodes[2] = vxCopyImageNode(
          graph, (vx_image)vxGetReferenceFromDelay(delay, -1), images[2]);
      ...
      if (status == VX_SUCCESS) {
        ...
        vxuFillImage(context, 0xAD,
                     (vx_image)vxGetReferenceFromDelay(delay, -1));
        vxuFillImage(context, 0x00,
                     (vx_image)vxGetReferenceFromDelay(delay, 0));
        ...
        if (status != VX_SUCCESS) {
          ...
          if (vxuCheckImage(context,
                            (vx_image)vxGetReferenceFromDelay(delay, 0), 0xBE,
                            &errors) == VX_SUCCESS &&
              vxuCheckImage(context,
                            (vx_image)vxGetReferenceFromDelay(delay, -1), 0xAD,
                            &errors) == VX_SUCCESS &&
            ...
            if (status == VX_SUCCESS &&
                vxuCheckImage(context,
                              (vx_image)vxGetReferenceFromDelay(delay, 0), 0xAD,
                              &errors) == VX_SUCCESS &&
                vxuCheckImage(context,
                              (vx_image)vxGetReferenceFromDelay(delay, -1),
                              0xBE, &errors) == VX_SUCCESS &&
                vxuCheckImage(context, images[1], 0xAD, &errors) ==
                    VX_SUCCESS &&
                vxuCheckImage(context, images[2], 0xAD, &errors) ==
                    VX_SUCCESS) {
              status = vxProcessGraph(graph);
              if (status == VX_SUCCESS) {
                if (vxuCheckImage(context,
                    ...
                    vxuCheckImage(context,
                                  (vx_image)vxGetReferenceFromDelay(delay, -1),
                                  0xBE, &errors) == VX_SUCCESS &&
                    vxuCheckImage(context, images[1], 0xBE, &errors) ==
                        VX_SUCCESS &&
                    vxuCheckImage(context, images[2], 0xBE, &errors) ==
                        VX_SUCCESS) {
                  ALARM("Passed!");
                  status = VX_SUCCESS;
                } else {
...
vx_status vx_test_framework_async(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    if (vxGetStatus((vx_reference)g1) == VX_SUCCESS &&
        vxGetStatus((vx_reference)g2) == VX_SUCCESS && status == VX_SUCCESS) {
      vx_node nodes[] = {
...
vx_status vx_test_direct_copy_image(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    vx_imagepatch_addressing_t addrs[] = {
        {WIDTH, HEIGHT, sizeof(vx_uint8), WIDTH * sizeof(vx_uint8),
    ...
    void *src_ptrs[] = {&images[0][0][0][0][0], &images[0][1][0][0][0],
                        &images[0][2][0][0][0]};
    void *dst_ptrs[] = {&images[1][0][0][0][0], &images[1][1][0][0][0],
                        &images[1][2][0][0][0]};
    vx_image input = vxCreateImageFromHandle(context, VX_DF_IMAGE_YUV4, addrs,
                                             src_ptrs, VX_MEMORY_TYPE_HOST);
    vx_image output = vxCreateImageFromHandle(context, VX_DF_IMAGE_YUV4, addrs,
                                              dst_ptrs, VX_MEMORY_TYPE_HOST);
    if (input && output) {
...
vx_status vx_test_graph_bikegray(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    vx_image images[] = {
        ...
        vxCreateImage(context, w, h,
                      VX_DF_IMAGE_U8), /* 16: EqualizeHistogram */
    ...
    status |= vxSetThresholdAttribute(thresh, VX_THRESHOLD_THRESHOLD_VALUE, &lo,
                                      sizeof(lo));
    status |= vxSetThresholdAttribute(hyst, VX_THRESHOLD_THRESHOLD_LOWER,
                                      &lower, sizeof(lower));
    status |= vxSetThresholdAttribute(hyst, VX_THRESHOLD_THRESHOLD_UPPER,
                                      &upper, sizeof(upper));
    status |= vxLoadKernels(context, "openvx-debug");
    if (status == VX_SUCCESS) {
      vx_graph graph = vxCreateGraph(context);
      if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
        vx_node nodes[] = {
            ...
            vxScaleImageNode(graph, images[0], images[1],
            ...
            vxMinMaxLocNode(graph, images[7], minVal, maxVal, minLoc, maxLoc, 0,
                            0),
            vxCannyEdgeDetectorNode(graph, images[0], hyst, 3, VX_NORM_L1,
                                    images[15]),
            vxEqualizeHistNode(graph, images[0], images[16]),
            vxConvertDepthNode(graph, images[3], images[17], policy, snoshift),
            vxRemapNode(graph, images[0], table,
                        VX_INTERPOLATION_NEAREST_NEIGHBOR, images[18]),
            vxRemapNode(graph, images[0], table, VX_INTERPOLATION_BILINEAR,
                        images[19]),
            ...
            vxFWriteImageNode(graph, images[2],
                              "obikegradh_300x200_P400_-16b.bw"),
            vxFWriteImageNode(graph, images[3],
                              "obikegradv_300x200_P400_-16b.bw"),
            ...
            vxFWriteImageNode(graph, images[14],
                              "obikediff_300x200_P400_16b.bw"),
            vxFWriteImageNode(graph, images[15], "obikecanny_640x480_P400.pgm"),
            vxFWriteImageNode(graph, images[16],
                              "obikeeqhist_640x480_P400.pgm"),
            vxFWriteImageNode(graph, images[17],
                              "obikegrady8_300x200_P400.pgm"),
            vxFWriteImageNode(graph, images[18],
                              "obikeremap_nn_300x200_P400.pgm"),
            vxFWriteImageNode(graph, images[19],
                              "obikeremap_bi_300x200_P400.pgm"),
            vxFWriteImageNode(graph, images[20],
                              "obikeremap_ab_300x200_P400.pgm"),
        };
        ...
        if (status == VX_SUCCESS) {
          ...
          if (status == VX_SUCCESS) {
            ...
            vxAccessArrayRange(minLoc, 0, 1, &stride, (void **)&p_min_l,
                               VX_READ_ONLY);
            vxCommitArrayRange(minLoc, 0, 0, p_min_l);
            vxAccessArrayRange(maxLoc, 0, 1, &stride, (void **)&p_max_l,
                               VX_READ_ONLY);
            ...
            vxMapDistribution(dist, &map_id, (void **)&histogram, VX_READ_ONLY,
...
vx_status vx_test_graph_lena(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    vx_pyramid pyramid = vxCreatePyramid(context, 4, VX_SCALE_PYRAMID_HALF, w,
                                         h, VX_DF_IMAGE_U8);
    ...
    CHECK_ALL_ITEMS(images, i, status, exit);
    vxCopyConvolutionCoefficients(conv[0], (vx_int16 *)mat1, VX_WRITE_ONLY,
                                  VX_MEMORY_TYPE_HOST);
    vxCopyConvolutionCoefficients(conv[1], (vx_int16 *)mat2, VX_WRITE_ONLY,
                                  VX_MEMORY_TYPE_HOST);
    vxSetAffineRotationMatrix(affine, 45.0, 0.5, (vx_float32)w / 2,
                              (vx_float32)h / 2);
    ...
    if (status == VX_SUCCESS) {
      ...
      if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
        vx_node nodes[] = {
            ...
            vxWarpAffineNode(graph, images[0], affine,
                             VX_INTERPOLATION_NEAREST_NEIGHBOR, images[8]),
            vxWarpPerspectiveNode(graph, images[0], perspective,
                                  VX_INTERPOLATION_NEAREST_NEIGHBOR, images[9]),
            ...
            vxFWriteImageNode(graph, images[4],
                              "olenacus1_512x512_P400_-16b.bw"),
            vxFWriteImageNode(graph, images[5],
                              "olenacus2_512x512_P400_-16b.bw"),
        ...
        if (status == VX_SUCCESS) {
          ...
          if (status == VX_SUCCESS) {
            vxuFWriteImage(context, vxGetPyramidLevel(pyramid, 0),
                           "olenapyr_512x512_P400.bw");
            vxuFWriteImage(context, vxGetPyramidLevel(pyramid, 1),
                           "olenapyr_256x256_P400.bw");
            vxuFWriteImage(context, vxGetPyramidLevel(pyramid, 2),
                           "olenapyr_128x128_P400.bw");
            vxuFWriteImage(context, vxGetPyramidLevel(pyramid, 3),
                           "olenapyr_64x64_P400.bw");
          } else {
            printf("Graph failed (%d)\n", status);
            for (i = 0; i < dimof(nodes); i++) {
              status = VX_SUCCESS;
    ...
    if (status == VX_SUCCESS) {
      ...
      if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
        vx_node nodes[] = {
            ...
            vxChannelCombineNode(graph, images[1], images[2], images[3], 0,
                                 images[4]),
    ...
    if (status == VX_SUCCESS) {
      ...
      if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
        vx_node nodes[] = {
            ...
            vxAccumulateWeightedImageNode(graph, images[0], scalars[0],
                                          images[2]),
            vxAccumulateSquareImageNode(graph, images[0], scalars[1],
                                        images[3]),
            vxFWriteImageNode(graph, images[1],
                              "obikeaccu_640x480_P400_16b.bw"),
            vxFWriteImageNode(graph, images[2],
                              "obikeaccw_640x480_P400_16b.bw"),
            vxFWriteImageNode(graph, images[3],
                              "obikeaccq_640x480_P400_16b.bw"),
        };
        CHECK_ALL_ITEMS(nodes, i, status, exit);
        if (status == VX_SUCCESS) {
          status = vxVerifyGraph(graph);
          if (status == VX_SUCCESS) {
            while (r-- && status == VX_SUCCESS) status = vxProcessGraph(graph);
          }
          if (status == VX_SUCCESS) {
            /* add a node which should cause the graph to be re-verified and
...
static vx_uint32 b_not_1st(vx_uint32 a, vx_uint32 b) {
  (void)b;
  return ~a;
}
static vx_uint32 b_not_2nd(vx_uint32 a, vx_uint32 b) {
  (void)a;
  return ~b;
}
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    vx_image images[] = {
        ...
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8), /* not (on first image of two) */
        /*
         ...
         */
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8), /* not (on second image of two) */
    ...
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
      ...
      subtest_t subtests[] = {OPX(and),     OPX(or),   OPX (xor), OPX(not_1st),
                              OPX(not_2nd), OPX(nand), OPX(nor),  OPX(xorn)};

      status = vxAccessImagePatch(images[0], &rect, 0, &image0_addr, &base0,
                                  VX_WRITE_ONLY);
      status |= vxAccessImagePatch(images[1], &rect, 0, &image1_addr, &base1,
                                   VX_WRITE_ONLY);
      if (status != VX_SUCCESS) FAIL(release_rect, "couldn't get image patch");
      CHECK_ALL_ITEMS(nodes, i, status, release_graph);

      /*
       * Fill the source images. As long as each image size (i.e. width*height)
       * is
      ...
      for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
          vx_uint8 *img0 =
              vxFormatImagePatchAddress2d(base0, x, y, &image0_addr);
          vx_uint8 *img1 =
              vxFormatImagePatchAddress2d(base1, x, y, &image1_addr);
      ...
      status = vxVerifyGraph(graph);
      if (status != VX_SUCCESS) FAIL(release_rect, "graph verification failed");

      status = vxProcessGraph(graph);
      if (status != VX_SUCCESS) FAIL(release_rect, "graph processing failed");
      ...
      for (i = 0; i < dimof(subtests); i++) {
        ...
        status = vxAccessImagePatch(images[i + 2], &rect, 0, &image_addr, &base,
                                    VX_READ_ONLY);
        if (status != VX_SUCCESS)
          VFAIL(release_rect, "couldn't get image patch for %s-image",
                subtests[i].s);

        for (y = 0; y < height; y++) {
          for (x = 0; x < width; x++) {
            vx_uint8 *img =
                vxFormatImagePatchAddress2d(base, x, y, &image_addr);
            vx_uint8 ref = subtests[i].op(v0, v1);

            if (ref != *img)
              VFAIL(release_rect,
                    "wrong result at (x, y) = (%d, %d) for bitwise %s(0x%x, "
                    "0x%x): 0x%x should be 0x%x",
    ...
    release_rect:
      for (i = 0; i < dimof(nodes); i++) vxReleaseNode(&nodes[i]);
 ...
 */
static vx_image vx_create_image_valuecovering(vx_context context,
                                              vx_df_image format,
  ...
  void *base = NULL;
  vx_status status =
      vxAccessImagePatch(image, &rect, 0, &addr, &base, VX_READ_AND_WRITE);
  vx_uint32 x, y;
  vx_uint32 period_counter = 0;

  if (status != VX_SUCCESS) goto release_rect;
...
static vx_int32 a_apply_policy(vx_int32 raw_result, vx_df_image destformat,
...
static vx_int32 a_mult(vx_int32 a, vx_int32 b, vx_float32 scale,
  ...
  vx_int32 policy_result =
      a_apply_policy(inttyped_raw_result, destformat, policy);
...
vx_status vx_test_graph_arit(int argc, char *argv[]) {
   ...
   */
  static const vx_float32 test_scales[] = {0,    0.125f, 0.25f, 0.5f,
                                           1.0f, 1.25f,  1.5f,  2.0f};

  /* Testing is made somewhat simpler by using the direct calls. */
  static const struct {
    vx_int32 (*refop)(vx_int32, vx_int32, vx_float32, vx_df_image,
                      enum vx_convert_policy_e);
    vx_status(VX_CALLBACK *operate_image)(vx_context, vx_image, vx_image,
                                          vx_enum, vx_image);
    int n_scales;
    const char *name;
  } op_tests[] = {
  ...
  static const vx_df_image test_formats[] = {VX_DF_IMAGE_U8, VX_DF_IMAGE_S16};
  ...
  for (ipolicy = 0; ipolicy < (int)dimof(test_policies); ipolicy++) {
    ...
    int isrc0_format;
    for (isrc0_format = 0; isrc0_format < (int)dimof(test_formats);
         isrc0_format++) {
      vx_df_image src0_format = test_formats[isrc0_format];
      vx_imagepatch_addressing_t image0_addr;
      int isrc1_format;

      src0_image = vx_create_image_valuecovering(context, src0_format, width,
                                                 height, 130613, 1);
      if (!src0_image)
      ...
      for (isrc1_format = 0; isrc1_format < (int)dimof(test_formats);
           isrc1_format++) {
        vx_df_image src1_format = test_formats[isrc1_format];
        vx_imagepatch_addressing_t image1_addr;
        int idest_format;

        /* We deliberately use periodicity 256 for both VX_DF_IMAGE_U8 and
         * VX_DF_IMAGE_S16. */
        src1_image = vx_create_image_valuecovering(context, src1_format, width,
                                                   height, 20130611, 256);
        if (!src1_image)
          VFAIL(release_src0_image,
                "can't create test-image for second operand (%d, %d, %d)",
                ipolicy, isrc0_format, isrc1_format);

        for (idest_format = 0; idest_format < (int)dimof(test_formats);
             idest_format++) {
          vx_df_image dest_format = test_formats[idest_format];
          int iops;

          if (dest_format == VX_DF_IMAGE_U8 &&
              (src0_format != VX_DF_IMAGE_U8 || src1_format != VX_DF_IMAGE_U8))
            continue;
            ...
            for (iscale = 0; iscale < op_tests[iops].n_scales; iscale++) {
              ...
              if (!dest_image)
                VFAIL(release_src1_image,
                      "can't create dest_image (%d, %d, %d, %d, %d, %d)",
                      ipolicy, isrc0_format, isrc1_format, idest_format, iops,
                      iscale);

              /*
               * Second part of test-fixture simplification: "multiply" is
               * assumed if there's more than one "scale" iteration, otherwise
               * "scale" is assumed not used.
               */
              if (op_tests[iops].n_scales > 1)
                status = vxuMultiply(context, src0_image, src1_image, scale,
                                     policy, rounding, dest_image);
              else
                status = op_tests[iops].operate_image(
                    context, src0_image, src1_image, policy, dest_image);
              if (status != VX_SUCCESS)
                VFAIL(release_dest_image,
                      "can't apply %s (%d, %d, %d, %d, %d, %d)",
                      op_tests[iops].name, ipolicy, isrc0_format, isrc1_format,
                      idest_format, iops, iscale);

              status = vxAccessImagePatch(src0_image, &rect, 0, &image0_addr,
                                          &base0, VX_READ_ONLY);
              if (status != VX_SUCCESS)
                VFAIL(release_dest_image,
                      "can't create patch to test-image for first arithmetic "
                      "operand (%d, %d)",
                      ipolicy, isrc0_format);

              status = vxAccessImagePatch(src1_image, &rect, 0, &image1_addr,
                                          &base1, VX_READ_ONLY);
              if (status != VX_SUCCESS)
                VFAIL(release_dest_image,
                      "can't create patch to test-image for second operand "
                      "(%d, %d, %d)",
                      ipolicy, isrc0_format, isrc1_format);

              status = vxAccessImagePatch(dest_image, &rect, 0, &dest_addr,
                                          &dbase, VX_READ_ONLY);
              if (status != VX_SUCCESS)
                VFAIL(
                    release_dest_image,
              ...
              for (y = 0; y < height; y++) {
                for (x = 0; x < width; x++) {
                  ...
                  void *img1 =
                      vxFormatImagePatchAddress2d(base1, x, y, &image1_addr);
                  void *imgd =
                      vxFormatImagePatchAddress2d(dbase, x, y, &dest_addr);
                  vx_int32 v0 = src0_format == VX_DF_IMAGE_U8
                                    ? *(vx_uint8 *)img0
                                    : *(vx_int16 *)img0;
                  vx_int32 v1 = src1_format == VX_DF_IMAGE_U8
                                    ? *(vx_uint8 *)img1
                                    : *(vx_int16 *)img1;
                  vx_int32 res = dest_format == VX_DF_IMAGE_U8
                                     ? *(vx_uint8 *)imgd
                                     : *(vx_int16 *)imgd;
                  vx_int32 ref =
                      op_tests[iops].refop(v0, v1, scale, dest_format, policy);

                  if (ref != res)
                    VFAIL(release_dest_image,
                          "wrong result at (x, y) = (%d, %d) for "
                          "%s:%s(0x%x:%s, 0x%x:%s, %f);"
                          " 0x%x:%s should be 0x%x:%s",
                          x, y, op_tests[iops].name,
                          policy == VX_CONVERT_POLICY_WRAP ? "WRAP" : "SAT", v0,
                          src0_format == VX_DF_IMAGE_U8 ? "U8" : "S16", v1,
                          src1_format == VX_DF_IMAGE_U8 ? "U8" : "S16", scale,
...
vx_status vx_test_graph_corners(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    vx_scalar scalars[] = {vxCreateScalar(context, VX_TYPE_FLOAT32, &str),
    ...
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
      vx_node nodes[] = {
          ...
          vxHarrisCornersNode(graph, images[1], scalars[0], scalars[1],
                              scalars[2], ws, bs, harris_arr, NULL),
          vxFastCornersNode(graph, images[1], scalars[3], vx_false_e, fast_arr,
                            NULL),
      };
      ...
      if (status == VX_SUCCESS) {
        ...
        vxQueryArray(harris_arr, VX_ARRAY_NUMITEMS, &num_items,
                     sizeof(num_items));
        printf("There are " VX_FMT_SIZE
               " number of points in the harris array!\n",
               num_items);
        vxQueryArray(fast_arr, VX_ARRAY_NUMITEMS, &num_items,
                     sizeof(num_items));
        printf("There are " VX_FMT_SIZE
               " number of points in the fast array!\n",
               num_items);
        printf("Rectangle from Gaussian is {%d, %d, %d, %d}\n", rect.start_x,
               rect.start_y, rect.end_x, rect.end_y);
      }
    exit:
      for (n = 0; n < dimof(nodes); n++) {
        vxReleaseNode(&nodes[n]);
...
vx_status vx_test_graph_tracker(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    vx_scalar min_distance_s =
        vxCreateScalar(context, VX_TYPE_FLOAT32, &min_distance);
    vx_scalar sensitivity_s =
        vxCreateScalar(context, VX_TYPE_FLOAT32, &sensitivity);
    vx_scalar sens_thresh_s =
        vxCreateScalar(context, VX_TYPE_FLOAT32, &sens_thresh);
    vx_scalar num_corners_s =
        vxCreateScalar(context, VX_TYPE_SIZE, &num_corners);
    ;

    vx_scalar scalars[] = {
        epsilon_s,     num_iterations_s, use_initial_estimate_s, min_distance_s,
        sensitivity_s, sens_thresh_s,    num_corners_s};

    vx_array arrays[] = {old_features, new_features};

    vx_image images[] = {
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_UYVY),  // index 0: Input 1
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_UYVY),  // index 1: Input 2
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8),  // index 2: Get Y channel
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8),  // index 3: Get Y channel
    };
    vx_pyramid pyramid_new =
        vxCreatePyramid(context, 4, 0.5f, width, height, VX_DF_IMAGE_U8);
    vx_pyramid pyramid_old =
        vxCreatePyramid(context, 4, 0.5f, width, height, VX_DF_IMAGE_U8);

    vx_pyramid pyramids[] = {pyramid_new, pyramid_old};
    ...
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
      vx_node nodes[] = {
          ...
          vxHarrisCornersNode(graph, images[3], sens_thresh_s, min_distance_s,
                              sensitivity_s, gradient_size, block_size,
                              old_features, num_corners_s),
          vxOpticalFlowPyrLKNode(graph, pyramid_old, pyramid_new, old_features,
                                 old_features, new_features, criteria,
                                 epsilon_s, num_iterations_s,
                                 use_initial_estimate_s, winSize)};

      CHECK_ALL_ITEMS(nodes, n, status, exit);
      status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
        status |= vxuFReadImage(context, "superresFirst_3072x2048_UYVY.yuv",
                                images[0]);
        status |= vxuFReadImage(context, "superresSecond_3072x2048_UYVY.yuv",
                                images[1]);
...
vx_status vx_xml_fullimport(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    vxDirective((vx_reference)context, VX_DIRECTIVE_ENABLE_PERFORMANCE);
    vxQueryContext(context, VX_CONTEXT_REFERENCES, &num_refs_orig,
                   sizeof(num_refs_orig));
    vxQueryContext(context, VX_CONTEXT_UNIQUE_KERNELS, &num_kernels_orig,
                   sizeof(num_kernels_orig));

    xml_import = vxImportFromXML(context, filename);
    if (xml_import &&
        (import_status = vxGetStatus((vx_reference)xml_import)) == VX_SUCCESS) {
      vxQueryContext(context, VX_CONTEXT_REFERENCES, &num_refs,
                     sizeof(num_refs));
      vxQueryContext(context, VX_CONTEXT_UNIQUE_KERNELS, &num_kernels,
                     sizeof(num_kernels));
      ...
      if ((status = vxQueryImport(xml_import, VX_IMPORT_ATTRIBUTE_COUNT, &count,
                                  sizeof(count))) == VX_SUCCESS) {
        /* Count returns the number of references explicitly found in the xml
         * file and returned in the refs array. num_nonkern_added can be greater
         * than count because of the following 2 reasons:
         *    1. if there are any non-unique kernels added from a library, this
         * will increase the num_refs count, but not the count of the number of
         * unique kernels
         *    2. if there are any virtual pyramids which do not have image
         * references in the xml file, there will be added references for each
         * level of the pyramid, but these will not be listed in the refs array
         */
      ...
      /* Now that the objects are imported, execute all graphs and report
       * performance */
      if (status == VX_SUCCESS) {
        vx_uint32 i;
        vx_enum type;
        ...
        for (i = 0; i < count; i++) {
          ...
          if (vxQueryReference(ref, VX_REF_ATTRIBUTE_TYPE, &type,
                               sizeof(type)) == VX_SUCCESS) {
            if (type == VX_TYPE_GRAPH) {
              vx_perf_t perf;
              graphStatus |= vxProcessGraph((vx_graph)ref);
              vxQueryGraph((vx_graph)ref, VX_GRAPH_PERFORMANCE, &perf,
                           sizeof(perf));
        ...
        if (graphStatus != VX_SUCCESS) status = VX_FAILURE;
        ...
        for (i = 0; i < 2; i++) {
          ...
          if (refs[i]) {
            ...
            vxQueryGraph((vx_graph)refs[i], VX_GRAPH_PERFORMANCE, &perf,
                         sizeof(perf));
            printf("Graph " VX_FMT_SIZE " avg time: %lu\n", (vx_size)refs[i],
                   perf.avg);
        ...
        /* Option 2.b: Use known names and release import object before graphs
         * (should work) */
...
vx_status vx_xml_fullexport(int argc, char *argv[]) {
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    ...
    vx_pyramid pyr = vxCreatePyramid(context, 4, VX_SCALE_PYRAMID_HALF, w * 4,
                                     h * 4, VX_DF_IMAGE_U8);
    vx_pyramid pyr2 = vxCreatePyramid(context, 4, VX_SCALE_PYRAMID_HALF, w * 4,
                                      h * 4, VX_DF_IMAGE_U8);
    vx_pyramid pyr3 = vxCreatePyramid(context, 4, VX_SCALE_PYRAMID_HALF, w * 4,
                                      h * 4, VX_DF_IMAGE_U8);
    vx_image fromPyr[] = {
    ...
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
      ...
      vx_image virts[] = {
          vxCreateVirtualImage(
              graph, 0, 0,
              VX_DF_IMAGE_VIRT), /* no specified dimension or format */
          vxCreateVirtualImage(graph, 0, 0,
                               VX_DF_IMAGE_S16), /* no specified dimension */
          vxCreateVirtualImage(graph, 320, 240,
                               VX_DF_IMAGE_VIRT), /* no specified format */
          vxCreateVirtualImage(graph, 640, 480,
                               VX_DF_IMAGE_U8), /* no user access */
      };
      vxCreateVirtualPyramid(
          graph, 4, VX_SCALE_PYRAMID_HALF, 0, 0,
          VX_DF_IMAGE_VIRT); /* no dimension and format specified for level 0 */
      vxCreateVirtualPyramid(graph, 4, VX_SCALE_PYRAMID_HALF, 640, 480,
                             VX_DF_IMAGE_VIRT); /* no format specified. */
      vxCreateVirtualPyramid(graph, 4, VX_SCALE_PYRAMID_HALF, 640, 480,
                             VX_DF_IMAGE_U8); /* no access */

      vxCreateVirtualArray(graph, 0, 0); /* totally unspecified */
      vxCreateVirtualArray(graph, VX_TYPE_KEYPOINT,
                           0); /* unspecified capacity */
      ...
      vx_node convNode =
          vxConvertDepthNode(graph, virts[0], images[1], policy, sshift);
      vx_node addNode =
          vxAddNode(graph, fromPyr[0], fromPyr[1], policy, fromPyr[2]);
      ...
      vx_parameter parameters[] = {vxGetParameterByIndex(sobelNode, 0),
      ...
      vx_keypoint_t ary5[] = {{0, 0, 2.3, 6.55555, 0.9059, 5, 3.5455},
                              {400, 235, 5.2222, 1.221, 0.5695, 8, 462.5}};
      ...
      for (x = 0u; x < 256; x++) mylut[x] = (vx_uint8)((x - 1) & 0xFF);
      ...
      vxSetConvolutionAttribute(conv, VX_CONVOLUTION_SCALE, &cscale,
                                sizeof(cscale));
      vxCopyConvolutionCoefficients(conv, (vx_int16 *)sobel_x, VX_WRITE_ONLY,
                                    VX_MEMORY_TYPE_HOST);
      vxSetThresholdAttribute(thresh, VX_THRESHOLD_THRESHOLD_VALUE, &th,
                              sizeof(th));
      vxSetThresholdAttribute(dblthr, VX_THRESHOLD_THRESHOLD_LOWER, &lwr,
                              sizeof(lwr));
      vxSetThresholdAttribute(dblthr, VX_THRESHOLD_THRESHOLD_UPPER, &th,
                              sizeof(th));
      {
        vx_map_id map_id = 0;
        vx_uint32 ptr = 0;
        ...
        vxMapDistribution(dist, &map_id, (void **)&ptr, VX_WRITE_ONLY,
                          VX_MEMORY_TYPE_HOST, 0);
...
vx_unittest unittests[] = {
    {VX_FAILURE, "Framework: Load XYZ Extension",
     &vx_test_framework_load_extension},
    {VX_FAILURE, "Framework: Load Kernel Node",
     &vx_test_framework_load_kernel_node},
    ...
    {VX_FAILURE, "Direct: Copy External Image",
     &vx_test_direct_copy_external_image},
    // graphs
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2 &&
      ((strncmp(argv[1], "-?", 2) == 0) ||
       ...
       (strncmp(argv[1], "-l", 2) == 0) || (strncmp(argv[1], "/?", 2) == 0))) {
    vx_uint32 t = 0;
