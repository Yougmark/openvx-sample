...
vx_graph vxPyramidIntegral(vx_context context, vx_image image,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    ...
    vx_node nodes[] = {
        ...
        vxConvertDepthNode(graph, images[0], images[4], VX_CONVERT_POLICY_WRAP,
                           sshift16),
        vxConvertDepthNode(graph, images[4], output, VX_CONVERT_POLICY_WRAP,
                           sshift8),
    };
    ...
    for (i = 0; i < dimof(images); i++) vxReleaseNode(&nodes[i]);
    vxReleasePyramid(&pyr);
  }
  return graph;
}

vx_graph vxPseudoCannyGraph(vx_context context, vx_image input,
                            vx_threshold thresh, vx_image output) {
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_uint32 n = 0;
    ...
    for (n = 0; n < dimof(nodes); n++) vxReleaseNode(&nodes[n]);
    for (n = 0; n < dimof(virts); n++) vxReleaseImage(&virts[n]);
  }
  return graph;
}
  ...
  if (ioctl(dev, VIDIOC_QUERYCTRL, &qctrl) == 0) {
    if ((qctrl.flags & V4L2_CTRL_TYPE_BOOLEAN) ||
      ...
      if ((min <= value) && (value <= max)) {
        if (step > 1) value = value - (value % step);
      } else {
        value = qctrl.default_value;
      }
      ctrl.id = control;
      ctrl.value = value;
      if (ioctl(dev, VIDIOC_S_CTRL, &ctrl) == 0) return 1;
    }
  }
  return 0;
}
...
void v4l2_close(int32_t dev) { close(dev); }
...
V4L2_to_VX_DF_IMAGE_t codes[] = {
    {V4L2_PIX_FMT_UYVY, VX_DF_IMAGE_UYVY, sizeof(uint16_t),
     VX_COLOR_SPACE_BT601_625},
    {V4L2_PIX_FMT_YUYV, VX_DF_IMAGE_YUYV, sizeof(uint16_t),
     VX_COLOR_SPACE_BT601_625},  // capture
    {V4L2_PIX_FMT_YUYV, VX_DF_IMAGE_YUYV, sizeof(uint16_t),
     VX_COLOR_SPACE_BT601_625},
...
void print_buffer(struct v4l2_buffer *desc) {
  printf(
      "struct v4l2_buffer [%u] type:%u used:%u len:%u flags=0x%08x field:%u "
      "memory:%u offset:%u\n",
      desc->index, desc->type, desc->bytesused, desc->length, desc->flags,
      desc->field, desc->memory, desc->m.offset);
}
...
// C99
int32_t vxReleaseV4L2Images(vx_uint32 count, vx_image images[count],
                            void *ptrs[count], int32_t dev) {
  uint32_t i = 0;
  int32_t ret = 0;
...
// C99
int32_t vxCreateV4L2Images(vx_context context, vx_uint32 count,
                           vx_image images[count], vx_uint32 width,
                           vx_uint32 height, vx_df_image format, int32_t dev,
                           void *ptrs[count]) {
  uint32_t c, i;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (buffers[i].flags != V4L2_BUF_FLAG_MAPPED) {
      ptrs[i] = mmap(NULL, buffers[i].length, PROT_READ | PROT_WRITE,
                     MAP_SHARED, dev, buffers[i].m.offset);
      if (ptrs[i] == MAP_FAILED) {
        ...
        images[i] = vxCreateImageFromHandle(context, format, &addr, &ptrs[i],
                                            VX_IMPORT_TYPE_HOST);
        vxSetImageAttribute(images[i], VX_IMAGE_SPACE, &codes[c].space,
                            sizeof(codes[c].space));
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 2) strncpy(xmlfile, argv[2], MAX_PATH);
  ...
  if (context && cam) {
    ...
    if (vxCreateV4L2Images(context, MAX_CAPTURE, images, width, height, format,
                           cam, captures)) {
      i += MAX_CAPTURE;
      ...
      for (s = 0; s < dimof(backplanes); s++, i++) {
        ...
        vx_imagepatch_addressing_t map[] = {
            {width, height, (depth / 8) * sizeof(vx_uint8),
             width * (depth / 8) * sizeof(vx_uint8), VX_SCALE_UNITY,
             VX_SCALE_UNITY, 1, 1}};
        void *ptrs[1];
        backplanes[s] = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height,
                                             depth, 0x000000FF, 0x0000FF00,
        ...
        printf("Mapping %p with stride %d to a vx_image\n", ptrs[0],
               map[0].stride_y);
        images[i] = vxCreateImageFromHandle(context, VX_DF_IMAGE_RGB, map, ptrs,
                                            VX_IMPORT_TYPE_HOST);
        assert(images[i] != 0);
    ...
    assert(i == (dimof(backplanes) + dimof(captures)));
    {
      ...
      vxSetThresholdAttribute(thresh, VX_THRESHOLD_THRESHOLD_LOWER, &bounds[0],
                              sizeof(bounds[0]));
      vxSetThresholdAttribute(thresh, VX_THRESHOLD_THRESHOLD_UPPER, &bounds[1],
                              sizeof(bounds[1]));
      // input is the YUYV image.
      // output is the RGB image
      graph = vxPseudoCannyGraph(context, images[0], thresh,
                                 images[dimof(captures)]);
      // graph = vxPyramidIntegral(context, images[0], thresh,
      // images[dimof(captures)]);
      if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    ...
    if (v4l2_start(cam)) {
      do {
        ...
        if (cap != UINT32_MAX) {
          ...
          status = vxSetGraphParameterByIndex(graph, 0,
                                              (vx_reference)images[camIdx]);
          assert(status == VX_SUCCESS);
          status = vxSetGraphParameterByIndex(graph, 2,
                                              (vx_reference)images[dispIdx]);
          assert(status == VX_SUCCESS);
          ...
          SDL_Flip(screen);
          if (v4l2_queue(cam, cap) == 0) printf("Failed to queue image!\n");
        } else {
