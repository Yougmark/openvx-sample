...
static vx_status execute_and_change_int32_parameter(vx_graph graph,
                                                    vx_node node, int paramno,
                                                    vx_int32 newval) {
...
int main(void) {
  ...
  vx_node NodeOk = vxConvertDepthNode(graph_ok, images[2], images[3],
                                      VX_CONVERT_POLICY_WRAP, shift_scalar);
  ...
  /* Then, a second run, showing the incorrect behavior for
