...
void ownPrintNode(vx_node_t *node) {
  if (node)
    VX_PRINT(VX_ZONE_NODE, "vx_node_t:%p %s:%d affinity:%s\n", node,
             node->kernel->name, node->kernel->enumeration,
    ...
    if (ownIsValidSpecificReference(&kernel->base, VX_TYPE_KERNEL) ==
      ...
      for (n = 0; n < VX_INT_MAX_REF; n++) {
        if (graph->nodes[n] == NULL) {
          node = (vx_node)ownCreateReference(graph->base.context, VX_TYPE_NODE,
                                             VX_EXTERNAL, &graph->base);
          if (vxGetStatus((vx_reference)node) == VX_SUCCESS &&
              node->base.type == VX_TYPE_NODE) {
            ...
            memcpy(&node->attributes, &kernel->attributes,
                   sizeof(vx_kernel_attr_t));

            /* setup our forward and back references to the node/graph */
            graph->nodes[n] = node;
            node->graph = graph;
            ownIncrementReference(&node->base,
                                  VX_INTERNAL); /* one for the graph */
      ...
      vxAddLogEntry((vx_reference)g, VX_ERROR_INVALID_REFERENCE,
                    "Kernel %p was invalid!\n", kernel);
      node = (vx_node_t *)ownGetErrorObject(graph->base.context,
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryNode(vx_node n, vx_enum attribute,
    ...
    switch (attribute) {
      ...
      case VX_NODE_BORDER:
        if (VX_CHECK_PARAM(ptr, size, vx_border_t, 0x3)) {
          ...
          memcpy((vx_border_t *)ptr, &node->attributes.borders,
                 sizeof(vx_border_t));
        } else {
          status = VX_ERROR_INVALID_PARAMETERS;
        }
...
VX_API_ENTRY vx_status VX_API_CALL vxSetNodeAttribute(vx_node n,
                                                      vx_enum attribute,
                                                      const void *ptr,
                                                      vx_size size) {
  vx_status status = VX_SUCCESS;
...
#endif
          {
            memcpy(&node->attributes.borders, (vx_border_t *)ptr,
                   sizeof(vx_border_t));
          }
        } else {
          status = VX_ERROR_INVALID_PARAMETERS;
...
void ownDestructNode(vx_reference ref) {
      ...
      if (ref->delay != NULL) {
        ...
        if (res == vx_false_e) {
          VX_PRINT(VX_ZONE_ERROR,
                   "Internal error removing delay association\n");
        }
      }
  ...
  ownReleaseReferenceInt((vx_reference *)&node->kernel, VX_TYPE_KERNEL,
                         VX_INTERNAL, NULL);
}

VX_API_ENTRY vx_status VX_API_CALL vxReleaseNode(vx_node *n) {
  return ownReleaseReferenceInt((vx_reference *)n, VX_TYPE_NODE, VX_EXTERNAL,
                                NULL);
}
...
vx_status ownRemoveNodeInt(vx_node *n) {
  ...
  if (node && ownIsValidSpecificReference(&node->base, VX_TYPE_NODE)) {
    if (node->graph) {
      ...
      if (removedFromGraph) {
        ownReleaseReferenceInt((vx_reference *)&node, VX_TYPE_NODE, VX_INTERNAL,
                               NULL);
        status = VX_SUCCESS;
      }
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxRemoveNode(vx_node *n) {
  ...
  if (node && ownIsValidSpecificReference(&node->base, VX_TYPE_NODE)) {
    ...
    if (status == VX_SUCCESS) {
      status = ownReleaseReferenceInt((vx_reference *)&node, VX_TYPE_NODE,
                                      VX_EXTERNAL, NULL);
      if (status == VX_SUCCESS) {
        *n = NULL;
      }
...
vxAssignNodeCallback(vx_node n, vx_nodecomplete_f callback) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
  vx_node_t *node = (vx_node_t *)n;
  if (ownIsValidSpecificReference(&node->base, VX_TYPE_NODE) == vx_true_e) {
...
VX_API_ENTRY vx_status VX_API_CALL
vxReplicateNode(vx_graph graph, vx_node first_node, vx_bool replicate[],
  ...
  if (ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) != vx_true_e) {
    VX_PRINT(VX_ZONE_ERROR, "Graph %p was invalid!\n", graph);
    vxAddLogEntry((vx_reference)graph, VX_ERROR_INVALID_REFERENCE,
                  "Graph %p as invalid!\n", graph);
    status = VX_ERROR_INVALID_REFERENCE;
  } else if (ownIsValidSpecificReference(&first_node->base, VX_TYPE_NODE) !=
             vx_true_e) {
    VX_PRINT(VX_ZONE_ERROR, "Node %p was invalid!\n", first_node);
    vxAddLogEntry((vx_reference)first_node, VX_ERROR_INVALID_REFERENCE,
                  "Node %p as invalid!\n", first_node);
    status = VX_ERROR_INVALID_REFERENCE;
    ...
    status = vxQueryNode(first_node, VX_NODE_PARAMETERS, &numParams,
                         sizeof(numParams));
    ...
    for (p = 0; (VX_SUCCESS == status) && p < number_of_parameters; p++) {
      ...
      if (replicate[p] == vx_false_e &&
          (dir == VX_OUTPUT || dir == VX_BIDIRECTIONAL))
        status = VX_FAILURE;

      if (replicate[p] == vx_true_e) {
        if (ownIsValidSpecificReference(ref, type) == vx_true_e) {
          vx_size items = 0;
          if (ownIsValidSpecificReference(ref->scope, VX_TYPE_PYRAMID) ==
              vx_true_e) {
            vx_pyramid pyramid = (vx_pyramid)ref->scope;
            vxQueryPyramid(pyramid, VX_PYRAMID_LEVELS, &items, sizeof(vx_size));
          } else if (ownIsValidSpecificReference(
                         ref->scope, VX_TYPE_OBJECT_ARRAY) == vx_true_e) {
          ...
          if (num_of_replicas == 0) num_of_replicas = items;
...
vx_status ownSetChildGraphOfNode(vx_node node, vx_graph graph) {
  ...
  if ((valid_node == vx_true_e) && graph &&
    ...
    /* check to make sure the signature of the node matches the signature of the
     * graph. */
    ...
    if (graph->numParams > 0) {
      ...
      for (p = 0; p < num; p++) {
        ...
        if (graph->parameters[p].node) {
          if ((node->kernel->signature.directions[p] !=
               graph->parameters[p]
                   .node->kernel->signature.directions[child_index]) ||
              (node->kernel->signature.states[p] !=
               graph->parameters[p]
                   .node->kernel->signature.states[child_index]) ||
              (node->kernel->signature.types[p] !=
               graph->parameters[p]
                   .node->kernel->signature.types[child_index])) {
            vxAddLogEntry(
                &graph->base, VX_ERROR_INVALID_PARAMETERS,
                "Graph parameter %u does not match node parameter %u\n"
                "\t%s[%u] dir:%d req:%d type:0%x\n"
                "\t%s[%u] dir:%d req:%d type:0%x\n",
                p, p, node->kernel->name, p,
                node->kernel->signature.directions[p],
                node->kernel->signature.states[p],
                node->kernel->signature.types[p],
                graph->parameters[p].node->kernel->name, p,
                graph->parameters[p]
                    .node->kernel->signature.directions[child_index],
                graph->parameters[p]
                    .node->kernel->signature.states[child_index],
          ...
          vxAddLogEntry(&graph->base, VX_ERROR_INVALID_NODE,
                        "WARNING: the child graph has declared graph parameter "
                        "index %u node reference to be NULL!\n",
                        p);
        }
      }
      if (match == vx_true_e) {
        ...
        VX_PRINT(VX_ZONE_GRAPH,
                 "Graph " VX_FMT_REF " set as child graph of " VX_FMT_REF "\n",
                 graph, node->graph);
        status = VX_SUCCESS;
      }
    } else {
      // vxAddLogEntry((vx_reference)child, VX_ERROR_INVALID_GRAPH, "Number of
      // parameters on graph does not equal number of parameters on node!");
      vxAddLogEntry(&graph->base, VX_ERROR_INVALID_GRAPH,
                    "Graph must have some parameters!");
    }
  } else if ((valid_node == vx_true_e) && (graph == NULL) &&
             (node->child != NULL)) {
    status = ownReleaseReferenceInt((vx_reference *)&node->child, VX_TYPE_GRAPH,
                                    VX_INTERNAL, NULL);
  }
    ...
    switch (target_enum) {
      case VX_TARGET_ANY:
        for (t = 0; (t < context->num_targets) && (kernel == NULL); t++) {
          ...
          kernel = findKernelByEnum(&context->targets[rt],
      ...
      case VX_TARGET_STRING: {
        ...
        if (target_lower_string) {
          ...
          for (t = 0; (t < context->num_targets) && (kernel == NULL); t++) {
            ...
            if (ownMatchTargetNameWithString(context->targets[rt].name,
                                             target_lower_string) ==
                vx_true_e) {
              kernel = findKernelByEnum(&context->targets[rt],
                                        node->kernel->enumeration);
