...
#define VX_IX_USE_IMPORT_AS_KERNEL               \
  (VX_ENUM_BASE(VX_ID_KHRONOS, VX_ENUM_IX_USE) + \
   0x3) /*!< \brief Graph exported as user kernel. */
#endif
...
conflicts in the 'uses' array values are tolerated and resolved in what I think
is a sensible way. No real attempt has been made to reduce the size (or speed)
of the export - this is a pragmatic implementation that seeks merely to satisfy
the specification.

*IMPORTANT NOTE*
================
This implementation requires that the exporting and importing implementations
have the same endianness.

*/
#define VX_IX_ID \
  (0x1234C0DE) /* To identify an export and make sure endian matches etc. */
...
#define USES_OFFSET \
  (sizeof(VXBinHeader)) /* Where the variable part of the blob starts */

#define MAX_CONCURRENT_EXPORTS \
  (10) /* Arbitrary limit on number of exports allowed at any one time */
...
#define IMAGE_SUB_TYPE_CHANNEL \
  (3) /* Code to indicate image created from channel */
#define IMAGE_SUB_TYPE_TENSOR \
  (4) /* Code to indicate an image created from a tensor */
#define IMAGE_SUB_TYPE_HANDLE \
  (5) /* Code to indicate an image created from handle */
...
struct VXBinExportS {
  ...
  VXBinExportRefTable
      *ref_table;  /* Table for all references compiled by export function */
...
static int exportToProcess(VXBinExport *xport, vx_size n, int calcSize) {
  return (calcSize ? OBJECT_UNSIZED : OBJECT_NOT_EXPORTED) ==
         xport->ref_table[n].status;
}

static int isFromHandle(vx_reference ref) {
  /* Return TRUE if the reference has been created from a handle (i.e., by the
   * application)*/
  return (VX_TYPE_IMAGE == ref->type &&
          VX_MEMORY_TYPE_NONE != ((vx_image)ref)->memory_type);
}

static int isImmutable(vx_kernel k, vx_uint32 p) {
  int retval = 0;
  /* return true if parameter p of kernel k is immutable
     This is based upon the parameter to a node function being not an OpenVX
     object
  */
  if (k->enumeration < VX_KERNEL_MAX_1_2 &&
      k->enumeration >= VX_KERNEL_COLOR_CONVERT) {
...
static void malloc_export(VXBinExport *xport) {
  ...
  if (xport->export_length && num_exports < MAX_CONCURRENT_EXPORTS) {
    for (int i = 0; i < MAX_CONCURRENT_EXPORTS && NULL == xport->export_buffer;
         ++i)
      if (NULL == exports[i]) {
  ...
  if (xport->export_buffer) ++num_exports;
}

static vx_size indexIn(const VXBinExportRefTable *ref_table,
                       const vx_reference ref, const vx_size num) {
...
static vx_uint32 indexOf(const VXBinExport *xport, const vx_reference ref) {
  /* Return the first index of the reference in the table or
  (xport->actual_numrefs + ref->type) if it is not there. Note that it's not an
  error if the reference is not found; it may be that an object is being
  exported without its parent, for example a pyramid level or element of an
  object array, or that we are looking for the context. We test for context
  specifically to make the search faster, as this could be a frequent case. The
  type of an unfound reference (usually scope) is encoded for error-checking
  upon import. We also return 0xffffffff if the reference was NULL; this is for
  ...
  */
  ...
  if (ref) /* Check for NULL: this could be an optional parameter */
  {
    if (VX_TYPE_CONTEXT != ref->type) {
      for (i = 0; i < xport->actual_numrefs && ref != xport->ref_table[i].ref;
           ++i) {
        ;
      }
      if (i == xport->actual_numrefs) /* not found; encode the type of the
                                         reference for future use on import */
        i += ref->type;
    }
  } else {
    i = 0xFFFFFFFF;
...
static vx_uint8 *addressOf(VXBinExport *xport, const vx_size n) {
  /* Return the address of the offset into the blob of a given reference by it's
   * index. */
  return xport->export_buffer + USES_OFFSET +
         xport->numrefs * sizeof(vx_uint32) + n * sizeof(vx_uint64);
}

static void calculateUses(VXBinExport *xport) {
  /* Calculate what the uses for each object should be. This is essentially
patching up uses assigned by putInTable...
  */
  vx_size i;
  for (i = xport->numrefs; i < xport->actual_numrefs; ++i) {
    switch (xport->ref_table[i].use) {
      ...
      default: /* All other cases, assume we need to export values unless
                  virtual */
...
static vx_size putInTable(VXBinExportRefTable *ref_table,
                          const vx_reference newref, vx_size actual_numrefs,
                          vx_enum use) {
  /* If the reference newref is NULL or is already in the table, just return
  actual_numrefs, else put it in the table and return actual_numrefs + 1. Note
  that NULL can occur for absent optional parameters and is treated specially
  during output and so does not appear in the table since it can't have been
  supplied in original list of references passed to the export function as they
  must pass getStatus(). */
  if (newref) {
    ...
    if (index == actual_numrefs) {
      ...
      ref_table[index].use =
          vx_true_e == newref->is_virtual ? VX_IX_USE_NO_EXPORT_VALUES : use;
      ++actual_numrefs;
    } else {
      /* Adjust use if required. This allows for inheriting of
      APPLICATION_CREATE and
    ...
    */
    ...
    if (OBJECT_UNPROCESSED ==
        ref_table[index]
            .status) { /* We make sure we are not processing something we have
                       already processed, otherwise we could get in a loop
                       because of processing parents that have
      ...
      if (VX_TYPE_GRAPH == newref->type) {
        /* Need to enumerate all graph parameters, nodes and node parameters in
         * a graph */
        ...
        for (ix = 0; ix < g->numParams; ++ix) {
          ...
          vx_reference r =
              g->parameters[ix].node->parameters[g->parameters[ix].index];
          actual_numrefs = putInTable(ref_table, r, actual_numrefs,
                                      VX_IX_USE_NO_EXPORT_VALUES);
        }
        for (ix = 0; ix < g->numNodes; ++ix) {
          vx_node n = g->nodes[ix];
          vx_uint32 p; /* for enumerating node parameters */
          /* We don't have to put in nodes are these are output as part of each
           * graph */
          /* Bugzilla 16337 - we need to add in the kernel object, however. */
          /* Bugzilla 16207 - may have to export kernel as
           * VX_IX_USE_EXPORT_VALUES rather than VX_IX_USE_APPLICATION_CREATE */
          ...
          actual_numrefs = putInTable(ref_table, (vx_reference)n->kernel,
                                      actual_numrefs, kernel_use);
          for (p = 0; p < n->kernel->signature.num_parameters; ++p) {
            /* Add in each parameter. Note that absent optional parameters
            (NULL) will not end up in the table and have special treatment on
            output (and input). */
            actual_numrefs =
                putInTable(ref_table, n->parameters[p], actual_numrefs,
                           n->kernel->signature.directions[p]);
            /* If a node is replicated, need to check replicated flags and put
             * parent in the list as well */
            if (n->is_replicated && n->replicated_flags[p]) {
              actual_numrefs =
                  putInTable(ref_table, n->parameters[p]->scope, actual_numrefs,
                             n->kernel->signature.directions[p]);
            }
          }
        }
        ...
        for (ix = 0; ix < VX_INT_MAX_REF; ++ix) {
          actual_numrefs =
              putInTable(ref_table, (vx_reference)g->delays[ix], actual_numrefs,
                         VX_IX_USE_NO_EXPORT_VALUES);
        }
        /* Now need to patch up uses; any objects read but not written become
         * VX_IX_EXPORT_VALUES */
      ...
      } else if (VX_TYPE_KERNEL == newref->type &&
                 VX_IX_USE_EXPORT_VALUES == ref_table[index].use) {
        /* Bugzilla 16207 */
        actual_numrefs = putInTable(ref_table, newref->scope, actual_numrefs,
                                    VX_IX_USE_EXPORT_VALUES);
        if (VX_TYPE_GRAPH != newref->scope->type) {
          DEBUGPRINTF(
              "Error in graph kernel export; scope of kernel is not a graph! "
              "Difference is %d\n",
        ...
        for (c = 0; c < delay->count; ++c) {
          /* if parent use is APPLICATION_CREATE or EXPORT_VALUES sub-object
           * inherits use */
          actual_numrefs = putInTable(ref_table, delay->refs[c], actual_numrefs,
                                      ref_table[index].use);
        }
      } else if (VX_TYPE_PYRAMID == newref->type &&
                 vx_false_e == newref->is_virtual) {
        ...
        for (c = 0; c < pyramid->numLevels; ++c) {
           ...
           * inherits use */
          actual_numrefs =
              putInTable(ref_table, (vx_reference)pyramid->levels[c],
                         actual_numrefs, ref_table[index].use);
        }
        ...
        for (c = 0; c < objArray->num_items; ++c) {
          /* if parent use is APPLICATION_CREATE or EXPORT_VALUES sub-object
           * inherits use */
          actual_numrefs = putInTable(ref_table, objArray->items[c],
                                      actual_numrefs, ref_table[index].use);
        }
      ...
      */
      ...
      need to put both the object array and the tensor in the table */
               ...
               VX_TYPE_IMAGE == newref->type && ((vx_image)newref)->parent &&
               VX_TYPE_TENSOR == ((vx_image)newref)->parent->base.type) {
        actual_numrefs = putInTable(ref_table, newref->scope, actual_numrefs,
                                    ref_table[index].use);
        actual_numrefs =
            putInTable(ref_table, (vx_reference)(((vx_image)newref)->parent),
                       actual_numrefs, ref_table[index].use);
      }
    }
  }
...
static vx_status exportBytes(VXBinExport *xport, const void *data,
                             const vx_size length, int calcSize) {
  vx_status status = VX_SUCCESS;
  if (calcSize) {
    xport->export_length += length;
  } else if (xport->export_length - (xport->curptr - xport->export_buffer) >=
             length) {
...
#define EXPORT_FUNCTION(funcname, datatype)                          \
  static vx_status funcname(VXBinExport *xport, const datatype data, \
                            int calcSize) {                          \
...
static vx_status exportHeader(VXBinExport *xport, int calcSize) {
  vx_status status = VX_SUCCESS;
  vx_size i;
  if (!calcSize && ((NULL == xport->export_buffer) ||
                    xport->actual_numrefs & 0xffffffff00000000U)) {
    ...
    if (calcSize) {
      xport->export_length += xport->actual_numrefs * sizeof(vx_uint64);
               ...
               xport->actual_numrefs * sizeof(vx_uint64)) {
...
static vx_status exportObjectHeader(VXBinExport *xport, vx_size n,
                                    int calcSize) {
  /* Set the index into the memory blob for entry ix to be the current pointer,
     and then exports the common information (header) for an object */
  vx_status status = VX_SUCCESS;
  if (calcSize) {
    ...
    xport->curptr = p;
    status |=
        exportVxEnum(xport, rt->ref->type, 0); /* Export type of this object */
    status |= exportVxEnum(xport, rt->use, 0); /* Export usage of this object */
    status |= exportBytes(xport, rt->ref->name, VX_MAX_REFERENCE_NAME,
                          0); /* Export name of this object */
    status |= exportVxUint8(xport, (vx_uint8)(rt->ref->is_virtual ? 1 : 0),
                            0); /* Export virtual flag */
    status |= exportVxUint32(xport, indexOf(xport, rt->ref->scope),
                             0); /* Export scope, for virtuals etc */
    xport->ref_table[n].status = OBJECT_EXPORTED;
  }
  return status;
}

static void computeROIStartXY(vx_image parent, vx_image roi, vx_uint32 *x,
                              vx_uint32 *y) {
  vx_uint32 offset = roi->memory.ptrs[0] - parent->memory.ptrs[0];
  *y = offset * roi->scale[0][VX_DIM_Y] / roi->memory.strides[0][VX_DIM_Y];
  *x = (offset -
        ((*y * roi->memory.strides[0][VX_DIM_Y]) / roi->scale[0][VX_DIM_Y])) *
       roi->scale[0][VX_DIM_X] / roi->memory.strides[0][VX_DIM_X];
}

static vx_status exportPixel(VXBinExport *xport, void *base, vx_uint32 x,
                             vx_uint32 y, vx_uint32 p,
                             vx_imagepatch_addressing_t *addr,
                             vx_df_image format, int calcSize) {
  /* export a pixel. For some YUV formats the meaning of the values changes for
   * odd pixels */
  vx_status status = VX_FAILURE;
  vx_pixel_value_t *pix =
      (vx_pixel_value_t *)vxFormatImagePatchAddress2d(base, x, y, addr);
  switch (format) {
    ...
    case VX_DF_IMAGE_NV21: /* second plane order is different for the two
                              formats */
      if (0 == p)
...
static vx_status exportSinglePixel(VXBinExport *xport, void *base, vx_uint32 p,
  ...
  switch (format) {
    ...
    case VX_DF_IMAGE_RGBX: /* single-plane 4 bytes */
      ...
      break;
    case VX_DF_IMAGE_UYVY: /* single plane, export 3 bytes for YUV mapping
                              UY0VY1 -> YUV*/
    {
    ...
    case VX_DF_IMAGE_YUYV: /* single plane, export 3 bytes for YUV mapping
                              Y0UY1V to YUV */
    {
      vx_uint8 *pix = vxFormatImagePatchAddress2d(base, 0, 0, addr);
      status = exportVxUint8(xport, pix[0], calcSize);
    ...
    case VX_DF_IMAGE_NV12: /* export 1 byte from 1st plane (Y) and 2 from second
                            */
    case VX_DF_IMAGE_NV21: /* second plane UV order is different for the two
                              formats */
    {
      vx_uint8 *pix = vxFormatImagePatchAddress2d(base, 0, 0, addr);
      if (0 == p)
...
static vx_status exportImage(VXBinExport *xport, const vx_size n,
                             int calcSize) {
  vx_image image = (vx_image)xport->ref_table[n].ref;
  vx_status status =
      exportObjectHeader(xport, n, calcSize); /* Export common information */
  if (VX_TYPE_IMAGE == image->base.scope->type) {
    ...
    vx_image parent = image->parent; /* Note: parent has already been exported
                                        as scope in the object header */
    ...
    if (parent->planes == image->planes) {
      ...
      status = exportVxUint8(xport, (vx_uint8)IMAGE_SUB_TYPE_ROI,
                             calcSize);             /* Export as ROI */
      status |= exportVxUint32(xport, x, calcSize); /* Export the dimensions */
      status |= exportVxUint32(xport, y,
                               calcSize); /* To use to make a vx_rectangle_t */
      ...
      status = exportVxUint8(xport, (vx_uint8)IMAGE_SUB_TYPE_CHANNEL,
                             calcSize); /* Export as image from channel */
      status |= exportVxUint32(xport, (vx_uint32)channel,
                               calcSize); /* Export channel */
    }
    ...
    */
    ...
    /* output type */
    status |= exportVxUint32(xport, (vx_uint32)image->width,
                             calcSize); /* Image width */
    status |= exportVxUint32(xport, (vx_uint32)image->height,
                             calcSize); /* Image height */
    status |= exportVxUint32(xport, (vx_uint32)image->format,
                             calcSize); /* Image format */
    /* Now we just output one pixel for each plane as the value to use;
       On import the number of planes is determined by the format, so this
       relies on a properly formed image...
       TODO for version 1.2 we can optimise this since there is a uniform image
       value attribute.
    */
    {
      vx_rectangle_t rect;
      ...
      for (p = 0U; p < image->planes; ++p) {
        ...
        status |=
            exportSinglePixel(xport, base, p, &addr, image->format, calcSize);
        status |= vxUnmapImagePatch(image, id);
      }
    }
  } else if (VX_IX_USE_NO_EXPORT_VALUES == xport->ref_table[n].use &&
             isFromHandle((vx_reference)image)) {
    ...
    status |=
        exportVxDfImage(xport, image->format, calcSize); /* Image format */
    status |=
        exportVxUint32(xport, image->planes,
                       calcSize); /* Number of planes, for ease of import */
    status |=
        exportVxEnum(xport, image->memory_type, calcSize); /* Memory type */
    for (p = 0; p < image->planes; ++p) {
      status |= exportVxInt32(xport, image->memory.strides[p][VX_DIM_X],
                              calcSize); /* x stride for plane p */
      status |= exportVxInt32(xport, image->memory.strides[p][VX_DIM_Y],
                              calcSize); /* y stride for plane p */
    }
  } else {
    /* Regular image */
    ...
    status |=
        exportVxDfImage(xport, image->format, calcSize); /* Image format */
    /* Now we have to output the data, but only if use ==
       VX_IX_USE_EXPORT_VALUES and not virtual
    */
    if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[n].use &&
        !image->base.is_virtual) {
      ...
      /* The number of planes is derived from format upon input; here we output
         all the available pixel data.
      */
      for (p = 0U; p < image->planes; ++p) {
        void *base = NULL;
        vx_uint32 x, y;
        ...
        */
        for (y = 0U; y < addr.dim_y; y += addr.step_y) {
          for (x = 0U; x < addr.dim_x; x += addr.step_x) {
            status |= exportPixel(xport, base, x, y, p, &addr, image->format,
                                  calcSize);
          }
        }
        status |= vxUnmapImagePatch(image, id);
...
static vx_status exportLUT(VXBinExport *xport, const vx_size i, int calcSize) {
  ...
  if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
    ...
    status |=
        exportVxUint32(xport, size, calcSize); /* Size of data following */
    status |= exportBytes(xport, lut->memory.ptrs[0], size,
                          calcSize); /* Export the data */
  } else {
    status |= exportVxUint32(xport, 0U,
                             calcSize); /* Output a zero to indicate no data */
  }
  return status;
}

static vx_status exportDistribution(VXBinExport *xport, const vx_size i,
                                    int calcSize) {
  vx_status status = VX_SUCCESS;
  ...
  if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
    ...
    status |=
        exportVxUint32(xport, size, calcSize); /* Size of data following */
    status |= exportBytes(xport, dist->memory.ptrs[0], size,
                          calcSize); /* Export the data */
  } else {
    status |= exportVxUint32(xport, 0U,
                             calcSize); /* Output a zero to indicate no data */
...
static vx_status exportThreshold(VXBinExport *xport, const vx_size i,
                                 int calcSize) {
  vx_status status = VX_SUCCESS;
...
static vx_status exportMatrix(VXBinExport *xport, const vx_size i,
                              int calcSize) {
  vx_status status = VX_SUCCESS;
  ...
  if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
    ...
    if (VX_TYPE_FLOAT32 ==
        mat->data_type) /* can only be int32, float32, or uint8 */
      size *= sizeof(vx_float32);
    else if (VX_TYPE_UINT32 == mat->data_type)
      size *= sizeof(vx_int32);
    status |=
        exportVxUint32(xport, size, calcSize); /* Size of data following */
    status |= exportBytes(xport, mat->memory.ptrs[0], size,
                          calcSize); /* Export the data */
  } else {
    status |= exportVxUint32(xport, 0U,
                             calcSize); /* Output a zero to indicate no data */
...
static vx_status exportConvolution(VXBinExport *xport, const vx_size i,
                                   int calcSize) {
  vx_status status = VX_SUCCESS;
  ...
  if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
    ...
    vx_uint32 size = conv->base.columns * conv->base.rows +
                     sizeof(vx_int16); /* only type supported is int16 */
    status |=
        exportVxUint32(xport, size, calcSize); /* Size of data following */
    status |= exportBytes(xport, conv->base.memory.ptrs[0], size,
                          calcSize); /* Export the data */
  } else {
    status |= exportVxUint32(xport, 0U,
                             calcSize); /* Output a zero to indicate no data */
...
static vx_status exportScalar(VXBinExport *xport, const vx_size i,
                              int calcSize) {
  ...
  if (VX_TYPE_USER_STRUCT_START <=
          scalar->data_type && /* Now output the name if it's a user struct
                                  (Bugzilla 16338 comment 2)*/
      VX_TYPE_USER_STRUCT_END >= scalar->data_type) {
    int ix = scalar->data_type - VX_TYPE_USER_STRUCT_START;
    if (scalar->base.context->user_structs[ix]
            .name[0]) /* Zero length name not allowed */
      status |= exportBytes(xport, &scalar->base.context->user_structs[ix].name,
                            VX_MAX_STRUCT_NAME, calcSize);
    else
  ...
  if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
    ...
    status |=
        exportVxUint32(xport, item_size, calcSize); /* Size of data following */
    status |= vxCopyScalar(scalar, data, VX_READ_ONLY,
                           VX_MEMORY_TYPE_HOST); /* Read the data */
    status |= exportBytes(xport, &scalar->data, item_size,
                          calcSize); /* Export the data */
    free(data);
  } else {
    status |= exportVxUint32(xport, 0U,
...
static vx_status exportArray(VXBinExport *xport, const vx_size i,
                             int calcSize) {
  vx_status status = VX_SUCCESS;
  ...
  if (VX_TYPE_USER_STRUCT_START <=
          arr->item_type && /* Now output the name if it's a user struct
                               (Bugzilla 16338 comment 2)*/
      VX_TYPE_USER_STRUCT_END >= arr->item_type) {
    int ix = arr->item_type - VX_TYPE_USER_STRUCT_START;
    if (arr->base.context->user_structs[ix]
            .name[0]) /* Zero length name not allowed */
      status |= exportBytes(xport, &arr->base.context->user_structs[ix].name,
                            VX_MAX_STRUCT_NAME, calcSize);
    else
      status = VX_ERROR_INVALID_TYPE;
  }
  if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
    ...
    status |=
        exportVxUint32(xport, size, calcSize); /* Size of data following */
    status |= exportBytes(xport, arr->memory.ptrs[0], size,
                          calcSize); /* Export the data */
  } else {
    status |= exportVxUint32(xport, 0U,
                             calcSize); /* Output a zero to indicate no data */
...
static vx_status exportRemap(VXBinExport *xport, const vx_size i,
                             int calcSize) {
  vx_status status = VX_SUCCESS;
  ...
  if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
    ...
    vx_uint32 size =
        remap->dst_width * remap->dst_height * sizeof(vx_float32) * 2;
    status |=
        exportVxUint32(xport, size, calcSize); /* Size of data following */
    status |= exportBytes(xport, remap->memory.ptrs[0], size,
                          calcSize); /* Export the data */
  } else {
    status |= exportVxUint32(xport, 0U,
                             calcSize); /* Output a zero to indicate no data */
...
static vx_status exportTensor(VXBinExport *xport, const vx_size i,
                              int calcSize) {
  vx_status status = VX_SUCCESS;
  ...
  if (VX_TYPE_TENSOR == tensor->base.scope->type) {
    ...
    status |= exportVxUint32(
        xport, (vx_uint8 *)tensor->parent->addr - (vx_uint8 *)tensor->addr,
        calcSize);
  } else {
    status |=
        exportVxUint32(xport, 0U, calcSize); /* No memory pointer offset */
  }
  /* export the size of each dimension */
  for (dim = 0; dim < tensor->number_of_dimensions && VX_SUCCESS == status;
       ++dim) {
    status =
        exportVxUint32(xport, (vx_uint32)tensor->dimensions[dim], calcSize);
  }
  ...
  if (VX_SUCCESS == status) {
    if (VX_IX_USE_EXPORT_VALUES == xport->ref_table[i].use &&
        vx_false_e == tensor->base.is_virtual && NULL != tensor->addr &&
      ...
      } else if (VX_SUCCESS == status) { /* Export the data */
        vx_size starts[VX_MAX_TENSOR_DIMENSIONS] = {0};
        status = vxCopyTensorPatch(
            tensor, tensor->number_of_dimensions, starts, tensor->dimensions,
...
static vx_status exportGraph(VXBinExport *xport, vx_size i, int calcSize) {
  ...
  /* Bugzilla 16207. Allow export of graphs as user kernels; in this case the
  name of the kernel will be the name of the reference to the graph and it must
  not have a zero length */
  ...
  */
  ...
  for (ix = 0; ix < VX_INT_MAX_REF && VX_SUCCESS == status; ++ix) {
    if (g->delays[ix])
      status |= exportVxUint32(
          xport, indexOf(xport, (vx_reference)g->delays[ix]), calcSize);
  }
  /* Now export the number of nodes */
  status |= exportVxUint32(xport, g->numNodes, calcSize);
  /* Now we will export all the nodes of the graph. All nodes are distinct so
     there is no point them being indexed in the ref_table, they are just put
     here.
   */
  for (ix = 0; ix < g->numNodes && VX_SUCCESS == status; ++ix) {
    ...
    vx_uint32 p;
    /* Bugzilla 16337 - user kernels, we have a kernel export function so we put
     * a link to the kernel here */
    status |= exportVxUint32(xport, indexOf(xport, (vx_reference)node->kernel),
                             calcSize);
    /* The parameter list */
    for (p = 0; p < num_parameters; ++p) {
      status |=
          exportVxUint32(xport, indexOf(xport, node->parameters[p]), calcSize);
    }
    /* Replicated flags */
    ...
    if (node->is_replicated) {
      for (p = 0; p < num_parameters; ++p) {
        status |=
            exportVxUint8(xport, node->replicated_flags[p] ? 1 : 0, calcSize);
      }
    }
    /* Affinity */
  ...
  for (ix = 0; ix < g->numParams && VX_SUCCESS == status; ++ix) {
    ...
    for (nix = 0; nix < g->numNodes; ++nix)
      if (g->nodes[nix] == g->parameters[ix].node) break;
...
static vx_status exportDelay(VXBinExport *xport, vx_size i, int calcSize) {
  ...
  status =
      exportObjectHeader(xport, i, calcSize); /* Export common information */
  status |=
      exportVxUint32(xport, delay->count, calcSize); /* Number of delay slots */
...
static vx_status exportObjectArray(VXBinExport *xport, vx_size i,
                                   int calcSize) {
  vx_status status = VX_SUCCESS;
  ...
  /* Look for the specical case of an object array of images whose parent is a
   * tensor */
  if (VX_TYPE_IMAGE == object_array->items[0]->type &&
    ...
    status |= exportVxEnum(xport, VX_TYPE_TENSOR,
                           calcSize); /* Indicate type of parent */
    ...
    div_t divmod = div((int64_t)image0->memory.ptrs[0] - (int64_t)tensor->addr,
                       image0->memory.strides[0][1]);
    rect.start_y = divmod.quot;
    rect.start_x =
        divmod.rem / (image0->memory.strides[0][1] / tensor->dimensions[0]);
    ...
    if (object_array->num_items > 1) {
      ...
      stride = (image1->memory.ptrs[0] - image0->memory.ptrs[0]) /
               image0->memory.strides[0][2];
    }
    ...
    status |= exportVxEnum(xport, VX_TYPE_INVALID,
                           calcSize); /* Indicate type of parent */
  for (c = 0; c < object_array->num_items; ++c) {
    status |=
        exportVxUint32(xport, indexOf(xport, object_array->items[c]), calcSize);
  }
  return status;
}
...
static vx_status exportPyramid(VXBinExport *xport, vx_size i, int calcSize) {
  ...
  for (c = 0; c < pyramid->numLevels; ++c) {
    /* Export each level */
    status |= exportVxUint32(
        xport, indexOf(xport, (vx_reference)pyramid->levels[c]), calcSize);
  }
  return status;
}
  ...
  for (i = 0; i < xport->actual_numrefs && VX_SUCCESS == status; ++i) {
    if (exportToProcess(xport, i, calcSize)) {
      switch (xport->ref_table[i].ref->type) {
        case VX_TYPE_IMAGE:
          status = exportImage(xport, i, calcSize);
          break;
        case VX_TYPE_LUT:
          status = exportLUT(xport, i, calcSize);
          break;
        case VX_TYPE_DISTRIBUTION:
          status = exportDistribution(xport, i, calcSize);
          break;
        case VX_TYPE_THRESHOLD:
          status = exportThreshold(xport, i, calcSize);
          break;
        case VX_TYPE_MATRIX:
          status = exportMatrix(xport, i, calcSize);
          break;
        case VX_TYPE_CONVOLUTION:
          status = exportConvolution(xport, i, calcSize);
          break;
        case VX_TYPE_SCALAR:
          status = exportScalar(xport, i, calcSize);
          break;
        case VX_TYPE_ARRAY:
          status = exportArray(xport, i, calcSize);
          break;
        case VX_TYPE_REMAP:
          status = exportRemap(xport, i, calcSize);
          break;
        case VX_TYPE_OBJECT_ARRAY:
          status = exportObjectArray(xport, i, calcSize);
          break;
        case VX_TYPE_PYRAMID:
          status = exportPyramid(xport, i, calcSize);
          break;
        case VX_TYPE_DELAY:
          status = exportDelay(xport, i, calcSize);
          break;
        case VX_TYPE_GRAPH:
          status = exportGraph(xport, i, calcSize);
          break;
        case VX_TYPE_TENSOR:
          status = exportTensor(xport, i, calcSize);
          break;
        case VX_TYPE_KERNEL:
          status = exportKernel(xport, i, calcSize);
          break;
...
VX_API_ENTRY vx_status VX_API_CALL vxExportObjectsToMemory(
    vx_context context, vx_size numrefs, const vx_reference *refs,
    const vx_enum *uses, const vx_uint8 **ptr, vx_size *length) {
  vx_status retval =
      VX_SUCCESS; /* To keep track of any errors, and the return value */
  VXBinExport xport = {
      .context = context,
      .refs = refs,
      .uses = uses,
  ...
  if (VX_SUCCESS != vxGetStatus((vx_reference)context) ||
      ...
      uses == NULL || ptr == NULL || length == NULL) {
        ...
        if (context != refs[i]->context) {
          ...
          if (refs[i]->name[0] != '\0') {
            for (i2 = i + 1; i2 < numrefs; ++i2) {
              if (refs[i] != refs[i2] &&
                  0 == strncmp(refs[i]->name, refs[i2]->name,
        ...
        switch (refs[i]->type) {
          ...
          case VX_TYPE_GRAPH:
            if (VX_IX_USE_EXPORT_VALUES != uses[i]) {
              ...
              retval =
                  vxVerifyGraph(g); /* export will fail if verification fails */
              if (retval) {
                DEBUGPRINTF("Graph verification failed!\n");
              }

              /* All graph parameters are automotically exported with
              VX_IX_USE_NO_EXPORT_VALUES unless already listed - see 16313. If
              attached to another graph somewhere *must* be listed */
              for (ix = 0; VX_SUCCESS == retval && ix < g->numParams; ++ix) {
                ...
                for (i2 = 0; i2 < numrefs && !found; ++i2)
                  found = (refs[i2] == r);
                if (!found) {
                  /* now check to make sure thay are not on another graph
                   * somewhere */
                  for (i2 = i + 1; VX_SUCCESS == retval && i2 < numrefs; ++i2) {
                    if (VX_TYPE_GRAPH == refs[i2]->type) {
                      vx_graph g2 = (vx_graph)refs[i2];
                      vx_uint32 n = 0;
                      for (n = 0; VX_SUCCESS == retval && n < g2->numNodes;
                           ++n) {
                        vx_uint32 p = 0;
                        for (p = 0;
                             VX_SUCCESS == retval &&
                             p < g2->nodes[n]->kernel->signature.num_parameters;
                             ++p) {
                          if (g2->nodes[n]->parameters[p] == r) {
                            DEBUGPRINTF(
                                "Unlisted graph parameter found in another "
                                "graph!\n");
                            retval = VX_ERROR_INVALID_PARAMETERS;
                          }
                        }
              ...
              /* "Immutable" node parameters can't be in refs, need to check all
               * nodes in the graph */
              for (ix = 0; VX_SUCCESS == retval && ix < g->numNodes; ++ix) {
                ...
                vx_uint32 p; /* for enumerating node parameters */
                for (p = 0;
                     VX_SUCCESS == retval && p < k->signature.num_parameters;
                     ++p)
                  if (isImmutable(k, p)) {
          ...
          case VX_TYPE_TENSOR: {
            vx_enum use = uses[i];
            if (refs[i]
                    ->is_virtual || /* No exported references may be virtual */
                /* Objects created from handles must use
                   VX_IX_USE_APPLICATION_CREATE or VX_IS_USE_NO_EXPORT_VALUES
                   (See bugzilla 16312)*/
                (isFromHandle(refs[i]) && VX_IX_USE_APPLICATION_CREATE != use &&
                 VX_IX_USE_NO_EXPORT_VALUES != use) ||
                /* uses must contain valid enumerants for objects that aren't
                   graphs */
              ...
              retval = VX_ERROR_INVALID_PARAMETERS;
              DEBUGPRINTF(
                  "Virtual or fromhandle found, i = %lu, use = %d, virtual = "
                  "%s\n",
          ...
          case VX_TYPE_KERNEL:
            /* Bugzilla 16207 comment 3. 'graph kernels' may be exported as
            kernels. The underlying graph is exported.
            */
            if (VX_IX_USE_EXPORT_VALUES == uses[i]) {
              if (vx_false_e == ((vx_kernel)refs[i])->user_kernel ||
                  VX_TYPE_GRAPH != refs[i]->scope->type) {
                /* Kernels being exported this way must be user kernels, and
                 * have their scope be a graph. */
                retval = VX_ERROR_INVALID_PARAMETERS;
              } else {
                /* We don't have to process the graph at this stage as it must
                have already been verified and checked. It will be placed in
                ref_table for export processing later by putInTable().
                */
              }
            }
            /* Bugzilla 16337 */
  ...
  if (VX_SUCCESS == retval) {
    /* First, make a table of unique references to export; first allocate an
     * array big enough for any eventuality */
    xport.ref_table = (VXBinExportRefTable *)calloc(sizeof(VXBinExportRefTable),
                                                    context->num_references);
    xport.actual_numrefs = numrefs;
    /* Now, populate the table and find the actual number of references to
     * export */
    if (xport.ref_table) {
      ...
      /* Next, populate with references from any compound objects; putInTable
         will expand them recursively, but will only process any object with
         status OBJECT_UNPROCESSED, then setting its status to OBJECT_UNSIZED.
      */
      for (i = 0; i < numrefs; ++i) {
        xport.actual_numrefs =
            putInTable(xport.ref_table, refs[i], xport.actual_numrefs, uses[i]);
      }
      /* Calculate the uses for each reference */
      calculateUses(&xport);
      /* Now, calculate the size of the export */
      retval = exportObjects(&xport, 1);
      /* Increment the length of the export to take into account a checksum at
       * the end */
      ...
      for (i = 0; i < numrefs; ++i) {
        ...
        for (i2 = i + 1; i2 < numrefs; ++i2)
          if (xport.ref_table[i].ref == xport.ref_table[i2].ref) {
            ...
            memcpy(addressOf(&xport, i2), addressOf(&xport, i),
                   sizeof(vx_uint64));
          }
      }
...
vxReleaseExportedMemory(vx_context context, const vx_uint8 **ptr) {
  vx_status retval = VX_ERROR_INVALID_PARAMETERS;
  /* First, check we have some exports to release, that the pointer provided is
     not NULL, and that the context is valid. Note that the list of exports is
     across all contexts */
  if (VX_SUCCESS == vxGetStatus((vx_reference)context) && *ptr && **ptr &&
      num_exports) {
    /* next, find the pointer in our list of pointers.*/
    for (int i = 0; i < MAX_CONCURRENT_EXPORTS && VX_SUCCESS != retval; ++i) {
      if (*ptr == exports[i]) {
        /* found it, all good, we can free the memory, zero the pointers and set
