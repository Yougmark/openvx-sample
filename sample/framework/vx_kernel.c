...
void vxPrintKernel(vx_kernel kernel) {
  VX_PRINT(VX_ZONE_KERNEL, "kernel[%u] enabled?=%s %s \n", kernel->enumeration,
           (kernel->enabled ? "TRUE" : "FALSE"), kernel->name);
}
...
vx_bool ownIsKernelUnique(vx_kernel kernel) {
  ...
  for (t = 0u; t < context->num_targets; t++) {
    for (k = 0u; k < VX_INT_MAX_KERNELS; k++) {
      if (context->targets[t].kernels[k].enabled &&
...
vx_kernel_t *ownAllocateKernel(vx_context context, vx_enum kenum,
                               ...
                               vx_uint32 numParams) {
  vx_kernel kernel = (vx_kernel)ownCreateReference(context, VX_TYPE_KERNEL,
                                                   VX_INTERNAL, &context->base);
...
vx_status ownInitializeKernel(
  ...
  if (kernel) {
    ...
    kernel->attributes.valid_rect_reset =
        vx_false_e; /* default value for std nodes */
...
vx_status ownDeinitializeKernel(vx_kernel *kernel) {
  ...
  if (kernel && ownIsValidSpecificReference(&((*kernel)->base),
                                            VX_TYPE_KERNEL) == vx_true_e) {
    VX_PRINT(VX_ZONE_KERNEL, "Releasing kernel " VX_FMT_REF "\n",
             (void *)*kernel);
    status = ownReleaseReferenceInt((vx_reference *)kernel, VX_TYPE_KERNEL,
                                    VX_INTERNAL, NULL);
    if (*kernel != NULL) {
  ...
  for (m = 0; m < VX_INT_MAX_MODULES; m++) {
    ...
    if (context->modules[m].handle == NULL) {
      ...
      if (context->modules[m].handle) {
        vx_symbol_t sym =
            ownGetSymbol(context->modules[m].handle, "vxPublishKernels");
...
VX_API_ENTRY vx_status VX_API_CALL vxUnloadKernels(vx_context context,
  ...
  for (m = 0; m < VX_INT_MAX_MODULES; m++) {
    ...
    if (context->modules[m].handle != NULL &&
      ...
      vx_symbol_t sym =
          ownGetSymbol(context->modules[m].handle, "vxUnpublishKernels");
      ...
      if (unpublish == NULL) {
        ...
        if (status != VX_SUCCESS) {
          VX_PRINT(VX_ZONE_ERROR,
                   "Failed to unpublish kernels in module (%s)\n", module);
        } else {
          ownUnloadModule(context->modules[m].handle);
          context->modules[m].handle = NULL;
          context->num_modules--;
...
VX_API_ENTRY vx_kernel VX_API_CALL vxGetKernelByName(
    vx_context context, const vx_char string[VX_MAX_KERNEL_NAME]) {
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    ...
    for (t = 0; t < context->num_targets && kern == NULL; t++) {
      ...
      if (target == NULL || target->enabled == vx_false_e) continue;
      if (target->funcs.supports(target, targetName, kernelName, &k) ==
          VX_SUCCESS) {
    ...
    if (kern == NULL) {
      ...
      kern = (vx_kernel_t *)ownGetErrorObject(context,
...
VX_API_ENTRY vx_kernel VX_API_CALL vxGetKernelByEnum(vx_context context,
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    ...
    for (t = 0; t < context->num_targets; t++) {
      ...
      for (k = 0; k < VX_INT_MAX_KERNELS; k++) {
        if (target->kernels[k].enumeration == kernelenum) {
          ...
          VX_PRINT(VX_ZONE_KERNEL,
                   "Found Kernel[%u] enum:%d name:%s in target[%u]=%s\n", k,
                   kernelenum, kernel->name, context->priority_targets[t],
                   target->name);
          break;
        }
      }
    ...
    if (kernel == NULL) {
      ...
      kernel = (vx_kernel_t *)ownGetErrorObject(context,
                                                VX_ERROR_INVALID_PARAMETERS);
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseKernel(vx_kernel *kernel) {
  ...
  if (kernel && ownIsValidSpecificReference(&((*kernel)->base),
    ...
    VX_PRINT(VX_ZONE_KERNEL, "Releasing kernel " VX_FMT_REF "\n",
             (void *)*kernel);
    ownReleaseReferenceInt((vx_reference *)kernel, VX_TYPE_KERNEL, VX_EXTERNAL,
                           NULL);
  } else {
    status = VX_ERROR_INVALID_REFERENCE;
    VX_PRINT(VX_ZONE_ERROR, "Invalid Reference!\n");
...
static vx_kernel addkernel(vx_context c, const vx_char name[VX_MAX_KERNEL_NAME],
  ...
  if (func_ptr == NULL ||
      ...
      numParams > VX_INT_MAX_PARAMS || numParams == 0 || name == NULL ||
      strncmp(name, "", VX_MAX_KERNEL_NAME) == 0)
  /* initialize and de-initialize can be NULL */
  {
    VX_PRINT(VX_ZONE_ERROR, "Invalid Parameters!\n");
    vxAddLogEntry(
        (vx_reference)c, VX_ERROR_INVALID_PARAMETERS,
        "Invalid Parameters supplied to vxAddKernel or vxAddUserKernel\n");
    kernel =
        (vx_kernel_t *)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
    return kernel;
  }
  ...
  if (target) {
    kernel = target->funcs.addkernel(target, name, enumeration, func_ptr,
                                     numParams, validate, input, output,
    ...
    vxAddLogEntry((vx_reference)c, VX_ERROR_NO_RESOURCES,
                  "No target named %s exists!\n", targetName);
...
VX_API_ENTRY vx_kernel VX_API_CALL vxAddKernel(
    vx_context c, const vx_char name[VX_MAX_KERNEL_NAME], vx_enum enumeration,
    vx_kernel_f func_ptr, vx_uint32 numParams, vx_kernel_input_validate_f input,
    vx_kernel_output_validate_f output, vx_kernel_initialize_f initialize,
    vx_kernel_deinitialize_f deinitialize) {
  return addkernel(c, name, enumeration, func_ptr, numParams, NULL, input,
                   output, initialize, deinitialize, vx_false_e);
}

/*
// add user kernels
*/
VX_API_ENTRY vx_kernel VX_API_CALL vxAddUserKernel(
    vx_context c, const vx_char name[VX_MAX_KERNEL_NAME], vx_enum enumeration,
    vx_kernel_f func_ptr, vx_uint32 numParams, vx_kernel_validate_f validate,
    vx_kernel_initialize_f initialize, vx_kernel_deinitialize_f deinitialize) {
  return addkernel(c, name, enumeration, func_ptr, numParams, validate, NULL,
                   NULL, initialize, deinitialize, vx_true_e);
}

#ifdef OPENVX_KHR_TILING
VX_API_ENTRY vx_kernel VX_API_CALL vxAddTilingKernel(
    vx_context c, vx_char name[VX_MAX_KERNEL_NAME], vx_enum enumeration,
  ...
  if ((flexible_func_ptr == NULL && fast_func_ptr == NULL) || input == NULL ||
      output == NULL || num_params > VX_INT_MAX_PARAMS || num_params == 0 ||
      name == NULL || strncmp(name, "", VX_MAX_KERNEL_NAME) == 0)
  /* initialize and de-initialize can be NULL */
  {
    VX_PRINT(VX_ZONE_ERROR, "Invalid Parameters!\n");
    vxAddLogEntry((vx_reference)c, VX_ERROR_INVALID_PARAMETERS,
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryKernel(vx_kernel kern,
...
VX_API_ENTRY vx_status VX_API_CALL vxAddParameterToKernel(vx_kernel kernel,
  ...
  VX_PRINT(VX_ZONE_KERNEL,
           "INFO: Adding index %u, data_type 0x%x, dir:%d state:%d\n", index,
           data_type, dir, state);
...
VX_API_ENTRY vx_status VX_API_CALL vxRemoveKernel(vx_kernel kernel) {
  ...
  if (kernel &&
    ...
    /* find back references to kernel's target and kernel in target->kernels
     * array */
    vx_uint32 index =
        (vx_uint32)(strnindex(kernel->name, ':', VX_MAX_TARGET_NAME));
    if (index == VX_MAX_TARGET_NAME) {
      strcpy(targetName, "khronos.any");
    } else {
...
VX_API_ENTRY vx_status VX_API_CALL vxSetKernelAttribute(vx_kernel k,
                                                        vx_enum attribute,
                                                        const void *ptr,
                                                        vx_size size) {
  vx_status status = VX_SUCCESS;
  ...
  if (!k || ownIsValidSpecificReference(&kernel->base, VX_TYPE_KERNEL) ==
                vx_false_e) {
    return VX_ERROR_INVALID_REFERENCE;
  }
  if (kernel->enabled == vx_true_e) {
  ...
  switch (attribute) {
    case VX_KERNEL_LOCAL_DATA_SIZE:
      if (VX_CHECK_PARAM(ptr, size, vx_size, 0x3)) {
        ...
        VX_PRINT(VX_ZONE_KERNEL,
                 "Set Local Data Size to " VX_FMT_SIZE " bytes\n",
