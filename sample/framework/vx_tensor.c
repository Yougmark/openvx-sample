...
void ownInitTensor(vx_tensor tensor, const vx_size *dimensions,
  ...
  for (vx_uint32 i = (vx_uint32)number_of_dimensions;
       i < VX_MAX_TENSOR_DIMENSIONS; i++) {
...
vxCreateTensor(vx_context context, vx_size number_of_dims, const vx_size *dims,
  ...
  if (number_of_dims < 1) {
    ...
    return (vx_tensor)ownGetErrorObject((vx_context)context,
  ...
  tensor = (vx_tensor)ownCreateReference(context, VX_TYPE_TENSOR, VX_EXTERNAL,
                                         &context->base);
...
VX_API_ENTRY vx_object_array VX_API_CALL vxCreateImageObjectArrayFromTensor(
    vx_tensor tensor, const vx_rectangle_t *rect, vx_size array_size,
    vx_size stride, vx_df_image image_format) {
  (void)array_size;
  (void)stride;

  vx_object_array images = (vx_object_array)ownCreateReference(
      tensor->base.context, VX_TYPE_OBJECT_ARRAY, VX_EXTERNAL,
      (vx_reference)tensor->base.context);  //  TODO scope?
  ...
  if (ownIsValidTensor(tensor) == vx_true_e) {
    ...
    if (ownAllocateTensorMemory(tensor) != NULL) {
      if ((tensor->data_type == VX_TYPE_INT16 &&
          ...
          (tensor->data_type == VX_TYPE_UINT8 &&
           image_format == VX_DF_IMAGE_U8)) {
        if (tensor->number_of_dimensions == 3) {
          if ((rect->end_y <= tensor->dimensions[1]) &&
              (rect->end_x <= tensor->dimensions[0])) {
            vx_uint32 image_byte_size =
                (vx_uint32)(tensor->dimensions[1] * tensor->dimensions[0] *
                            ownSizeOfType(tensor->data_type));
            ...
            addr.stride_y = (vx_uint32)(tensor->dimensions[0] *
                                        ownSizeOfType(tensor->data_type));
            // vx_image exemplar = vxCreateImage (tensor->base.context,
            // addr.dim_x, addr.dim_y, image_format);
            if (vxGetStatus((vx_reference)images) == VX_SUCCESS &&
                images->base.type == VX_TYPE_OBJECT_ARRAY) {
              int i;
              ...
              for (i = 0; i < (int)tensor->dimensions[2]; i++) {
                vx_image subimage = vxCreateImage(
                    tensor->base.context, addr.dim_x, addr.dim_y, image_format);
                if (subimage) {
                  vx_uint32 p = 0;
                  subimage->parent = (vx_image)tensor;
                  subimage->base.scope = (vx_reference_t *)tensor;
                  subimage->memory.allocated = vx_true_e;
                  subimage->memory.ptrs[0] = (vx_uint8 *)(tensor->addr) +
                                             rect->start_y * addr.stride_y +
                                             rect->start_x * addr.stride_x +
                                             i * image_byte_size;
            ...
            images = (vx_object_array)ownGetErrorObject(
                tensor->base.context, VX_ERROR_INVALID_DIMENSION);
          }
        } else {
          VX_PRINT(VX_ZONE_ERROR,
                   "Failed to create image array from a non-3d tensor!\n");
          images = (vx_object_array)ownGetErrorObject(
              tensor->base.context, VX_ERROR_INVALID_DIMENSION);
        }
      } else {
        VX_PRINT(VX_ZONE_ERROR, "Failed to allocate tensor memory!\n");
        images = (vx_object_array)ownGetErrorObject(tensor->base.context,
                                                    VX_ERROR_NO_MEMORY);
      }
    } else {
      VX_PRINT(VX_ZONE_ERROR,
               "Requested image format does not match tensor type!\n");
      images = (vx_object_array)ownGetErrorObject(tensor->base.context,
...
VX_API_ENTRY vx_tensor VX_API_CALL
vxCreateTensorFromView(vx_tensor tensor, vx_size number_of_dimensions,
                       const vx_size *view_start, const vx_size *view_end) {
  vx_tensor subtensor = NULL;
  (void)number_of_dimensions;

  if ((ownIsValidTensor(tensor) == vx_true_e) && (NULL != view_start) &&
      (NULL != view_end)) {
    /* perhaps the parent hasn't been allocated yet? */
    if (ownAllocateTensorMemory(tensor) != NULL) {
      subtensor = (vx_tensor)ownCreateReference(tensor->base.context,
                                                VX_TYPE_TENSOR, VX_EXTERNAL,
                                                &tensor->base.context->base);
      if (subtensor) {
        vx_uint32 p = 0;
        ...
        for (vx_uint32 i = 0; i < subtensor->number_of_dimensions; i++) {
          ...
          //((vx_uint8*)(subtensor->addr)) += view->start[i] *
          // tensor->stride[i];
          subtensor->addr =
              (vx_uint8 *)(subtensor->addr) + view_start[i] * tensor->stride[i];
        }
      } else {
        VX_PRINT(VX_ZONE_ERROR, "Child tensor failed to allocate!\n");
      }
    } else {
      subtensor = (vx_tensor)ownGetErrorObject(tensor->base.context,
                                               VX_ERROR_NO_MEMORY);
...
VX_API_ENTRY vx_tensor VX_API_CALL vxCreateVirtualTensor(
    vx_graph graph, vx_size number_of_dims, const vx_size *dims,
    vx_enum data_type, vx_int8 fixed_point_position) {
  vx_tensor tensor = NULL;
  ...
  if (ownIsValidSpecificReference(gref, VX_TYPE_GRAPH) == vx_true_e) {
    if (data_type == VX_TYPE_INVALID ||
        validFormat(data_type, fixed_point_position)) {
      tensor = (vx_tensor)ownCreateReference(gref->context, VX_TYPE_TENSOR,
                                             VX_EXTERNAL, &gref->context->base);
      if (vxGetStatus((vx_reference)tensor) == VX_SUCCESS &&
          tensor->base.type == VX_TYPE_TENSOR) {
        ownInitTensor(tensor, dims, number_of_dims, data_type,
      ...
      tensor = (vx_tensor)ownGetErrorObject(graph->base.context,
                                            VX_ERROR_INVALID_FORMAT);
    }
  }
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseTensor(vx_tensor *tensor) {
  ...
  if (tensor) {
    // TODO: I assume we need to release from parent like we do in vx_image.c:
    // vxReleaseImage
  }

  return ownReleaseReferenceInt((vx_reference *)tensor, VX_TYPE_TENSOR,
                                VX_EXTERNAL, NULL);
}
  ...
  if (ownIsValidTensor(tensor) == vx_true_e) {
    switch (attribute) {
      case VX_TENSOR_NUMBER_OF_DIMS:
        ...
        break;
      case VX_TENSOR_DIMS:
        if (size >= (sizeof(vx_size) * tensor->number_of_dimensions) &&
            ((vx_size)ptr & 0x3) == 0) {
          memcpy(ptr, tensor->dimensions,
                 sizeof(vx_size) * tensor->number_of_dimensions);
        } else {
          status = VX_ERROR_INVALID_PARAMETERS;
        }
        break;
...
int CheckSizes(vx_size *dimensions, const vx_size *view_start,
               const vx_size *view_end, vx_size number_of_dimensions) {
  for (vx_size i = 0; i < number_of_dimensions; i++) {
    if (view_end[i] <= view_start[i] || view_end[i] > dimensions[i]) return -1;
  }
  return 0;
}

vx_size ComputePatchSize(const vx_size *view_start, const vx_size *view_end,
...
void ComputePositionsFromIndex(vx_size index, const vx_size *start,
                               const vx_size *end, const vx_size *tensor_stride,
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyTensorPatch(
    vx_tensor tensor, vx_size number_of_dimensions, const vx_size *view_start,
  ...
  /* bad parameters */
  if ((view_start == NULL) || (view_end == NULL) || (user_stride == NULL) ||
      (user_ptr == NULL)) {
    status = VX_ERROR_INVALID_PARAMETERS;
    goto exit;
  }
  ...
  if (tensor->number_of_dimensions < number_of_dimensions ||
      number_of_dimensions == 0) {
    VX_PRINT(VX_ZONE_ERROR, "Invalid number of patch dimensions\n");
    status = VX_ERROR_INVALID_PARAMETERS;
    goto exit;
  }
  if (CheckSizes(tensor->dimensions, view_start, view_end,
                 number_of_dimensions) != 0) {
  ...
  vx_size patch_size =
      ComputePatchSize(view_start, view_end, number_of_dimensions);
  for (vx_size i = 0; i < patch_size; i++) {
    vx_size patch_pos = 0;
    vx_size tensor_pos = 0;
    ComputePositionsFromIndex(i, view_start, view_end, tensor->stride,
                              user_stride, number_of_dimensions, &tensor_pos,
                              &patch_pos);
    if (usage == VX_READ_ONLY)
      memcpy(user_curr_ptr + patch_pos, tensor_ptr + tensor_pos,
             tensor->stride[0]);
    else
      memcpy(tensor_ptr + tensor_pos, user_curr_ptr + patch_pos,
             tensor->stride[0]);
  }
  status = VX_SUCCESS;
...
void ownDestructTensor(vx_reference ref) {
  ...
  if (tensor->parent == NULL) {
    ...
    ownReleaseReferenceInt((vx_reference *)&tensor->parent, VX_TYPE_TENSOR,
                           VX_INTERNAL, NULL);
  }
}
