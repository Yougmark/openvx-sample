...
static vx_bool vxIsImgInVirtPyramid(vx_reference ref) {
  ...
  if (ref->scope && (ref->scope->type == VX_TYPE_PYRAMID) &&
      ref->scope->is_virtual) {
    match = vx_true_e;
  }
...
static vx_bool vxIsDelayInDelay(vx_reference ref) {
  ...
  if (ref->scope && (ref->type == VX_TYPE_DELAY) &&
      (ref->scope->type == VX_TYPE_DELAY)) {
    match = vx_true_e;
  }
...
#if defined(_LITTLE_ENDIAN_) || (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__) || \
...
static void vxComputeROIStartXY(vx_image parent, vx_image roi, vx_uint32 p,
  ...
  *x = (offset -
        ((*y * roi->memory.strides[p][VX_DIM_Y]) / roi->scale[p][VX_DIM_Y])) *
       roi->scale[p][VX_DIM_X] / roi->memory.strides[p][VX_DIM_X];
}

static vx_status vxExportToXMLROI(FILE *fp, vx_reference refs[], vx_uint32 r,
                                  vx_uint32 id, vx_uint32 numrefs) {
  vx_status status = VX_SUCCESS;
  vx_image_t *image = (vx_image_t *)refs[r];
  vx_char indent[10] = {0};
  vx_uint32 i, start_x, start_y;
  for (i = 0; i < (dimof(indent) - 1) && i < id; i++) indent[i] = '\t';
  indent[i] = '\0';

  vxComputeROIStartXY(image->parent, image, 0, &start_x, &start_y);
  fprintf(fp,
          "%s<roi reference=\"%u\" start_x=\"%u\" start_y=\"%u\" end_x=\"%u\" "
          "end_y=\"%u\"%s",
          indent, r, start_x, start_y, start_x + image->width,
          start_y + image->height, refNameStr);
...
static vx_status vxExportToXMLImage(FILE *fp, vx_reference refs[], vx_uint32 r,
                                    vx_uint32 id, vx_uint32 numrefs) {
  vx_status status = VX_SUCCESS;
  vx_image_t *image = (vx_image_t *)refs[r];
  vx_char indent[10] = {0};
  ...
  fprintf(
      fp,
      "%s<image reference=\"%u\" width=\"%u\" height=\"%u\" format=\"%s\"%s",
      indent, r, image->width, image->height, vxFourCCString(image->format),
      refNameStr);
  ...
  if (refs[r]->is_virtual == vx_true_e) {
    ...
    if (vx_true_e == image->constant) {
      ...
      for (p = 0u; p < image->planes; p++) {
        void *base = NULL;
        status |= vxAccessImagePatch((vx_image)image, &rect, p, &addr, &base,
                                     VX_READ_ONLY);
        ...
        for (y = 0u; y < addr.dim_y; y += addr.step_y) {
          for (x = 0u; x < addr.dim_x; x += addr.step_x) {
            if ((image->format == VX_DF_IMAGE_U8) ||
              ...
              vx_uint8 *ptr = vxFormatImagePatchAddress2d(base, x, y, &addr);
              fprintf(fp,
                      "%s\t\t\t<rgb x=\"%u\" y=\"%u\">#%02x%02x%02x</rgb>\n",
                      indent, x, y, ptr[0], ptr[1], ptr[2]);
            } else if (image->format == VX_DF_IMAGE_RGBX) {
              vx_uint8 *ptr = vxFormatImagePatchAddress2d(base, x, y, &addr);
              fprintf(
                  fp,
                  "%s\t\t\t<rgba x=\"%u\" y=\"%u\">#%02x%02x%02x%02x</rgba>\n",
...
static vx_status vxExportToXMLPyramid(FILE *fp, vx_reference refs[],
                                      vx_uint32 r, vx_uint32 id,
                                      vx_uint32 numrefs) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<pyramid reference=\"%u\" width=\"%u\" height=\"%u\" "
          "format=\"%s\" scale=\"%f\" levels=\"" VX_FMT_SIZE "\"%s",
          indent, r, pyr->width, pyr->height, vxFourCCString(pyr->format),
          pyr->scale, pyr->numLevels, refNameStr);
...
static vx_status vxExportToXMLArray(FILE *fp, vx_reference refs[], vx_uint32 r,
                                    vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<array reference=\"%u\" capacity=\"" VX_FMT_SIZE "\" elemType=",
          indent, r, array->capacity);

  if (j == -1) { /* Type was not found, check if it is a user type */
    if (array->item_type >= VX_TYPE_USER_STRUCT_START &&
        array->item_type <
            VX_TYPE_USER_STRUCT_START + VX_INT_MAX_USER_STRUCTS) {
      fprintf(fp, "\"USER_STRUCT_%d\"",
              array->item_type - VX_TYPE_USER_STRUCT_START);
      ...
      switch (array->item_type) {
        case VX_TYPE_CHAR: {
          ...
          for (j = 0; j < array->num_items; j++) {
            /* character array is a string in xsd, not space-separated as lists
             * are */
            fprintf(fp, "%c", ptr[j]);
          }
          fprintf(fp, "</char>\n");
        ...
        case VX_TYPE_COORDINATES2D: {
          vx_coordinates2d_t *cord2d =
              (vx_coordinates2d_t *)array->memory.ptrs[0];
          for (j = 0; j < array->num_items; j++) {
        ...
        case VX_TYPE_COORDINATES3D: {
          vx_coordinates3d_t *cord3d =
              (vx_coordinates3d_t *)array->memory.ptrs[0];
          for (j = 0; j < array->num_items; j++) {
...
static vx_status vxExportToXMLLut(FILE *fp, vx_reference refs[], vx_uint32 r,
                                  vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<lut reference=\"%u\" count=\"" VX_FMT_SIZE "\" elemType=\"%s\"%s",
          indent, r, lut->num_items, type_pairs[j].name, refNameStr);

  if (refs[r]->is_virtual ==
      vx_true_e) /* is not virtual in 1.0, but check anyway */
  {
    fprintf(fp, " />\n");
...
static vx_status vxExportToXMLMatrix(FILE *fp, vx_reference refs[], vx_uint32 r,
                                     vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<matrix reference=\"%u\" elemType=\"%s\" rows=\"%zu\" "
          "columns=\"%zu\"%s",
      ...
      for (ri = 0ul; ri < mat->rows; ri++) {
        for (ci = 0ul; ci < mat->columns; ci++) {
          if (mat->data_type == VX_TYPE_INT32) {
            ...
            fprintf(fp,
                    "%s\t<float32 row=\"%zu\" column=\"%zu\">%f</float32>\n",
                    indent, ri, ci, value);
          }
        }
      }
...
static vx_status vxExportToXMLConvolution(FILE *fp, vx_reference refs[],
                                          vx_uint32 r, vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<convolution reference=\"%u\" rows=\"%zu\" columns=\"%zu\" "
          "scale=\"%u\"%s",
          indent, r, conv->base.rows, conv->base.columns, conv->scale,
          refNameStr);
      ...
      vx_true_e) /* is not virtual in 1.0, but check anyway */
  {
    fprintf(fp, " />\n");
    fprintf(fp,
            "%s<!-- Virtual Convolution not supported in OpenVX 1.0 spec -->\n",
            indent);
  } else {
    fprintf(fp, ">\n");
...
static vx_status vxExportToXMLDistribution(FILE *fp, vx_reference refs[],
                                           vx_uint32 r, vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<distribution reference=\"%u\" bins=\"%u\" offset=\"%u\" "
          "range=\"%u\"%s",
      ...
      vx_true_e) /* is not virtual in 1.0, but check anyway */
  {
    ...
    fprintf(
        fp,
        "%s<!-- Virtual Distribution not supported in OpenVX 1.0 spec -->\n",
        indent);
  } else {
    fprintf(fp, ">\n");
...
static vx_status vxExportToXMLRemap(FILE *fp, vx_reference refs[], vx_uint32 r,
                                    vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<remap reference=\"%u\" src_width=\"%u\" src_height=\"%u\" "
          "dst_width=\"%u\" dst_height=\"%u\"%s",
          indent, r, remap->src_width, remap->src_height, remap->dst_width,
          remap->dst_height, refNameStr);
    ...
    if (refs[r]->write_count > 0) {
      for (y = 0u; y < remap->dst_height; y++) {
        for (x = 0u; x < remap->dst_width; x++) {
          ...
          fprintf(fp,
                  "%s\t<point src_x=\"%lf\" src_y=\"%lf\" dst_x=\"%u\" "
                  "dst_y=\"%u\" />\n",
...
static vx_status vxExportToXMLThreshold(FILE *fp, vx_reference refs[],
                                        vx_uint32 r, vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp,
          "%s<threshold reference=\"%u\" elemType=\"%s\" true_value=\"%d\" "
          "false_value=\"%d\"%s",
          indent, r, type_pairs[j].name, thresh->true_value,
          thresh->false_value, refNameStr);
      ...
      vx_true_e) /* is not virtual in 1.0, but check anyway */
  {
    fprintf(fp, " />\n");
    fprintf(fp,
            "%s<!-- Virtual Threshold not supported in OpenVX 1.0 spec -->\n",
            indent);
  } else {
    fprintf(fp, ">\n");
...
static vx_status vxExportToXMLScalar(FILE *fp, vx_reference refs[], vx_uint32 r,
                                     vx_uint32 id) {
  vx_status status = VX_SUCCESS;
  ...
  fprintf(fp, "%s<scalar reference=\"%u\" elemType=\"%s\"%s", indent, r,
          type_pairs[ownStringFromType(scalar->data_type)].name, refNameStr);
    ...
    switch (scalar->data_type) {
      ...
      case VX_TYPE_BOOL:
        fprintf(fp, "%s\t<bool>%s</bool>\n", indent,
                (scalar->data.boolean ? "true" : "false"));
        break;
      case VX_TYPE_DF_IMAGE:
        fprintf(fp, "%s\t<df_image>%s</df_image>\n", indent,
                vxFourCCString(scalar->data.fcc));
        break;
      case VX_TYPE_SIZE:
...
VX_API_ENTRY vx_status VX_API_CALL vxExportToXML(vx_context context,
  ...
  vx_enum skipTypes[] = {
      VX_TYPE_ERROR,     VX_TYPE_KERNEL,  VX_TYPE_TARGET,
      VX_TYPE_PARAMETER, VX_TYPE_CONTEXT, VX_TYPE_IMPORT,
  };
  ...
  for (r = 0u, r1 = 0u; r1 < context->num_references; r++) {
    if (context->reftable[r]) {
      r1++;
      if (vxIsMemberOf(context->reftable[r]->type, skipTypes,
                       dimof(skipTypes)) == vx_true_e ||
  ...
  /* check the number */
  if (numrefs == 0 || numrefs > VX_INT_MAX_REF) return VX_ERROR_NOT_SUPPORTED;
  ...
  if (!refs)
    ...
    for (r = 0u, r1 = 0u, r2 = 0u; r1 < context->num_references; r2++) {
      if (context->reftable[r2]) {
        r1++;
        if (vxIsMemberOf(context->reftable[r2]->type, skipTypes,
                         dimof(skipTypes)) == vx_true_e ||
        ...
        for (n = 0u; n < graph->numNodes; n++) {
          for (r2 = 0u; r2 < numrefs; r2++) {
            if (refs[r2] == (vx_reference)graph->nodes[n]) {
              ...
              if (graph->nodes[n]->attributes.borders.mode !=
              ...
              for (p = 0u;
                   p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
                for (r3 = 0u; r3 < numrefs; r3++) {
                  if (refs[r3] == graph->nodes[n]->parameters[p]) {
                    fprintf(fp,
                            "\t\t\t<parameter index=\"%u\" reference=\"%u\"", p,
                            r3);
                    if (graph->nodes[n]->is_replicated == vx_true_e) {
        ...
        for (p = 0u; p < graph->numParams; p++) {
          for (r2 = 0u; r2 < numrefs; r2++) {
            if (refs[r2] == (vx_reference)graph->parameters[p].node) {
              fprintf(fp,
                      "\t\t<parameter index=\"%u\" node=\"%u\" "
                      "parameter=\"%u\" />\n",
                      p, r2, graph->parameters[p].index);
            }
          }
        }
        /* scan for "virtual" objects in the context and if they match this
         * scope, print them */
        for (r2 = 0u; r2 < numrefs; r2++) {
