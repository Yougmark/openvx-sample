...
static vx_reference vxLocateBaseLocation(vx_reference ref, vx_size *start,
                                         vx_size *end) {
  ...
  while ((ref->type == VX_TYPE_IMAGE && ((vx_image)ref)->parent &&
    ...
    if (ref->type == VX_TYPE_IMAGE) {
      ...
      vx_uint32 dx = (vx_uint32)(
          (plane_offset -
           (dy * img->memory.strides[0][VX_DIM_Y] / img->scale[0][VX_DIM_Y])) *
      ...
      for (vx_int32 i = tensor->number_of_dimensions - 1; i >= 0; i--) {
        start[i] = ((vx_uint8 *)tensor->addr -
                    (vx_uint8 *)tensor->parent->addr - offset) /
                   tensor->stride[i];
...
static vx_tensor vxLocateView(vx_tensor mddata, vx_size *start, vx_size *end) {
  ...
  while (mddata->parent && mddata->parent != mddata) {
    ...
    for (vx_int32 i = mddata->number_of_dimensions - 1; i >= 0; i--) {
      start[i] = ((vx_uint8 *)mddata->addr - (vx_uint8 *)mddata->parent->addr -
                  offset) /
                 mddata->stride[i];
...
static vx_bool vxCheckWriteDependency(vx_reference ref1, vx_reference ref2) {
  ...
  if (ref1->type == VX_TYPE_PYRAMID && ref2->type == VX_TYPE_IMAGE) {
    ...
    if (img->base.scope == ref1) return vx_true_e;
  }
  ...
  if (ref2->type == VX_TYPE_PYRAMID && ref1->type == VX_TYPE_IMAGE) {
    ...
    while (img->parent && img->parent != img) img = img->parent;
    if (img->base.scope == ref2) return vx_true_e;
  }

  // two images or ROIs
  if (ref1->type == VX_TYPE_IMAGE && ref2->type == VX_TYPE_IMAGE) {
    vx_size rr_start[VX_MAX_TENSOR_DIMENSIONS],
        rw_start[VX_MAX_TENSOR_DIMENSIONS], rr_end[VX_MAX_TENSOR_DIMENSIONS],
        rw_end[VX_MAX_TENSOR_DIMENSIONS];
    ...
    if (refr == refw) {
      if (refr->type == VX_TYPE_IMAGE) {
        ...
        if (rr_start[0] < rw_end[0] && rr_end[0] > rw_start[0] &&
            rr_start[1] < rw_end[1] && rr_end[1] > rw_start[1])
          return vx_true_e;
      } else {
        if (refr->type == VX_TYPE_TENSOR) {
          for (vx_uint32 i = 0; i < ((vx_tensor)refr)->number_of_dimensions;
               i++) {
...
vx_status ownFindNodesWithReference(vx_graph graph, vx_reference ref,
  ...
  *count = 0;
  ...
  VX_PRINT(VX_ZONE_GRAPH,
           "Find nodes with reference " VX_FMT_REF
           " type %d over %u nodes upto %u finds\n",
  ...
  for (n = 0; n < graph->numNodes; n++) {
    for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
      ...
      /* VX_PRINT(VX_ZONE_GRAPH,"\tchecking node[%u].parameter[%u] dir = %d ref
       * = "VX_FMT_REF" (=?%d:"VX_FMT_REF")\n", n, p, dir, thisref, reftype,
       * ref); */
      if ((dir == reftype) && vxCheckWriteDependency(thisref, ref)) {
        if (nc < max) {
          VX_PRINT(VX_ZONE_GRAPH, "match at node[%u].parameter[%u]\n", n, p);
          if (refnodes) refnodes[nc] = n;
          nc++;
          status = VX_SUCCESS;
        } else {
  ...
  *count = nc;
  VX_PRINT(VX_ZONE_GRAPH,
           "Found %u nodes with reference " VX_FMT_REF " status = %d\n", nc,
           ref, status);
  return status;
}
...
void ownClearVisitation(vx_graph graph) {
  vx_uint32 n = 0;
  for (n = 0; n < graph->numNodes; n++) graph->nodes[n]->visited = vx_false_e;
}

void ownClearExecution(vx_graph graph) {
  vx_uint32 n = 0;
  for (n = 0; n < graph->numNodes; n++) graph->nodes[n]->executed = vx_false_e;
}
...
vx_status ownTraverseGraph(vx_graph graph, vx_uint32 parentIndex,
  ...
  } else if (depth > graph->numNodes) /* should be impossible under normal
                                         circumstances */
  {
    /* there's a cycle in the graph */
    status = VX_ERROR_INVALID_GRAPH;
    ...
    if (parentIndex == VX_INT_MAX_NODES) {
      ...
      VX_PRINT(VX_ZONE_GRAPH,
               "Starting head-first traverse of graph from node[%u]\n",
               thisIndex);
    } else {
      thisIndex = childIndex;
      VX_PRINT(
          VX_ZONE_GRAPH,
          "continuing traverse of graph from node[%u] on node[%u] start=%u\n",
          parentIndex, thisIndex, refStart);
    }

    for (p = 0; p < graph->nodes[thisIndex]->kernel->signature.num_parameters;
         p++) {
      ...
      if (dir != VX_INPUT && ref != NULL) {
        VX_PRINT(VX_ZONE_GRAPH,
                 "[traverse] node[%u].parameter[%u] = " VX_FMT_REF "\n",
                 thisIndex, p, ref);
        /* send the maximum number of possible nodes to find */
        refCount = dimof(refNodes) - refStart;
        status = ownFindNodesWithReference(graph, ref, &refNodes[refStart],
                                           &refCount, VX_INPUT);
        ...
        if (status == VX_SUCCESS) {
          ...
          for (refIndex = refStart; refIndex < refStop; refIndex++) {
            ...
            child_status =
                ownTraverseGraph(graph, thisIndex, refNodes[refIndex]);
            if (child_status != VX_SUCCESS) status = child_status;
        ...
        {
          VX_PRINT(VX_ZONE_GRAPH,
                   "[Ok] No link found for node[%u].parameter[%u]\n", thisIndex,
                   p);
          status = VX_SUCCESS;
        }
      } else {
        VX_PRINT(VX_ZONE_GRAPH,
                 "[ ignore ] node[%u].parameter[%u] = " VX_FMT_REF " type %d\n",
                 childIndex, p, ref, dir);
      }
      if (status == VX_ERROR_INVALID_GRAPH) break;
    }
...
void ownFindNextNodes(vx_graph graph, vx_uint32 last_nodes[VX_INT_MAX_REF],
  ...
  for (n = 0; n < *numLeft; n++) {
    VX_PRINT(VX_ZONE_GRAPH, "leftover: node[%u] = %s\n", left_nodes[n],
             graph->nodes[left_nodes[n]]->kernel->name);
  }
  ...
  for (i = 0; i < numLast; i++) {
    ...
    for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
      ...
      if (((dir == VX_OUTPUT) || (dir == VX_BIDIRECTIONAL)) && (ref != NULL)) {
        ...
        if (ownFindNodesWithReference(graph, ref, &poss_next[numPoss], &n1,
                                      VX_INPUT) == VX_SUCCESS) {
  ...
  for (i = 0; i < numPoss; i++) {
    ...
    for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
      if (graph->nodes[n]->kernel->signature.directions[p] == VX_INPUT) {
        VX_PRINT(VX_ZONE_GRAPH,
                 "nodes[%u].parameter[%u] predicate needs to be checked\n", n,
                 p);
...
void ownContaminateGraphs(vx_reference ref) {
  if (ownIsValidReference(ref) == vx_true_e) {
    ...
    for (r = 0u; r < context->num_references; r++) {
      if (context->reftable[r] == NULL) continue;
      if (context->reftable[r]->type == VX_TYPE_GRAPH) {
        ...
        for (n = 0u; n < (graph->numNodes) && (found == vx_false_e); n++) {
          vx_uint32 p;
          for (p = 0u; p < graph->nodes[n]->kernel->signature.num_parameters;
               p++) {
...
VX_API_ENTRY vx_graph VX_API_CALL vxCreateGraph(vx_context c) {
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    graph = (vx_graph)ownCreateReference(context, VX_TYPE_GRAPH, VX_EXTERNAL,
                                         &context->base);
    if (vxGetStatus((vx_reference)graph) == VX_SUCCESS &&
        graph->base.type == VX_TYPE_GRAPH) {
...
void ownDestructGraph(vx_reference ref) {
  ...
  while (graph->numNodes) {
    ...
    /* Interpretation of spec is to release all external references of Nodes
       when vxReleaseGraph() is called AND all graph references count == 0
       (garbage collection). However, it may be possible that the user would
       have already released its external reference so we need to check. */
    if (node->base.external_count) {
      ownReleaseReferenceInt((vx_reference *)&node, VX_TYPE_NODE, VX_EXTERNAL,
                             NULL);
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseGraph(vx_graph *g) {
  return ownReleaseReferenceInt((vx_reference *)g, VX_TYPE_GRAPH, VX_EXTERNAL,
                                NULL);
}
...
static void vxTopologicalSort(vx_graph graph, vx_node *list, vx_uint32 nnodes) {
  ...
  vx_uint32
      nremain; /* Number of remaining objects to be "output"; "N" in TAoCP. */
  ...
  while (f != 0) {
    ...
    if (x[f].ref->type == VX_TYPE_NODE) list[outputnr++] = (vx_node)x[f].ref;
    nremain--;
    p = x[f].top;
  ...
  if (nremain != 0)
    for (n = 0; n < nnodes; n++) list[n] = (vx_node)x[n + 1].ref;

  free(suc_next_table);
  free(x);
}

static vx_bool setup_output(vx_graph graph, vx_uint32 n, vx_uint32 p,
                            vx_reference_t **vref, vx_meta_format *meta,
  ...
  *meta = ownCreateMetaFormat(graph->base.context);
  ...
  if ((*vref)->is_virtual == vx_false_e) {
    *vref = NULL;
    ...
    VX_PRINT(VX_ZONE_GRAPH,
             "Virtual Reference detected at kernel %s parameter %u\n",
    ...
    if ((*vref)->scope->type == VX_TYPE_GRAPH &&
      ...
      *status = VX_ERROR_INVALID_SCOPE;
      vxAddLogEntry((vx_reference)(*vref), *status,
                    "Virtual Reference is in the wrong scope, created from "
                    "another graph!\n");
      (*num_errors)++;
  ...
  /* the type of the parameter is known by the system, so let the system set it
   * by default. */
...
*/
static vx_bool postprocess_output_data_type(vx_graph graph, vx_uint32 n,
                                            vx_uint32 p, vx_reference_t *item,
                                            vx_reference_t *vref,
                                            vx_meta_format meta,
                                            vx_status *status,
                                            vx_uint32 *num_errors) {
  ...
  if (meta->type == VX_TYPE_IMAGE) {
    ...
    VX_PRINT(VX_ZONE_GRAPH, "meta: type 0x%08x, %ux%u\n", meta->type,
             meta->dim.image.width, meta->dim.image.height);
    if (vref == (vx_reference_t *)img) {
      VX_PRINT(VX_ZONE_GRAPH, "Creating Image From Meta Data!\n");
      /*! \todo need to worry about images that have a format, but no dimensions
       * too */
      if (img->format == VX_DF_IMAGE_VIRT ||
      ...
      /* check the data that came back from the output validator against the
       * object */
    ...
    if (vx_false_e == graph->nodes[n]->attributes.valid_rect_reset &&
      ...
      if (VX_SUCCESS !=
          vxQueryNode(node, VX_NODE_PARAMETERS, &nparams, sizeof(nparams))) {
        *status = VX_FAILURE;
        return vx_false_e;
      }
      ...
      in_rect =
          (vx_rectangle_t **)malloc(num_in_images * sizeof(vx_rectangle_t *));
      if (NULL == in_rect) {
        *status = VX_FAILURE;
        return vx_false_e;
      }
      ...
      for (i = 0; i < nparams; i++) {
        if (VX_INPUT == node->kernel->signature.directions[i] &&
          ...
          if (VX_SUCCESS != vxGetValidRegionImage((vx_image)node->parameters[i],
                                                  in_rect[i])) {
            *status = VX_FAILURE;
            res = vx_false_e;
            break;
      ...
      if (vx_false_e != res) {
        ...
        if (VX_SUCCESS == meta->set_valid_rectangle_callback(
                              graph->nodes[n], p,
                              (const vx_rectangle_t *const *)in_rect,
                              (vx_rectangle_t *const *)out_rect)) {
          /* set output image valid rectangle */
          if (VX_SUCCESS != vxSetImageValidRectangle(
                                img, (const vx_rectangle_t *)out_rect[0])) {
            *status = VX_FAILURE;
            res = vx_false_e;
          }
        } else {
      ...
      if (NULL != in_rect) free(in_rect);
      if (NULL != out_rect[0]) free(out_rect[0]);
    ...
    if (vref == (vx_reference_t *)arr) {
      VX_PRINT(VX_ZONE_GRAPH,
               "Creating Array From Meta Data %x and " VX_FMT_SIZE "!\n",
               meta->dim.array.item_type, meta->dim.array.capacity);
      if (ownInitVirtualArray(arr, meta->dim.array.item_type,
                              meta->dim.array.capacity) != vx_true_e) {
        *status = VX_ERROR_INVALID_DIMENSION;
        vxAddLogEntry(&graph->base, VX_ERROR_INVALID_DIMENSION,
                      "Node: %s: meta[%u] has an invalid item type 0x%08x or "
                      "capacity " VX_FMT_SIZE "\n",
                      graph->nodes[n]->kernel->name, p,
                      meta->dim.array.item_type, meta->dim.array.capacity);
        VX_PRINT(VX_ZONE_ERROR,
                 "Node: %s: meta[%u] has an invalid item type 0x%08x or "
                 "capacity " VX_FMT_SIZE "\n",
                 graph->nodes[n]->kernel->name, p, meta->dim.array.item_type,
                 meta->dim.array.capacity);
        (*num_errors)++;
        return vx_false_e;  // break;
      }
    } else {
      if (ownValidateArray(arr, meta->dim.array.item_type,
                           meta->dim.array.capacity) != vx_true_e) {
        *status = VX_ERROR_INVALID_DIMENSION;
        vxAddLogEntry(&graph->base, VX_ERROR_INVALID_DIMENSION,
                      "Node: %s: parameter[%u] has an invalid item type 0x%08x "
                      "or capacity " VX_FMT_SIZE "\n",
                      graph->nodes[n]->kernel->name, p, arr->item_type,
                      arr->capacity);
        VX_PRINT(VX_ZONE_ERROR,
                 "Node: %s: parameter[%u] has an invalid item type 0x%08x or "
                 "capacity " VX_FMT_SIZE "\n",
                 graph->nodes[n]->kernel->name, p, arr->item_type,
                 arr->capacity);
        (*num_errors)++;
    ...
    VX_PRINT(VX_ZONE_GRAPH, "meta: type 0x%08x, %ux%u:%u:%lf\n", meta->type,
             ...
             meta->dim.pyramid.levels, meta->dim.pyramid.scale);
    ...
    if ((pyramid->numLevels != meta->dim.pyramid.levels) ||
      ...
      *status = VX_ERROR_INVALID_VALUE;
      vxAddLogEntry(&graph->base, *status,
                    "Either levels (%u?=%u) or scale (%lf?=%lf) are invalid\n",
    ...
    if ((pyramid->format != VX_DF_IMAGE_VIRT) &&
      ...
      *status = VX_ERROR_INVALID_FORMAT;
      vxAddLogEntry(&graph->base, *status,
                    "Invalid pyramid format %x, needs %x\n", pyramid->format,
    ...
    if (((pyramid->width != 0) &&
      ...
      *status = VX_ERROR_INVALID_DIMENSION;
      vxAddLogEntry(&graph->base, *status,
                    "Invalid pyramid dimensions %ux%u, needs %ux%u\n",
                    pyramid->width, pyramid->height, meta->dim.pyramid.width,
    ...
    if (vref == (vx_reference_t *)pyramid) {
      ownInitPyramid(pyramid, meta->dim.pyramid.levels, meta->dim.pyramid.scale,
                     meta->dim.pyramid.width, meta->dim.pyramid.height,
                     meta->dim.pyramid.format);
    }
    ...
    if (vx_false_e == graph->nodes[n]->attributes.valid_rect_reset &&
      ...
      for (i = 0; i < num_in_images; i++) in_rect[i] = NULL;
      ...
      if (vx_false_e != res) {
        out_rect = (vx_rectangle_t **)malloc(meta->dim.pyramid.levels *
                                             sizeof(vx_rectangle_t *));
        if (NULL != out_rect) {
          vx_uint32 k;
          for (k = 0; k < meta->dim.pyramid.levels; k++) out_rect[k] = NULL;
      ...
      if (vx_false_e != res) {
        ...
        if (VX_SUCCESS == meta->set_valid_rectangle_callback(
          ...
          for (i = 0; i < meta->dim.pyramid.levels; i++) {
            ...
            if (vx_false_e ==
                ownIsValidSpecificReference((vx_reference)img, VX_TYPE_IMAGE)) {
              *status = VX_FAILURE;
              res = vx_false_e;
              vxReleaseImage(
                  &img); /* already on error path, ignore additional errors */
              break;
            }
      ...
      for (i = 0; i < meta->dim.pyramid.levels; i++) {
        if (NULL != out_rect && NULL != out_rect[i]) free(out_rect[i]);
      }

      if (NULL != out_rect) free(out_rect);
    ...
    if (matrix->columns != meta->dim.matrix.cols ||
        matrix->rows != meta->dim.matrix.rows) {
      *status = VX_ERROR_INVALID_DIMENSION;
      vxAddLogEntry(
          &graph->base, VX_ERROR_INVALID_DIMENSION,
          "Node: %s: parameter[%u] has an invalid matrix dimention %ux%u\n",
          graph->nodes[n]->kernel->name, p, matrix->data_type, matrix->rows,
          matrix->columns);
      (*num_errors)++;
      return vx_false_e;  // break;
    }
    ...
    if (distribution->offset_x != meta->dim.distribution.offset ||
      ...
      *status = VX_ERROR_INVALID_VALUE;
      vxAddLogEntry(&graph->base, VX_ERROR_INVALID_VALUE,
                    "Node: %s: parameter[%u] has an invalid offset %u, number "
                    "of bins %u or range %u\n",
                    graph->nodes[n]->kernel->name, p, distribution->offset_x,
                    distribution->memory.dims[0][VX_DIM_X],
                    distribution->range_x);
      (*num_errors)++;
      return vx_false_e;  // break;
    }
  } else if (meta->type == VX_TYPE_REMAP) {
    vx_remap_t *remap = (vx_remap_t *)item;
    if (remap->src_width != meta->dim.remap.src_width ||
        remap->src_height != meta->dim.remap.src_height) {
      *status = VX_ERROR_INVALID_DIMENSION;
      vxAddLogEntry(
          &graph->base, VX_ERROR_INVALID_DIMENSION,
    ...
    if (remap->dst_width != meta->dim.remap.dst_width ||
        remap->dst_height != meta->dim.remap.dst_height) {
      *status = VX_ERROR_INVALID_DIMENSION;
      vxAddLogEntry(
          &graph->base, VX_ERROR_INVALID_DIMENSION,
    ...
    if (lut->item_type != meta->dim.lut.type ||
        lut->num_items != meta->dim.lut.count) {
      *status = VX_ERROR_INVALID_DIMENSION;
      vxAddLogEntry(&graph->base, VX_ERROR_INVALID_DIMENSION,
                    "Node: %s: parameter[%u] has an invalid item type 0x%08x "
                    "or count " VX_FMT_SIZE "\n",
                    graph->nodes[n]->kernel->name, p, lut->item_type,
                    lut->num_items);
      (*num_errors)++;
      return vx_false_e;  // break;
    }
  ...
  if (meta->type == VX_TYPE_TENSOR) {
    ...
    if (vref == (vx_reference_t *)tensor) {
      ...
      if ((tensor->data_type != VX_TYPE_INVALID) &&
          (tensor->data_type != meta->dim.tensor.data_type ||
           tensor->fixed_point_position !=
               meta->dim.tensor.fixed_point_position)) {
        *status = VX_ERROR_INVALID_FORMAT;
        vxAddLogEntry(&graph->base, *status,
                      "Node: %s: parameter[%u] has invalid data type %08x or "
                      "fixed point position %d!\n",
                      graph->nodes[n]->kernel->name, p, tensor->data_type,
                      tensor->fixed_point_position);
        VX_PRINT(VX_ZONE_ERROR,
                 "Node: %s: parameter[%u] has invalid data type %08x or fixed "
                 "point position %d!\n",
                 graph->nodes[n]->kernel->name, p, tensor->data_type,
                 tensor->fixed_point_position);
        (*num_errors)++;
      ...
      if (tensor->number_of_dimensions != 0) {
        for (unsigned i = 0; i < tensor->number_of_dimensions; i++) {
          if (tensor->dimensions[i] != 0 &&
              tensor->dimensions[i] != meta->dim.tensor.dimensions[i]) {
            *status = VX_ERROR_INVALID_DIMENSION;
            vxAddLogEntry(&graph->base, *status,
                          "Node: %s: parameter[%u] has invalid dimension size "
                          "%d in dimension %d!\n",
                          graph->nodes[n]->kernel->name, p,
                          tensor->dimensions[i], i);
            VX_PRINT(VX_ZONE_ERROR,
                     "Node: %s: parameter[%u] has invalid dimension size %d in "
                     "dimension %d!\n",
      ...
      } else if (tensor->number_of_dimensions !=
                 meta->dim.tensor.number_of_dimensions) {
        *status = VX_ERROR_INVALID_DIMENSION;
        vxAddLogEntry(&graph->base, *status,
      ...
      ownInitTensor(tensor, meta->dim.tensor.dimensions,
                    meta->dim.tensor.number_of_dimensions,
                    meta->dim.tensor.data_type,
                    meta->dim.tensor.fixed_point_position);
      ownAllocateTensorMemory(tensor);
    } else {
      if (tensor->number_of_dimensions !=
          meta->dim.tensor.number_of_dimensions) {
        *status = VX_ERROR_INVALID_DIMENSION;
        vxAddLogEntry(
            &graph->base, *status,
      ...
      if (tensor->data_type != meta->dim.tensor.data_type) {
        *status = VX_ERROR_INVALID_FORMAT;
        vxAddLogEntry(&graph->base, *status,
                      ...
                      graph->nodes[n]->kernel->name, p, tensor->data_type);
        VX_PRINT(VX_ZONE_ERROR,
                 "Node: %s: parameter[%u] has invalid data type %08x!\n",
                 graph->nodes[n]->kernel->name, p, tensor->data_type);
        (*num_errors)++;
        return vx_false_e;  // exit on error
      }
      if (tensor->fixed_point_position !=
          meta->dim.tensor.fixed_point_position) {
        *status = VX_ERROR_INVALID_FORMAT;
        vxAddLogEntry(&graph->base, *status,
                      "Node: %s: parameter[%u] has an invalid fixed point "
                      "position %08x!\n",
                      graph->nodes[n]->kernel->name, p,
                      tensor->fixed_point_position);
        VX_PRINT(
            VX_ZONE_ERROR,
            "Node: %s: parameter[%u] has invalid fixed point position  %08x!\n",
  ...
  /*! \todo support other output types for safety checks in graph verification
     parameters phase */
  else {
...
//   meta - parameter meta info
*/
static vx_bool postprocess_output(vx_graph graph, vx_uint32 n, vx_uint32 p,
                                  vx_reference_t *vref, vx_meta_format meta,
                                  vx_status *status, vx_uint32 *num_errors) {
  if (ownIsValidType(meta->type) == vx_false_e) {
    *status = VX_ERROR_INVALID_TYPE;
  ...
  if (meta->type == VX_TYPE_OBJECT_ARRAY) {
    vx_object_array_t *objarr =
        (vx_object_array_t *)graph->nodes[n]->parameters[p];
    VX_PRINT(VX_ZONE_GRAPH, "meta: type 0x%08x, 0x%08x " VX_FMT_SIZE "\n",
             meta->type, meta->dim.object_array.item_type,
             meta->dim.object_array.num_items);

    if (ownValidateObjectArray(objarr, meta->dim.object_array.item_type,
                               meta->dim.object_array.num_items) != vx_true_e) {
      *status = VX_ERROR_INVALID_DIMENSION;
      vxAddLogEntry(&graph->base, VX_ERROR_INVALID_DIMENSION,
                    "Node: %s: parameter[%u] has an invalid item type 0x%08x "
                    "or num_items " VX_FMT_SIZE "\n",
                    graph->nodes[n]->kernel->name, p, objarr->item_type,
                    objarr->num_items);
      VX_PRINT(VX_ZONE_ERROR,
               "Node: %s: parameter[%u] has an invalid item type 0x%08x or "
               "num_items " VX_FMT_SIZE "\n",
               graph->nodes[n]->kernel->name, p, objarr->item_type,
               objarr->num_items);
      (*num_errors)++;
      return vx_false_e;  // break;
    ...
    if (vref == (vx_reference_t *)objarr) {
      VX_PRINT(VX_ZONE_GRAPH,
               "Creating Object Array From Meta Data %x and " VX_FMT_SIZE "!\n",
               meta->dim.object_array.item_type,
               meta->dim.object_array.num_items);
      for (vx_uint32 i = 0; i < meta->dim.object_array.num_items; i++) {
        vx_reference item = vxGetObjectArrayItem(objarr, i);

        if (!postprocess_output_data_type(graph, n, p, item, vref, meta, status,
                                          num_errors)) {
      ...
      for (vx_uint32 i = 0; i < meta->dim.object_array.num_items; i++) {
        ...
        if (!postprocess_output_data_type(graph, n, p, item, itemref, meta,
                                          status, num_errors)) {
          vxReleaseReference(&item);
          *status = VX_ERROR_INVALID_PARAMETERS;
          vxAddLogEntry(&graph->base, VX_ERROR_INVALID_PARAMETERS,
    ...
    return postprocess_output_data_type(graph, n, p,
                                        graph->nodes[n]->parameters[p], vref,
                                        meta, status, num_errors);
  }
...
VX_API_ENTRY vx_status VX_API_CALL vxVerifyGraph(vx_graph graph) {
  ...
  vx_bool first_time_verify =
      ((graph->verified == vx_false_e) && (graph->reverify == vx_false_e))
          ? vx_true_e
          : vx_false_e;
  ...
  if (ownIsValidReference(&graph->base) == vx_true_e) {
      ...
      if (node->kernel->user_kernel) {
        if (!first_time_verify)  // re-verify
        {
          if (node->kernel->deinitialize) {
            ...
            status = node->kernel->deinitialize(
                (vx_node)node, (vx_reference *)node->parameters,
    ...
    for (n = 0; n < graph->numNodes; n++) {
      ...
      for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
        if (graph->nodes[n]->kernel->signature.states[p] ==
          ...
          if (graph->nodes[n]->parameters[p] == NULL) {
            ...
            VX_PRINT(VX_ZONE_ERROR,
                     ...
                     graph->nodes[n], graph->nodes[n]->kernel->name, p);
      ...
      /* debugging, show that we can detect "constant" data or "unreferenced
       * data" */
      for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
        ...
        if (ref) {
          if (ref->external_count == 0) {
            VX_PRINT(VX_ZONE_INFO,
                     "%s[%u] = " VX_FMT_REF " (CONSTANT) type:%08x\n",
                     graph->nodes[n]->kernel->name, p, ref, ref->type);
          } else {
            VX_PRINT(VX_ZONE_INFO,
                     "%s[%u] = " VX_FMT_REF " (MUTABLE) type:%08x count:%d\n",
                     graph->nodes[n]->kernel->name, p, ref, ref->type,
                     ref->external_count);
          }
        }
      ...
      if (graph->nodes[n]->kernel->validate != NULL) {
        ...
        for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters;
             p++) {
          if ((graph->nodes[n]->parameters[p] != NULL) &&
              (graph->nodes[n]->kernel->signature.directions[p] == VX_OUTPUT)) {
            if (setup_output(graph, n, p, &vref[p], &metas[p], &status,
        ...
        if (status == VX_SUCCESS) {
          validation_status = graph->nodes[n]->kernel->validate(
              (vx_node)graph->nodes[n], graph->nodes[n]->parameters,
              graph->nodes[n]->kernel->signature.num_parameters, metas);
          if (validation_status != VX_SUCCESS) {
            ...
            vxAddLogEntry(&graph->base, status,
                          ...
                          graph->nodes[n]->kernel->name);
            VX_PRINT(VX_ZONE_GRAPH,
                     "Failed on validation of parameter(s) of kernel %s in "
                     "node #%d (status=%d)\n",
        ...
        if (status == VX_SUCCESS) {
          for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters;
               p++) {
            if ((graph->nodes[n]->parameters[p] != NULL) &&
                (graph->nodes[n]->kernel->signature.directions[p] ==
                 VX_OUTPUT)) {
              if (postprocess_output(graph, n, p, vref[p], metas[p], &status,
                                     &num_errors) == vx_false_e)
                break;
            }
          }
        ...
        for (p = 0; p < dimof(metas); p++) {
          if (metas[p]) ownReleaseMetaFormat(&metas[p]);
        }
      } else /* old style validators */
      {
        ...
        for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters;
             p++) {
          if (((graph->nodes[n]->kernel->signature.directions[p] ==
                VX_BIDIRECTIONAL) ||
               (graph->nodes[n]->kernel->signature.directions[p] ==
                VX_INPUT)) &&
              (graph->nodes[n]->parameters[p] != NULL)) {
            vx_status input_validation_status =
                graph->nodes[n]->kernel->validate_input(
                    (vx_node)graph->nodes[n], p);
            if (input_validation_status != VX_SUCCESS) {
              ...
              vxAddLogEntry(
                  ...
                  graph->nodes[n]->kernel->name, p);
              VX_PRINT(VX_ZONE_GRAPH,
                       "Failed on validation of parameter %u of kernel %s in "
                       "node #%d (status=%d)\n",
        ...
        for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters;
          ...
          if (meta) ownReleaseMetaFormat(&meta);
        }
        if (meta) ownReleaseMetaFormat(&meta);
      }
    }
      ...
      for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
        if (graph->nodes[n]->parameters[p] &&
          ...
          for (n1 = vxNextNode(graph, n); n1 != n; n1 = vxNextNode(graph, n1)) {
            for (p1 = 0; p1 < graph->nodes[n]->kernel->signature.num_parameters;
                 p1++) {
              if ((graph->nodes[n1]->kernel->signature.directions[p1] ==
                   VX_OUTPUT) ||
                  (graph->nodes[n1]->kernel->signature.directions[p1] ==
                   VX_BIDIRECTIONAL)) {
                if (vxCheckWriteDependency(graph->nodes[n]->parameters[p],
                                           graph->nodes[n1]->parameters[p1])) {
                  status = VX_ERROR_MULTIPLE_WRITERS;
                  VX_PRINT(
                      VX_ZONE_GRAPH,
                      "Multiple Writer to a reference found, check log!\n");
                  vxAddLogEntry(&graph->base, status,
                                "Node %u and Node %u are trying to output to "
                                "the same reference " VX_FMT_REF "\n",
      ...
      for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters; p++) {
        if (graph->nodes[n]->parameters[p]) {
          ...
          if (graph->nodes[n]->parameters[p]->type == VX_TYPE_IMAGE) {
            if (ownAllocateImage(
                    (vx_image_t *)graph->nodes[n]->parameters[p]) ==
                vx_false_e) {
              vxAddLogEntry(
                  &graph->base, VX_ERROR_NO_MEMORY,
                  "Failed to allocate image at node[%u] %s parameter[%u]\n", n,
                  graph->nodes[n]->kernel->name, p);
              VX_PRINT(VX_ZONE_ERROR, "See log\n");
            }
          } else if ((VX_TYPE_IS_SCALAR(
                         graph->nodes[n]->parameters[p]->type)) ||
                     (graph->nodes[n]->parameters[p]->type ==
                      VX_TYPE_RECTANGLE) ||
                     (graph->nodes[n]->parameters[p]->type ==
            ...
            if (ownAllocateMemory(graph->base.context, &lut->memory) ==
                vx_false_e) {
              vxAddLogEntry(
                  &graph->base, VX_ERROR_NO_MEMORY,
          ...
          } else if (graph->nodes[n]->parameters[p]->type ==
                     VX_TYPE_DISTRIBUTION) {
            vx_distribution_t *dist =
                (vx_distribution_t *)graph->nodes[n]->parameters[p];
            if (ownAllocateMemory(graph->base.context, &dist->memory) ==
                vx_false_e) {
              vxAddLogEntry(&graph->base, VX_ERROR_NO_MEMORY,
                            "Failed to allocate distribution at node[%u] %s "
                            "parameter[%u]\n",
            ...
            for (i = 0; i < pyr->numLevels; i++) {
              if (ownAllocateImage((vx_image_t *)pyr->levels[i]) ==
                  vx_false_e) {
                vxAddLogEntry(&graph->base, VX_ERROR_NO_MEMORY,
                              "Failed to allocate pyramid image at node[%u] %s "
                              "parameter[%u]\n",
                              n, graph->nodes[n]->kernel->name, p);
                VX_PRINT(VX_ZONE_ERROR, "See log\n");
              }
            ...
            if (ownAllocateMemory(graph->base.context, &mat->memory) ==
                vx_false_e) {
              vxAddLogEntry(&graph->base, VX_ERROR_NO_MEMORY,
                            "Failed to allocate matrix (or subtype) at "
                            "node[%u] %s parameter[%u]\n",
                            n, graph->nodes[n]->kernel->name, p);
              VX_PRINT(VX_ZONE_ERROR, "See log\n");
            }
          } else if (graph->nodes[n]->kernel->signature.types[p] ==
                     VX_TYPE_ARRAY) {
            if (ownAllocateArray(
                    (vx_array_t *)graph->nodes[n]->parameters[p]) ==
                vx_false_e) {
              vxAddLogEntry(
                  &graph->base, VX_ERROR_NO_MEMORY,
    ...
    for (n = 0; (n < graph->numNodes) && (status == VX_SUCCESS); n++) {
      uint32_t n1, p1;
      vx_bool isAHead =
          vx_true_e; /* assume every node is a head until proven otherwise */
      ...
      for (p = 0; p < graph->nodes[n]->kernel->signature.num_parameters &&
        ...
        if ((graph->nodes[n]->kernel->signature.directions[p] == VX_INPUT) &&
            (graph->nodes[n]->parameters[p] != NULL)) {
          /* ring loop over the node array, checking every node but this nth
           * node. */
          for (n1 = vxNextNode(graph, n); (n1 != n) && (isAHead == vx_true_e);
               n1 = vxNextNode(graph, n1)) {
            for (p1 = 0;
                 p1 < graph->nodes[n1]->kernel->signature.num_parameters &&
                 isAHead == vx_true_e;
                 p1++) {
              if (graph->nodes[n1]->kernel->signature.directions[p1] !=
                  VX_INPUT) {
                ...
                if (vxCheckWriteDependency(graph->nodes[n]->parameters[p],
                  ...
                  VX_PRINT(VX_ZONE_GRAPH,
                           ...
                           "nodes[%u].parameter[%u]\n",
                           n, p, n1, p1);
                  isAHead = vx_false_e; /* this will cause all the loops to
                                           break too. */
                }
              }
            }
    ...
    for (h = 0; h < graph->numHeads; h++) {
      vx_status cycle_status =
          ownTraverseGraph(graph, VX_INT_MAX_NODES, graph->heads[h]);
    ...
    for (n = 0; (n < graph->numNodes) && (status == VX_SUCCESS); n++) {
      ...
      if (target) {
        vx_status target_verify_status =
            target->funcs.verify(target, graph->nodes[n]);
    ...
    for (n = 0; (n < graph->numNodes) && (status == VX_SUCCESS); n++) {
      ...
      if (node->kernel->initialize) {
        ...
        kernel_init_status = node->kernel->initialize(
            (vx_node)node, (vx_reference *)node->parameters,
            node->kernel->signature.num_parameters);
        node->local_data_change_is_enabled = vx_false_e;
      ...
      if ((node->attributes.localDataSize > 0) &&
        ...
        node->attributes.localDataPtr =
            calloc(1, node->attributes.localDataSize);
        if (node->kernel->user_kernel == vx_true_e)
          node->local_data_set_by_implementation = vx_true_e;
        VX_PRINT(VX_ZONE_GRAPH,
                 "Local Data Allocated " VX_FMT_SIZE
                 " bytes for node into %p\n!",
                 node->attributes.localDataSize, node->attributes.localDataPtr);
      }
#ifdef OPENVX_KHR_TILING
      /* if this is a tiling kernel, we can also have tile memory (the sample
       * only makes 1 buffer) */
    ...
    for (n = 0; (n < graph->numNodes) && (status == VX_SUCCESS); n++) {
          ...
          switch (ref->type) {
            case VX_TYPE_IMAGE: {
              ...
              for (i = 0; i < image->memory.nptrs; i++)
                graph->nodes[n]->costs.bandwidth +=
                    ownComputeMemorySize(&image->memory, i);
              break;
            }
            case VX_TYPE_ARRAY: {
              vx_array array = (vx_array)ref;
              graph->nodes[n]->costs.bandwidth +=
                  ownComputeMemorySize(&array->memory, 0);
              break;
            }
            ...
            default:
              // VX_PRINT(VX_ZONE_WARNING, "Node[%u].parameter[%u] Unknown
              // bandwidth cost!\n", n, p);
              break;
          }
        }
      ...
      VX_PRINT(VX_ZONE_GRAPH,
               "Node[%u] has bandwidth cost of " VX_FMT_SIZE " bytes\n", n,
               graph->nodes[n]->costs.bandwidth);
    }
...
static vx_status vxExecuteGraph(vx_graph graph, vx_uint32 depth) {
  ...
  do {
      ...
      if (graph->nodes[next_nodes[n]]->executed == vx_false_e) {
...
#endif
        {
          ...
          VX_PRINT(VX_ZONE_GRAPH, "Returned Node[%u] %s:%s Action %d\n",
                   next_nodes[n], target->name, node->kernel->name, action);
...
#if defined(OPENVX_USE_SMP)
    if (depth == 1 && graph->should_serialize == vx_false_e) {
      if (ownIssueThreadpool(graph->base.context->workers, workitems,
                             numNext) == vx_true_e) {
        /* do a blocking complete */
        VX_PRINT(VX_ZONE_GRAPH, "Issued %u work items!\n", numNext);
        if (ownCompleteThreadpool(graph->base.context->workers, vx_true_e) ==
            vx_true_e) {
    ...
    ownFindNextNodes(graph, last_nodes, numLast, next_nodes, &numNext,
                     left_nodes, &numLeft);
  ...
  for (n = 0; n < VX_INT_MAX_REF; n++) {
    if (graph->delays[n] &&
        ownIsValidSpecificReference(&graph->delays[n]->base, VX_TYPE_DELAY) ==
            vx_true_e)
  ...
  if (context->perf_enabled) {
    for (n = 0; n < graph->numNodes; n++) {
      VX_PRINT(VX_ZONE_PERF,
               "nodes[%u] %s[%d] last:" VX_FMT_TIME "ms avg:" VX_FMT_TIME
               "ms min:" VX_FMT_TIME "ms max:" VX_FMT_TIME "\n",
...
VX_API_ENTRY vx_status VX_API_CALL vxAddParameterToGraph(vx_graph graph,
  ...
  if ((ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) == vx_true_e) &&
      (ownIsValidSpecificReference(&param->base, VX_TYPE_PARAMETER) ==
       vx_true_e)) {
    graph->parameters[graph->numParams].node = param->node;
    graph->parameters[graph->numParams].index = param->index;
    graph->numParams++;
    status = VX_SUCCESS;
  } else if ((ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) ==
              vx_true_e) &&
             (ownIsValidSpecificReference(&param->base, VX_TYPE_PARAMETER) ==
              vx_false_e)) {
  ...
  if (ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) == vx_true_e) {
    if (index < VX_INT_MAX_PARAMS) {
      status = vxSetParameterByIndex((vx_node)graph->parameters[index].node,
                                     graph->parameters[index].index, value);
    } else {
      status = VX_ERROR_INVALID_VALUE;
    }
  }
...
VX_API_ENTRY vx_parameter VX_API_CALL
vxGetGraphParameterByIndex(vx_graph graph, vx_uint32 index) {
  ...
  if (ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) == vx_true_e) {
    if ((index < VX_INT_MAX_PARAMS) && (index < graph->numParams)) {
      ...
      parameter = vxGetParameterByIndex((vx_node)graph->parameters[index].node,
                                        node_index);
    }
  } else {
    VX_PRINT(VX_ZONE_ERROR, "Invalid Graph!\n");
    vxAddLogEntry(&graph->base, VX_ERROR_INVALID_REFERENCE,
                  "Invalid Graph given to %s\n", __FUNCTION__);
  }
  return parameter;
}
