...
static vx_bool vxIsValidArray(vx_array arr) {
  ...
  if (arr != NULL) res = ownIsValidSpecificReference(&arr->base, VX_TYPE_ARRAY);
...
void ownPrintArray(vx_array array) {
  VX_PRINT(VX_ZONE_INFO,
           "Array:%p has %zu elements of %04x type of %zu size each.\n", array,
           array->capacity, array->item_type, array->item_size);
}

vx_array ownCreateArrayInt(vx_context context, vx_enum item_type,
                           vx_size capacity, vx_bool is_virtual, vx_enum type) {
  vx_array arr =
      (vx_array)ownCreateReference(context, type, VX_EXTERNAL, &context->base);
...
vx_bool ownInitVirtualArray(vx_array arr, vx_enum item_type, vx_size capacity) {
  ...
  if ((vxIsValidArrayItemType(arr->base.context, item_type) == vx_true_e) &&
    ...
    if ((arr->item_type == VX_TYPE_INVALID) || (arr->item_type == item_type)) {
      ...
      if (arr->capacity == 0) arr->capacity = capacity;
...
vx_bool ownValidateArray(vx_array arr, vx_enum item_type, vx_size capacity) {
  ...
  if ((vxIsValidArrayItemType(arr->base.context, item_type)) &&
      (arr->item_type == item_type)) {
    /* if the required capacity is > 0 and the objects capacity is not
     * sufficient */
...
vx_status ownAccessArrayRangeInt(vx_array arr, vx_size start, vx_size end,
                                 vx_size *pStride, void **ptr, vx_enum usage) {
  vx_status status = VX_FAILURE;

  /* bad parameters */
  if ((usage < VX_READ_ONLY) || (VX_READ_AND_WRITE < usage) || (ptr == NULL) ||
  ...
  if (*ptr == NULL) {
    if ((usage == VX_WRITE_ONLY) || (usage == VX_READ_AND_WRITE)) {
      ...
      *stride_save = arr->item_size;
      ...
      if (ownAddAccessor(arr->base.context, size, usage, *ptr, &arr->base, &a,
                         stride_save) == vx_true_e) {
        vx_size offset;
        *ptr = arr->base.context->accessors[a].ptr;
        ...
        vxAddLogEntry(
            (vx_reference)arr, status,
            "Failed to allocate memory for COPY-ON-READ! Size=" VX_FMT_SIZE
            "\n",
            size);
      }
    }
      ...
      *ptr = arr->base.context->accessors[a].ptr;
      ...
      if (status == VX_SUCCESS) {
        if (usage != VX_WRITE_ONLY) {
          ...
          for (i = (int)start, pDest = *ptr,
              pSrc = &arr->memory.ptrs[0][start * arr->item_size];
               i < (int)end; i++, pDest += *pStride, pSrc += arr->item_size) {
...
vx_status ownCommitArrayRangeInt(vx_array arr, vx_size start, vx_size end,
                                 const void *ptr) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;

  vx_bool external = vx_true_e;  // assume that it was an allocated buffer

  if ((ptr == NULL) || (start > end) || (end > arr->num_items)) {
    return VX_ERROR_INVALID_PARAMETERS;
  }
...

  {
    ...
    if (zero_area == vx_false_e) {
      ...
      if (internal == vx_true_e &&
          arr->base.context->accessors[index].usage == VX_READ_ONLY) {
        ...
        if (external == vx_true_e || internal == vx_true_e) {
          ...
          if (internal == vx_true_e) {
            vx_size stride =
                *(vx_size *)arr->base.context->accessors[index].extra_data;
            ...
            if (stride == arr->item_size) {
              ...
              for (i = (int)start, pSrc = ptr, pDest = &beg_ptr[offset];
                   i < (int)end; i++, pSrc += stride, pDest += arr->item_size) {
                memcpy(pDest, pSrc, arr->item_size);
              }
            }
...
vx_status ownCopyArrayRangeInt(vx_array arr, vx_size start, vx_size end,
  ...
  /* bad parameters */
  if (((usage != VX_READ_ONLY) && (VX_WRITE_ONLY != usage)) || (ptr == NULL) ||
      (stride < arr->item_size) || (start >= end) || (end > arr->num_items)) {
    return VX_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (usage == VX_READ_ONLY) {
    VX_PRINT(VX_ZONE_ARRAY,
             "CopyArrayRange from " VX_FMT_REF " to ptr %p from %u to %u\n",
             arr, ptr, start, end);
    ...
    VX_PRINT(VX_ZONE_ARRAY,
             "CopyArrayRange from ptr %p to " VX_FMT_REF " from %u to %u\n",
             arr, ptr, start, end);
...
vx_status ownMapArrayRangeInt(vx_array arr, vx_size start, vx_size end,
  ...
  /* bad parameters */
  if ((usage < VX_READ_ONLY) || (VX_READ_AND_WRITE < usage) || (ptr == NULL) ||
      (stride == NULL) || (start >= end) || (end > arr->num_items)) {
    return VX_ERROR_INVALID_PARAMETERS;
  }
  ...
  VX_PRINT(VX_ZONE_ARRAY,
           "MapArrayRange from " VX_FMT_REF " to ptr %p from %u to %u\n", arr,
           *ptr, start, end);
  ...
  vx_uint8 *buf = NULL;
  vx_size size = (end - start) * arr->item_size;
  if (ownMemoryMap(arr->base.context, (vx_reference)arr, size, usage, mem_type,
                   flags, &extra, (void **)&buf, map_id) == vx_true_e) {
...
vx_status ownUnmapArrayRangeInt(vx_array arr, vx_map_id map_id) {
  ...
  /* bad parameters */
  if (ownFindMemoryMap(arr->base.context, (vx_reference)arr, map_id) !=
      vx_true_e) {
...
VX_API_ENTRY vx_array VX_API_CALL vxCreateArray(vx_context context,
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    if ((vxIsValidArrayItemType(context, item_type) == vx_true_e) &&
        (capacity > 0)) {
      arr = (vx_array)ownCreateArrayInt(context, item_type, capacity,
                                        vx_false_e, VX_TYPE_ARRAY);
...
VX_API_ENTRY vx_array VX_API_CALL vxCreateVirtualArray(vx_graph graph,
  ...
  if (ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) == vx_true_e) {
    if (((vxIsValidArrayItemType(graph->base.context, item_type) ==
          vx_true_e) ||
         item_type == VX_TYPE_INVALID)) {
      arr = (vx_array)ownCreateArrayInt(graph->base.context, item_type,
                                        capacity, vx_true_e, VX_TYPE_ARRAY);

      if (arr && arr->base.type == VX_TYPE_ARRAY) {
        arr->base.scope = (vx_reference_t *)graph;
      } else {
        arr = (vx_array)ownGetErrorObject(graph->base.context,
                                          VX_ERROR_NO_MEMORY);
      }
    } else {
      arr = (vx_array)ownGetErrorObject(graph->base.context,
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseArray(vx_array *a) {
  ...
  return ownReleaseReferenceInt((vx_reference_t **)a, VX_TYPE_ARRAY,
                                VX_EXTERNAL, NULL);
}

VX_API_ENTRY vx_status VX_API_CALL vxQueryArray(vx_array arr, vx_enum attribute,
...
VX_API_ENTRY vx_status VX_API_CALL vxAddArrayItems(vx_array arr, vx_size count,
                                                   const void *ptr,
                                                   vx_size stride) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
  if (vxIsValidArray(arr) == vx_true_e) {
      ...
      if ((count > 0) && (ptr != NULL) && (stride >= arr->item_size)) {
        ...
        if (arr->num_items + count <= arr->capacity) {
          ...
          for (i = 0; i < count; ++i) {
            ...
            memcpy(&dst_ptr[i * arr->item_size], &tmp[i * stride],
                   arr->item_size);
          }
...
vxAccessArrayRange(vx_array arr, vx_size start, vx_size end, vx_size *stride,
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyArrayRange(vx_array arr, vx_size start,
                                                    vx_size end, vx_size stride,
                                                    void *ptr, vx_enum usage,
                                                    vx_enum mem_type) {
  vx_status status = VX_FAILURE;
  /* bad references */
...
VX_API_ENTRY vx_status VX_API_CALL
vxMapArrayRange(vx_array arr, vx_size start, vx_size end, vx_map_id *map_id,
  ...
  status = ownMapArrayRangeInt(arr, start, end, map_id, stride, ptr, usage,
                               mem_type, flags);

  return status;
}

VX_API_ENTRY vx_status VX_API_CALL vxUnmapArrayRange(vx_array arr,
                                                     vx_map_id map_id) {
  vx_status status = VX_FAILURE;
  /* bad references */
  if (vxIsValidArray(arr) == vx_false_e) {
