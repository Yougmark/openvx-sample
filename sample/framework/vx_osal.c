...
vx_bool ownCreateSem(vx_sem_t *sem, vx_uint32 count) {
  ...
  VX_PRINT(VX_ZONE_OSAL, "sem_init(%p,%u)=>%d,%d errno=%d\n", sem, count, ret0,
           ret1, errno);
  if (ret0 == 0 && ret1 == 0)
#elif defined(__linux__) || defined(__ANDROID__) || defined(__QNX__) || \
    defined(__CYGWIN__)
...
vx_bool ownSemWait(vx_sem_t *sem) {
  ...
  VX_PRINT(VX_ZONE_OSAL, "sem_wait(%p)=>%d v=%d errno=%d\n", sem,
           res == vx_true_e ? 0 : -1, sem->count, errno);
  if (res != vx_false_e)
#elif defined(__linux__) || defined(__ANDROID__) || defined(__QNX__) || \
    defined(__CYGWIN__)
...
vx_bool ownSemTryWait(vx_sem_t *sem) {
  ...
  VX_PRINT(VX_ZONE_OSAL, "sem_trywait(%p)=>%d v=%d errno=%d\n", sem,
           res != vx_false_e ? 0 : -1, sem->count, errno);
  if (res != vx_false_e)
#elif defined(__linux__) || defined(__ANDROID__) || defined(__QNX__) || \
    defined(__CYGWIN__)
...
vx_bool ownWaitEvent(vx_event_t *e, vx_uint32 timeout) {
  ...
  if (status == WAIT_OBJECT_0) ret = vx_true_e;
#endif
  return ret;
}
...
#if defined(__linux__) || defined(__ANDROID__) || defined(__QNX__) || \
    defined(__CYGWIN__) || defined(__APPLE__)
    if (pthread_join(thread, (void **)value) == 0) joined = vx_true_e;
#elif defined(_WIN32) || defined(UNDER_CE)
...
#elif defined(_WIN32) || defined(UNDER_CE)
  thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)func, arg,
...
vx_threadpool_t *ownCreateThreadpool(vx_uint32 numThreads,
  ...
  if (pool) {
    ...
    pool->workers = (vx_threadpool_worker_t *)calloc(
        pool->numWorkers, sizeof(vx_threadpool_worker_t));
    if (pool->workers) {
...
vx_bool ownIssueThreadpool(vx_threadpool_t *pool, vx_value_set_t workitems[],
  ...
  ownSemWait(&pool->sem);
  ownResetEvent(&pool->completed); /* we're going to have items to work on, so
                                      clear the event */
  for (i = 0u; i < numWorkItems; i++) {
    ...
    /* there's too much work to do, there's an overflow condition. some of the
     * work may have been issued, others not. */
    if (wrote == vx_false_e) {
      break;
    }
  }

  /* if none of the writes worked for whatever reason, and we didn't have any
   * previous pending work (how?) then set the completed event */
...
void ownStartCapture(vx_perf_t *perf) { perf->beg = ownCaptureTime(); }
...
void ownPrintPerf(vx_perf_t *perf) {
  VX_PRINT(VX_ZONE_PERF,
           "beg:" VX_FMT_TIME "ms end:" VX_FMT_TIME "ms tmp:" VX_FMT_TIME
           "ms sum:" VX_FMT_TIME "ms num:%d avg:" VX_FMT_TIME "ms\n",
           ownTimeToMS(perf->beg), ownTimeToMS(perf->end),
           ownTimeToMS(perf->tmp), ownTimeToMS(perf->sum), (int)perf->num,
...
void vxPrintQueue(vx_queue_t *q) {
  ...
  VX_PRINT(VX_ZONE_OSAL, "Queue: %p, lock=%p s,e=[%d,%d] popped=%s\n", q,
           &q->lock, q->start_index, q->end_index, (q->popped ? "yes" : "no"));
  for (i = 0; i < VX_INT_MAX_QUEUE_DEPTH; i++) {
    if (q->data[i]) {
      VX_PRINT(VX_ZONE_OSAL,
               "[%u] = {" VX_FMT_VALUE ", " VX_FMT_VALUE ", " VX_FMT_VALUE
               "}\n",
    ...
    while (ownWaitEvent(&q->writeEvent, VX_INT_FOREVER) == vx_true_e) {
      ...
      if (q->popped == vx_false_e) {
        ...
        if (q->start_index != q->end_index) {
          ...
          if (q->start_index != q->end_index) ownSetEvent(&q->writeEvent);
          vxPrintQueue(q);
        } else {
        ...
        if (q->end_index != -1) ownSetEvent(&q->readEvent);
      }
      ownSemPost(&q->lock);
      if (q->popped == vx_true_e || wrote == vx_true_e) break;
    }
  }
  return wrote;
}
...
vx_bool ownReadQueue(vx_queue_t *q, vx_value_set_t **data) {
  ...
  if (q) {
    ...
    while (ownWaitEvent(&q->readEvent, VX_INT_FOREVER) == vx_true_e) {
      ...
      ownSemPost(&q->lock);
      if (q->popped == vx_true_e || red == vx_true_e) break;
    }
