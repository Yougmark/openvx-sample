  ...
  switch (scalar->data_type) {
    ...
    case VX_TYPE_BOOL:
      VX_PRINT(VX_ZONE_SCALAR, "scalar " VX_FMT_REF " = %s\n", scalar,
               (scalar->data.boolean == vx_true_e ? "TRUE" : "FALSE"));
      break;
    default:
...
VX_API_ENTRY vx_scalar VX_API_CALL vxCreateScalar(vx_context context,
  ...
  if (ownIsValidContext(context) == vx_false_e) return NULL;

  if (!VX_TYPE_IS_SCALAR(data_type)) {
    VX_PRINT(VX_ZONE_ERROR, "Invalid type to scalar\n");
    vxAddLogEntry(&context->base, VX_ERROR_INVALID_TYPE,
                  "Invalid type to scalar\n");
    scalar = (vx_scalar)ownGetErrorObject(context, VX_ERROR_INVALID_TYPE);
  } else {
    scalar = (vx_scalar)ownCreateReference(context, VX_TYPE_SCALAR, VX_EXTERNAL,
                                           &context->base);
    if (vxGetStatus((vx_reference)scalar) == VX_SUCCESS &&
        scalar->base.type == VX_TYPE_SCALAR) {
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseScalar(vx_scalar *s) {
  return ownReleaseReferenceInt((vx_reference *)s, VX_TYPE_SCALAR, VX_EXTERNAL,
                                NULL);
} /* vxReleaseScalar() */
...
static vx_status own_scalar_to_host_mem(vx_scalar scalar, void *user_ptr) {
  ...
  switch (scalar->data_type) {
    case VX_TYPE_CHAR:
      *(vx_char *)user_ptr = scalar->data.chr;
      break;
    case VX_TYPE_INT8:
      *(vx_int8 *)user_ptr = scalar->data.s08;
      break;
    case VX_TYPE_UINT8:
      *(vx_uint8 *)user_ptr = scalar->data.u08;
      break;
    case VX_TYPE_INT16:
      *(vx_int16 *)user_ptr = scalar->data.s16;
      break;
    case VX_TYPE_UINT16:
      *(vx_uint16 *)user_ptr = scalar->data.u16;
      break;
    case VX_TYPE_INT32:
      *(vx_int32 *)user_ptr = scalar->data.s32;
      break;
    case VX_TYPE_UINT32:
      *(vx_uint32 *)user_ptr = scalar->data.u32;
      break;
    case VX_TYPE_INT64:
      *(vx_int64 *)user_ptr = scalar->data.s64;
      break;
    case VX_TYPE_UINT64:
      *(vx_uint64 *)user_ptr = scalar->data.u64;
      break;
#if OVX_SUPPORT_HALF_FLOAT
    case VX_TYPE_FLOAT16:
...
#endif
    case VX_TYPE_FLOAT32:
      *(vx_float32 *)user_ptr = scalar->data.f32;
      break;
    case VX_TYPE_FLOAT64:
      *(vx_float64 *)user_ptr = scalar->data.f64;
      break;
    case VX_TYPE_DF_IMAGE:
      *(vx_df_image *)user_ptr = scalar->data.fcc;
      break;
    case VX_TYPE_ENUM:
      *(vx_enum *)user_ptr = scalar->data.enm;
      break;
    case VX_TYPE_SIZE:
      *(vx_size *)user_ptr = scalar->data.size;
      break;
    case VX_TYPE_BOOL:
      *(vx_bool *)user_ptr = scalar->data.boolean;
      break;
...
static vx_status own_host_mem_to_scalar(vx_scalar scalar, void *user_ptr) {
  ...
  switch (scalar->data_type) {
    case VX_TYPE_CHAR:
      scalar->data.chr = *(vx_char *)user_ptr;
      break;
    case VX_TYPE_INT8:
      scalar->data.s08 = *(vx_int8 *)user_ptr;
      break;
    case VX_TYPE_UINT8:
      scalar->data.u08 = *(vx_uint8 *)user_ptr;
      break;
    case VX_TYPE_INT16:
      scalar->data.s16 = *(vx_int16 *)user_ptr;
      break;
    case VX_TYPE_UINT16:
      scalar->data.u16 = *(vx_uint16 *)user_ptr;
      break;
    case VX_TYPE_INT32:
      scalar->data.s32 = *(vx_int32 *)user_ptr;
      break;
    case VX_TYPE_UINT32:
      scalar->data.u32 = *(vx_uint32 *)user_ptr;
      break;
    case VX_TYPE_INT64:
      scalar->data.s64 = *(vx_int64 *)user_ptr;
      break;
    case VX_TYPE_UINT64:
      scalar->data.u64 = *(vx_uint64 *)user_ptr;
      break;
#if OVX_SUPPORT_HALF_FLOAT
    case VX_TYPE_FLOAT16:
      scalar->data.f16 = *(vx_float16 *)user_ptr;
      break;
#endif
    case VX_TYPE_FLOAT32:
      scalar->data.f32 = *(vx_float32 *)user_ptr;
      break;
    case VX_TYPE_FLOAT64:
      scalar->data.f64 = *(vx_float64 *)user_ptr;
      break;
    case VX_TYPE_DF_IMAGE:
      scalar->data.fcc = *(vx_df_image *)user_ptr;
      break;
    case VX_TYPE_ENUM:
      scalar->data.enm = *(vx_enum *)user_ptr;
      break;
    case VX_TYPE_SIZE:
      scalar->data.size = *(vx_size *)user_ptr;
      break;
    case VX_TYPE_BOOL:
      scalar->data.boolean = *(vx_bool *)user_ptr;
      break;
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyScalar(vx_scalar scalar,
  ...
  switch (usage) {
    case VX_READ_ONLY:
      ...
      break;
    case VX_WRITE_ONLY:
      status = own_host_mem_to_scalar(scalar, user_ptr);
      break;
...
VX_API_ENTRY vx_status VX_API_CALL vxReadScalarValue(vx_scalar scalar,
                                                     void *ptr) {
  vx_status status = VX_SUCCESS;
  ...
  if (ptr == NULL) return VX_ERROR_INVALID_PARAMETERS;
...
VX_API_ENTRY vx_status VX_API_CALL vxWriteScalarValue(vx_scalar scalar,
                                                      const void *ptr) {
  vx_status status = VX_SUCCESS;
