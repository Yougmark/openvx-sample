...
vx_import ownCreateImportInt(vx_context context, vx_enum type,
                             vx_uint32 count) {
  vx_import import = NULL;

  if (ownIsValidContext(context) == vx_false_e) return 0;

  import = (vx_import)ownCreateReference(context, VX_TYPE_IMPORT, VX_EXTERNAL,
                                         &context->base);
...
void ownDestructImport(vx_reference ref) {
  ...
  for (i = 0; i < import->count; i++) {
    ownReleaseReferenceInt(&import->refs[i], import->refs[i]->type, VX_INTERNAL,
                           NULL);
  }
  if (import->refs) {
    free(import->refs);
...
VX_API_ENTRY vx_reference VX_API_CALL
vxGetImportReferenceByIndex(vx_import import, vx_uint32 index) {
  ...
  if (import && import->base.type == VX_TYPE_IMPORT) {
    if (index < import->count) {
      ...
      vxAddLogEntry(&import->base.context->base, VX_ERROR_INVALID_PARAMETERS,
                    "Incorrect index value\n");
      ref = (vx_reference_t *)ownGetErrorObject(import->base.context,
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryImport(vx_import import,
  ...
  if (ownIsValidSpecificReference((vx_reference_t *)import, VX_TYPE_IMPORT) ==
      vx_true_e) {
    switch (attribute) {
...
vxGetImportReferenceByName(vx_import import, const vx_char *name) {
  ...
  if (import && import->base.type == VX_TYPE_IMPORT) {
    ...
    for (index = 0; index < import->count; index++) {
      if (strncmp(name, import->refs[index]->name, VX_MAX_REFERENCE_NAME) ==
          0) {
        ref = (vx_reference_t *)import->refs[index];
        ownIncrementReference(ref, VX_EXTERNAL);
        break;
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseImport(vx_import *import) {
  return ownReleaseReferenceInt((vx_reference *)import, VX_TYPE_IMPORT,
                                VX_EXTERNAL, NULL);
}
