...
vx_status ownUnloadTarget(vx_context_t* context, vx_uint32 index,
  ...
  if (index < VX_INT_MAX_NUM_TARGETS) {
    ...
    if (ownDecrementReference(&context->targets[index].base, VX_INTERNAL) ==
        0) {
      /* The ownReleaseReferenceInt() below errors out if the internal index is
       * 0 */
      ...
      memset(context->targets[index].module.name, 0,
             sizeof(context->targets[index].module.name));
      ownReleaseReferenceInt((vx_reference*)targ, VX_TYPE_TARGET, VX_INTERNAL,
                             NULL);
    }
    status = VX_SUCCESS;
  }
...
#if defined(WIN32)
    _snprintf(module, VX_INT_MAX_PATH, VX_MODULE_NAME("%s"),
              (name ? name : "openvx-target"));
    module[VX_INT_MAX_PATH - 1] = 0;  // for MSVC which is not C99 compliant
#else
    snprintf(module, VX_INT_MAX_PATH, VX_MODULE_NAME("%s"),
             (name ? name : "openvx-target"));
#endif
    context->targets[index].module.handle = ownLoadModule(module);
    if (context->targets[index].module.handle) {
      ownInitReference(&context->targets[index].base, context, VX_TYPE_TARGET,
                       &context->base);
      ownIncrementReference(&context->targets[index].base, VX_INTERNAL);
      if (ownAddReference(context, &context->targets[index].base) ==
          vx_true_e) {
        context->targets[index].funcs.init = (vx_target_init_f)ownGetSymbol(
            context->targets[index].module.handle, "vxTargetInit");
        context->targets[index].funcs.deinit = (vx_target_deinit_f)ownGetSymbol(
            context->targets[index].module.handle, "vxTargetDeinit");
        context->targets[index].funcs.supports =
            (vx_target_supports_f)ownGetSymbol(
                context->targets[index].module.handle, "vxTargetSupports");
        context->targets[index].funcs.process =
            (vx_target_process_f)ownGetSymbol(
                context->targets[index].module.handle, "vxTargetProcess");
        context->targets[index].funcs.verify = (vx_target_verify_f)ownGetSymbol(
            context->targets[index].module.handle, "vxTargetVerify");
        context->targets[index].funcs.addkernel =
            (vx_target_addkernel_f)ownGetSymbol(
                context->targets[index].module.handle, "vxTargetAddKernel");
#ifdef OPENVX_KHR_TILING
        context->targets[index].funcs.addtilingkernel =
            (vx_target_addtilingkernel_f)ownGetSymbol(
                context->targets[index].module.handle,
        ...
        /* tiling kernel function can be NULL */
        {
          ...
          VX_PRINT(VX_ZONE_ERROR,
                   "Failed to load target %s due to missing symbols!\n",
                   module);
...
vx_status ownInitializeTarget(vx_target target,
  ...
  for (k = 0u; k < numkernels; k++) {
    status = ownInitializeKernel(
        target->base.context, &target->kernels[k], kernels[k]->enumeration,
        kernels[k]->function, kernels[k]->name, kernels[k]->parameters,
        kernels[k]->numParams, kernels[k]->validate, kernels[k]->input_validate,
        kernels[k]->output_validate, kernels[k]->initialize,
...
vx_bool ownMatchTargetNameWithString(const char* target_name,
  ...
  if (ptr != NULL) {
    ...
    if ((!(begin > 0) || ((begin > 0) && (target_name[begin - 1] == dot))) ||
        (!(end < name_len) ||
         ((end < name_len) && (target_name[end] == dot)))) {
      match = vx_true_e;
    }
  }
  return match;
