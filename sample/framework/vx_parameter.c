...
vx_bool ownIsValidDirection(vx_enum dir) {
  if ((dir == VX_INPUT) ||
      (dir == VX_OUTPUT)) /* Bidirectional is not valid for user kernels */
  {
    return vx_true_e;
  } else {
    return vx_false_e;
...
vxGetKernelParameterByIndex(vx_kernel kernel, vx_uint32 index) {
  ...
  if (ownIsValidSpecificReference(&kernel->base, VX_TYPE_KERNEL) == vx_true_e) {
    if (index < VX_INT_MAX_PARAMS && index < kernel->signature.num_parameters) {
      parameter = (vx_parameter)ownCreateReference(
          kernel->base.context, VX_TYPE_PARAMETER, VX_EXTERNAL,
          &kernel->base.context->base);
      if (vxGetStatus((vx_reference)parameter) == VX_SUCCESS &&
          parameter->base.type == VX_TYPE_PARAMETER) {
      ...
      vxAddLogEntry(&kernel->base, VX_ERROR_INVALID_PARAMETERS,
                    "Index %u out of range for node %s (numparams = %u)!\n",
                    index, kernel->name, kernel->signature.num_parameters);
      parameter = (vx_parameter_t *)ownGetErrorObject(
          kernel->base.context, VX_ERROR_INVALID_PARAMETERS);
    }
  }
...
VX_API_ENTRY vx_parameter VX_API_CALL vxGetParameterByIndex(vx_node node,
  ...
  if (node->kernel == NULL) {
    ...
    vxAddLogEntry(&node->base, VX_ERROR_INVALID_NODE,
                  "Node was created without a kernel! Fatal Error!\n");
    param = (vx_parameter_t *)ownGetErrorObject(node->base.context,
                                                VX_ERROR_INVALID_NODE);
  } else {
    if (/*0 <= index &&*/ index < VX_INT_MAX_PARAMS &&
        index < node->kernel->signature.num_parameters) {
      param = (vx_parameter)ownCreateReference(
          node->base.context, VX_TYPE_PARAMETER, VX_EXTERNAL, &node->base);
      if (vxGetStatus((vx_reference)param) == VX_SUCCESS &&
          param->base.type == VX_TYPE_PARAMETER) {
        param->index = index;
        param->node = node;
...
void ownDestructParameter(vx_reference ref) {
  ...
  if (param->node) {
    if (ownIsValidSpecificReference(&param->node->base, VX_TYPE_NODE) ==
        vx_true_e) {
  ...
  if (param->kernel) {
    if (ownIsValidSpecificReference(&param->kernel->base, VX_TYPE_KERNEL) ==
        vx_true_e) {
      vx_kernel kernel = (vx_kernel)param->kernel;
      ownReleaseReferenceInt((vx_reference *)&kernel, VX_TYPE_KERNEL,
                             VX_INTERNAL, NULL);
    }
  }
}
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseParameter(vx_parameter *param) {
  return ownReleaseReferenceInt((vx_reference *)param, VX_TYPE_PARAMETER,
                                VX_EXTERNAL, NULL);
}
...
VX_API_ENTRY vx_status VX_API_CALL vxSetParameterByIndex(vx_node node,
  ...
  VX_PRINT(VX_ZONE_PARAMETER,
           ...
           "\n",
           index, node->kernel->name, node->kernel->enumeration, value);

  /* is the index out of bounds? */
  if ((index >= node->kernel->signature.num_parameters) ||
      (index >= VX_INT_MAX_PARAMS)) {
  ...
  if ((value == 0) &&
      (node->kernel->signature.states[index] == VX_PARAMETER_STATE_OPTIONAL)) {
    status = VX_SUCCESS;
    goto exit;
  }
  ...
  VX_PRINT(VX_ZONE_PARAMETER,
           "Query returned type %08x for ref " VX_FMT_REF "\n", type, value);
  ...
  if (node->kernel->signature.types[index] != type) {
    /* Check special case where signature is a specific scalar type.
       ...
       doesn't specify that only VX_TYPE_SCALAR should be used for scalar types
       in this function. */
    if (node->kernel->signature.types[index] == VX_TYPE_REFERENCE) {
      if (!VX_TYPE_IS_DATA_OBJECT(type)) {
        ...
        goto exit;
               ...
               (vxQueryScalar((vx_scalar)value, VX_SCALAR_TYPE, &data_type,
                              sizeof(data_type)) == VX_SUCCESS)) {
      ...
      VX_PRINT(VX_ZONE_ERROR,
               "Invalid type %s (0x%08x), expected %s (0x%08x)\n",
  ...
  if (node->parameters[index]) {
    if (node->parameters[index]->delay != NULL) {
      ...
      vx_bool res =
          ownRemoveAssociationToDelay(node->parameters[index], node, index);
      if (res == vx_false_e) {
  ...
  /* Note that we don't need to do anything special for parameters to child
   * graphs. */

exit:
  if (status == VX_SUCCESS) {
    VX_PRINT(VX_ZONE_PARAMETER,
             "Assigned Node[%u] %p type:%08x ref=" VX_FMT_REF "\n", index, node,
             type, value);
  } else {
    VX_PRINT(VX_ZONE_ERROR,
             "Specified: parameter[%u] type:%08x => " VX_FMT_REF "\n", index,
             type, value);
...
vxSetParameterByReference(vx_parameter parameter, vx_reference value) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (ownIsValidSpecificReference((vx_reference_t *)parameter,
                                  VX_TYPE_PARAMETER) == vx_true_e) {
    if (parameter->node) {
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryParameter(vx_parameter parameter,
  ...
  vx_status status = VX_SUCCESS;
  if (ownIsValidSpecificReference(&parameter->base, VX_TYPE_PARAMETER) ==
      vx_true_e) {
    switch (attribute) {
      case VX_PARAMETER_DIRECTION:
        if (VX_CHECK_PARAM(ptr, size, vx_enum, 0x3))
          *(vx_enum *)ptr =
              parameter->kernel->signature.directions[parameter->index];
        else
      ...
      case VX_PARAMETER_TYPE:
        if (VX_CHECK_PARAM(ptr, size, vx_enum, 0x3))
          *(vx_enum *)ptr =
              parameter->kernel->signature.types[parameter->index];
        else
          status = VX_ERROR_INVALID_PARAMETERS;
        break;
      case VX_PARAMETER_STATE:
        if (VX_CHECK_PARAM(ptr, size, vx_enum, 0x3))
          *(vx_enum *)ptr =
              (vx_enum)parameter->kernel->signature.states[parameter->index];
        else
          status = VX_ERROR_INVALID_PARAMETERS;
        break;
