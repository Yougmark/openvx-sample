...
#define vxIsValidDelay(d) \
  ((d) && ownIsValidSpecificReference((vx_reference)(d), VX_TYPE_DELAY))
#define vxIsValidGraph(g) \
  ((g) && ownIsValidSpecificReference((vx_reference)(g), VX_TYPE_GRAPH))
...
vx_bool ownAddAssociationToDelay(vx_reference value, vx_node n, vx_uint32 i) {
  ...
  vx_int32 index =
      (delay->index + delay->count - abs(delay_index)) % (vx_int32)delay->count;
...
vx_bool ownRemoveAssociationToDelay(vx_reference value, vx_node n,
                                    vx_uint32 i) {
  vx_delay delay = value->delay;
  vx_int32 delay_index = value->delay_slot_index;
  ...
  if (delay->set[index].node == n &&
      delay->set[index].index == i)  // head is a match
  {
...
VX_API_ENTRY vx_reference VX_API_CALL vxGetReferenceFromDelay(vx_delay delay,
  ...
  if (vxIsValidDelay(delay) == vx_true_e) {
    if ((vx_uint32)abs(index) < delay->count) {
      ...
      ref = delay->refs[i];
      VX_PRINT(VX_ZONE_DELAY,
               "Retrieving relative index %d => " VX_FMT_REF
               " from Delay (%d)\n",
               index, ref, i);
    } else {
      vxAddLogEntry(&delay->base, VX_ERROR_INVALID_PARAMETERS,
                    "Failed to retrieve reference from delay by index %d\n",
                    index);
      ref = (vx_reference_t *)ownGetErrorObject(delay->base.context,
...
void ownDestructDelay(vx_reference ref) {
  ...
  if (vxIsValidDelay(delay) && (delay->type == VX_TYPE_PYRAMID) &&
      (delay->pyr != NULL)) {
    vx_uint32 numLevels = (vx_uint32)(((vx_pyramid)delay->refs[0])->numLevels);
    /* release pyramid delays */
    for (i = 0; i < numLevels; ++i) {
      ownReleaseReferenceInt((vx_reference *)&(delay->pyr[i]), VX_TYPE_DELAY,
                             VX_INTERNAL, &ownDestructDelay);
    }
    free(delay->pyr);
  }
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseDelay(vx_delay *d) {
  return ownReleaseReferenceInt((vx_reference *)d, VX_TYPE_DELAY, VX_EXTERNAL,
                                &ownDestructDelay);
}
...
VX_API_ENTRY vx_delay VX_API_CALL vxCreateDelay(vx_context context,
  ...
  vx_enum invalid_types[] = {
      VX_TYPE_CONTEXT, VX_TYPE_GRAPH,     VX_TYPE_NODE,      VX_TYPE_KERNEL,
      VX_TYPE_TARGET,  VX_TYPE_PARAMETER, VX_TYPE_REFERENCE,
  ...
  if (ownIsValidContext(context) == vx_false_e) return 0;
  ...
  for (t = 0u; t < dimof(invalid_types); t++) {
    if (exemplar->type == invalid_types[t]) {
      ...
      vxAddLogEntry((vx_reference)context, VX_ERROR_INVALID_REFERENCE,
                    "Attempted to create delay of invalid object type!\n");
      return (vx_delay)ownGetErrorObject(context, VX_ERROR_INVALID_REFERENCE);
    }
  }

  delay = (vx_delay)ownCreateReference(context, VX_TYPE_DELAY, VX_EXTERNAL,
                                       &context->base);
  if (vxGetStatus((vx_reference)delay) == VX_SUCCESS &&
      delay->base.type == VX_TYPE_DELAY) {
    vx_size i = 0;
    delay->pyr = NULL;
    ...
    for (i = 0; i < count; i++) {
      switch (exemplar->type) {
        case VX_TYPE_IMAGE: {
          ...
          delay->refs[i] = (vx_reference)vxCreateImage(
              context, image->width, image->height, image->format);
          break;
        }
        case VX_TYPE_ARRAY: {
          vx_array arr = (vx_array)exemplar;
          delay->refs[i] = (vx_reference)vxCreateArray(context, arr->item_type,
                                                       arr->capacity);
          break;
        }
        case VX_TYPE_MATRIX: {
          vx_matrix mat = (vx_matrix)exemplar;
          delay->refs[i] = (vx_reference)vxCreateMatrix(
              context, mat->data_type, mat->columns, mat->rows);
          break;
        }
        case VX_TYPE_CONVOLUTION: {
          vx_convolution conv = (vx_convolution)exemplar;
          vx_convolution conv2 =
              vxCreateConvolution(context, conv->base.columns, conv->base.rows);
        ...
        case VX_TYPE_DISTRIBUTION: {
          ...
          delay->refs[i] = (vx_reference)vxCreateDistribution(
              context, dist->memory.dims[0][VX_DIM_X], dist->offset_x,
              dist->range_x);
          break;
        }
        case VX_TYPE_REMAP: {
          vx_remap remap = (vx_remap)exemplar;
          delay->refs[i] = (vx_reference)vxCreateRemap(
              context, remap->src_width, remap->src_height, remap->dst_width,
              remap->dst_height);
          break;
        }
        case VX_TYPE_LUT: {
          ...
          delay->refs[i] =
              (vx_reference)vxCreateLUT(context, lut->item_type, lut->capacity);
          break;
        }
        case VX_TYPE_PYRAMID: {
          vx_pyramid pyramid = (vx_pyramid)exemplar;
          delay->refs[i] = (vx_reference)vxCreatePyramid(
              context, pyramid->numLevels, pyramid->scale, pyramid->width,
              pyramid->height, pyramid->format);
          break;
        }
        case VX_TYPE_THRESHOLD: {
          vx_threshold thresh = (vx_threshold)exemplar;
          delay->refs[i] = (vx_reference)vxCreateThreshold(
              context, thresh->thresh_type, VX_TYPE_UINT8);
          break;
        }
        case VX_TYPE_SCALAR: {
          vx_scalar scalar = (vx_scalar)exemplar;
          delay->refs[i] =
              (vx_reference)vxCreateScalar(context, scalar->data_type, NULL);
          break;
        }
        default:
          break;
    ...
    if (exemplar->type == VX_TYPE_PYRAMID) {
      ...
      for (j = 0; j < numLevels; ++j) {
        pyrdelay = (vx_delay)ownCreateReference(
            context, VX_TYPE_DELAY, VX_INTERNAL, (vx_reference)delay);
        delay->pyr[j] = pyrdelay;
        if (vxGetStatus((vx_reference)pyrdelay) == VX_SUCCESS &&
            pyrdelay->base.type == VX_TYPE_DELAY) {
          pyrdelay->set =
              (vx_delay_param_t *)calloc(count, sizeof(vx_delay_param_t));
          pyrdelay->refs = (vx_reference *)calloc(count, sizeof(vx_reference));
          pyrdelay->type = VX_TYPE_IMAGE;
          pyrdelay->count = count;
          for (i = 0; i < count; i++) {
            pyrdelay->refs[i] = (vx_reference)vxGetPyramidLevel(
                (vx_pyramid)delay->refs[i], (vx_uint32)j);
            /* set the object as a delay element */
...
VX_API_ENTRY vx_status VX_API_CALL vxAgeDelay(vx_delay delay) {
  ...
  if (vxIsValidDelay(delay) == vx_true_e) {
    ...
    // increment the index
    delay->index =
        (delay->index + (vx_uint32)delay->count - 1) % (vx_uint32)delay->count;
    ...
    for (i = 0; i < (vx_int32)delay->count; i++) {
      ...
      do {
        if (param->node != 0) {
          ownNodeSetParameter(param->node, param->index, delay->refs[j]);
        }
        param = param->next;
      } while (param != NULL);
    }
    ...
    if (delay->type == VX_TYPE_PYRAMID && delay->pyr != NULL) {
      ...
      for (i = 0; i < numLevels; ++i) vxAgeDelay(delay->pyr[i]);
    }
  } else {
    status = VX_ERROR_INVALID_REFERENCE;
  }
...
VX_API_ENTRY vx_status VX_API_CALL vxRegisterAutoAging(vx_graph graph,
  ...
  if (vxIsValidDelay(delay) == vx_false_e) return VX_ERROR_INVALID_REFERENCE;

  /* check if this particular delay is already registered in the graph */
  for (i = 0; i < VX_INT_MAX_REF; i++) {
    if (graph->delays[i] && vxIsValidDelay(graph->delays[i]) &&
