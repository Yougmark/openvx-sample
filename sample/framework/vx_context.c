    ...
    if (ownReadQueue(&proc->input, &data) == vx_true_e) {
      ...
      if (ownWriteQueue(&proc->output, data) == vx_false_e)
        VX_PRINT(VX_ZONE_ERROR,
                 "Failed to write graph=" VX_FMT_REF " status=%d\n", g, s);
    }
  }
...
VX_INT_API vx_bool ownIsValidContext(vx_context context) {
  vx_bool ret = vx_false_e;
  if ((context != NULL) && (context->base.magic == VX_MAGIC) &&
...
VX_INT_API vx_bool ownAddAccessor(vx_context context, vx_size size,
                                  vx_enum usage, void *ptr, vx_reference ref,
  ...
  for (a = 0u; a < dimof(context->accessors); a++) {
    if (context->accessors[a].used == vx_false_e) {
      ...
      if (size > 0ul && ptr == NULL) {
        ...
        if (context->accessors[a].ptr == NULL) return vx_false_e;
...
VX_INT_API vx_bool ownFindAccessor(vx_context context, const void *ptr,
                                   vx_uint32 *pIndex) {
  vx_uint32 a;
  vx_bool worked = vx_false_e;
  for (a = 0u; a < dimof(context->accessors); a++) {
    ...
    for (id = 0u; id < dimof(context->memory_maps); id++) {
      if (context->memory_maps[id].used == vx_false_e) {
        ...
        if (VX_TYPE_IMAGE == ref->type) {
          context->memory_maps[id].extra.image_data.plane_index =
              extra->image_data.plane_index;
          context->memory_maps[id].extra.image_data.rect =
              extra->image_data.rect;
        } else if (VX_TYPE_ARRAY == ref->type || VX_TYPE_LUT == ref->type) {
          vx_memory_map_extra *extra = (vx_memory_map_extra *)extra_data;
          context->memory_maps[id].extra.array_data.start =
              extra->array_data.start;
...
VX_INT_API vx_bool ownFindMemoryMap(vx_context context, vx_reference ref,
  ...
  if (id < dimof(context->memory_maps)) {
    ...
    if (vx_true_e == ownSemWait(&context->memory_maps_lock)) {
      if ((context->memory_maps[id].used == vx_true_e) &&
          (context->memory_maps[id].ref == ref)) {
        worked = vx_true_e;
      }
...
#endif
{
  ...
  if (single_context == NULL) {
    ...
    if (context) {
      ...
      context->workers = ownCreateThreadpool(
          VX_INT_HOST_CORES, VX_INT_MAX_REF, /* very deep queues! */
          sizeof(vx_work_t), vxWorkerNode, context);
      ...
      for (t = 0u; t < context->num_targets; t++) {
        if (context->targets[t].module.handle) {
          ...
          if (context->targets[t].funcs.init(&context->targets[t]) !=
              VX_SUCCESS) {
      ...
      // create the internal thread which processes graphs for asynchronous
      // mode.
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseContext(vx_context *c) {
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    if (ownDecrementReference(&context->base, VX_EXTERNAL) == 0) {
      ...
      /* Details:
       *   1. This loop will warn of references which have not been released by
       * the user.
       *   2. It will close all internally opened error references.
       *   3. It will close the external references, which in turn will
       * internally close any internally dependent references that they
       * reference, assuming the reference counting has been done properly in
       * the framework.
       *   4. This garbage collection must be done before the targets are
       * released since some of these external references may have internal
       * references to target kernels.
       */
      for (r = 0; r < VX_INT_MAX_REF; r++) {
        vx_reference_t *ref = context->reftable[r];

        /* Warnings should only come when users have not released all external
         * references */
        if (ref && ref->external_count > 0) {
          VX_PRINT(VX_ZONE_WARNING,
                   ...
                   ref, ownGetObjectTypeName(ref->type), ref->external_count,
                   ref->internal_count);
        }

        /* These were internally opened during creation, so should internally
         * close ERRORs */
        if (ref && ref->type == VX_TYPE_ERROR) {
          ownReleaseReferenceInt(&ref, ref->type, VX_INTERNAL, NULL);
        }

        /* Warning above so user can fix release external objects, but close
         * here anyway */
      ...
      for (a = 0; a < dimof(context->accessors); ++a)
        if (context->accessors[a].used) ownRemoveAccessor(context, a);
      ...
      /* Normally destroy sem is part of release reference, but can't for
       * context */
  ...
  if (ownIsValidContext(context) == vx_false_e) {
    ...
    switch (attribute) {
      ...
      case VX_CONTEXT_UNIQUE_KERNEL_TABLE:
        if ((size ==
             (context->num_unique_kernels * sizeof(vx_kernel_info_t))) &&
            (ptr != NULL)) {
          vx_uint32 k = 0u;
          vx_uint32 t = 0u;
          vx_uint32 k2 = 0u;
          ...
          for (t = 0; t < context->num_targets; t++) {
            for (k = 0u; k < VX_INT_MAX_KERNELS; k++) {
              ...
              VX_PRINT(VX_ZONE_INFO, "Checking uniqueness of %s (%d)\n",
                       ...
                       context->targets[t].kernels[k].enumeration);
              for (k2 = 0u; k2 < numk; k2++) {
                if (VX_KERNEL_INVALID <
                        context->targets[t].kernels[k].enumeration &&
                    VX_KERNEL_MAX_1_0 >
                        context->targets[t].kernels[k].enumeration &&
                    table[k2].enumeration ==
                        context->targets[t].kernels[k].enumeration) {
                  found = vx_true_e;
                  break;
                }
              ...
              if (VX_KERNEL_INVALID <
                ...
                VX_PRINT(VX_ZONE_INFO, "Kernel %s is unique\n",
                         context->targets[t].kernels[k].name);
                table[numk].enumeration =
                    context->targets[t].kernels[k].enumeration;
                strncpy(table[numk].name, context->targets[t].kernels[k].name,
                        VX_MAX_KERNEL_NAME);
                numk++;
              }
            }
...
VX_API_ENTRY vx_status VX_API_CALL vxHint(vx_reference reference, vx_enum hint,
  ...
  if (ownIsValidContext((vx_context)reference) == vx_false_e &&
      ownIsValidReference(reference) == vx_false_e)
...
VX_API_ENTRY vx_enum VX_API_CALL vxRegisterUserStruct(vx_context context,
  ...
  if ((ownIsValidContext(context) == vx_true_e) && (size != 0)) {
...
VX_API_ENTRY vx_enum VX_API_CALL vxGetUserStructByName(vx_context context,
  ...
  if (VX_MAX_STRUCT_NAME < len) len = VX_MAX_STRUCT_NAME;
...
VX_API_ENTRY vx_enum VX_API_CALL vxRegisterUserStructWithName(
    vx_context context, vx_size size, const vx_char *name) {
  vx_enum type = VX_TYPE_INVALID;
  vx_uint32 i = 0;
...
vxAllocateUserKernelId(vx_context context, vx_enum *pKernelEnumId) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
  if ((ownIsValidContext(context) == vx_true_e) && pKernelEnumId) {
    status = VX_ERROR_NO_RESOURCES;
    if (context->next_dynamic_user_kernel_id <= VX_KERNEL_MASK) {
      *pKernelEnumId = VX_KERNEL_BASE(VX_ID_USER, 0) +
                       context->next_dynamic_user_kernel_id++;
      status = VX_SUCCESS;
    }
  }
  return status;
...
VX_API_ENTRY vx_status VX_API_CALL
vxAllocateUserKernelLibraryId(vx_context context, vx_enum *pLibraryId) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
...
static vx_target_t *findTargetByString(vx_context_t *context,
  ...
  if (target_lower_string) {
    ...
    for (t = 0; t < context->num_targets; t++) {
      ...
      if (ownMatchTargetNameWithString(curr_target->name,
                                       target_lower_string) == vx_true_e) {
        target = curr_target;
      }
    }
...
VX_API_ENTRY vx_status VX_API_CALL vxSetImmediateModeTarget(
    vx_context context, vx_enum target_enum, const char *target_string) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
  if (ownIsValidContext(context) == vx_true_e) {
    vx_target_t *target = NULL;
    switch (target_enum) {
      case VX_TARGET_ANY:
        context->imm_target_enum = VX_TARGET_ANY;
        memset(context->imm_target_string, 0,
      ...
      case VX_TARGET_STRING:
        ...
        if (target != NULL) /* target was found */
        {
          ...
          strncpy(context->imm_target_string, target_string,
                  sizeof(context->imm_target_string));
          context->imm_target_string[sizeof(context->imm_target_string) - 1] =
              '\0';
          status = VX_SUCCESS;
