...
void ownDestructPyramid(vx_reference ref) {
  ...
  for (i = 0; i < pyramid->numLevels; i++) {
    ownReleaseReferenceInt((vx_reference *)&pyramid->levels[i], VX_TYPE_IMAGE,
                           VX_INTERNAL, NULL);
  }
  free(pyramid->levels);
}
...
VX_API_ENTRY vx_status VX_API_CALL vxReleasePyramid(vx_pyramid *pyr) {
  return ownReleaseReferenceInt((vx_reference *)pyr, VX_TYPE_PYRAMID,
                                VX_EXTERNAL, NULL);
}
...
vx_status ownInitPyramid(vx_pyramid pyramid, vx_size levels, vx_float32 scale,
  ...
  const vx_float32 c_orbscale[4] = {
      0.5f, VX_SCALE_PYRAMID_ORB, VX_SCALE_PYRAMID_ORB * VX_SCALE_PYRAMID_ORB,
    ...
    if (pyramid->width != 0 && pyramid->height != 0 &&
      ...
      for (i = 0; i < (vx_int32)pyramid->numLevels; i++) {
        ...
        if (pyramid->levels[i] == 0) {
           ...
           */
          ownIncrementReference((vx_reference_t *)pyramid->levels[i],
                                VX_INTERNAL);
          ownDecrementReference((vx_reference_t *)pyramid->levels[i],
                                VX_EXTERNAL);

          /* remember that the scope of the image is the pyramid */
          ((vx_image_t *)pyramid->levels[i])->base.scope =
              (vx_reference_t *)pyramid;
...
static vx_pyramid vxCreatePyramidInt(vx_context context, vx_size levels,
                                     vx_float32 scale, vx_uint32 width,
  ...
  if ((scale != VX_SCALE_PYRAMID_HALF) && (scale != VX_SCALE_PYRAMID_ORB)) {
    ...
    pyramid =
        (vx_pyramid_t *)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
  } else if (levels == 0 || levels > 8) {
    VX_PRINT(VX_ZONE_ERROR, "Invalid number of levels for pyramid!\n", levels);
    vxAddLogEntry((vx_reference)context, VX_ERROR_INVALID_PARAMETERS,
                  "Invalid number of levels for pyramid!\n", levels);
    pyramid =
        (vx_pyramid_t *)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
  } else {
    pyramid = (vx_pyramid)ownCreateReference(context, VX_TYPE_PYRAMID,
    ...
    if (vxGetStatus((vx_reference)pyramid) == VX_SUCCESS &&
      ...
      if (status != VX_SUCCESS) {
        vxAddLogEntry((vx_reference)pyramid, status,
                      "Failed to initialize pyramid\n");
      ...
      vxAddLogEntry((vx_reference)context, VX_ERROR_NO_MEMORY,
...
VX_API_ENTRY vx_pyramid VX_API_CALL
vxCreateVirtualPyramid(vx_graph graph, vx_size levels, vx_float32 scale,
                       vx_uint32 width, vx_uint32 height, vx_df_image format) {
  vx_pyramid pyramid = NULL;

  if (ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) == vx_true_e) {
    pyramid = vxCreatePyramidInt(graph->base.context, levels, scale, width,
                                 height, format, vx_true_e);
    if (vxGetStatus((vx_reference)pyramid) == VX_SUCCESS &&
        pyramid->base.type == VX_TYPE_PYRAMID) {
...
VX_API_ENTRY vx_pyramid VX_API_CALL
vxCreatePyramid(vx_context context, vx_size levels, vx_float32 scale,
                vx_uint32 width, vx_uint32 height, vx_df_image format) {
  vx_pyramid pyr = NULL;
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    if ((width == 0) || (height == 0) || (format == VX_DF_IMAGE_VIRT)) {
      ...
      pyr = (vx_pyramid)vxCreatePyramidInt(context, levels, scale, width,
                                           height, format, vx_false_e);
    }
  }
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryPyramid(vx_pyramid pyramid,
                                                  vx_enum attribute, void *ptr,
                                                  vx_size size) {
  vx_status status = VX_SUCCESS;
  if (ownIsValidSpecificReference(&pyramid->base, VX_TYPE_PYRAMID) ==
      vx_true_e) {
    switch (attribute) {
...
VX_API_ENTRY vx_image VX_API_CALL vxGetPyramidLevel(vx_pyramid pyramid,
                                                    vx_uint32 index) {
  vx_image image = 0;
  if (ownIsValidSpecificReference(&pyramid->base, VX_TYPE_PYRAMID) ==
      vx_true_e) {
    if (index < pyramid->numLevels) {
      ...
      vxAddLogEntry(&pyramid->base, VX_ERROR_INVALID_PARAMETERS,
                    "Failed to get pyramid level %d\n", index);
      image = (vx_image_t *)ownGetErrorObject(pyramid->base.context,
