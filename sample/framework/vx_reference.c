...
void ownInitReferenceForDelay(vx_reference ref, vx_delay d,
                              vx_int32 slot_index) {
  ref->delay = d;
...
vx_status ownReleaseReferenceInt(vx_reference *pref, vx_enum type,
                                 vx_enum reftype,
  ...
  if (ref && ownIsValidSpecificReference(ref, type) == vx_true_e) {
    ...
    if (ownDecrementReference(ref, reftype) == 0) {
      ...
      VX_PRINT(VX_ZONE_REFERENCE,
               ">>>> Reference count was zero, destructed object " VX_FMT_REF
               "\n",
               ref);

      ownDestroySem(&ref->lock);
      ref->magic = VX_BAD_MAGIC; /* make sure no existing copies of refs can use
                                    ref again */

      /* some objects are statically allocated. */
      for (d = 0; d < dimof(static_objects); d++) {
        if (type == static_objects[d]) break;
      }
  ...
  if (ref) {
    ...
    if (ownAddReference(context, ref) == vx_false_e) {
      ...
      vxAddLogEntry(&context->base, VX_ERROR_NO_RESOURCES,
                    "Failed to add to resources table\n");
...
vx_bool ownIsValidReference(vx_reference ref) {
  ...
  if (ref != NULL) {
    ownPrintReference(ref);
    if ((ref->magic == VX_MAGIC) && (ownIsValidType(ref->type) == vx_true_e) &&
        (((ref->type != VX_TYPE_CONTEXT) &&
...
vx_bool ownIsValidSpecificReference(vx_reference ref, vx_enum type) {
  ...
  if (ref != NULL) {
    ownPrintReference(ref);
    if ((ref->magic == VX_MAGIC) && (ref->type == type) &&
        (ownIsValidContext(ref->context) == vx_true_e)) {
      ret = vx_true_e;
    } else if (ref->type != VX_TYPE_CONTEXT) {
      VX_PRINT(VX_ZONE_ERROR, "%p is not a valid reference!\n", ref);
      // DEBUG_BREAK(); // this will catch any "invalid" objects, but is not
      // useful otherwise.
...
void ownPrintReference(vx_reference ref) {
  if (ref) {
    VX_PRINT(
        VX_ZONE_REFERENCE,
        "vx_reference_t:%p magic:%08x type:%08x count:[%u,%u] context:%p\n",
...
vx_uint32 ownIncrementReference(vx_reference ref, vx_enum reftype) {
  ...
  if (ref) {
    ownSemWait(&ref->lock);
    if (reftype == VX_EXTERNAL || reftype == VX_BOTH) ref->external_count++;
    if (reftype == VX_INTERNAL || reftype == VX_BOTH) ref->internal_count++;
    count = ref->internal_count + ref->external_count;
    VX_PRINT(VX_ZONE_REFERENCE,
             "Incremented Total Reference Count to %u on " VX_FMT_REF "\n",
             count, ref);
    ownSemPost(&ref->lock);
  }
  return count;
  ...
  if (ref) {
    ...
    if (reftype == VX_INTERNAL || reftype == VX_BOTH) {
      if (ref->internal_count == 0) {
        VX_PRINT(VX_ZONE_WARNING,
                 "#### INTERNAL REF COUNT IS ALREADY ZERO!!! " VX_FMT_REF
                 " type:%08x #####\n",
                 ref, ref->type);
        DEBUG_BREAK();
      } else {
    ...
    if (reftype == VX_EXTERNAL || reftype == VX_BOTH) {
      if (ref->external_count == 0) {
        VX_PRINT(VX_ZONE_WARNING,
                 "#### EXTERNAL REF COUNT IS ALREADY ZERO!!! " VX_FMT_REF
                 " type:%08x #####\n",
                 ref, ref->type);
        DEBUG_BREAK();
      } else {
    ...
    VX_PRINT(VX_ZONE_REFERENCE,
             "Decremented Total Reference Count to %u on " VX_FMT_REF
             " type:%08x\n",
             result, ref, ref->type);
    ownSemPost(&ref->lock);
  }
