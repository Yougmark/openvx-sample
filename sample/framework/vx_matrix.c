...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseMatrix(vx_matrix *mat) {
  return ownReleaseReferenceInt((vx_reference *)mat, VX_TYPE_MATRIX,
                                VX_EXTERNAL, NULL);
}

VX_API_ENTRY vx_matrix VX_API_CALL vxCreateMatrix(vx_context context,
                                                  vx_enum data_type,
                                                  vx_size columns,
                                                  vx_size rows) {
  ...
  } else if ((data_type == VX_TYPE_INT32) || (data_type == VX_TYPE_UINT32) ||
  ...
  if ((columns == 0ul) || (rows == 0ul)) {
    ...
    vxAddLogEntry(&context->base, VX_ERROR_INVALID_DIMENSION,
                  "Invalid dimensions to matrix\n");
    return (vx_matrix)ownGetErrorObject(context, VX_ERROR_INVALID_DIMENSION);
  }
  matrix = (vx_matrix)ownCreateReference(context, VX_TYPE_MATRIX, VX_EXTERNAL,
                                         &context->base);
...
VX_API_ENTRY vx_matrix VX_API_CALL vxCreateMatrixFromPattern(vx_context context,
  ...
  if ((columns > VX_INT_MAX_NONLINEAR_DIM) ||
      (rows > VX_INT_MAX_NONLINEAR_DIM)) {
    VX_PRINT(VX_ZONE_ERROR, "Invalid dimensions to matrix\n");
    vxAddLogEntry(&context->base, VX_ERROR_INVALID_DIMENSION,
                  "Invalid dimensions to matrix\n");
    return (vx_matrix)ownGetErrorObject(context, VX_ERROR_INVALID_DIMENSION);
  ...
  if (ownIsValidSpecificReference(&matrix->base, VX_TYPE_MATRIX) == vx_true_e) {
    if (ownAllocateMemory(matrix->base.context, &matrix->memory) == vx_true_e) {
        ...
        for (x = 0; x < columns; ++x) {
          ...
          switch (pattern) {
            ...
            case VX_PATTERN_CROSS:
              value = ((y == rows / 2) || (x == columns / 2)) ? 255 : 0;
              break;
            case VX_PATTERN_DISK:
              value = (((y - rows / 2.0 + 0.5) * (y - rows / 2.0 + 0.5)) /
                           ((rows / 2.0) * (rows / 2.0)) +
                       ((x - columns / 2.0 + 0.5) * (x - columns / 2.0 + 0.5)) /
                           ((columns / 2.0) * (columns / 2.0))) <= 1
                          ? 255
                          : 0;
              break;
          }
      ...
      vxAddLogEntry(&context->base, VX_ERROR_NO_MEMORY,
...
VX_API_ENTRY vx_matrix VX_API_CALL vxCreateMatrixFromPatternAndOrigin(
    ...
    vx_size origin_col, vx_size origin_row) {
    ...
    if (vxGetStatus((vx_reference)matrix) == VX_SUCCESS &&
      ...
      if (origin_col < columns && origin_row < rows) {
        ...
        vxAddLogEntry(&context->base, VX_ERROR_INVALID_PARAMETERS,
                      "Origin point error\n");
        matrix =
            (vx_matrix)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
      }
    }
  }
  return matrix;
}
VX_API_ENTRY vx_status VX_API_CALL vxQueryMatrix(vx_matrix matrix,
                                                 vx_enum attribute, void *ptr,
                                                 vx_size size) {
  vx_status status = VX_SUCCESS;
  if (ownIsValidSpecificReference(&matrix->base, VX_TYPE_MATRIX) ==
      vx_false_e) {
  ...
  switch (attribute) {
    ...
    case VX_MATRIX_SIZE:
      if (VX_CHECK_PARAM(ptr, size, vx_size, 0x3)) {
        *(vx_size *)ptr =
            matrix->columns * matrix->rows * matrix->memory.dims[0][0];
      } else {
        status = VX_ERROR_INVALID_PARAMETERS;
      }
      break;
...
VX_API_ENTRY vx_status VX_API_CALL vxWriteMatrix(vx_matrix matrix,
                                                 const void *array) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
  if (ownIsValidSpecificReference(&matrix->base, VX_TYPE_MATRIX) == vx_true_e) {
    if (ownAllocateMemory(matrix->base.context, &matrix->memory) == vx_true_e) {
...
vx_status VX_API_CALL vxCopyMatrix(vx_matrix matrix, void *ptr, vx_enum usage,
                                   vx_enum mem_type) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
  (void)mem_type;
  ...
  if (ownIsValidSpecificReference(&matrix->base, VX_TYPE_MATRIX) == vx_true_e) {
    if (ownAllocateMemory(matrix->base.context, &matrix->memory) == vx_true_e) {
      if (usage == VX_READ_ONLY) {
        ...
        if (ptr) {
          vx_size size =
              matrix->memory.strides[0][1] * matrix->memory.dims[0][1];
