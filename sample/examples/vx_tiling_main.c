...
vx_node vxTilingAddNode(vx_graph graph, vx_image in0, vx_image in1,
                        vx_image out) {
  vx_reference params[] = {
      (vx_reference)in0,
      (vx_reference)in1,
  ...
  return vxCreateNodeByStructure(graph, VX_KERNEL_ADD_TILING, params,
                                 dimof(params));
}

vx_node vxTilingAlphaNode(vx_graph graph, vx_image in, vx_scalar alpha,
                          vx_image out) {
  vx_reference params[] = {
      (vx_reference)in,
      (vx_reference)alpha,
  ...
  return vxCreateNodeByStructure(graph, VX_KERNEL_ALPHA_TILING, params,
                                 dimof(params));
}

vx_node vxTilingBoxNode(vx_graph graph, vx_image in, vx_image out,
                        vx_uint32 width, vx_uint32 height) {
  vx_reference params[] = {
      (vx_reference)in,
      (vx_reference)out,
  };
  vx_node node = vxCreateNodeByStructure(graph, VX_KERNEL_BOX_MxN_TILING,
                                         params, dimof(params));
...
vx_node vxTilingGaussianNode(vx_graph graph, vx_image in, vx_image out) {
  ...
  return vxCreateNodeByStructure(graph, VX_KERNEL_GAUSSIAN_3x3_TILING, params,
                                 dimof(params));
}
