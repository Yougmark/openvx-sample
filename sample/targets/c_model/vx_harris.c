...
static vx_status VX_CALLBACK vxHarrisCornersKernel(
...
static vx_status VX_CALLBACK vxHarrisInputValidator(vx_node node,
                                                    vx_uint32 index) {
  ...
  if (index == 0) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      status = vxQueryParameter(param, VX_PARAMETER_REF, &strength_thresh,
                                sizeof(strength_thresh));
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      status = vxQueryParameter(param, VX_PARAMETER_REF, &min_dist,
                                sizeof(min_dist));
      if ((status == VX_SUCCESS) && (min_dist)) {
        ...
        if (type == VX_TYPE_FLOAT32) {
          vx_float32 d = 0.0f;
          status =
              vxCopyScalar(min_dist, &d, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
...
static vx_status VX_CALLBACK vxHarrisOutputValidator(vx_node node,
                                                     vx_uint32 index,
...
static vx_status VX_CALLBACK vxHarrisInitializer(
  ...
  if (num == dimof(harris_kernel_params)) {
    ...
    if (status == VX_SUCCESS) {
      ...
      vx_image virts[] = {
          ...
          vxCreateVirtualImage(subgraph, 0, 0, VX_DF_IMAGE_VIRT),  // Suppressed
                                                                   /*
                                                                                   ...
                                                                                   vxCreateVirtualImage(g, 0, 0, VX_DF_IMAGE_U8), //
                                                                   Shifted Suppressed Log10                                                          #endif
                                                                   */
      };

      vx_node nodes[] = {
          vxSobelMxNNode(subgraph, src, win, virts[0], virts[1]),
          vxHarrisScoreNode(subgraph, virts[0], virts[1], sen, win, blk,
                            virts[2]),
      ...
      status |=
          vxAddParameterToGraphByIndex(subgraph, nodes[3], 2);  // num_corners
...
static vx_status VX_CALLBACK vxHarrisDeinitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  (void)parameters;
...
vx_kernel_description_t harris_kernel = {
    VX_KERNEL_HARRIS_CORNERS,    "org.khronos.openvx.harris_corners",
    vxHarrisCornersKernel,       harris_kernel_params,
    dimof(harris_kernel_params), NULL,
    vxHarrisInputValidator,      vxHarrisOutputValidator,
    vxHarrisInitializer,         vxHarrisDeinitializer,
};
