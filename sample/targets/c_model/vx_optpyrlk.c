...
static vx_status LKTracker(const vx_image prevImg, const vx_image prevDerivIx,
                           const vx_image prevDerivIy, const vx_image nextImg,
  ...
  /* TODO: prevPts_stride and nextPts_stride should be used to access array
     elements as it may differ from sizeof(vx_keypoint_t_optpyrlk_internal)
  */
  ...
  vxMapArrayRange(prevPts, 0, list_length, &prevPtsFirstItem_map_id,
                  &prevPts_stride, &prevPtsFirstItem, VX_READ_ONLY,
                  VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
  vxMapArrayRange(nextPts, 0, list_length, &nextPtsFirstItem_map_id,
                  &nextPts_stride, &nextPtsFirstItem, VX_READ_AND_WRITE,
                  VX_MEMORY_TYPE_HOST, VX_NOGAP_X);

  vx_keypoint_t_optpyrlk_internal* nextPt_item =
      (vx_keypoint_t_optpyrlk_internal*)nextPtsFirstItem;
  vx_keypoint_t_optpyrlk_internal* prevPt_item =
      (vx_keypoint_t_optpyrlk_internal*)prevPtsFirstItem;

  int num_iterations_i;
  vx_float32 epsilon_f;

  vxCopyScalar(winSize_s, &winSize, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
  vxCopyScalar(num_iterations, &num_iterations_i, VX_READ_ONLY,
               VX_MEMORY_TYPE_HOST);
  vxCopyScalar(epsilon, &epsilon_f, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
  vxCopyScalar(criteria_s, &termination_Criteria, VX_READ_ONLY,
               VX_MEMORY_TYPE_HOST);
  ...
  IWinBuf = vxCreateImage(context_lk_internal, (vx_uint32)winSize,
                          (vx_uint32)winSize, VX_DF_IMAGE_S16);
  derivIWinBuf_x = vxCreateImage(context_lk_internal, (vx_uint32)winSize,
                                 (vx_uint32)winSize, VX_DF_IMAGE_S16);
  derivIWinBuf_y = vxCreateImage(context_lk_internal, (vx_uint32)winSize,
                                 (vx_uint32)winSize, VX_DF_IMAGE_S16);

  ownInitImage((vx_image_t*)IWinBuf, (vx_uint32)winSize, (vx_uint32)winSize,
               VX_DF_IMAGE_S16);
  ownAllocateImage((vx_image_t*)IWinBuf);
  ownInitImage((vx_image_t*)derivIWinBuf_x, (vx_uint32)winSize,
               (vx_uint32)winSize, VX_DF_IMAGE_S16);
  ownAllocateImage((vx_image_t*)derivIWinBuf_x);
  ownInitImage((vx_image_t*)derivIWinBuf_y, (vx_uint32)winSize,
               (vx_uint32)winSize, VX_DF_IMAGE_S16);
  ownAllocateImage((vx_image_t*)derivIWinBuf_y);

  vxQueryImage(derivIx, VX_IMAGE_FORMAT, &derivIx_format,
               sizeof(derivIx_format));
  vxQueryImage(J, VX_IMAGE_FORMAT, &J_format, sizeof(J_format));
  vxQueryImage(derivIy, VX_IMAGE_FORMAT, &derivIy_format,
               sizeof(derivIy_format));
  ...
  status |= vxAccessImagePatch(derivIx, &rect, 0, &derivIx_addr,
                               (void**)&derivIx_base, VX_READ_ONLY);
  status |=
      vxAccessImagePatch(J, &rect, 0, &J_addr, (void**)&J_base, VX_READ_ONLY);
  status |= vxAccessImagePatch(derivIy, &rect, 0, &derivIy_addr,
                               (void**)&derivIy_base, VX_READ_ONLY);
  status |=
      vxAccessImagePatch(I, &rect, 0, &I_addr, (void**)&I_base, VX_READ_ONLY);

  vxQueryImage(IWinBuf, VX_IMAGE_FORMAT, &IWinBuf_format,
               sizeof(IWinBuf_format));
  vxQueryImage(derivIWinBuf_x, VX_IMAGE_FORMAT, &derivIWinBuf_x_format,
               sizeof(derivIWinBuf_x_format));
  vxQueryImage(derivIWinBuf_y, VX_IMAGE_FORMAT, &derivIWinBuf_y_format,
               sizeof(derivIWinBuf_y_format));
  ...
  status |= vxAccessImagePatch(IWinBuf, &rect, 0, &IWinBuf_addr,
                               (void**)&IWinBuf_base, VX_READ_AND_WRITE);
  status |= vxAccessImagePatch(derivIWinBuf_x, &rect, 0, &derivIWinBuf_x_addr,
                               (void**)&derivIWinBuf_x_base, VX_READ_AND_WRITE);
  status |= vxAccessImagePatch(derivIWinBuf_y, &rect, 0, &derivIWinBuf_y_addr,
                               (void**)&derivIWinBuf_y_base, VX_READ_AND_WRITE);

  for (list_indx = 0; list_indx < list_length; list_indx++) {
    // Does not compute a point that shouldn't be tracked
    if (prevPt_item[list_indx].tracking_status == 0) continue;
    ...
    // extract the patch from the first image, compute covariation matrix of
    // derivatives
    int x;
    int y;
    for (y = 0; y < (int)winSize; y++) {
      unsigned char* src = (unsigned char*)vxFormatImagePatchAddress2d(
          I_base, (vx_uint32)iprevPt.x, (vx_uint32)(y + iprevPt.y), &I_addr);
      short* dsrc_x = (short*)vxFormatImagePatchAddress2d(
          derivIx_base, (vx_uint32)iprevPt.x, (vx_uint32)(y + iprevPt.y),
          &derivIx_addr);
      short* dsrc_y = (short*)vxFormatImagePatchAddress2d(
          derivIy_base, (vx_uint32)iprevPt.x, (vx_uint32)(y + iprevPt.y),
          &derivIy_addr);

      short* Iptr = (short*)vxFormatImagePatchAddress2d(IWinBuf_base, 0, y,
                                                        &IWinBuf_addr);
      short* dIptr_x = (short*)vxFormatImagePatchAddress2d(
          derivIWinBuf_x_base, 0, y, &derivIWinBuf_x_addr);
      short* dIptr_y = (short*)vxFormatImagePatchAddress2d(
          derivIWinBuf_y_base, 0, y, &derivIWinBuf_y_addr);

      x = 0;

      for (; x < (int)winSize; x++, dsrc_x++, dsrc_y++) {
        int ival =
            INT_ROUND(src[x] * iw00 + src[x + 1] * iw01 +
                          src[x + stepI] * iw10 + src[x + stepI + 1] * iw11,
                      W_BITS1 - 5);
        int ixval =
            INT_ROUND(dsrc_x[0] * iw00 + dsrc_x[1] * iw01 +
                          dsrc_x[dstep_x] * iw10 + dsrc_x[dstep_x + 1] * iw11,
                      W_BITS1);
        int iyval =
            INT_ROUND(dsrc_y[0] * iw00 + dsrc_y[1] * iw01 +
                          dsrc_y[dstep_y] * iw10 + dsrc_y[dstep_y + 1] * iw11,
                      W_BITS1);
    ...
    float minEig =
        (float)(A22 + A11 - sqrt((A11 - A22) * (A11 - A22) + 4.f * A12 * A12)) /
        (2 * winSize * winSize);
    ...
    j = 0;
    while (j < num_iterations_i ||
           termination_Criteria == VX_TERM_CRITERIA_EPSILON) {
      ...
      for (y = 0; y < (int)winSize; y++) {
        unsigned char* Jptr = (unsigned char*)vxFormatImagePatchAddress2d(
            J_base, (vx_uint32)inextPt.x, (vx_uint32)(y + inextPt.y), &J_addr);
        short* Iptr = (short*)vxFormatImagePatchAddress2d(IWinBuf_base, 0, y,
                                                          &IWinBuf_addr);
        short* dIptr_x = (short*)vxFormatImagePatchAddress2d(
            derivIWinBuf_x_base, 0, y, &derivIWinBuf_x_addr);
        ...
        for (; x < (int)winSize; x++) {
          int diff =
              INT_ROUND(Jptr[x] * iw00 + Jptr[x + 1] * iw01 +
      ...
      // (int)list_indx, (int)j, nextPt.x, nextPt.y, delta_x, delta_y); delta =
      // -delta;
      ...
      if ((delta_x * delta_x + delta_y * delta_y) <= epsilon_f &&
          (termination_Criteria == VX_TERM_CRITERIA_EPSILON ||
           termination_Criteria == VX_TERM_CRITERIA_BOTH))
        break;
  ...
  status |=
      vxCommitImagePatch(IWinBuf, NULL, 0, &IWinBuf_addr, (void*)IWinBuf_base);
  status |= vxCommitImagePatch(derivIWinBuf_x, NULL, 0, &derivIWinBuf_x_addr,
                               (void*)derivIWinBuf_x_base);
  status |= vxCommitImagePatch(derivIWinBuf_y, NULL, 0, &derivIWinBuf_y_addr,
                               (void*)derivIWinBuf_y_base);
  /* no needs to commit for read-only images */
  status |=
      vxCommitImagePatch(derivIx, NULL, 0, &derivIx_addr, (void*)derivIx_base);
  status |= vxCommitImagePatch(J, NULL, 0, &J_addr, (void*)J_base);
  status |=
      vxCommitImagePatch(derivIy, NULL, 0, &derivIy_addr, (void*)derivIy_base);
...
static vx_status VX_CALLBACK vxOpticalFlowPyrLKKernel(
  ...
  if (num == dimof(optpyrlk_kernel_params)) {
    ...
    vxCopyScalar(use_initial_estimate, &use_initial_estimate_b, VX_READ_ONLY,
                 VX_MEMORY_TYPE_HOST);

    vxQueryPyramid(old_pyramid, VX_PYRAMID_LEVELS, &maxLevel, sizeof(maxLevel));
    vxQueryPyramid(old_pyramid, VX_PYRAMID_SCALE, &pyramid_scale,
                   sizeof(pyramid_scale));
    ...
    for (level = (vx_int32)maxLevel; level > 0; level--) {
      ...
      vxMapArrayRange(prevPts, 0, list_length, &prevPtsFirstItem_map_id,
                      &prevPts_stride, &prevPtsFirstItem, VX_READ_AND_WRITE,
                      VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
      prevPt = (vx_keypoint_t_optpyrlk_internal*)prevPtsFirstItem;

      if (level == (vx_int32)maxLevel) {
        if (use_initial_estimate_b) {
          vx_size list_length2 = 0;
          vxQueryArray(estimatedPts, VX_ARRAY_NUMITEMS, &list_length2,
                       sizeof(list_length2));
          if (list_length2 != list_length) return VX_ERROR_INVALID_PARAMETERS;

          initialPtsFirstItem = NULL;
          vxMapArrayRange(estimatedPts, 0, list_length,
                          &initialPtsFirstItem_map_id, &estimatedPts_stride,
                          &initialPtsFirstItem, VX_READ_ONLY,
                          VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
      ...
      for (list_indx = 0; list_indx < list_length; list_indx++) {
        if (level == (vx_int32)maxLevel) {
          ...
          (prevPt)->x = (((vx_keypoint_t*)prevPt))->x *
                        (float)((pow(pyramid_scale, level - 1)));
          (prevPt)->y = (((vx_keypoint_t*)prevPt))->y *
                        (float)((pow(pyramid_scale, level - 1)));

          if (use_initial_estimate_b) {
            /* Estimate point coordinates not already adjusted */
            keypoint.x =
                (initialPt)->x * (float)((pow(pyramid_scale, level - 1)));
            keypoint.y =
                (initialPt)->y * (float)((pow(pyramid_scale, level - 1)));
          } else {
      ...
      //            rect.start_x = 0;
      //            rect.start_y = 0;
      //            vxQueryImage(old_image, VX_IMAGE_WIDTH,  &rect.end_x,
      //            sizeof(rect.end_x)); vxQueryImage(old_image,
      //            VX_IMAGE_HEIGHT, &rect.end_y, sizeof(rect.end_y));

      // printf("%ux%u - %ux%u\n", rec.start_x, rec.start_y, rec.end_x,
      // rec.end_y);

      {
        ...
        if (vxGetStatus((vx_reference)context_scharr) == VX_SUCCESS) {
          ...
          vx_status extras_status =
              vxLoadKernels(context_scharr, "openvx-extras");

          vx_graph graph_scharr = vxCreateGraph(context_scharr);

          if (vxGetStatus((vx_reference)graph_scharr) == VX_SUCCESS &&
              extras_status == VX_SUCCESS) {
            vx_image shar_images[] = {
                old_image,  // index 0: Input
                vxCreateImage(context_scharr, width, height,
                              VX_DF_IMAGE_S16),  // index 1: Output dx
                vxCreateImage(context_scharr, width, height,
                              VX_DF_IMAGE_S16),  // index 2: Output dy
            };

            vx_node scharr_nodes[] = {
                vxScharr3x3Node(graph_scharr, shar_images[0], shar_images[1],
                                shar_images[2]),
            };
            ...
            if (status == VX_SUCCESS) {
              ...
              status |=
                  LKTracker(old_image, shar_images[1], shar_images[2],
                            new_image, prevPts, nextPts, window_dimension,
                            termination, level - 1, epsilon, num_iterations);
            }
...
static vx_status VX_CALLBACK vxOpticalFlowPyrLKInputValidator(vx_node node,
                                                              vx_uint32 index) {
...
static vx_status VX_CALLBACK vxOpticalFlowPyrLKOutputValidator(
...
extern "C"
    ...
    vx_kernel_description_t optpyrlk_kernel = {
        ...
        optpyrlk_kernel_params,
        dimof(optpyrlk_kernel_params),
        NULL,
