...
static vx_status VX_CALLBACK vxRemapKernel(vx_node node,
      ...
      for (y = 0u; y < height; y++) {
        for (x = 0u; x < width; x++) {
          ...
          if (policy == VX_INTERPOLATION_NEAREST_NEIGHBOR) {
            ...
            read_pixel(src_base, &src_addr, src_x + 0.5f, src_y + 0.5f,
                       &borders, dst);
            ...
            defined &=
                read_pixel(src_base, &src_addr, xf + 0, yf + 0, &borders, &tl);
            defined &=
                read_pixel(src_base, &src_addr, xf + 1, yf + 0, &borders, &tr);
            defined &=
                read_pixel(src_base, &src_addr, xf + 0, yf + 1, &borders, &bl);
            defined &=
                read_pixel(src_base, &src_addr, xf + 1, yf + 1, &borders, &br);
            if (defined) {
...
static vx_status VX_CALLBACK vxRemapInputValidator(vx_node node,
                                                   vx_uint32 index) {
...
static vx_status VX_CALLBACK vxRemapOutputValidator(vx_node node,
                                                    vx_uint32 index,
