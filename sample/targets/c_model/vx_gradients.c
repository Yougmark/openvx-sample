...
static vx_status VX_CALLBACK ownSobel3x3Kernel(vx_node node,
                                               const vx_reference parameters[],
                                               vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (NULL != node && NULL != parameters &&
      num == dimof(sobel3x3_kernel_params)) {
...
own_sobel3x3_validator(vx_node node, const vx_reference parameters[],
                       vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;

  if (NULL != node && NULL != parameters &&
      num == dimof(sobel3x3_kernel_params) && NULL != metas) {
    ...
    if (VX_SUCCESS == vxGetStatus((vx_reference)param1) &&
      ...
      status =
          vxQueryParameter(param1, VX_PARAMETER_REF, &input, sizeof(input));

      status |=
          vxQueryImage(input, VX_IMAGE_WIDTH, &src_width, sizeof(src_width));
      status |=
          vxQueryImage(input, VX_IMAGE_HEIGHT, &src_height, sizeof(src_height));
      status |=
