...
static vx_status VX_CALLBACK vxNonMaxSuppressionKernel(
  ...
  if (num == 4) {
    ...
    vx_scalar win_size = (vx_scalar)parameters[2];
    vx_image output = (vx_image)parameters[3];
    status = vxNonMaxSuppression(input, mask, win_size, output);
  }
  return status;
}

static vx_status VX_CALLBACK
vxNonMaxSuppressionInputValidator(vx_node node, vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
    ...
    vx_image input = 0;
    vx_parameter param[2] = {vxGetParameterByIndex(node, 0),
    ...
    if (input && win_size) {
      ...
      if (type == VX_TYPE_INT32) {
        if ((wsize <= (vx_int32)width) && (wsize <= (vx_int32)height) &&
            (wsize % 2 == 1)) {
          status = VX_SUCCESS;
        }
      } else {
...
static vx_status VX_CALLBACK vxNonMaxSuppressionOutputValidator(
