...
/*! \note Look at \ref vxCannyEdgeDetectorNode to see how this pyramid
 * construction works */
...
static vx_status VX_CALLBACK vxCannyEdgeKernel(vx_node node,
...
static vx_status VX_CALLBACK vxCannyEdgeInputValidator(vx_node node,
                                                       vx_uint32 index) {
  ...
  if (index == 0) {
    ...
    if (hyst) {
      ...
      if (type == VX_THRESHOLD_TYPE_RANGE) {
        vx_enum data_type = 0;
        vxQueryThreshold(hyst, VX_THRESHOLD_DATA_TYPE, &data_type,
                         sizeof(data_type));
...
static vx_status VX_CALLBACK vxCannyEdgeOutputValidator(vx_node node,
                                                        vx_uint32 index,
...
static vx_status VX_CALLBACK vxCannyEdgeInitializer(
  ...
  if (num == dimof(canny_kernel_params)) {
    ...
    if (status == VX_SUCCESS) {
      ...
      vx_image virts[] = {
          ...
          vxCreateVirtualImage(subgraph, 0, 0,
                               VX_DF_IMAGE_VIRT), /* 4: Nonmax */
      };

      vx_node nodes[] = {
          vxSobelMxNNode(subgraph, input, gradient_size, virts[0], virts[1]),
          vxElementwiseNormNode(subgraph, virts[0], virts[1], norm_type,
                                virts[2]),
      ...
      status |=
          vxAddParameterToGraphByIndex(subgraph, nodes[0], 0); /* input image */
      status |=
          vxAddParameterToGraphByIndex(subgraph, nodes[4], 1); /* threshold */
      status |= vxAddParameterToGraphByIndex(subgraph, nodes[0],
                                             1); /* gradient size */
      status |=
          vxAddParameterToGraphByIndex(subgraph, nodes[1], 2); /* norm type */
      status |= vxAddParameterToGraphByIndex(subgraph, nodes[4],
                                             2); /* output image */
...
static vx_status VX_CALLBACK vxCannyEdgeDeinitializer(
    vx_node node, const vx_reference parameters[], vx_uint32 num) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  (void)parameters;
