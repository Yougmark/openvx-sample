...
static vx_status VX_CALLBACK tensorTansposeKernel(
  ...
  if (num == TRANSPOSE_PARAMS_NUMBER) {
    ...
    status = TransposeTensorKernelImpl(in, dim1, dim2, output,
                                       ownSizeOfType(output->data_type));
  }

  return status;
}

static vx_status SetTransposeOutputMetaFormat(vx_tensor in1, vx_tensor out,
                                              vx_size out_dims[],
                                              vx_meta_format *meta) {
  ...
  if ((format_out != VX_TYPE_INVALID && format_out != format1) ||
      ...
      (format1 != VX_TYPE_INT16 && format1 != VX_TYPE_UINT8 &&
       format1 != VX_TYPE_INT8) ||
      (format1 == VX_TYPE_INT16 && fixed_point_pos1 != 0 &&
       fixed_point_pos1 != Q78_FIXED_POINT_POSITION)) {
    status = VX_ERROR_INVALID_FORMAT;
  } else {
    status |= vxQueryTensor(in1, VX_TENSOR_NUMBER_OF_DIMS, &out_num_dims,
                            sizeof(out_num_dims));
...
static vx_status validateDimensions(vx_tensor in, vx_tensor out,
                                    vx_scalar sc_dim1, vx_scalar sc_dim2,
                                    vx_size out_dims[]) {
  vx_status status = VX_SUCCESS;
  ...
  status |= vxQueryTensor(in, VX_TENSOR_NUMBER_OF_DIMS, &in1_num_dims,
                          sizeof(in1_num_dims));
  status |= vxQueryTensor(out, VX_TENSOR_NUMBER_OF_DIMS, &out_num_dims,
                          sizeof(out_num_dims));

  // TODO: this is exactly the kind of issue I mentioned earlier: here we
  // possibly query more dims than the tensor actually has. should it be filled
  // with 0s or must we query only sizeof(in1_dim[0]) * in1_num_dims bytes?
  status |= vxQueryTensor(in, VX_TENSOR_DIMS, in1_dim, sizeof(in1_dim));
  status |= vxQueryTensor(out, VX_TENSOR_DIMS, out_dims,
                          sizeof(*out_dims) * out_num_dims);
  ...
  if (out_num_dims != 0) {
    if (in1_num_dims != out_num_dims) {
      ...
      for (vx_size i = 0; i < out_num_dims; i++) {
        if (out_dims[i] == 0) {
          if (i == dim1) {
    ...
    for (vx_size i = 0; i < in1_num_dims; i++) {
      if (i == dim1)
        out_dims[i] = in1_dim[dim2];
      else if (i == dim2)
        out_dims[i] = in1_dim[dim1];
...
tensorTransposeValidator(vx_node node, const vx_reference parameters[],
  ...
  status =
      validateDimensions(in, out, (vx_scalar)parameters[TRANSPOSE_PARAM_DIM1],
                         (vx_scalar)parameters[TRANSPOSE_PARAM_DIM2], out_dims);
  status |= SetTransposeOutputMetaFormat(in, out, out_dims,
                                         &metas[TRANSPOSE_PARAM_TENSOR_OUT]);
...
vx_kernel_description_t tensor_transpose_kernel = {
    ...
    tensor_transpose_kernel_params,
    dimof(tensor_transpose_kernel_params),
