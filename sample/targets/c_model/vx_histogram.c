...
static vx_status VX_CALLBACK vxHistogramKernel(vx_node node,
...
static vx_status VX_CALLBACK vxEqualizeHistKernel(
...
static vx_status VX_CALLBACK vxHistogramInputValidator(vx_node node,
                                                       vx_uint32 index) {
...
static vx_status VX_CALLBACK vxHistogramOutputValidator(vx_node node,
                                                        vx_uint32 index,
  ...
  if (index == 1) {
    ...
    if ((src) && (dist)) {
      ...
      vx_df_image format;
      vx_size numBins = 0;
      vxQueryDistribution(dist, VX_DISTRIBUTION_BINS, &numBins,
                          sizeof(numBins));
...
static vx_status VX_CALLBACK vxEqualizeHistInputValidator(vx_node node,
                                                          vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
...
static vx_status VX_CALLBACK vxEqualizeHistOutputValidator(
    vx_node node, vx_uint32 index, vx_meta_format_t *ptr) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 1) {
    vx_parameter param =
        vxGetParameterByIndex(node, 0); /* we reference the input image */
...
vx_kernel_description_t histogram_kernel = {
    ...
    vxHistogramKernel,
    histogram_kernel_params,
    dimof(histogram_kernel_params),
    NULL,
...
vx_kernel_description_t equalize_hist_kernel = {
    ...
    equalize_hist_kernel_params,
    dimof(equalize_hist_kernel_params),
    NULL,
