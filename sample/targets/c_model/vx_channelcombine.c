...
static vx_status VX_CALLBACK vxChannelCombineKernel(
...
static vx_status VX_CALLBACK vxChannelCombineInputValidator(vx_node node,
                                                            vx_uint32 index) {
...
static vx_status VX_CALLBACK vxChannelCombineOutputValidator(
  ...
  if (index == 4) {
    ...
    vx_parameter params[] = {
        vxGetParameterByIndex(node, 0), vxGetParameterByIndex(node, 1),
        vxGetParameterByIndex(node, 2), vxGetParameterByIndex(node, 3),
        vxGetParameterByIndex(node, index)};
    vx_bool planes_present[4] = {vx_false_e, vx_false_e, vx_false_e,
                                 vx_false_e};
        ...
        if (image) {
          ...
          if (width == 0 && height == 0) {
            ...
            if (uv_x_scale == 0 || uv_y_scale == 0 || uv_y_scale > uv_x_scale) {
              ...
              vxAddLogEntry(
                  (vx_reference)image, status,
                  "Input image channel %u does not match in dimensions!\n", p);
              goto exit;
            }
    ...
    if (params[index]) {
      vx_image output = 0;
      vxQueryParameter(params[index], VX_PARAMETER_REF, &output,
                       sizeof(output));
      if (output) {
        vx_df_image format = VX_DF_IMAGE_VIRT;
        vx_bool supported_format = vx_true_e;
        vx_bool correct_planes =
            planes_present[0] && planes_present[1] && planes_present[2];
        ...
        switch (format) {
          ...
          case VX_DF_IMAGE_YUV4:
            correct_planes =
                correct_planes && uv_y_scale == 1 && uv_x_scale == 1;
            break;
          case VX_DF_IMAGE_RGBX:
            correct_planes = correct_planes && planes_present[3] &&
                             uv_y_scale == 1 && uv_x_scale == 1;
            break;
          case VX_DF_IMAGE_YUYV:
          case VX_DF_IMAGE_UYVY:
            correct_planes =
                correct_planes && uv_y_scale == 1 && uv_x_scale == 2;
            break;
          case VX_DF_IMAGE_NV12:
          case VX_DF_IMAGE_NV21:
          case VX_DF_IMAGE_IYUV:
            correct_planes =
                correct_planes && uv_y_scale == 2 && uv_x_scale == 2;
            break;
          default:
...
vx_kernel_description_t channelcombine_kernel = {
    ...
    channel_combine_kernel_params,
    dimof(channel_combine_kernel_params),
