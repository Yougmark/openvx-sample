...
static vx_status VX_CALLBACK vxMatchTemplateKernel(
...
static vx_status VX_CALLBACK vxMatchTemplateInputValidator(vx_node node,
                                                           vx_uint32 index) {
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (scalar) {
        ...
        if (stype == VX_TYPE_ENUM) {
          ...
          if ((metric == VX_COMPARE_HAMMING) || (metric == VX_COMPARE_L1) ||
              (metric == VX_COMPARE_L2) || (metric == VX_COMPARE_CCORR) ||
...
static vx_status VX_CALLBACK vxMatchTemplateOutputValidator(
...
vx_kernel_description_t match_template_kernel = {
    ...
    match_template_kernel_params,
    dimof(match_template_kernel_params),
    NULL,
