...
static vx_status VX_CALLBACK vxHoughLinesPKernel(vx_node node,
...
static vx_status VX_CALLBACK vxHoughLinesPInputValidator(vx_node node,
                                                         vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
...
static vx_status VX_CALLBACK vxHoughLinesPOutputValidator(
...
vx_kernel_description_t houghlinesp_kernel = {
    ...
    vxHoughLinesPKernel,
    houghlinesp_kernel_params,
    dimof(houghlinesp_kernel_params),
    NULL,
