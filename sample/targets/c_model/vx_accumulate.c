...
static vx_status VX_CALLBACK vxAccumulateKernel(vx_node node,
...
static vx_status VX_CALLBACK vxAccumulateWeightedKernel(
...
static vx_status VX_CALLBACK vxAccumulateSquareKernel(
    vx_node node, const vx_reference *parameters, vx_uint32 num) {
  (void)node;
...
static vx_status VX_CALLBACK vxAccumulateInputValidator(vx_node node,
                                                        vx_uint32 index) {
...
vxAccumulateWeightedInputValidator(vx_node node, vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
    vx_parameter param = vxGetParameterByIndex(node, index);
    ...
    if (images[0] && images[1]) {
      ...
      vxQueryImage(images[1], VX_IMAGE_FORMAT, &format[1], sizeof(format[1]));
      if (width[0] == width[1] && height[0] == height[1] &&
          format[0] == VX_DF_IMAGE_U8 && format[1] == VX_DF_IMAGE_U8) {
        status = VX_SUCCESS;
      }
      vxReleaseImage(&images[0]);
      vxReleaseImage(&images[1]);
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (scalar) {
        ...
        if (type == VX_TYPE_FLOAT32) {
          ...
          if ((vxCopyScalar(scalar, &alpha, VX_READ_ONLY,
...
static vx_status VX_CALLBACK
vxAccumulateSquaredInputValidator(vx_node node, vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_image input = 0;
    vx_parameter param = vxGetParameterByIndex(node, index);
    ...
    if (vxGetStatus((vx_reference)param) == VX_SUCCESS) {
      ...
      if (scalar) {
        ...
        if (type == VX_TYPE_UINT32) {
          vx_uint32 shift = 0u;
          if ((vxCopyScalar(scalar, &shift, VX_READ_ONLY,
                            VX_MEMORY_TYPE_HOST) == VX_SUCCESS) &&
...
static vx_status VX_CALLBACK vxAccumulateOutputValidator(
...
vx_kernel_description_t accumulate_kernel = {
    ...
    vxAccumulateKernel,
    accumulate_kernel_params,
    dimof(accumulate_kernel_params),
    NULL,
...
vx_kernel_description_t accumulate_weighted_kernel = {
    ...
    accumulate_scaled_kernel_params,
    dimof(accumulate_scaled_kernel_params),
    NULL,
