...
static vx_status setNodeTarget(vx_node node) {
  ...
  return vxSetNodeTarget(node, context->imm_target_enum,
...
VX_API_ENTRY vx_status VX_API_CALL vxuColorConvert(vx_context context,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    ...
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      ...
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
...
vxuChannelCombine(vx_context context, vx_image plane0, vx_image plane1,
                  vx_image plane2, vx_image plane3, vx_image output) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node =
        vxChannelCombineNode(graph, plane0, plane1, plane2, plane3, output);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
static const vx_enum border_modes_2[] = {VX_BORDER_UNDEFINED,
                                         VX_BORDER_CONSTANT};

static const vx_enum border_modes_3[] = {
    VX_BORDER_UNDEFINED, VX_BORDER_CONSTANT, VX_BORDER_REPLICATE};

static vx_status vx_isBorderModeSupported(vx_enum mode_to_check,
                                          const vx_enum supported_modes[],
                                          vx_size num_modes) {
...
static vx_status vx_useImmediateBorderMode(vx_context context, vx_node node,
                                           ...
                                           vx_size num_modes) {
  vx_border_t border;
  vx_status status = vxQueryContext(context, VX_CONTEXT_IMMEDIATE_BORDER,
  ...
  if (status == VX_SUCCESS) {
    ...
    if (status == VX_ERROR_NOT_SUPPORTED) {
      vx_enum policy;
      status = vxQueryContext(context, VX_CONTEXT_IMMEDIATE_BORDER_POLICY,
...
VX_API_ENTRY vx_status VX_API_CALL vxuSobel3x3(vx_context context, vx_image src,
                                               vx_image output_x,
                                               vx_image output_y) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    ...
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = vx_useImmediateBorderMode(context, node, border_modes_3,
                                         dimof(border_modes_3));
      if (status == VX_SUCCESS) status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
        status = vxProcessGraph(graph);
      }
...
VX_API_ENTRY vx_status VX_API_CALL vxuPhase(vx_context context, vx_image grad_x,
                                            vx_image grad_y, vx_image dst) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuAbsDiff(vx_context context, vx_image in1,
...
VX_API_ENTRY vx_status VX_API_CALL vxuBox3x3(vx_context context, vx_image input,
                                             vx_image output) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuLaplacianReconstruct(vx_context context,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    ...
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = vx_useImmediateBorderMode(context, node, border_modes_2,
                                         dimof(border_modes_2));
      if (status == VX_SUCCESS) status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
        status = vxProcessGraph(graph);
...
VX_API_ENTRY vx_status VX_API_CALL vxuAccumulateWeightedImage(
    vx_context context, vx_image input, vx_scalar scale, vx_image accum) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuMinMaxLoc(
    vx_context context, vx_image input, vx_scalar minVal, vx_scalar maxVal,
...
VX_API_ENTRY vx_status VX_API_CALL vxuCannyEdgeDetector(
    vx_context context, vx_image input, vx_threshold hyst,
    vx_int32 gradient_size, vx_enum norm_type, vx_image output) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node = vxCannyEdgeDetectorNode(graph, input, hyst, gradient_size,
                                           norm_type, output);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
...
VX_API_ENTRY vx_status VX_API_CALL vxuAnd(vx_context context, vx_image in1,
                                          vx_image in2, vx_image out) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuOr(vx_context context, vx_image in1,
                                         vx_image in2, vx_image out) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuXor(vx_context context, vx_image in1,
                                          vx_image in2, vx_image out) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuNot(vx_context context, vx_image input,
                                          vx_image out) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuMultiply(
    vx_context context, vx_image in1, vx_image in2, vx_float32 scale_val,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    ...
    vx_node node = vxMultiplyNode(graph, in1, in2, scale, overflow_policy,
                                  rounding_policy, out);
    if ((VX_SUCCESS == vxGetStatus((vx_reference)node)) &&
...
VX_API_ENTRY vx_status VX_API_CALL vxuAdd(vx_context context, vx_image in1,
...
VX_API_ENTRY vx_status VX_API_CALL vxuSubtract(vx_context context, vx_image in1,
                                               vx_image in2, vx_enum policy,
                                               vx_image out) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
...
VX_API_ENTRY vx_status VX_API_CALL vxuTensorMultiply(
    vx_context context, vx_tensor input1, vx_tensor input2, vx_scalar scale,
    vx_enum overflow_policy, vx_enum rounding_policy, vx_tensor output) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node = vxTensorMultiplyNode(
        graph, input1, input2, scale, overflow_policy, rounding_policy, output);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuTensorTranspose(vx_context context,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node =
        vxTensorTransposeNode(graph, input, output, dimension1, dimension2);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL
vxuTensorConvertDepth(vx_context context, vx_tensor input, vx_enum policy,
                      vx_scalar norm, vx_scalar offset, vx_tensor output) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node =
        vxTensorConvertDepthNode(graph, input, policy, norm, offset, output);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuTensorMatrixMultiply(
    vx_context context, vx_tensor input1, vx_tensor input2, vx_tensor input3,
    const vx_tensor_matrix_multiply_params_t *matrix_multiply_params,
    vx_tensor output) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node = vxTensorMatrixMultiplyNode(graph, input1, input2, input3,
                                              matrix_multiply_params, output);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuHarrisCorners(
    vx_context context, vx_image input, vx_scalar strength_thresh,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node = vxHarrisCornersNode(graph, input, strength_thresh,
                                       min_distance, sensitivity, gradient_size,
                                       block_size, corners, num_corners);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL
vxuFastCorners(vx_context context, vx_image input, vx_scalar sens,
               vx_bool nonmax, vx_array corners, vx_scalar num_corners) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node =
        vxFastCornersNode(graph, input, sens, nonmax, corners, num_corners);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuNonMaxSuppression(vx_context context,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node =
        vxNonMaxSuppressionNode(graph, input, mask, win_size, output);
    if (node) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuOpticalFlowPyrLK(
    vx_context context, vx_pyramid old_images, vx_pyramid new_images,
  ...
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node = vxOpticalFlowPyrLKNode(
...
VX_API_ENTRY vx_status VX_API_CALL vxuRemap(vx_context context, vx_image input,
...
VX_API_ENTRY vx_status VX_API_CALL vxuMin(vx_context context, vx_image in1,
                                          vx_image in2, vx_image out) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuMax(vx_context context, vx_image in1,
                                          vx_image in2, vx_image out) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuLBP(vx_context context, vx_image in,
...
VX_API_ENTRY vx_status VX_API_CALL vxuBilateralFilter(
    vx_context context, vx_tensor src, vx_int32 diameter, vx_float32 sigmaSpace,
    vx_float32 sigmaValues, vx_tensor dst) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (graph) {
    vx_node node = vxBilateralFilterNode(graph, src, diameter, sigmaSpace,
                                         sigmaValues, dst);
    if (node) {
      status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuMatchTemplate(vx_context context,
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node =
        vxMatchTemplateNode(graph, src, templateImage, matchingMethod, output);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = setNodeTarget(node);
      if (status == VX_SUCCESS) status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_status VX_API_CALL vxuHoughLinesP(
    vx_context context, vx_image input, const vx_hough_lines_p_t *params,
    vx_array lines_array, vx_scalar num_lines) {
  vx_status status = VX_FAILURE;
  vx_graph graph = vxCreateGraph(context);
  if (graph) {
    vx_node node =
        vxHoughLinesPNode(graph, input, params, lines_array, num_lines);
    if (node) {
      status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
        status = vxProcessGraph(graph);
...
vx_status VX_API_CALL vxuHOGCells(vx_context context, vx_image input,
  ...
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node = vxHOGCellsNode(graph, input, cell_width, cell_height,
...
vx_status VX_API_CALL vxuHOGFeatures(vx_context context, vx_image input,
  ...
  vx_graph graph = vxCreateGraph(context);
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    vx_node node = vxHOGFeaturesNode(graph, input, magnitudes, bins, params,
                                     hog_param_size, features);
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      status = vxVerifyGraph(graph);
      if (status == VX_SUCCESS) {
        status = vxProcessGraph(graph);
