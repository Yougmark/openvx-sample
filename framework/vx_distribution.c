...
VX_API_ENTRY vx_distribution VX_API_CALL vxCreateDistribution(
    vx_context context, vx_size numBins, vx_int32 offset, vx_uint32 range) {
  vx_distribution distribution = NULL;

  if (ownIsValidContext(context) == vx_true_e) {
    if ((numBins != 0) && (range != 0)) {
      distribution = (vx_distribution)ownCreateReference(
          context, VX_TYPE_DISTRIBUTION, VX_EXTERNAL, &context->base);
      ...
      distribution = (vx_distribution)ownGetErrorObject(
          context, VX_ERROR_INVALID_PARAMETERS);
    }
  }
...
VX_API_ENTRY vx_distribution VX_API_CALL vxCreateVirtualDistribution(
    vx_graph graph, vx_size numBins, vx_int32 offset, vx_uint32 range) {
  vx_distribution distribution = NULL;
...
vx_status vxReleaseDistributionInt(vx_distribution *distribution) {
  return ownReleaseReferenceInt((vx_reference *)distribution,
                                VX_TYPE_DISTRIBUTION, VX_INTERNAL,
                                &ownDestructDistribution);
}

VX_API_ENTRY vx_status VX_API_CALL vxReleaseDistribution(vx_distribution *d) {
  return ownReleaseReferenceInt((vx_reference *)d, VX_TYPE_DISTRIBUTION,
                                VX_EXTERNAL, &ownDestructDistribution);
}

VX_API_ENTRY vx_status VX_API_CALL vxQueryDistribution(
    vx_distribution distribution, vx_enum attribute, void *ptr, vx_size size) {
...
vxAccessDistribution(vx_distribution distribution, void **ptr, vx_enum usage) {
  vx_status status = VX_FAILURE;
  (void)usage;

  if ((ownIsValidSpecificReference(&distribution->base, VX_TYPE_DISTRIBUTION) ==
       vx_true_e) &&
      (ownAllocateMemory(distribution->base.context, &distribution->memory) ==
       vx_true_e)) {
    if (ptr != NULL) {
...
VX_API_ENTRY vx_status VX_API_CALL
vxCommitDistribution(vx_distribution distribution, const void *ptr) {
  vx_status status = VX_FAILURE;
  if ((ownIsValidSpecificReference(&distribution->base, VX_TYPE_DISTRIBUTION) ==
       vx_true_e) &&
      (ownAllocateMemory(distribution->base.context, &distribution->memory) ==
      ...
      ownSemWait(&distribution->base.lock);
      {
        if (ptr != distribution->memory.ptrs[0]) {
          ...
          VX_PRINT(VX_ZONE_INFO,
                   "Copied distribution from %p to %p for " VX_FMT_SIZE
                   " bytes\n",
...
VX_API_ENTRY vx_status VX_API_CALL
vxCopyDistribution(vx_distribution distribution, void *user_ptr, vx_enum usage,
                   vx_enum mem_type) {
  vx_status status = VX_FAILURE;
  vx_size size = 0;
...
VX_API_ENTRY vx_status VX_API_CALL
vxMapDistribution(vx_distribution distribution, vx_map_id *map_id, void **ptr,
                  vx_enum usage, vx_enum mem_type, vx_bitfield flags) {
  vx_status status = VX_FAILURE;
  vx_size size = 0;
  ...
  if (ownMemoryMap(distribution->base.context, (vx_reference)distribution, size,
                   usage, mem_type, flags, NULL, ptr, map_id) == vx_true_e) {
    switch (usage) {
      case VX_READ_ONLY:
...
VX_API_ENTRY vx_status VX_API_CALL
vxUnmapDistribution(vx_distribution distribution, vx_map_id map_id) {
  vx_status status = VX_FAILURE;
  vx_size size = 0;
  ...
  /* bad parameters */
  if (ownFindMemoryMap(distribution->base.context, (vx_reference)distribution,
                       map_id) != vx_true_e) {
    status = VX_ERROR_INVALID_PARAMETERS;
...

  {
    ...
    /* regardless of the current status, if we're here, so previous call to
     * vxMapDistribution()
     * was successful and thus ref was locked once by a call to
