...
void ownReleaseMetaFormat(vx_meta_format *pmeta) {
  ownReleaseReferenceInt((vx_reference *)pmeta, VX_TYPE_META_FORMAT,
                         VX_EXTERNAL, NULL);
}
...
vx_meta_format ownCreateMetaFormat(vx_context context) {
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    meta = (vx_meta_format)ownCreateReference(context, VX_TYPE_META_FORMAT,
  ...
  switch (attribute) {
    ...
    case VX_VALID_RECT_CALLBACK:
      if (VX_CHECK_PARAM(ptr, size, vx_kernel_image_valid_rectangle_f, 0x0)) {
        meta->set_valid_rectangle_callback =
            *(vx_kernel_image_valid_rectangle_f *)ptr;
      } else {
        status = VX_ERROR_INVALID_PARAMETERS;
      }
      break;
    ...
    case VX_TENSOR_NUMBER_OF_DIMS:
      ...
      break;
    case VX_TENSOR_DIMS:
      if (size <= (sizeof(vx_size) * VX_MAX_TENSOR_DIMENSIONS) &&
          ((vx_size)ptr & 0x3) == 0) {
...
vxSetMetaFormatFromReference(vx_meta_format meta, vx_reference examplar) {
  vx_status status = VX_SUCCESS;
  ...
  switch (examplar->type) {
    case VX_TYPE_TENSOR: {
      ...
      memcpy(meta->dim.tensor.dimensions, tensor->dimensions,
