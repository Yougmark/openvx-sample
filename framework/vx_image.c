...
static vx_uint32 vxComputePatchOffset(vx_uint32 x, vx_uint32 y,
                                      const vx_imagepatch_addressing_t *addr) {
  return ((addr->stride_y * ((addr->scale_y * y) / VX_SCALE_UNITY)) +
          (addr->stride_x * ((addr->scale_x * x) / VX_SCALE_UNITY)));
}

static vx_uint32 vxComputePlaneOffset(vx_image image, vx_uint32 x, vx_uint32 y,
                                      vx_uint32 p) {
  return (
      ((y * image->memory.strides[p][VX_DIM_Y]) / image->scale[p][VX_DIM_Y]) +
      ((x * image->memory.strides[p][VX_DIM_X]) / image->scale[p][VX_DIM_X]));
}

static vx_uint32 vxComputePatchRangeSize(
    vx_uint32 range, const vx_imagepatch_addressing_t *addr) {
  return (range * addr->stride_x * addr->scale_x) / VX_SCALE_UNITY;
}

static vx_uint32 vxComputePlaneRangeSize(vx_image image, vx_uint32 range,
                                         vx_uint32 p) {
  return (range * image->memory.strides[p][VX_DIM_X]) /
...
vx_bool ownAllocateImage(vx_image image) {
   ...
   * 'vx_false_e' when there is no handle and no memory allocation should be
   * done by OpenVX. ownAllocateMemory then must not be called for images
   * created from handle.
   */
...
vx_bool vxIsValidDimensions(vx_uint32 width, vx_uint32 height,
                            vx_df_image color) {
  if (vxIsOdd(width) &&
      (color == VX_DF_IMAGE_UYVY || color == VX_DF_IMAGE_YUYV)) {
    return vx_false_e;
  } else if ((vxIsOdd(width) || vxIsOdd(height)) &&
             (color == VX_DF_IMAGE_IYUV || color == VX_DF_IMAGE_NV12 ||
...
static vx_image_t *vxCreateImageInt(vx_context_t *context, vx_uint32 width,
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    if (ownIsSupportedFourcc(color) == vx_true_e) {
      if (vxIsValidDimensions(width, height, color) == vx_true_e) {
        image = (vx_image)ownCreateReference(context, VX_TYPE_IMAGE,
                                             VX_EXTERNAL, &context->base);
        if (vxGetStatus((vx_reference)image) == VX_SUCCESS &&
            image->base.type == VX_TYPE_IMAGE) {
        ...
        vxAddLogEntry((vx_reference)image, VX_ERROR_INVALID_DIMENSION,
                      "Requested Image Dimensions was invalid!\n");
        image = (vx_image_t *)ownGetErrorObject(context,
                                                VX_ERROR_INVALID_DIMENSION);
      }
    } else {
      VX_PRINT(VX_ZONE_ERROR, "Requested Image Format was invalid!\n");
      vxAddLogEntry((vx_reference)context, VX_ERROR_INVALID_FORMAT,
                    "Requested Image Format was invalid!\n");
...
VX_API_ENTRY vx_image VX_API_CALL vxCreateImage(vx_context context,
                                                ...
                                                vx_df_image format) {
  if ((width == 0) || (height == 0) ||
      (ownIsSupportedFourcc(format) == vx_false_e) ||
...
vxCreateUniformImage(vx_context context, vx_uint32 width, vx_uint32 height,
                     vx_df_image format, const vx_pixel_value_t *value) {
  vx_image image = 0;
  ...
  if (vxGetStatus((vx_reference)image) == VX_SUCCESS) {
    ...
    for (p = 0; p < planes; p++) {
      ...
      void *base = NULL;
      if (vxAccessImagePatch(image, &rect, p, &addr, &base, VX_WRITE_ONLY) ==
          VX_SUCCESS) {
...
VX_API_ENTRY vx_image VX_API_CALL
vxCreateImageFromROI(vx_image image, const vx_rectangle_t *rect) {
  ...
  if (ownIsValidImage(image) == vx_true_e) {
    if (!rect || rect->start_x > rect->end_x || rect->start_y > rect->end_y ||
      ...
      subimage =
          (vx_image)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
    } else {
      /* perhaps the parent hasn't been allocated yet? */
      if (ownAllocateImage(image) == vx_true_e) {
        subimage = (vx_image)ownCreateReference(image->base.context,
                                                VX_TYPE_IMAGE, VX_EXTERNAL,
                                                &image->base.context->base);
        if (vxGetStatus((vx_reference)subimage) == VX_SUCCESS) {
          ...
          if (rect->start_x > image_rect.end_x ||
            ...
            subimage->region.start_x =
                VX_MAX(image_rect.start_x, rect->start_x) - rect->start_x;
            subimage->region.start_y =
                VX_MAX(image_rect.start_y, rect->start_y) - rect->start_y;
            subimage->region.end_x =
                VX_MIN(image_rect.end_x, rect->end_x) - rect->start_x;
            subimage->region.end_y =
                VX_MIN(image_rect.end_y, rect->end_y) - rect->start_y;
          }
          ...
          for (p = 0; p < subimage->planes; p++) {
            vx_uint32 offset =
                vxComputePlaneOffset(image, rect->start_x, rect->start_y, p);
            VX_PRINT(VX_ZONE_IMAGE,
                     "Offsetting SubImage plane[%u] by %u bytes!\n", p, offset);
...
VX_API_ENTRY vx_image VX_API_CALL vxCreateImageFromChannel(vx_image image,
                                                           vx_enum channel) {
  vx_image_t *subimage = NULL;
  ...
  if (ownIsValidImage(image) == vx_true_e) {
    ...
    if (ownAllocateImage(image) == vx_true_e) {
      ...
      switch (channel) {
        case VX_CHANNEL_Y: {
          if (VX_DF_IMAGE_YUV4 != image->format &&
            ...
            vx_context context = vxGetContext((vx_reference)image);
            subimage = (vx_image)ownGetErrorObject(context,
      ...
      subimage =
          (vx_image)ownCreateReference(image->base.context, VX_TYPE_IMAGE,
                                       VX_EXTERNAL, &image->base.context->base);
      if (vxGetStatus((vx_reference)subimage) == VX_SUCCESS) {
        vx_uint32 p = 0;
        ...
        switch (image->format) {
          case VX_DF_IMAGE_YUV4: {
            ...
            subimage->bounds[0][VX_DIM_X][VX_BOUND_START] =
                image->bounds[p][VX_DIM_X][VX_BOUND_START];
            subimage->bounds[0][VX_DIM_X][VX_BOUND_END] =
                image->bounds[p][VX_DIM_X][VX_BOUND_END];
            subimage->bounds[0][VX_DIM_Y][VX_BOUND_START] =
                image->bounds[p][VX_DIM_Y][VX_BOUND_START];
            subimage->bounds[0][VX_DIM_Y][VX_BOUND_END] =
                image->bounds[p][VX_DIM_Y][VX_BOUND_END];

            subimage->memory.dims[0][VX_DIM_C] =
                image->memory.dims[p][VX_DIM_C];
            subimage->memory.dims[0][VX_DIM_X] =
                image->memory.dims[p][VX_DIM_X];
            subimage->memory.dims[0][VX_DIM_Y] =
                image->memory.dims[p][VX_DIM_Y];

            subimage->memory.strides[0][VX_DIM_C] =
                image->memory.strides[p][VX_DIM_C];
            subimage->memory.strides[0][VX_DIM_X] =
                image->memory.strides[p][VX_DIM_X];
            subimage->memory.strides[0][VX_DIM_Y] =
...
VX_API_ENTRY vx_image VX_API_CALL
vxCreateImageFromHandle(vx_context context, vx_df_image color,
  ...
  if (vxGetStatus((vx_reference)image) == VX_SUCCESS &&
    ...
    image->memory.allocated =
        vx_true_e; /* don't let the system realloc this memory */

    /* now assign the plane pointers, assume linearity */
    for (p = 0; p < image->planes; p++) {
      /* ensure row-major memory layout */
      if (addrs[p].stride_x <= 0 ||
          addrs[p].stride_y < (vx_int32)(addrs[p].stride_x * addrs[p].dim_x)) {
        vxReleaseImage(&image);
        return (vx_image)ownGetErrorObject(context,
                                           VX_ERROR_INVALID_PARAMETERS);
      }
...
VX_API_ENTRY vx_status VX_API_CALL vxSwapImageHandle(vx_image image,
  ...
  if (ownIsValidImage(image) == vx_true_e) {
    if (image->memory_type != VX_MEMORY_TYPE_NONE) {
      ...
      if (new_ptrs != NULL) {
        for (p = 0; p < image->planes; p++) {
          if (new_ptrs[p] == NULL) return VX_ERROR_INVALID_PARAMETERS;
        }
      }
      ...
      for (i = 0; i < VX_INT_MAX_REF; i++) {
        if (image->subimages[i] != NULL) {
          if (new_ptrs == NULL)
            status = vxSwapImageHandle(image->subimages[i], NULL, NULL,
                                       image->planes);
          else {
            vx_uint8 *ptrs[4];
            ...
            status = vxSwapImageHandle(image->subimages[i], (void **)ptrs, NULL,
                                       image->planes);
          }
    ...
    switch (attribute) {
      ...
      case VX_IMAGE_SIZE:
        if (VX_CHECK_PARAM(ptr, size, vx_size, 0x3)) {
          ...
          vx_uint32 p;
          for (p = 0; p < image->planes; p++) {
            size += (abs(image->memory.strides[p][VX_DIM_Y]) *
                     image->memory.dims[p][VX_DIM_Y]);
          }
          *(vx_size *)ptr = size;
        } else {
...
void ownDestructImage(vx_reference ref) {
  ...
  if ((image->memory_type == VX_MEMORY_TYPE_NONE) && (image->parent == NULL)) {
    ...
    ownReleaseReferenceInt((vx_reference *)&image->parent, VX_TYPE_IMAGE,
                           VX_INTERNAL, NULL);
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseImage(vx_image *image) {
  if (image != NULL) {
    ...
    if (ownIsValidSpecificReference((vx_reference)this_image, VX_TYPE_IMAGE) ==
        vx_true_e) {
      vx_image parent = this_image->parent;

      /* clear this image from its parent' subimages list */
      if (parent && ownIsValidSpecificReference((vx_reference)parent,
                                                VX_TYPE_IMAGE) == vx_true_e) {
        vx_uint32 n;
  ...
  return ownReleaseReferenceInt((vx_reference *)image, VX_TYPE_IMAGE,
                                VX_EXTERNAL, NULL);
}
  ...
  if ((ownIsValidImage(image) == vx_true_e) && (rect)) {
    ...
    if (image->memory.ptrs[0] == NULL) {
      if (ownAllocateImage(image) == vx_false_e) {
        vxAddLogEntry((vx_reference)image, VX_ERROR_NO_MEMORY,
    ...
    if (plane_index < image->planes) {
      vx_size numPixels =
          ((end_x - start_x) / image->scale[plane_index][VX_DIM_X]) *
          ((end_y - start_y) / image->scale[plane_index][VX_DIM_Y]);
      vx_size pixelSize = image->memory.strides[plane_index][VX_DIM_X];
      ownPrintImage(image);
      VX_PRINT(VX_ZONE_IMAGE,
               "numPixels = " VX_FMT_SIZE " pixelSize = " VX_FMT_SIZE "\n",
               numPixels, pixelSize);
      size = numPixels * pixelSize;
    } else {
      vxAddLogEntry((vx_reference)image, VX_ERROR_INVALID_PARAMETERS,
                    "Plane index %u is out of bounds!", plane_index);
    }

    VX_PRINT(
        VX_ZONE_IMAGE,
        "image %p for patch {%u,%u to %u,%u} has a byte size of " VX_FMT_SIZE
        "\n",
...
VX_API_ENTRY vx_status VX_API_CALL vxAccessImagePatch(
  ...
  vx_bool zero_area =
      ((((end_x - start_x) == 0) || ((end_y - start_y) == 0)) ? vx_true_e
                                                              : vx_false_e);

  /* bad parameters */
  if ((usage < VX_READ_ONLY) || (VX_READ_AND_WRITE < usage) || (addr == NULL) ||
      (ptr == NULL)) {
    status = VX_ERROR_INVALID_PARAMETERS;
    goto exit;
  }
  ...
  /* bad references */
  if ((!rect) || (ownIsValidImage(image) == vx_false_e)) {
    status = VX_ERROR_INVALID_REFERENCE;
    goto exit;
  }
  ...
  if (zero_area == vx_false_e &&
      ((plane_index >= image->memory.nptrs) || (plane_index >= image->planes) ||
  ...
  if ((image->memory.ptrs[0] == NULL) &&
      (ownAllocateImage(image) == vx_false_e)) {
  ...
  if ((image->constant == vx_true_e) &&
    ...
    vxAddLogEntry(&image->base, status,
  ...
  /*************************************************************************/
  VX_PRINT(VX_ZONE_IMAGE,
           "AccessImagePatch from " VX_FMT_REF
           " to ptr %p from {%u,%u} to {%u,%u} plane %u\n",
           image, *ptr, rect->start_x, rect->start_y, rect->end_x, rect->end_y,
           plane_index);
  ...
  if ((*ptr == NULL) && ((usage == VX_READ_ONLY) || (usage == VX_WRITE_ONLY) ||
  ...
  if (mapped == vx_true_e) {
    ...
    *ptr = &p[index];
    VX_PRINT(VX_ZONE_IMAGE,
             "Returning mapped pointer %p which is offset by %lu\n", *ptr,
             index);
  ...
  /* COPY mode */
    ...
    vx_imagepatch_addressing_t *addr_save =
        calloc(1, sizeof(vx_imagepatch_addressing_t));
    ...
    addr->scale_x = addr_save->scale_x =
        VX_SCALE_UNITY / image->scale[plane_index][VX_DIM_X];
    addr->scale_y = addr_save->scale_y =
        VX_SCALE_UNITY / image->scale[plane_index][VX_DIM_Y];

    if (ownAddAccessor(image->base.context, size, usage, *ptr, &image->base, &a,
                       addr_save) == vx_false_e) {
      status = VX_ERROR_NO_MEMORY;
      vxAddLogEntry(
          &image->base, status,
          "Failed to allocate memory for COPY! Size=" VX_FMT_SIZE "\n", size);
      goto exit;
    }

    {
      ...
      /*! \todo implement overlapping multi-writers lock, not just single writer
       * lock */
      ...
      if ((usage == VX_READ_ONLY) || (usage == VX_READ_AND_WRITE)) {
        if (addr_save->stride_x ==
            image->memory.strides[plane_index][VX_DIM_X]) {
          /* Both have compact lines */
          for (y = rect->start_y; y < rect->end_y; y += addr_save->step_y) {
            vx_uint32 i =
                vxComputePlaneOffset(image, rect->start_x, y, plane_index);
            vx_uint32 j = vxComputePatchOffset(0, (y - rect->start_y), addr);
            vx_uint32 len =
                vxComputePlaneRangeSize(image, addr_save->dim_x, plane_index);
            VX_PRINT(VX_ZONE_IMAGE, "%p[%u] <= %p[%u] for %u\n", tmp, j,
                     image->memory.ptrs[plane_index], i, len);
...
VX_API_ENTRY vx_status VX_API_CALL vxCommitImagePatch(
  ...
  vx_bool zero_area =
      ((((end_x - start_x) == 0) || ((end_y - start_y) == 0)) ? vx_true_e
                                                              : vx_false_e);
  vx_uint32 index =
      UINT32_MAX;  // out of bounds, if given to remove, won't do anything

  if (ownIsValidImage(image) == vx_false_e) return VX_ERROR_INVALID_REFERENCE;

  VX_PRINT(VX_ZONE_IMAGE,
           "CommitImagePatch to " VX_FMT_REF
           " from ptr %p plane %u to {%u,%u},{%u,%u}\n",
  ...
  if (zero_area == vx_false_e &&
      ((plane_index >= image->planes) || (plane_index >= image->memory.nptrs) ||
       (ptr == NULL) || (addr == NULL))) {
    return VX_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (zero_area == vx_false_e &&
      ((start_x >= end_x) || ((end_x - start_x) > addr->dim_x) ||
       ...
       (end_x > (vx_uint32)image->memory.dims[plane_index][VX_DIM_X] *
                    (vx_uint32)image->scale[plane_index][VX_DIM_X]) ||
       (end_y > (vx_uint32)image->memory.dims[plane_index][VX_DIM_Y] *
                    (vx_uint32)image->scale[plane_index][VX_DIM_X]))) {
    VX_PRINT(VX_ZONE_ERROR,
...

  {
     ...
     * 4.) EXTERNAL - dependant on area (do nothing on zero, determine on
     * non-zero) 5.) !INTERNAL && !EXTERNAL == MAPPED
     */
    ...
    if ((zero_area == vx_false_e) && (image->constant == vx_true_e)) {
      ...
      if (internal == vx_true_e &&
          image->base.context->accessors[index].usage == VX_READ_ONLY) {
        /* this is a buffer that we allocated on behalf of the user and now they
         * are done. Do nothing else*/
        ownRemoveAccessor(image->base.context, index);
      } else {
        /* determine if this grows the valid region */
        if (image->region.start_x > start_x) image->region.start_x = start_x;
        if (image->region.start_y > start_y) image->region.start_y = start_y;
        if (image->region.end_x < end_x) image->region.end_x = end_x;
        if (image->region.end_y < end_y) image->region.end_y = end_y;

        /* index of 1 pixel line past last. */
        i = (image->memory.dims[plane_index][VX_DIM_Y] *
             image->memory.strides[plane_index][VX_DIM_Y]);

        VX_PRINT(VX_ZONE_IMAGE, "base:%p tmp:%p end:%p\n",
                 image->memory.ptrs[plane_index], ptr,
                 &image->memory.ptrs[plane_index][i]);
        ...
        if ((image->memory.ptrs[plane_index] <= (vx_uint8 *)ptr) &&
          ...
          if (image->memory.strides[plane_index][VX_DIM_Y] !=
              ((int)image->memory.dims[plane_index][VX_DIM_X] *
               image->memory.strides[plane_index][VX_DIM_X])) {
            /* determine if the pointer is within the image boundary. */
            vx_uint8 *base = image->memory.ptrs[plane_index];
            vx_size offset = ((vx_size)(tmp - base)) %
                             image->memory.strides[plane_index][VX_DIM_Y];
            if (offset <
                (vx_size)(image->memory.dims[plane_index][VX_DIM_X] *
                          image->memory.strides[plane_index][VX_DIM_X])) {
          ...
          if (internal == vx_true_e) {
            ...
            if (addr->stride_x ==
              ...
              for (y = start_y; y < end_y; y += addr->step_y) {
                vx_uint32 i =
                    vxComputePlaneOffset(image, start_x, y, plane_index);
                vx_uint32 j = vxComputePatchOffset(0, (y - start_y), addr);
                vx_uint32 len =
                    vxComputePatchRangeSize((end_x - start_x), addr);
                VX_PRINT(VX_ZONE_IMAGE, "%p[%u] <= %p[%u] for %u\n",
...
VX_API_ENTRY vx_status VX_API_CALL
vxCopyImagePatch(vx_image image, const vx_rectangle_t *rect,
                 vx_uint32 plane_index, const vx_imagepatch_addressing_t *addr,
  ...
  /* bad parameters */
  if (((VX_READ_ONLY != usage) && (VX_WRITE_ONLY != usage)) || (rect == NULL) ||
      (addr == NULL) || (ptr == NULL)) {
    status = VX_ERROR_INVALID_PARAMETERS;
    goto exit;
  }
  ...
  /*************************************************************************/
  VX_PRINT(VX_ZONE_IMAGE,
           "CopyImagePatch from " VX_FMT_REF
           " to ptr %p from {%u,%u} to {%u,%u} plane %u\n",
  ...
  if (usage == VX_READ_ONLY) {
    ...
    if (addr_save.stride_x == image->memory.strides[plane_index][VX_DIM_X]) {
      ...
      for (y = start_y; y < end_y; y += addr_save.step_y) {
        ...
        vx_uint32 dstOffset =
            vxComputePatchOffset(0, (y - start_y), &addr_save);
        vx_uint8 *pDstLine = &pDst[dstOffset];

        vx_uint32 len = vxComputePlaneRangeSize(
            image, end_x - start_x /*image->width*/, plane_index);
...
VX_API_ENTRY vx_status VX_API_CALL vxMapImagePatch(
  ...
  /* MAP mode */
  {
    ...
    if (VX_MEMORY_TYPE_HOST == image->memory_type &&
        vx_true_e == ownMemoryMap(image->base.context, (vx_reference)image, 0,
                                  usage, mem_type, flags, &extra, (void **)&buf,
                                  map_id)) {
      /* use the addressing of the internal format */
      ...
      vx_uint32 index =
          vxComputePatchOffset(rect->start_x, rect->start_y, addr);
      *ptr = &buf[index];
      ...
      status = VX_SUCCESS;
        ...
        if (vx_true_e == ownMemoryMap(image->base.context, (vx_reference)image,
                                      size, usage, mem_type, flags, &extra,
                                      (void **)&buf, map_id)) {
      /* use the addressing of the internal format */
      addr->dim_x = end_x - start_x;
      addr->dim_y = end_y - start_y;
      addr->stride_x = image->memory.strides[plane_index][VX_DIM_X];
      addr->stride_y =
          addr->stride_x * addr->dim_x / image->scale[plane_index][VX_DIM_Y];
      addr->step_x = image->scale[plane_index][VX_DIM_X];
      addr->step_y = image->scale[plane_index][VX_DIM_Y];
      addr->scale_x = VX_SCALE_UNITY / image->scale[plane_index][VX_DIM_X];
      addr->scale_y = VX_SCALE_UNITY / image->scale[plane_index][VX_DIM_Y];

      /* initialize mapping buffer with image patch data for
       * read/read-modify-write access */
      if (VX_READ_ONLY == usage || VX_READ_AND_WRITE == usage) {
        /* lock image plane even for read access to avoid mix of simultaneous
         * read/write */
        if (vx_true_e == ownSemWait(&image->memory.locks[plane_index])) {
          ...
          for (y = start_y; y < end_y; y += addr->step_y) {
            ...
            vx_uint32 len =
                vxComputePlaneRangeSize(image, addr->dim_x, plane_index);

            VX_PRINT(VX_ZONE_IMAGE, "%p[%u] <= %p[%u] for %u\n", pDst,
                     dstOffset, pSrc, srcOffset, len);
...
VX_API_ENTRY vx_status VX_API_CALL vxUnmapImagePatch(vx_image image,
  ...
  /* bad parameters */
  if (ownFindMemoryMap(image->base.context, (vx_reference)image, map_id) !=
      vx_true_e) {
    status = VX_ERROR_INVALID_PARAMETERS;
...

  {
    ...
    if (map->used && map->ref == (vx_reference)image) {
      ...
      if ((VX_WRITE_ONLY == map->usage || VX_READ_AND_WRITE == map->usage) &&
        ...
        if (vx_true_e == ownSemWait(&image->memory.locks[plane_index])) {
          ...
          for (y = rect.start_y; y < rect.end_y; y += addr.step_y) {
            vx_uint32 srcOffset =
                vxComputePatchOffset(0, (y - rect.start_y), &addr);
            vx_uint8 *pSrcLine = &pSrc[srcOffset];
            ...
            vx_uint32 len =
                vxComputePatchRangeSize((rect.end_x - rect.start_x), &addr);

            VX_PRINT(VX_ZONE_IMAGE, "%p[%u] <= %p[%u] for %u\n", pDst,
                     srcOffset, pSrc, dstOffset, len);
...
VX_API_ENTRY void *VX_API_CALL vxFormatImagePatchAddress1d(
    void *ptr, vx_uint32 index, const vx_imagepatch_addressing_t *addr) {
  ...
  if (ownIsValidImage(image) == vx_true_e) {
    ...
    if (rect) {
      if ((image->region.start_x <= image->region.end_x) &&
          (image->region.start_y <= image->region.end_y)) {
...
VX_API_ENTRY vx_status VX_API_CALL
vxSetImageValidRectangle(vx_image image, const vx_rectangle_t *rect) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
...
void ownPrintImage(vx_image image) {
  ...
  VX_PRINT(VX_ZONE_IMAGE, "vx_image_t:%p %s %ux%u (%s)\n", image, df_image,
           ...
           (image->constant ? "CONSTANT" : "MUTABLE"));
  for (p = 0; p < image->planes; p++) {
    VX_PRINT(VX_ZONE_IMAGE,
             "\tplane[%u] ptr:%p dim={%u,%u,%u} stride={%d,%d,%d} "
             "scale={%u,%u,%u} bounds={%u,%ux%u,%u}\n",
             p, image->memory.ptrs[p], image->memory.dims[p][VX_DIM_C],
             image->memory.dims[p][VX_DIM_X], image->memory.dims[p][VX_DIM_Y],
             image->memory.strides[p][VX_DIM_C],
             image->memory.strides[p][VX_DIM_X],
             image->memory.strides[p][VX_DIM_Y], image->scale[p][VX_DIM_C],
             image->scale[p][VX_DIM_X], image->scale[p][VX_DIM_Y],
             image->bounds[p][VX_DIM_X][VX_BOUND_START],
             image->bounds[p][VX_DIM_X][VX_BOUND_END],
             image->bounds[p][VX_DIM_Y][VX_BOUND_START],
             image->bounds[p][VX_DIM_Y][VX_BOUND_END]);
