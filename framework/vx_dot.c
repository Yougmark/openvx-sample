...
VX_API_ENTRY vx_status VX_API_CALL vxExportGraphToDot(vx_graph graph,
                                                      vx_char dotfile[],
                                                      vx_bool showData) {
  ...
  if (ownIsValidSpecificReference(&graph->base, VX_TYPE_GRAPH) == vx_true_e) {
    ...
    if (fp) {
        ...
        if (showData) {
          for (p = 0u; p < node->kernel->signature.num_parameters; p++) {
            ...
            for (d = 0u; d < num_data; d++) {
              if (data[d] == node->parameters[p]) break;
            }
            if (d == num_data) {
        ...
        for (d = 0u; d < num_data; d++) {
          ...
          if (data[d]->type == VX_TYPE_IMAGE) {
            ...
            fprintf(fp, "\tD%u [shape=box label=\"%ux%u\\n%4s\"];\n", d,
                    image->width, image->height, fcc);
          } else if (data[d]->type == VX_TYPE_ARRAY) {
            vx_array_t *arr = (vx_array_t *)data[d];
            if (arr->item_type == VX_TYPE_CHAR ||
                arr->item_size == sizeof(char))
            ...
            fprintf(fp,
                    "\tD%u [shape=triangle label=\"%lfx" VX_FMT_REF
                    "\\nPyramid\"];\n",
      ...
      memcpy(next_nodes, graph->heads,
             graph->numHeads * sizeof(graph->heads[0]));
      ...
      do {
        for (n = 0; n < num_next; n++) {
          ...
          for (p = 0; p < node->kernel->signature.num_parameters; p++) {
            ...
            if (showData && node->kernel->signature.directions[p] == VX_INPUT) {
              ownFindNodesWithReference(graph, node->parameters[p], NULL,
                                        &count, VX_OUTPUT);
              if (count > 0) continue;
              for (d = 0u; d < num_data; d++)
                if (data[d] == node->parameters[p]) break;
              ...
              status = ownFindNodesWithReference(graph, node->parameters[p],
                                                 dep_nodes, &count, VX_INPUT);
        ...
        num_last = num_next;
        num_next = 0;
        ownFindNextNodes(graph, last_nodes, num_last, next_nodes, &num_next,
                         left_nodes, &num_left);
      } while (num_next > 0);
      ownClearVisitation(graph);
      ownClearExecution(graph);
