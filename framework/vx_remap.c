...
VX_API_ENTRY vx_remap VX_API_CALL vxCreateRemap(vx_context context,
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    if (src_width != 0 && src_height != 0 && dst_width != 0 &&
        dst_height != 0) {
      remap = (vx_remap)ownCreateReference(context, VX_TYPE_REMAP, VX_EXTERNAL,
                                           &context->base);
      if (vxGetStatus((vx_reference)remap) == VX_SUCCESS &&
          remap->base.type == VX_TYPE_REMAP) {
      ...
      remap =
          (vx_remap_t *)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
    }
  }
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseRemap(vx_remap *r) {
  return ownReleaseReferenceInt((vx_reference *)r, VX_TYPE_REMAP, VX_EXTERNAL,
                                NULL);
}

VX_API_ENTRY vx_status VX_API_CALL vxQueryRemap(vx_remap remap,
...
VX_API_ENTRY vx_status VX_API_CALL vxSetRemapPoint(vx_remap remap,
                                                   vx_uint32 dst_x,
                                                   vx_uint32 dst_y,
                                                   vx_float32 src_x,
                                                   vx_float32 src_y) {
  ...
  if ((ownIsValidSpecificReference(&remap->base, VX_TYPE_REMAP) == vx_true_e) &&
    ...
    if ((dst_x < remap->dst_width) && (dst_y < remap->dst_height)) {
      vx_float32 *coords[] = {
...
VX_API_ENTRY vx_status VX_API_CALL vxGetRemapPoint(vx_remap remap,
                                                   vx_uint32 dst_x,
                                                   vx_uint32 dst_y,
                                                   vx_float32 *src_x,
                                                   vx_float32 *src_y) {
...
VX_API_ENTRY vx_remap VX_API_CALL vxCreateVirtualRemap(vx_graph graph,
  ...
  if (ownIsValidSpecificReference(gref, VX_TYPE_GRAPH) == vx_true_e) {
    remap = vxCreateRemap(gref->context, src_width, src_height, dst_width,
                          dst_height);
    if (vxGetStatus((vx_reference)remap) == VX_SUCCESS &&
        remap->base.type == VX_TYPE_REMAP) {
...
static vx_status vxSetCoordValue(vx_remap remap, vx_uint32 dst_x,
                                 vx_uint32 dst_y, vx_float32 src_x,
                                 vx_float32 src_y) {
  vx_status status = VX_FAILURE;
  if ((ownIsValidSpecificReference(&remap->base, VX_TYPE_REMAP) == vx_true_e) &&
      (ownAllocateMemory(remap->base.context, &remap->memory) == vx_true_e)) {
...
static vx_status vxGetCoordValue(vx_remap remap, vx_uint32 dst_x,
                                 vx_uint32 dst_y, vx_float32 *src_x,
                                 vx_float32 *src_y) {
  vx_status status = VX_FAILURE;
  if (ownIsValidSpecificReference(&remap->base, VX_TYPE_REMAP) == vx_true_e) {
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyRemapPatch(
    vx_remap remap, const vx_rectangle_t *rect, vx_size user_stride_y,
    void *user_ptr, vx_enum user_coordinate_type, vx_enum usage,
  ...
  /* more bad parameters */
  if ((user_stride_y <
       sizeof(vx_coordinates2df_t) * (rect->end_x - rect->start_x)) ||
  ...
  if (user_mem_type != VX_MEMORY_TYPE_HOST &&
      user_mem_type != VX_MEMORY_TYPE_NONE) {
    status = VX_ERROR_INVALID_PARAMETERS;
    goto exit;
  }
  ...
  /* more bad parameters */
  if (zero_area == vx_false_e && ((0 >= remap->memory.nptrs) ||
...
vxMapRemapPatch(vx_remap remap, const vx_rectangle_t *rect, vx_map_id *map_id,
  ...
  if (ownMemoryMap(remap->base.context, (vx_reference)remap, size, usage,
                   mem_type, flags, &extra, (void **)&buf,
                   map_id) == vx_true_e) {
...
VX_API_ENTRY vx_status VX_API_CALL vxUnmapRemapPatch(vx_remap remap,
  ...
  /* bad parameters */
  if (ownFindMemoryMap(remap->base.context, (vx_reference)remap, map_id) !=
      vx_true_e) {
    status = VX_ERROR_INVALID_PARAMETERS;
    goto exit;
  }
