  ...
  if (ownIsValidContext(context) == vx_true_e) {
    if (data_type == VX_TYPE_UINT8) {
      ...
      if (count != 256) {
        ...
        lut =
            (vx_lut_t *)ownGetErrorObject(context, VX_ERROR_INVALID_PARAMETERS);
      } else
#endif
      {
        lut = (vx_lut_t *)ownCreateArrayInt(context, VX_TYPE_UINT8, count,
                                            vx_false_e, VX_TYPE_LUT);
      ...
      if (!(count <= 65536)) {
        ...
        lut = (vx_lut_t *)ownCreateArrayInt(context, VX_TYPE_INT16, count,
                                            vx_false_e, VX_TYPE_LUT);
        if (vxGetStatus((vx_reference)lut) == VX_SUCCESS &&
            lut->base.type == VX_TYPE_LUT) {
          lut->num_items = count;
      ...
      lut = (vx_lut_t *)ownCreateArrayInt(context, VX_TYPE_UINT16, count,
                                          vx_false_e, VX_TYPE_LUT);
      if (vxGetStatus((vx_reference)lut) == VX_SUCCESS &&
          lut->base.type == VX_TYPE_LUT) {
        lut->num_items = count;
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseLUT(vx_lut *l) {
  return ownReleaseReferenceInt((vx_reference *)l, VX_TYPE_LUT, VX_EXTERNAL,
                                NULL);
}

VX_API_ENTRY vx_status VX_API_CALL vxQueryLUT(vx_lut l, vx_enum attribute,
...
VX_API_ENTRY vx_status VX_API_CALL vxAccessLUT(vx_lut l, void **ptr,
                                               vx_enum usage) {
  vx_status status = VX_FAILURE;
  vx_lut_t *lut = (vx_lut_t *)l;
  if (ownIsValidSpecificReference(&lut->base, VX_TYPE_LUT) == vx_true_e) {
    status = ownAccessArrayRangeInt((vx_array_t *)l, 0, lut->num_items, NULL,
                                    ptr, usage);
  } else {
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyLUT(vx_lut l, void *user_ptr,
                                             vx_enum usage,
  ...
  if (ownIsValidSpecificReference(&lut->base, VX_TYPE_LUT) == vx_true_e) {
    ...
    status = ownCopyArrayRangeInt((vx_array_t *)l, 0, lut->num_items, stride,
                                  user_ptr, usage, user_mem_type);
  } else {
    VX_PRINT(VX_ZONE_ERROR, "Not a valid object!\n");
  }
  return status;
...
VX_API_ENTRY vx_status VX_API_CALL vxMapLUT(vx_lut l, vx_map_id *map_id,
  ...
  if (ownIsValidSpecificReference(&lut->base, VX_TYPE_LUT) == vx_true_e) {
    vx_size stride = lut->item_size;
    status = ownMapArrayRangeInt((vx_array_t *)lut, 0, lut->num_items, map_id,
                                 &stride, ptr, usage, mem_type, flags);
  } else {
    VX_PRINT(VX_ZONE_ERROR, "Not a valid object!\n");
  }
  return status;
