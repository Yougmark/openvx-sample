...
#define XML_FOREACH_CHILD_TAG(child, tag, tags)                                \
  for (child = (xmlFirstElementChild(child)                                    \
                    ? (tag = xml_match_tag(xmlFirstElementChild(child), tags,  \
                                           dimof(tags)),                       \
                      xmlFirstElementChild(child))                             \
                    : (tag = 0, child));                                       \
       tag && child;                                                           \
       child = (xmlNextElementSibling(child)                                   \
                    ? (tag = xml_match_tag(xmlNextElementSibling(child), tags, \
                                           dimof(tags)),                       \
...
#define REFNUM_ERROR                                                 \
  VX_PRINT(VX_ZONE_ERROR,                                            \
           "Reference number out of bounds: \"references\" tag=%d, " \
           "\"reference\"=%d\n",                                     \
           total, refIdx);                                           \
...
static ulong xml_ulong(xmlNodePtr cur) {
  xmlChar *string = xmlNodeListGetString(cur->doc, cur->children, 1);
  ulong value = 0ul;
  if (string) value = strtoul((const char *)string, NULL, 10);
  free(string);
  return value;
}
...
static ulong xml_long(xmlNodePtr cur) {
  xmlChar *string = xmlNodeListGetString(cur->doc, cur->children, 1);
  long value = 0l;
  if (string) value = strtol((const char *)string, NULL, 10);
  free(string);
  return value;
}
...
static float xml_float(xmlNodePtr cur) {
  xmlChar *string = xmlNodeListGetString(cur->doc, cur->children, 1);
  float value = 0.0f;
  if (string) sscanf((const char *)string, "%f", &value);
  free(string);
  return value;
}
...
static double xml_double(xmlNodePtr cur) {
  xmlChar *string = xmlNodeListGetString(cur->doc, cur->children, 1);
  double value = 0.0f;
  if (string) sscanf((const char *)string, "%lf", &value);
  free(string);
  return value;
}
...
*/
static void xml_prop_string(xmlNodePtr cur, char prop[], char out[],
                            size_t nchar) {
  xmlChar *string = xmlGetProp(cur, (const xmlChar *)prop);
  if (string) {
    strncpy(out, (char *)string, nchar);
...
static int32_t xml_match_tag(xmlNodePtr cur, xml_tag_t tags[],
                             size_t num_tags) {
  int32_t tag = 0;
  if (cur) {
    uint32_t i = 0u;
...
static vx_status vxReserveReferences(vx_context context, vx_uint32 num) {
  ...
  for (r = 0u; (r < VX_INT_MAX_REF) && (count < num); r++) {
    if (context->reftable[r] == NULL) {
      /* 1 is used as a flag that this is reserved since it is not a valid
       * handle */
      context->reftable[r] = (vx_reference)1;
      count++;
    }
  }
  if (count == num) status = VX_SUCCESS;
...
/* Adds the numeric label for a user struct enumeration to a local table so that
 * it can be correlated to usage in
 * arrays that reference the label from the XML */
static vx_status vxStructSetEnum(xml_struct_t ptable[VX_INT_MAX_USER_STRUCTS],
                                 vx_char *slabel, vx_enum e_struct_type) {
  vx_status status = VX_FAILURE;
  vx_uint32 r;
  vx_uint32 label;

  if (sscanf(slabel, "USER_STRUCT_%u", &label) != 1) return status;
...
/* Gets the enumeration associated with a numeric label for a user struct from a
 * local table */
static vx_status vxStructGetEnum(xml_struct_t ptable[VX_INT_MAX_USER_STRUCTS],
                                 vx_uint32 label, vx_enum *e_struct_type) {
  vx_status status = VX_FAILURE;
  vx_uint32 r;
...
static vx_status vxImportFromXMLRoi(vx_image parent, xmlNodePtr cur,
  ...
  if (refIdx >= total) return VX_ERROR_INVALID_PARAMETERS;
    ...
    if (tag == ROI_TAG) {
      ...
      XML_FOREACH_CHILD_TAG(cur, tag, tags) {
        if (tag == START_X_TAG) {
          ...
          status |= vxAccessImagePatch(image, &rect, pIdx, &addr, &base,
                                       VX_WRITE_ONLY);
          if (status == VX_SUCCESS) {
            XML_FOREACH_CHILD_TAG(cur, tag, tags) {
              ...
              if (tag == UINT8_TAG) {
                ...
                xml_string(cur, values, sizeof(values));
                if (values[0] == '#') {
                  sscanf(values, "#%02x%02x%02x%02x", &tmp[0], &tmp[1], &tmp[2],
                         &tmp[3]);
                  ptr[0] = (vx_uint8)tmp[0];
                  ptr[1] = (vx_uint8)tmp[1];
                  ptr[2] = (vx_uint8)tmp[2];
...
static vx_status vxLoadDataForUniformImage(xmlNodePtr cur,
...
static vx_status vxLoadDataForPyramid(vx_pyramid pyr, xmlNodePtr cur,
                                      vx_reference refs[], vx_size total,
                                      vx_size levels) {
  vx_status status = VX_SUCCESS;
  vx_xml_tag_e tag = UNKNOWN_TAG;
  XML_FOREACH_CHILD_TAG(cur, tag, tags) {
    if (tag == IMAGE_TAG) {
      ...
      vx_int32 level = -1; /* use -1 to indicate not specified */
      {
        ...
        if (string) level = strtoul((const char *)string, NULL, 10);
        free(string);
      }
      if (refIdx < total) {
        if ((level >= 0) && (level < levels)) {
          if (pyr->levels[level]->width == width &&
            ...
            VX_PRINT(VX_ZONE_ERROR,
                     "Pyramid image settings doesn't match generated pyramid "
                     "image!\n");
...
static vx_status vxLoadDataForMatrix(vx_matrix matrix, xmlNodePtr cur,
                                     vx_size cols, vx_size rows, vx_enum type) {
  vx_status status = VX_SUCCESS;
  vx_xml_tag_e tag = UNKNOWN_TAG;
  ...
  if (XML_HAS_CHILD(cur)) {
    ...
    if (ptr) {
      if ((status = vxCopyMatrix(matrix, ptr, VX_READ_ONLY,
...
static vx_status vxLoadDataForLut(vx_lut lut, xmlNodePtr cur, vx_enum type,
                                  vx_size count) {
  vx_status status = VX_SUCCESS;
  vx_xml_tag_e tag = UNKNOWN_TAG;
...
static vx_status vxLoadDataForConvolution(vx_convolution conv, xmlNodePtr cur,
                                          vx_size cols, vx_size rows) {
  vx_status status = VX_SUCCESS;
  vx_xml_tag_e tag = UNKNOWN_TAG;
  ...
  if (XML_HAS_CHILD(cur)) {
    ...
    if (ptr) {
      if ((status = vxCopyConvolutionCoefficients(
               conv, ptr, VX_READ_ONLY, VX_MEMORY_TYPE_HOST)) == VX_SUCCESS) {
        XML_FOREACH_CHILD_TAG(cur, tag, tags) {
          vx_uint32 row = xml_prop_ulong(cur, "row");
          vx_uint32 col = xml_prop_ulong(cur, "column");
          vx_uint32 off = (row * cols) + col;
        ...
        status = vxCopyConvolutionCoefficients(conv, ptr, VX_WRITE_ONLY,
                                               VX_MEMORY_TYPE_HOST);
      }
      free(ptr);
    } else {
...
static vx_status vxLoadDataForDistribution(vx_distribution dist, xmlNodePtr cur,
                                           vx_size bins) {
  vx_status status = VX_SUCCESS;
  vx_xml_tag_e tag = UNKNOWN_TAG;
  ...
  if (XML_HAS_CHILD(cur)) {
    vx_map_id map_id = 0;
    void *ptr = NULL;
    if ((status = vxMapDistribution(dist, &map_id, &ptr, VX_WRITE_ONLY,
...
static vx_status vxLoadDataForScalar(vx_scalar scalar, xmlNodePtr cur) {
  ...
  XML_FOREACH_CHILD_TAG(cur, tag, tags) {
    ...
    if (tag == CHAR_TAG) {
      ...
      vx_bool v = vx_false_e;
      xml_string(cur, value, sizeof(value));
      if (strncmp(value, "true", sizeof(value)) == 0) v = vx_true_e;
...
static vx_status vxImportFromXMLImage(vx_reference ref, xmlNodePtr cur,
                                      vx_reference refs[], vx_size total,
                                      vx_bool is_virtual) {
  vx_status status = VX_SUCCESS;
  vx_uint32 width = xml_prop_ulong(cur, "width");
  ...
  if (is_virtual == vx_true_e) {
    ...
    if (status == VX_SUCCESS) {
      image = vxCreateUniformImage((vx_context)ref, width, height, format,
                                   &image_data);
      status = vxGetStatus((vx_reference)image);
    } else {
...
static vx_status vxImportFromXMLArray(vx_reference ref, xmlNodePtr cur,
                                      vx_reference refs[], vx_size total,
                                      vx_bool is_virtual) {
  vx_status status = VX_SUCCESS;
  vx_char typename[32];
  ...
  if (ownTypeFromString(typename, &type) !=
      VX_SUCCESS) { /* Type was not found, check if it is a user type */
...
static vx_status vxImportFromXMLPyramid(vx_reference ref, xmlNodePtr cur,
                                        vx_reference refs[], vx_size total,
                                        vx_bool is_virtual) {
  vx_status status = VX_SUCCESS;
  vx_uint32 refIdx = xml_prop_ulong(cur, "reference");
  ...
  if (refIdx >= total) return VX_ERROR_INVALID_PARAMETERS;
  if (is_virtual == vx_true_e) {
    pyr = vxCreateVirtualPyramid((vx_graph)ref, levels, scale, width, height,
                                 format);
    status = vxGetStatus((vx_reference)pyr);
  } else {
    /* Internally creates "levels" number of image objects */
    pyr =
        vxCreatePyramid((vx_context)ref, levels, scale, width, height, format);
    status = vxGetStatus((vx_reference)pyr);
    if (status == VX_SUCCESS) {
...
VX_API_ENTRY vx_import VX_API_CALL vxImportFromXML(vx_context context,
  ...
  if (doc == NULL) {
    ...
    vxAddLogEntry(&context->base, VX_ERROR_INVALID_PARAMETERS,
                  "Could not parse %s\n", xmlfile);
  ...
  if (root == NULL || xmlStrcmp(cur->name, (const xmlChar *)"openvx") != 0) {
    ...
    vxAddLogEntry(&context->base, VX_ERROR_INVALID_FORMAT,
                  "%s is not wellformed\n", xmlfile);
  ...
  if (total > VX_INT_MAX_REF) {
    VX_PRINT(VX_ZONE_ERROR,
             "Total references = %d too high for this implementation\n", total);
    vxAddLogEntry(&context->base, VX_ERROR_INVALID_FORMAT,
                  "Total references = %d too high for this implementation\n",
                  total);
    import = (vx_import)ownGetErrorObject(context, VX_ERROR_INVALID_FORMAT);
  ...
  XML_FOREACH_CHILD_TAG(cur, tag, tags) {
    if (tag == LIBRARY_TAG) {
      ...
      status = vxImportFromXMLImage((vx_reference)context, cur, refs, total,
                                    vx_false_e);
    } else if (tag == ARRAY_TAG) {
      status = vxImportFromXMLArray((vx_reference)context, cur, refs, total,
                                    vx_false_e);
    } else if (tag == PYRAMID_TAG) {
      status = vxImportFromXMLPyramid((vx_reference)context, cur, refs, total,
                                      vx_false_e);
      ...
      XML_FOREACH_CHILD_TAG(cur, tag, tags) {
        if (tag == NODE_TAG) {
          ...
          vxSetNodeAttribute((vx_node)refs[refIdx], VX_NODE_BORDER, &border,
                             sizeof(border));
        } else if (tag == IMAGE_TAG) {
          status |= vxImportFromXMLImage((vx_reference)graph, cur, refs, total,
                                         vx_true_e);
        } else if (tag == ARRAY_TAG) {
          status |= vxImportFromXMLArray((vx_reference)graph, cur, refs, total,
                                         vx_true_e);
        } else if (tag == PYRAMID_TAG) {
          status |= vxImportFromXMLPyramid((vx_reference)graph, cur, refs,
                                           total, vx_true_e);
        }
      }
      ...
      if ((status |= vxGetStatus(refs[refIdx])) == VX_SUCCESS) {
        ...
        vxQueryScalar((vx_scalar)refs[refIdx], VX_SCALAR_TYPE, &scalartype,
                      sizeof(scalartype));
        if (type != scalartype) {
      ...
      if ((status |= vxGetStatus(refs[refIdx])) == VX_SUCCESS) {
        status |=
            vxLoadDataForMatrix((vx_matrix)refs[refIdx], cur, cols, rows, type);
      }
      ...
      if ((status |= vxGetStatus(refs[refIdx])) == VX_SUCCESS) {
        if (!scale) scale = 1;
        status |= vxSetConvolutionAttribute((vx_convolution)refs[refIdx],
                                            VX_CONVOLUTION_SCALE, &scale,
                                            sizeof(scale));
        status |= vxLoadDataForConvolution((vx_convolution)refs[refIdx], cur,
                                           cols, rows);
      }
    } else if (tag == LUT_TAG) {
      vx_uint32 refIdx = xml_prop_ulong(cur, "reference");
      ...
      refs[refIdx] = (vx_reference)vxCreateRemap(context, src_width, src_height,
      ...
      refs[refIdx] =
          (vx_reference)vxCreateDistribution(context, bins, offset, range);
      vxSetName(refs[refIdx], cur);
      vxInternalizeReference(refs[refIdx]);
      if ((status |= vxGetStatus(refs[refIdx])) == VX_SUCCESS) {
        status |=
            vxLoadDataForDistribution((vx_distribution)refs[refIdx], cur, bins);
      }
      ...
      if (refIdx < total) {
        ...
        XML_FOREACH_CHILD_TAG(cur, tag, tags) {
          if (tag == BINARY_TAG) {
            ...
            refs[refIdx] = (vx_reference)vxCreateThreshold(
                context, VX_THRESHOLD_TYPE_BINARY, type);
            vxSetName(refs[refIdx], cur);
            vxInternalizeReference(refs[refIdx]);
            ...
            refs[refIdx] = (vx_reference)vxCreateThreshold(
                context, VX_THRESHOLD_TYPE_RANGE, type);
            vxSetName(refs[refIdx], cur);
            vxInternalizeReference(refs[refIdx]);
            status |= vxSetThresholdAttribute((vx_threshold)refs[refIdx],
    ...
    } else if (tag == DELAY_TAG || tag == OBJECT_ARRAY_TAG) {
      typedef vx_reference (*createFunction)(
        ...
        XML_FOREACH_CHILD_TAG(cur, tag, tags) {
          switch (tag) {
            case IMAGE_TAG: {
              ...
              vx_uint32 height = xml_prop_ulong(cur, "height");
              if (childNum ==
                  0) { /* Create parent object based on first child */
              ...
              childNum++;
              status |=
                  vxLoadDataForImage((vx_image)refs[refIdx], cur, refs, total);
              break;
            }
            case ARRAY_TAG: {
              vx_size capacity = xml_prop_ulong(cur, "capacity");
              ...
              if (ownTypeFromString(typename, &type) !=
                  VX_SUCCESS) { /* Type was not found, check if it is a user
                                   type */
                if (sscanf(typename, "USER_STRUCT_%d", &userNum) == 1) {
                  if (vxStructGetEnum(user_struct_table, userNum, &type) !=
                      VX_SUCCESS) {
              ...
              if (childNum ==
                  0) { /* Create delay object based on first child */
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_array)internalRefs[childNum])->capacity ==
                          capacity &&
            ...
            case PYRAMID_TAG: {
              ...
              if (childNum ==
                ...
                exemplar = vxCreatePyramid(context, levels, scale, width,
                                           height, format);
                status |=
                    vxReleaseReferences(context, count * (levels + 1) + 1);
                status |= vxGetStatus((vx_reference)exemplar);
                if (status == VX_SUCCESS) {
                  parentReference =
                      createFn(context, (vx_reference)exemplar, count);
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_pyramid)internalRefs[childNum])->width == width &&
                      ...
                      ((vx_pyramid)internalRefs[childNum])->numLevels ==
                          levels &&
                    ...
                    status = VX_ERROR_INVALID_PARAMETERS;
                    VX_PRINT(VX_ZONE_ERROR,
                             "%s pyramid settings doesn't match generated %s "
                             "pyramid!\n",
                             objectName, objectName);
                    goto exit_error;
                  }
              ...
              childNum++;
              status |= vxLoadDataForPyramid((vx_pyramid)refs[refIdx], cur,
                                             refs, total, levels);
              break;
            }
            case MATRIX_TAG: {
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_matrix)internalRefs[childNum])->rows == rows &&
                    ...
                    status = VX_ERROR_INVALID_PARAMETERS;
                    VX_PRINT(VX_ZONE_ERROR,
                             "%s matrix settings doesn't match generated %s "
                             "matrix!\n",
                             objectName, objectName);
                    goto exit_error;
                  }
              ...
              childNum++;
              status |= vxLoadDataForMatrix((vx_matrix)refs[refIdx], cur, cols,
                                            rows, type);
              break;
            }
            case LUT_TAG: {
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_lut_t *)internalRefs[childNum])->num_items ==
                          lut_count &&
              ...
              childNum++;
              status |=
                  vxLoadDataForLut((vx_lut)refs[refIdx], cur, type, lut_count);
              break;
            }
            case CONVOLUTION_TAG: {
              ...
              if (childNum ==
                ...
                if (status == VX_SUCCESS) {
                  ...
                  status |= vxSetConvolutionAttribute(
                      exemplar, VX_CONVOLUTION_SCALE, &scale, sizeof(scale));
                  parentReference =
                      createFn(context, (vx_reference)exemplar, count);
                  internalRefs = getRefsFromParent(parentReference, parentType);
                  status = vxGetStatus(parentReference);
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_convolution)internalRefs[childNum])->base.rows ==
                          rows &&
                      ((vx_convolution)internalRefs[childNum])->base.columns ==
                          cols) {
                    refs[refIdx] = internalRefs[childNum];
                    vxSetName(refs[refIdx], cur);
                    ownIncrementReference(refs[refIdx], VX_INTERNAL);
                    ...
                    status = VX_ERROR_INVALID_PARAMETERS;
                    VX_PRINT(VX_ZONE_ERROR,
                             "%s convolution settings doesn't match generated "
                             "&s convolution!\n",
                             objectName, objectName);
                    goto exit_error;
                  }
              ...
              childNum++;
              status |= vxLoadDataForConvolution((vx_convolution)refs[refIdx],
                                                 cur, cols, rows);
              break;
            }
            case REMAP_TAG: {
              ...
              if (childNum ==
                ...
                status = vxReserveReferences(context, count + 1);
                exemplar = vxCreateRemap(context, src_width, src_height,
                                         dst_width, dst_height);
                status |= vxReleaseReferences(context, count + 1);
                status |= vxGetStatus((vx_reference)exemplar);
                if (status == VX_SUCCESS) {
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_remap)internalRefs[childNum])->src_width ==
                          src_width &&
                      ((vx_remap)internalRefs[childNum])->src_height ==
                          src_height &&
                      ((vx_remap)internalRefs[childNum])->dst_width ==
                          dst_width &&
                      ((vx_remap)internalRefs[childNum])->dst_height ==
                          dst_height) {
                    refs[refIdx] = internalRefs[childNum];
                    vxSetName(refs[refIdx], cur);
                    ownIncrementReference(refs[refIdx], VX_INTERNAL);
            ...
            case DISTRIBUTION_TAG: {
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_distribution)internalRefs[childNum])->range_x ==
                      ...
                      ((vx_distribution)internalRefs[childNum])->offset_x ==
                          offset) {
                    refs[refIdx] = internalRefs[childNum];
                    vxSetName(refs[refIdx], cur);
                    ownIncrementReference(refs[refIdx], VX_INTERNAL);
                    ...
                    status = VX_ERROR_INVALID_PARAMETERS;
                    VX_PRINT(VX_ZONE_ERROR,
                             "%s distribution settings doesn't match generated "
                             "%s distribution!\n",
                             objectName, objectName);
                    goto exit_error;
                  }
              ...
              childNum++;
              status |= vxLoadDataForDistribution((vx_distribution)refs[refIdx],
                                                  cur, bins);
              break;
            }
            case THRESHOLD_TAG: {
              ...
              vx_int32 false_value =
                  (vx_int32)xml_prop_ulong(cur, "false_value");
              if (childNum ==
                  0) { /* Create delay object based on first child */
                ...
                XML_FOREACH_CHILD_TAG(cur, tag, tags) {
                  if (tag == BINARY_TAG) {
                    vx_int32 value = (vx_int32)xml_ulong(cur);
                    exemplar = vxCreateThreshold(
                        context, VX_THRESHOLD_TYPE_BINARY, type);
                    status |= vxSetThresholdAttribute(
                        exemplar, VX_THRESHOLD_THRESHOLD_VALUE, &value,
                        sizeof(value));
                  } else if (tag == RANGE_TAG) {
                    vx_int32 upper = (vx_int32)xml_prop_ulong(cur, "upper");
                    vx_int32 lower = (vx_int32)xml_prop_ulong(cur, "lower");
                    exemplar = vxCreateThreshold(context,
                                                 VX_THRESHOLD_TYPE_RANGE, type);
                    status |= vxSetThresholdAttribute(
                        exemplar, VX_THRESHOLD_THRESHOLD_UPPER, &upper,
                        sizeof(upper));
                    status |= vxSetThresholdAttribute(
                  ...
                  status |=
                      vxSetThresholdAttribute(exemplar, VX_THRESHOLD_TRUE_VALUE,
                                              &true_value, sizeof(true_value));
                  status |= vxSetThresholdAttribute(
                      exemplar, VX_THRESHOLD_FALSE_VALUE, &false_value,
                      sizeof(false_value));
                }
                status |= vxReleaseReferences(context, count + 1);
                status |= vxGetStatus((vx_reference)exemplar);
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  ...
                  if (((vx_threshold)internalRefs[childNum])->thresh_type ==
                      thresh_type) {
                    refs[refIdx] = internalRefs[childNum];
                    vxSetName(refs[refIdx], cur);
                    ownIncrementReference(refs[refIdx], VX_INTERNAL);
                    ...
                    status = VX_ERROR_INVALID_PARAMETERS;
                    VX_PRINT(VX_ZONE_ERROR,
                             "%s threshold settings doesn't match generated %s "
                             "threshold!\n",
                             objectName, objectName);
                    goto exit_error;
                  }
              ...
              XML_FOREACH_CHILD_TAG(cur, tag, tags) {
                if (tag == BINARY_TAG) {
                  vx_int32 value = (vx_int32)xml_ulong(cur);
                  status |= vxSetThresholdAttribute(
                      (vx_threshold)refs[refIdx], VX_THRESHOLD_THRESHOLD_VALUE,
                      &value, sizeof(value));
                } else if (tag == RANGE_TAG) {
                  vx_int32 upper = (vx_int32)xml_prop_ulong(cur, "upper");
                  vx_int32 lower = (vx_int32)xml_prop_ulong(cur, "lower");
                  status |= vxSetThresholdAttribute(
                      (vx_threshold)refs[refIdx], VX_THRESHOLD_THRESHOLD_UPPER,
                      &upper, sizeof(upper));
                  status |= vxSetThresholdAttribute(
                      (vx_threshold)refs[refIdx], VX_THRESHOLD_THRESHOLD_LOWER,
                      &lower, sizeof(lower));
                }
                status |= vxSetThresholdAttribute(
                    (vx_threshold)refs[refIdx], VX_THRESHOLD_TRUE_VALUE,
                    &true_value, sizeof(true_value));
                status |= vxSetThresholdAttribute(
                    (vx_threshold)refs[refIdx], VX_THRESHOLD_FALSE_VALUE,
                    &false_value, sizeof(false_value));
              }
              break;
            }
              ...
              if (refIdx < total) {
                if (childNum < count) {
                  if (((vx_scalar)internalRefs[childNum])->data_type == type) {
                    ...
                    status = VX_ERROR_INVALID_PARAMETERS;
                    VX_PRINT(VX_ZONE_ERROR,
                             "%s scalar settings doesn't match generated %s "
                             "scalar!\n",
                             objectName, objectName);
                    goto exit_error;
                  }
      ...
      XML_FOREACH_CHILD_TAG(cur, tag, tags) {
        if (tag == NODE_TAG) {
          ...
          XML_FOREACH_CHILD_TAG(cur, tag, tags) {
            if (tag == PARAMETER_TAG) {
              ...
              if (strcmp(replicated, "true") == 0) {
                ...
                xml_prop_string(cur, "replicate_flag", replicateFlag,
                                sizeof(replicateFlag));
              ...
              status = vxSetParameterByIndex((vx_node)refs[nidx], index,
                                             refs[refIdx]);
              if (status != VX_SUCCESS) {
                VX_PRINT(VX_ZONE_ERROR,
                         "Failed to set parameter %u on node[%u] in graph[%u] "
                         "reference=%u\n",
          ...
          if (strcmp(replicated, "true") == 0) {
            ...
            vxQueryNode((vx_node)refs[nidx], VX_NODE_PARAMETERS, &numParams,
                        sizeof(numParams));
            status = vxReplicateNode((vx_graph)refs[gidx], (vx_node)refs[nidx],
                                     replicate_flags, numParams);
            if (status != VX_SUCCESS) {
              VX_PRINT(VX_ZONE_ERROR,
                       "Failed to replicate node[%u] in graph[%u]\n", nidx,
                       gidx);
              goto exit_error;
            }
          }
          ...
          if (tag == PARAMETER_TAG) {
            ...
            if (gpidx == index) {
              ...
              status = vxAddParameterToGraphByIndex((vx_graph)refs[gidx],
                                                    (vx_node)refs[nidx], pidx);
              if (status != VX_SUCCESS) {
                VX_PRINT(VX_ZONE_ERROR,
                         "Failed to set graph[%u].parameter[%u] from "
                         "node[%u].parameter[%u]!\n",
...
exit_error:
  ...
  if (status == VX_SUCCESS && counted != total) {
    /* The XML file indicated that there was a different number of references
     * than were imported */
    VX_PRINT(
        VX_ZONE_ERROR,
        "Reference count mismatch: xml \"references\" tag=%d, imported=%d\n",
        total, counted);
    vxAddLogEntry(
        &context->base, VX_ERROR_NOT_COMPATIBLE,
        "Reference count mismatch: xml \"references\" tag=%d, imported=%d\n",
        total, counted);
