...
static vx_bool vxIsValidThresholdDataType(vx_enum data_type) {
  ...
  if (data_type == VX_TYPE_INT8 || data_type == VX_TYPE_UINT8 ||
      data_type == VX_TYPE_INT16 || data_type == VX_TYPE_UINT16 ||
      data_type == VX_TYPE_INT32 || data_type == VX_TYPE_UINT32) {
    ret = vx_true_e;
  }
  return ret;
}
...
VX_API_ENTRY vx_status VX_API_CALL vxReleaseThreshold(vx_threshold *t) {
  return ownReleaseReferenceInt((vx_reference *)t, VX_TYPE_THRESHOLD,
                                VX_EXTERNAL, NULL);
}

VX_API_ENTRY vx_threshold VX_API_CALL vxCreateThreshold(vx_context context,
                                                        vx_enum thresh_type,
                                                        vx_enum data_type) {
  ...
  if (ownIsValidContext(context) == vx_true_e) {
    if (vxIsValidThresholdDataType(data_type) == vx_true_e) {
      if (vxIsValidThresholdType(thresh_type) == vx_true_e) {
        ...
        threshold =
            (vx_threshold)ownGetErrorObject(context, VX_ERROR_INVALID_TYPE);
      }
    } else {
...
VX_API_ENTRY vx_status VX_API_CALL vxSetThresholdAttribute(
    vx_threshold threshold, vx_enum attribute, const void *ptr, vx_size size) {
  vx_status status = VX_SUCCESS;
  if (ownIsValidSpecificReference(&threshold->base, VX_TYPE_THRESHOLD) ==
      vx_true_e) {
    switch (attribute) {
...
VX_API_ENTRY vx_status VX_API_CALL vxQueryThreshold(vx_threshold threshold,
                                                    vx_enum attribute,
...
static vx_bool vxIsValidThresholdFormat(vx_df_image format) {
  ...
  if (format == VX_DF_IMAGE_U8 || format == VX_DF_IMAGE_S16 ||
      format == VX_DF_IMAGE_U16 || format == VX_DF_IMAGE_S32 ||
...
static vx_bool vxIsValidThresholdFormatEx(vx_df_image format) {
  ...
  if (format == VX_DF_IMAGE_RGB || format == VX_DF_IMAGE_RGBX ||
      format == VX_DF_IMAGE_NV12 || format == VX_DF_IMAGE_NV21 ||
      format == VX_DF_IMAGE_UYVY || format == VX_DF_IMAGE_YUYV ||
...
vxCreateThresholdForImage(vx_context context, vx_enum thresh_type,
  ...
  threshold = (vx_threshold)ownCreateReference(context, VX_TYPE_THRESHOLD,
...
VX_API_ENTRY vx_threshold VX_API_CALL vxCreateVirtualThresholdForImage(
    vx_graph graph, vx_enum thresh_type, vx_df_image input_format,
  ...
  if (ownIsValidSpecificReference(gref, VX_TYPE_GRAPH) == vx_true_e) {
    threshold = vxCreateThresholdForImage(gref->context, thresh_type,
    ...
    if (vxGetStatus((vx_reference)threshold) == VX_SUCCESS &&
      ...
      threshold = (vx_threshold)ownGetErrorObject(graph->base.context,
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyThresholdOutput(
    vx_threshold threshold, vx_pixel_value_t *true_value_ptr,
    vx_pixel_value_t *false_value_ptr, vx_enum usage, vx_enum user_mem_type) {
...
VX_API_ENTRY vx_status VX_API_CALL vxCopyThresholdRange(
    vx_threshold threshold, vx_pixel_value_t *lower_value_ptr,
    vx_pixel_value_t *upper_value_ptr, vx_enum usage, vx_enum user_mem_type) {
  vx_status status = VX_ERROR_INVALID_REFERENCE;
...
vxCopyThresholdValue(vx_threshold threshold, vx_pixel_value_t *value_ptr,
