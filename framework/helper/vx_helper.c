...
// no clue if this belongs here, but i don't know where better to put it. needed
// by c_scale.c and vx_scale.c
...
vx_status vxGetLogEntry(vx_reference r, char message[VX_MAX_LOG_MESSAGE_LEN]) {
  ...
  for (cur = helper_log.first; cur != helper_log.last;
       cur = (cur + 1) % helper_log.count) {
    // if reference match or context was given
    if (((isContext == vx_true_e) ||
         (r == helper_log.entries[cur].reference)) &&
...
static void VX_CALLBACK vxHelperLogCallback(vx_context context,
                                            vx_reference ref, vx_status status,
  ...
  strncpy(helper_log.entries[helper_log.last].message, string,
          VX_MAX_LOG_MESSAGE_LEN - 1);
...
vx_bool vxFindOverlapRectangle(vx_rectangle_t *rect_a, vx_rectangle_t *rect_b,
                               vx_rectangle_t *rect_res) {
  vx_bool res = vx_false_e;

  if ((rect_a) && (rect_b) && (rect_res)) {
    enum { sx = 0, sy = 1, ex = 2, ey = 3 };
    vx_uint32 a[4] = {rect_a->start_x, rect_a->start_y, rect_a->end_x,
                      rect_a->end_y};
    vx_uint32 b[4] = {rect_b->start_x, rect_b->start_y, rect_b->end_x,
                      rect_b->end_y};
    vx_uint32 c[4] = {0};
...
/* Note: with the current sample implementation structure, we have no other
choice than returning 0 in case of errors not due to vxCreateGenericNode,
because vxGetErrorObject is internal to another library and is not exported.
This is not an issue since vxGetStatus correctly manages a ref == 0 */
vx_node vxCreateNodeByStructure(vx_graph graph, vx_enum kernelenum,
                                vx_reference params[], vx_uint32 num) {
  vx_status status = VX_SUCCESS;
  vx_node node = 0;
    ...
    if (vxGetStatus((vx_reference)node) == VX_SUCCESS) {
      ...
      for (p = 0; p < num; p++) {
        ...
        if (status != VX_SUCCESS) {
          vxAddLogEntry((vx_reference)graph, status,
                        "Kernel %d Parameter %u is invalid.\n", kernelenum, p);
          vxReleaseNode(&node);
          node = 0;
          break;
        }
      ...
      vxAddLogEntry((vx_reference)graph, VX_ERROR_INVALID_PARAMETERS,
                    "Failed to create node with kernel enum %d\n", kernelenum);
      status = VX_ERROR_NO_MEMORY;
    }
    vxReleaseKernel(&kernel);
  } else {
    vxAddLogEntry((vx_reference)graph, VX_ERROR_INVALID_PARAMETERS,
                  "failed to retrieve kernel enum %d\n", kernelenum);
...
vx_status vxLinkParametersByIndex(vx_node node_a, vx_uint32 index_a,
                                  vx_node node_b, vx_uint32 index_b) {
...
vx_status vxLinkParametersByReference(vx_parameter a, vx_parameter b) {
  ...
  status =
      vxQueryParameter(a, VX_PARAMETER_DIRECTION, &dirs[0], sizeof(dirs[0]));
  if (status != VX_SUCCESS) return status;

  status =
      vxQueryParameter(b, VX_PARAMETER_DIRECTION, &dirs[1], sizeof(dirs[1]));
  if (status != VX_SUCCESS) return status;
...
vx_status vxSetAffineRotationMatrix(vx_matrix matrix, vx_float32 angle,
                                    vx_float32 scale, vx_float32 center_x,
                                    vx_float32 center_y) {
  ...
  if ((columns == 2) && (rows == 3) && (type == VX_TYPE_FLOAT32)) {
    ...
    vxAddLogEntry(
        (vx_reference)matrix, status,
        "Failed to set affine matrix due to type or dimension mismatch!\n");
  }
  return status;
}

vx_status vxAlterRectangle(vx_rectangle_t *rect, vx_int32 dsx, vx_int32 dsy,
                           vx_int32 dex, vx_int32 dey) {
  if (rect) {
    rect->start_x += dsx;
    rect->start_y += dsy;
    rect->end_x += dex;
...
void vxReadRectangle(const void *base, const vx_imagepatch_addressing_t *addr,
                     const vx_border_t *borders, vx_df_image type,
                     vx_uint32 center_x, vx_uint32 center_y, vx_uint32 radius_x,
                     vx_uint32 radius_y, void *destination) {
#if 0
      ...
      for (kx = -(int32_t)radius_x; kx <= (int32_t)radius_x;
        ...
        switch (type) {
          ...
          case VX_DF_IMAGE_U32:
            ...
            break;
          case VX_DF_IMAGE_F32:
            ((vx_float32 *)destination)[dest_index] =
                *(vx_float32 *)(ptr + y * stride_y + x * stride_x);
            break;
          default:
            abort();
        }
...
static vx_float32 vxh_matrix_trace_f32(vx_size columns, vx_size rows,
                                       vx_float32 matrix[rows][columns]) {
...
static vx_int32 vxh_matrix_trace_i32(vx_size columns, vx_size rows,
                                     vx_int32 matrix[rows][columns]) {
  vx_int32 trace = 0;
  vx_size i = 0ul;
  for (i = 0ul; i < columns && i < rows; i++) {
    trace += matrix[i][i];
...
vx_status vxMatrixTrace(vx_matrix matrix, vx_scalar trace) {
  ...
  if (status != VX_SUCCESS) return VX_ERROR_INVALID_REFERENCE;
  if (mtype == VX_TYPE_INVALID || mtype != stype) return VX_ERROR_INVALID_TYPE;
  if (columns == 0 || columns != rows) return VX_ERROR_INVALID_DIMENSION;
...
static vx_float32 vxh_matrix_determinant_f32(vx_size columns, vx_size rows,
                                             vx_float32 matrix[rows][columns]) {
  vx_float32 det = 0.0f;
  if (rows == 2 && columns == 2) {
...
static void vxh_matrix_inverse_f32(vx_uint32 columns, vx_uint32 rows,
                                   vx_float32 out[rows][columns],
                                   vx_float32 in[rows][columns]) {
  if (rows == 2 && columns == 2) {
...
vx_status vxMatrixInverse(vx_matrix input, vx_matrix output) {
  ...
  if (status != VX_SUCCESS) return status;
  if (ci != co || ri != ro || ci != ri) return VX_ERROR_INVALID_DIMENSION;
  if (ti != to) return VX_ERROR_INVALID_TYPE;
