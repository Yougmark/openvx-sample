...
#endif
VX_API_ENTRY vx_enum VX_API_CALL vxGetUserStructByName(vx_context context,
                                                       const vx_char *name);
...
have the same endianness. This implementation does not support exports
originating from previous versions!
...
static vx_status graph_kernel_validate(vx_node node,
                                       const vx_reference parameters[],
                                       vx_uint32 num, vx_meta_format metas[]) {
  vx_status status = VX_SUCCESS;
  /* Detect recursion using the kernel local data size TODO - is this safe
   * during pipelining? */
  ...
  if (node->kernel->attributes.localDataSize) {
    ...
    if (VX_SUCCESS == status) status = vxVerifyGraph(graph);
...
static vx_status graph_kernel_init(vx_node node, const vx_reference *parameters,
                                   vx_uint32 num) {
  (void)node;
  (void)parameters;
  (void)num;
...
static vx_status graph_kernel_deinit(vx_node node,
                                     const vx_reference *parameters,
                                     vx_uint32 num) {
  (void)node;
  (void)parameters;
...
static vx_status graph_kernel_function(vx_node node,
                                       const vx_reference *parameters,
                                       vx_uint32 num) {
  ...
  /* Execute the graph */
  if (VX_SUCCESS == status) status = vxProcessGraph(graph);
  return status;
}
...
static vx_kernel createGraphKernel(vx_context context, vx_graph graph) {
  ...
  */
  ...
  vx_kernel kernel =
      vxAddUserKernel(context, graph->base.name, kernel_enum,
                      (vx_kernel_f)graph_kernel_function, graph->numParams,
                      (vx_kernel_validate_f)graph_kernel_validate,
                      (vx_kernel_initialize_f)graph_kernel_init,
                      (vx_kernel_deinitialize_f)graph_kernel_deinit);
  ...
  if (VX_SUCCESS == vxGetStatus((vx_reference)kernel)) {
    kernel->base.scope =
        (vx_reference)graph; /* The crucial (and sneaky!) bit. */
    for (p = 0; p < graph->numParams && VX_SUCCESS == status; ++p) {
      ...
      vxQueryParameter(parameter, VX_PARAMETER_DIRECTION, &direction,
                       sizeof(direction));
...
#define VX_IMPORT_TYPE_BINARY                                           \
  (1) /* Can't be returned to application with IX extension, but if XML \
...
#define getOffsets(ptr)                    \
  ((const vx_uint64 *)(ptr + USES_OFFSET + \
                       sizeof(vx_uint32) * getHeader(ptr)->numrefs))
#define checkIndex(ptr, index) (index < getHeader(ptr)->actual_numrefs)
#define checkSize(ptr, curptr, length) \
  (curptr && (curptr + length <= getEnd(ptr)))

static vx_enum getObjectType(const vx_uint8 *ptr, vx_uint32 index) {
  return checkIndex(ptr, index) ? *((const vx_enum *)(((const vx_uint8 *)ptr) +
                                                      getOffsets(ptr)[index]))
                                : VX_TYPE_INVALID;
}

static const vx_char *getObjectName(const vx_uint8 *ptr, vx_uint32 index) {
  return checkIndex(ptr, index)
             ? ((const vx_char *)(((const vx_uint8 *)ptr) +
                                  getOffsets(ptr)[index] + 2 * sizeof(vx_enum)))
             : "";
}

static vx_status importObject(vx_context context, const vx_uint8 *ptr,
                              vx_reference *ref_table, vx_uint32 n);

static void importBytes(VXBinObjectHeader *hdr, void *dest,
                        const vx_size length) {
  if (hdr->curptr) {
...
static vx_status importObjectHeader(const vx_uint8 *ptr,
                                    VXBinObjectHeader *object_header,
                                    vx_uint32 index) {
  /* import the object header for the given index, and return a pointer to the
   * next byte */
...
static vx_int32 getFirstIndex(const vx_uint8 *ptr, const vx_uint32 index) {
  /* check to see if there is a duplicate entry in the table at a lower index.
  Note we are guaranteed no duplicates after the first numrefs entries */
  vx_uint32 i;
  vx_uint64 offset = getOffsets(ptr)[index];
  for (i = 0; i < index; ++i)
    if (getOffsets(ptr)[i] == offset) break;
  return i;
}

static vx_status importImage(vx_context context, const vx_uint8 *ptr,
                             vx_reference *ref_table, vx_uint32 n) {
  /* Export format:
  ...
  union {
      ...
      struct {    //Regular image (but may be part of a pyramid, delay or object
  array, and it may be virtual) width, height, format, data flag valid region
  rectangle and pixel data for each plane (if data flag non-zero)
      }
  }
  ...
  */
  ...
  switch (sub_type) {
    case IMAGE_SUB_TYPE_ROI:
      ...
      if (checkIndex(ptr, header.scope)) {
        ...
        if (VX_SUCCESS == status) {
          ...
          if (NULL == ref_table[n]) {
            ref_table[n] = (vx_reference)vxCreateImageFromROI(
                (vx_image)ref_table[header.scope], &rect);
    ...
    case IMAGE_SUB_TYPE_CHANNEL:
      ...
      if (checkIndex(ptr, header.scope)) {
        ...
        if (VX_SUCCESS == status) {
          ...
          if (NULL == ref_table[n]) {
            ref_table[n] = (vx_reference)vxCreateImageFromChannel(
                (vx_image)ref_table[header.scope], channel);
            status = vxGetStatus(ref_table[n]);
          } else /* check meta-data */
          {
            vx_image image = (vx_image)ref_table[n];
    ...
    case IMAGE_SUB_TYPE_UNIFORM: {
      ...
      if (NULL == ref_table[n]) {
        ref_table[n] = (vx_reference)vxCreateUniformImage(
    ...
    case IMAGE_SUB_TYPE_HANDLE: /* bugzilla 16312 */
    {
      ...
      if (NULL == ref_table[n]) {
        ...
        ref_table[n] = (vx_reference)vxCreateImageFromHandle(
            context, format, addrs, ptrs, memory_type);
        status = vxGetStatus(ref_table[n]);
      }
    } break;
    case IMAGE_SUB_TYPE_NORMAL: {
      ...
      if (NULL == ref_table[n]) {
        if (header.is_virtual) {
          if (checkIndex(ptr, header.scope)) {
            ref_table[n] = (vx_reference)vxCreateVirtualImage(
                (vx_graph)ref_table[header.scope], width, height, format);
          }
        } else {
          ref_table[n] =
              (vx_reference)vxCreateImage(context, width, height, format);
        }
        status = vxGetStatus(ref_table[n]);
      } else /* check the meta-data */
      {
      ...
      if (data && VX_SUCCESS == status && !header.is_virtual) {
        ...
        DEBUGPRINTF(
            "Importing image data for rect (start_x=%u, end_x=%u, start_y=%u, "
            "end_y=%u)\n",
          ...
          for (y = 0U; y < addr.dim_y; y += addr.step_y) {
            for (x = 0U; x < addr.dim_x; x += addr.step_x) {
              ...
              vx_pixel_value_t *pix =
                  (vx_pixel_value_t *)vxFormatImagePatchAddress2d(base, x, y,
                                                                  &addr);
              switch (format) {
                ...
                case VX_DF_IMAGE_RGBX: /* single-plane 4 bytes */
                  ...
                  break;
                case VX_DF_IMAGE_UYVY: /* single plane, import 2 bytes for UY or
                                          VY */
                case VX_DF_IMAGE_YUYV: /* single plane, import 2 bytes for YU or
                                          YV */
                  importBytes(&header, pix->YUV, 2);
                  break;
                case VX_DF_IMAGE_NV12: /* import 1 byte from 1st plane and 2
                                          from second */
                case VX_DF_IMAGE_NV21: /* second plane order is different for
                                          the two formats */
                  if (0 == p) {
...
static vx_status importLUT(vx_context context, const vx_uint8 *ptr,
                           vx_reference *ref_table, vx_uint32 n) {
  /*
  LUT format:
  vx_uint32 num_items
  ...
  */
  ...
  if (NULL == ref_table[n]) { /* create object */
  ...
  } else /* Just check meta-data */
  {
    ...
    if (VX_TYPE_LUT != ref_table[n]->type || lut->num_items != num_items ||
        lut->offset != offset || lut->item_type != item_type) {
      status = VX_ERROR_INVALID_PARAMETERS;
    }
  }
  if (VX_SUCCESS == status && size) { /* read data */
    vx_lut_t *lut = (vx_lut_t *)ref_table[n];
...
static vx_status importDistribution(vx_context context, const vx_uint8 *ptr,
                                    vx_reference *ref_table, vx_uint32 n) {
  /*
  Distribution format:
  vx_uint32 bins
  ...
  */
  ...
  if (NULL == ref_table[n]) { /* create object */
    ref_table[n] =
        (vx_reference)vxCreateDistribution(context, bins, offset_x, range_x);
    if (header.is_virtual) {
      if (checkIndex(ptr, header.scope)) {
        ref_table[n]->scope = (vx_reference)ref_table[header.scope];
        ref_table[n]->is_virtual = vx_true_e;
  ...
  } else /* Just check meta-data */
  {
    ...
    if (VX_TYPE_DISTRIBUTION != ref_table[n]->type ||
        dist->memory.dims[0][VX_DIM_X] != bins || dist->offset_x != offset_x ||
...
static vx_status importThreshold(vx_context context, const vx_uint8 *ptr,
                                 vx_reference *ref_table, vx_uint32 n) {
  /*
  Threshold format:
  vx_enum thresh_type
  ...
  */
  ...
  status = VX_ERROR_NOT_SUPPORTED;
  if (NULL == ref_table[n]) {
    vx_threshold threshold = (vx_threshold)ownCreateReference(
        context, VX_TYPE_THRESHOLD, VX_EXTERNAL, &context->base);
    ...
    if (VX_TYPE_THRESHOLD != ref_table[n]->type ||
        ...
        (VX_THRESHOLD_TYPE_RANGE == thresh_type &&
         (threshold->lower.S32 != lower || threshold->upper.S32 != upper)) ||
        threshold->true_value.S32 != true_value ||
        threshold->false_value.S32 != false_value) {
      status = VX_ERROR_INVALID_PARAMETERS;
    }
...
static vx_status importMatrix(vx_context context, const vx_uint8 *ptr,
                              vx_reference *ref_table, vx_uint32 n) {
  /*
  Matrix format:
  vx_uint32 columns
  ...
  */
  ...
  if (NULL == ref_table[n]) { /* create object */
    ref_table[n] =
        (vx_reference)vxCreateMatrix(context, data_type, columns, rows);
    if (header.is_virtual) {
      if (checkIndex(ptr, header.scope)) {
        ref_table[n]->scope = (vx_reference)ref_table[header.scope];
        ref_table[n]->is_virtual = vx_true_e;
  ...
  } else /* Just check meta-data */
  {
    ...
    if (VX_TYPE_MATRIX != ref_table[n]->type || matrix->columns != columns ||
        matrix->rows != rows || matrix->data_type != data_type ||
        matrix->pattern != pattern || matrix->origin.x != origin.x ||
...
static vx_status importConvolution(vx_context context, const vx_uint8 *ptr,
                                   vx_reference *ref_table, vx_uint32 n) {
  /*
  Convolution format:
  vx_uint32 columns
  ...
  */
  ...
  } else /* Just check meta-data */
  {
    ...
    if (VX_TYPE_CONVOLUTION != ref_table[n]->type ||
        ...
        convolution->base.rows != rows || convolution->scale != scale) {
      status = VX_ERROR_INVALID_PARAMETERS;
    }
  }
  if (VX_SUCCESS == status && size) { /* read data */
...
static vx_status importScalar(vx_context context, const vx_uint8 *ptr,
                              vx_reference *ref_table, vx_uint32 n) {
  /*
  Format:
      vx_enum data_type
  ...
  */
  ...
  if (VX_TYPE_USER_STRUCT_START <=
          data_type && /* Now import the name if it's a user struct (Bugzilla
                          16338)*/
    ...
    if (VX_TYPE_INVALID == data_type ||
        item_size !=
            context->user_structs[data_type - VX_TYPE_USER_STRUCT_START].size)
  ...
  if (VX_SUCCESS == status) {
    ...
    if (NULL == ref_table[n]) { /* create object */
      ref_table[n] =
          (vx_reference)vxCreateScalar(context, data_type, header.curptr);
      if (header.is_virtual) {
        if (checkIndex(ptr, header.scope)) {
          ref_table[n]->scope = (vx_reference)ref_table[header.scope];
          ref_table[n]->is_virtual = vx_true_e;
  ...
  if (VX_SUCCESS == status && size) {
    ...
    if (data) {
      ...
      status = vxCopyScalar((vx_scalar)ref_table[n], data, VX_WRITE_ONLY,
                            VX_MEMORY_TYPE_HOST);
      free(data);
    } else {
      status = VX_ERROR_INVALID_PARAMETERS;
...
static vx_status importArray(vx_context context, const vx_uint8 *ptr,
                             vx_reference *ref_table, vx_uint32 n) {
  /*
  Array format:
      vx_uint32 num_items
      ...
      vx_enum item_type
      vx_uint32 item_size
      if (VX_USER_STRUCT_START <= item_type <= VX_USER_STRUCT_END)    # Not for
  version < 1.2! vx_char struct_name[VX_MAX_STRUCT_NAME] vx_uint32 size # Size
  of data exported vx_uint8 data[size]
  */
  vx_status status = VX_SUCCESS;
  VXBinObjectHeader header;
  ...
  importVxUint32(&header, &item_size);
  if (VX_TYPE_USER_STRUCT_START <=
          item_type && /* Now import the name if it's a user struct (Bugzilla
                          16338)*/
    ...
    if (VX_TYPE_INVALID == item_type ||
        item_size !=
            context->user_structs[item_type - VX_TYPE_USER_STRUCT_START].size)
      status = VX_ERROR_INVALID_TYPE;
  }
  if (VX_SUCCESS == status) {
    importVxUint32(&header, &size);
    if (NULL == ref_table[n]) { /* create object */
      if (header.is_virtual) {
        if (checkIndex(ptr, header.scope)) {
          ref_table[n] = (vx_reference)vxCreateVirtualArray(
              (vx_graph)ref_table[header.scope], item_type, capacity);
        }
      } else
        ref_table[n] =
            (vx_reference)vxCreateArray(context, item_type, capacity);
      status = vxGetStatus(ref_table[n]);
    } else /* Just check meta-data */
    {
      vx_array array = (vx_array)ref_table[n];
      if (VX_TYPE_ARRAY != ref_table[n]->type || array->capacity != capacity ||
          array->item_size != item_size || array->item_type != item_type) {
        status = VX_ERROR_INVALID_PARAMETERS;
      }
    }
  }
...
static vx_status importRemap(vx_context context, const vx_uint8 *ptr,
                             vx_reference *ref_table, vx_uint32 n) {
  /*
  Remap format:
  vx_uint32 src_width
  ...
  */
  ...
  importVxUint32(&header, &size);
  if (NULL == ref_table[n]) { /* create object */
    ref_table[n] = (vx_reference)vxCreateRemap(context, src_width, src_height,
  ...
  } else /* Just check meta-data */
  {
    ...
    if (VX_TYPE_REMAP != ref_table[n]->type || remap->src_width != src_width ||
        remap->src_height != src_height || remap->dst_width != dst_width ||
...
static vx_status importObjectArray(vx_context context, const vx_uint8 *ptr,
                                   vx_reference *ref_table, vx_uint32 n,
                                   vx_enum required_type) {
  /* Object array format:
  vx_uint32 num_items
  vx_enum parent_type
  if VX_TYPE_TENSOR == parent_type
      vx_uint32 tensor reference, stride, rect.start_x, rect.start_y,
  rect.end_x. rect.end_y vx_df_image format vx_uint32 indices[num_items]
  */
  vx_status status = VX_SUCCESS;
  VXBinObjectHeader header;
  vx_uint32 num_items;
  ...
  if (NULL == ref_table[n]) {
    if (VX_TYPE_TENSOR == parent_type) {
      ...
      if (VX_SUCCESS == status) {
        ref_table[n] = (vx_reference)vxCreateImageObjectArrayFromTensor(
            (vx_tensor)ref_table[tensor_ix], &rect, num_items, stride, format);
        status = vxGetStatus(ref_table[n]);
      }
    } else if (VX_TYPE_INVALID == parent_type) {
      vx_uint32 exemplar_ix;
      const vx_uint8 *ixcurptr = header.curptr;
      importVxUint32(&header, &exemplar_ix); /* First item in array as exemplar,
                                                don't increment curptr */
      header.curptr = ixcurptr;
      if (checkIndex(ptr, exemplar_ix) && NULL == ref_table[exemplar_ix]) {
        status = importObject(context, ptr, ref_table,
                              exemplar_ix); /* import exemplar */
        if (VX_SUCCESS == status) {
          if (header.is_virtual) {
            if (checkIndex(ptr, header.scope)) {
              ref_table[n] = (vx_reference)vxCreateVirtualObjectArray(
                  (vx_graph)ref_table[header.scope], ref_table[exemplar_ix],
                  num_items);
            }
          } else
            ref_table[n] = (vx_reference)vxCreateObjectArray(
                context, ref_table[exemplar_ix], num_items);
          status = vxGetStatus(ref_table[n]);
          vxReleaseReference(
              &ref_table[exemplar_ix]); /* Release the exemplar */
        }
        ref_table[exemplar_ix] =
            NULL; /* make entry zero as we will set it to new value */
      } else {
        status = VX_ERROR_INVALID_PARAMETERS; /* References in an object array
                                                 can't be supplied by user */
      }
    } else {
      status = VX_ERROR_INVALID_PARAMETERS; /* parent type must be INVALID or
                                               TENSOR */
  ...
  for (c = 0; c < num_items && VX_SUCCESS == status; ++c) {
    /* import each object - note that as they have already been created, only a
    meta-data check and data import will be done. We do not increment the
    reference for the objects in the delay - this
    will be done at the end of the main function if the objects are on the
    import list */
    vx_uint32 object_ix;
    ...
    if (checkIndex(ptr, object_ix)) {
      ...
      if (object_ix > getHeader(ptr)->numrefs)
        vxRetainReference(ref_table[object_ix]); /* need to fix this up becuase
                                                    of a release later */
...
static vx_status importPyramid(vx_context context, const vx_uint8 *ptr,
                               vx_reference *ref_table, vx_uint32 n) {
  /* Pyramid format:
  vx_uint32 width
  vx_uint32 height
  ...
  */
  ...
  if (NULL == ref_table[n]) {
    if (header.is_virtual) {
      if (checkIndex(ptr, header.scope)) {
        ref_table[n] = (vx_reference)vxCreateVirtualPyramid(
            (vx_graph)ref_table[header.scope], num_levels, scale, width, height,
            format);
      }
    } else
      ref_table[n] = (vx_reference)vxCreatePyramid(context, num_levels, scale,
                                                   width, height, format);
    status = vxGetStatus(ref_table[n]);
  } else /* Check meta-data */
  {
    vx_pyramid pyramid = (vx_pyramid)ref_table[n];
    if (VX_TYPE_PYRAMID != ref_table[n]->type || pyramid->width != width ||
        pyramid->height != height || pyramid->format != format ||
        pyramid->numLevels != num_levels || pyramid->scale != scale) {
      status = VX_ERROR_INVALID_PARAMETERS;
    }
  }
  for (c = 0; c < num_levels && VX_SUCCESS == status; ++c) {
    /* Import each level - notice that importImage will import only the data
       since the image has been created. We do not increment the reference count
       for the images; they will be incremented if and only if they are on the
       export list, at the end of the main function. */
    vx_uint32 level_ix;
    importVxUint32(&header, &level_ix);
    if (checkIndex(ptr, level_ix)) {
      ref_table[level_ix] =
          (vx_reference)(((vx_pyramid)ref_table[n])->levels[c]);
...
static vx_status importDelay(vx_context context, const vx_uint8 *ptr,
                             vx_reference *ref_table, vx_uint32 n) {
  /* Delay format:
  vx_uint32 num_slots
  ...
  */
  ...
  importVxUint32(&header, &exemplar_ix); /* First item in array as examplar,
                                            don't increment curptr */
  header.curptr = ixcurptr;
  if (NULL == ref_table[n]) {
    if (checkIndex(ptr, exemplar_ix) && NULL == ref_table[exemplar_ix]) {
      ...
      if (VX_SUCCESS == status) {
        if (header.is_virtual)
          ...
          ref_table[n] = (vx_reference)vxCreateDelay(
              context, ref_table[exemplar_ix], num_slots);
          status = vxGetStatus(ref_table[n]);
        }
  ...
  for (c = 0; c < num_slots && VX_SUCCESS == status; ++c) {
    /* import each object - note that as they have already been created, only a
    meta-data check and data import will be done.  We do not increment the
    reference count for the objects in the slots; they will be incremented if
    and only if they are on the export list, at the end of the main function.
    */
    vx_uint32 object_ix;
    importVxUint32(&header, &object_ix);
    if (checkIndex(ptr, object_ix)) {
...
static vx_status importTensor(vx_context context, const vx_uint8 *ptr,
                              vx_reference *ref_table, vx_uint32 n) {
  /*
  ...
  */
  ...
  if (0U == number_of_dimensions ||
    ...
    if (NULL == ref_table[n]) {
      ...
      if (VX_TYPE_TENSOR == getObjectType(ptr, header.scope)) {
        ...
        if (NULL == ref_table[header.scope])
          status = importObject(context, ptr, ref_table, header.scope);
        if (VX_SUCCESS == status) {
          vx_size starts[VX_MAX_TENSOR_DIMENSIONS],
              ends[VX_MAX_TENSOR_DIMENSIONS];
          ...
          ref_table[n] = (vx_reference)vxCreateTensorFromView(
              (vx_tensor)ref_table[header.scope], number_of_dimensions, starts,
              ends);
          status = vxGetStatus(ref_table[n]);
        }
      } else {
        if (header.is_virtual) {
          if (checkIndex(ptr, header.scope)) {
            ref_table[n] = (vx_reference)vxCreateVirtualTensor(
                (vx_graph)ref_table[header.scope], number_of_dimensions,
                dimensions, data_type, fixed_point_position);
          }
        } else {
          ref_table[n] = (vx_reference)vxCreateTensor(
              context, number_of_dimensions, dimensions, data_type,
        ...
        status = vxGetStatus(ref_table[n]);
               ...
               number_of_dimensions !=
                   ((vx_tensor)ref_table[n])->number_of_dimensions ||
               data_type != ((vx_tensor)ref_table[n])->data_type ||
               fixed_point_position !=
                   ((vx_tensor)ref_table[n])->fixed_point_position) {
      status = VX_ERROR_INVALID_PARAMETERS;
    } else { /* Most meta-data correct, check each dimension */
    ...
    if (VX_SUCCESS == status && size && !header.is_virtual) { /* read data */
      vx_size starts[VX_MAX_TENSOR_DIMENSIONS] = {0};
      status = vxCopyTensorPatch(
          (vx_tensor)ref_table[n], number_of_dimensions, starts, dimensions,
...
static vx_status importKernel(vx_context context, const vx_uint8 *ptr,
                              vx_reference *ref_table, vx_uint32 n) {
  /* See bugzilla 16337 */
  /* Format of data:
          ...
          vx_uint32 graph_index               # Only for 'graph kernels' with
  VX_IX_USE_IMPORT_AS_KERNEL (Bugzilla 16207)

  This function is called twice for each kernel, once with required_use ==
  VX_IX_USE_APPLICATION_CREATE and once with VX_IX_USE_IMPORT_AS_KERNEL. This is
  because in the first case the kernels are required before graphs are built,
  and in the second graphs must be built before the kernel can be created.
  */
  vx_status status = VX_SUCCESS;
  VXBinObjectHeader header;
  vx_enum enumeration;
  ...
  if (VX_SUCCESS == status) {
    ...
    if (NULL == ref_table[n]) {
      ...
      if (VX_KERNEL_BASE(VX_ID_USER, 0) ==
          (VX_KERNEL_BASE(VX_ID_USER, 0) & enumeration)) {
...
static vx_status importObject(vx_context context, const vx_uint8 *ptr,
  ...
  if (first < n) {
    ...
    switch (getObjectType(ptr, n)) {
      case VX_TYPE_IMAGE:
        status = importImage(context, ptr, ref_table, n);
        break;
      case VX_TYPE_LUT:
        status = importLUT(context, ptr, ref_table, n);
        break;
      case VX_TYPE_DISTRIBUTION:
        status = importDistribution(context, ptr, ref_table, n);
        break;
      case VX_TYPE_THRESHOLD:
        status = importThreshold(context, ptr, ref_table, n);
        break;
      case VX_TYPE_MATRIX:
        status = importMatrix(context, ptr, ref_table, n);
        break;
      case VX_TYPE_CONVOLUTION:
        status = importConvolution(context, ptr, ref_table, n);
        break;
      case VX_TYPE_SCALAR:
        status = importScalar(context, ptr, ref_table, n);
        break;
      case VX_TYPE_ARRAY:
        status = importArray(context, ptr, ref_table, n);
        break;
      case VX_TYPE_REMAP:
        status = importRemap(context, ptr, ref_table, n);
        break;
      case VX_TYPE_OBJECT_ARRAY:
        status = importObjectArray(context, ptr, ref_table, n, VX_TYPE_INVALID);
        break;
      case VX_TYPE_PYRAMID:
        status = importPyramid(context, ptr, ref_table, n);
        break;
      case VX_TYPE_DELAY:
        status = importDelay(context, ptr, ref_table, n);
        break;
      case VX_TYPE_TENSOR:
        status = importTensor(context, ptr, ref_table, n);
        break;
      case VX_TYPE_KERNEL:
        status = importKernel(context, ptr, ref_table, n);
        break;
      default: /* something we don't know how to import - graphs have been
                  created already */
        status = VX_ERROR_NOT_SUPPORTED;
    }
...
static vx_status createGraphObjects(vx_context context, const vx_uint8 *ptr,
                                    vx_reference *ref_table) {
  vx_status status = VX_SUCCESS;
  vx_uint32 i;
...
static vx_status importObjectsOfType(vx_context context, const vx_uint8 *ptr,
                                     vx_reference *ref_table, vx_enum vx_type) {
  vx_status status = VX_SUCCESS;
  vx_uint32 i;
  for (i = 0; i < getHeader(ptr)->actual_numrefs && VX_SUCCESS == status; ++i) {
...
static vx_status importObjectArraysFromTensor(vx_context context,
                                              const vx_uint8 *ptr,
                                              vx_reference *ref_table) {
  vx_status status = VX_SUCCESS;
  vx_uint32 i;
...
static vx_status buildGraphs(vx_context context, const vx_uint8 *ptr,
                             vx_reference *ref_table) {
  ...
  for (i = 0; i < getHeader(ptr)->actual_numrefs && VX_SUCCESS == status; ++i) {
    /* Format of data:
        ...
        vx_uint32 num_nodes
        struct
            vx_uint32 kernel_index          # index into offsets table for the
       kernel vx_uint32 params[num_kernel_params]    # num_kernel_params from
       the kernel entry # index into offsets table for each parameter; #
       0xFFFFFFFF is used for absent optional parameters vx_uint8 is_replicated
       flag case is_replicated of 0:                              # no data

            default:                        # non-zero or true value
                vx_uint8 replicated flags[num_params]
            end_case
            vx_uint32 affinity              # note sample implementation has
       illegal zero affinity vx_border_t border mode end_struct nodes[num_nodes]
    ...
    */
    if (VX_TYPE_GRAPH == getObjectType(ptr, i)) {
      ...
      if (first == i) {
        ...
        for (c = 0; c < num_nodes && VX_SUCCESS == status; ++c) {
          ...
          if (checkIndex(ptr, kernel_ix)) {
            ...
            if (VX_SUCCESS == status) {
              ...
              status = vxSetNodeAttribute(node, VX_NODE_BORDER, &border,
                                          sizeof(vx_border_t));
              if (status) {
        ...
        if (NULL == header.curptr) status = VX_FAILURE;
      } else {
  ...
  /* Now verify all the graphs and create any graph kernels
     In this sample implementation verification is necessary, but for other
     implementations we hope that in fact we have an immutable graph, and here
     we just call validation and initialisation functions of any user nodes.
   */
  for (i = 0; i < getHeader(ptr)->actual_numrefs && VX_SUCCESS == status; ++i) {
    if (VX_TYPE_GRAPH == getObjectType(ptr, i)) {
      vx_uint32 first = getFirstIndex(ptr, i);
...
VX_API_ENTRY vx_import VX_API_CALL vxImportObjectsFromMemory(
    vx_context context, vx_size numrefs, vx_reference *refs,
  ...
  for (curptr = ptr; curptr < ptr + header->length - sizeof(checksum);
       ++curptr) {
    checksum += *curptr;
  }
  if ((VX_IX_VERSION == header->version &&
       checksum !=
           *(vx_uint32 *)curptr) || /* only check checksum if version match */
      VX_SUCCESS != vxGetStatus((vx_reference)context) ||
      NULL == refs || NULL == uses || NULL == ptr ||
      length <= sizeof(VXBinHeader) || VX_IX_ID != header->vx_ix_id ||
      VX_IX_VERSION <
          header->version || /* Should always be backwards compatible */
      length != header->length ||
      numrefs != header->numrefs || header->actual_numrefs < header->numrefs ||
      length <= sizeof(VXBinHeader) + numrefs * sizeof(vx_enum) +
                    header->actual_numrefs * sizeof(vx_uint64)) {
    status = VX_ERROR_INVALID_PARAMETERS;
    DEBUGPRINTF("Initial checks failed\n");
  } else /* Check that uses match and that refs is suitably populated with valid
            references where required */
  {
    vx_uint32 i;
    const vx_enum *header_uses = getUses(ptr);
    for (i = 0; i < numrefs && NULL == import; ++i) {
      if (uses[i] != header_uses[i] ||
          (VX_IX_USE_APPLICATION_CREATE == uses[i] &&
           VX_SUCCESS != vxGetStatus(refs[i]))) {
  ...
  if (VX_SUCCESS == status) {
    vx_uint32 i;
    vx_reference *ref_table =
        (vx_reference *)calloc(sizeof(vx_reference), header->actual_numrefs);
    if (ref_table) {
      for (i = 0; i < numrefs; ++i) {
      ...
      status = createGraphObjects(context, ptr,
                                  ref_table); /* Create graph objects */
    } else
      status = VX_ERROR_NO_MEMORY;
    /* Import all data objects.
       The order is important, since we need to create parents first; these will
       in turn create their children.
    */
    if (VX_SUCCESS == status)
      status = importObjectsOfType(context, ptr, ref_table, VX_TYPE_DELAY);
    if (VX_SUCCESS == status)
      status =
          importObjectsOfType(context, ptr, ref_table, VX_TYPE_OBJECT_ARRAY);
    if (VX_SUCCESS == status)
    ...
    if (VX_SUCCESS == status)
      status =
          buildGraphs(context, ptr, ref_table); /* Build and verify graphs */
    if (VX_SUCCESS == status) {
      /* Now we need to clean up kernels that are private to this import. These
      are graph kernels not listed, with use VX_IX_USE_EXPORT_VALES. We simply
      set their names to zero length, which ensures that they can't be
    ...
    if (VX_SUCCESS == status) {
      ...
      if (import && VX_TYPE_IMPORT == import->base.type && import->refs) {
        ...
        for (i = numrefs; i < header->actual_numrefs; ++i)
          if (ref_table[i]->external_count) vxReleaseReference(&ref_table[i]);
      }
    }
    free(ref_table);
  }
