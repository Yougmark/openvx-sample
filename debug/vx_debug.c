...
struct vx_string_and_enum_e enumnames[] = {
    ...
    _STR2(VX_ZONE_INFO),  _STR2(VX_ZONE_PERF),      _STR2(VX_ZONE_CONTEXT),
    _STR2(VX_ZONE_OSAL),  _STR2(VX_ZONE_REFERENCE), _STR2(VX_ZONE_ARRAY),
    _STR2(VX_ZONE_IMAGE), _STR2(VX_ZONE_SCALAR),    _STR2(VX_ZONE_KERNEL),
    _STR2(VX_ZONE_GRAPH), _STR2(VX_ZONE_NODE),      _STR2(VX_ZONE_PARAMETER),
    _STR2(VX_ZONE_DELAY), _STR2(VX_ZONE_TARGET),    _STR2(VX_ZONE_LOG),
...
void vx_print(vx_enum zone, char *string, ...) {
  if (vx_zone_mask & ZONE_BIT(zone)) {
    char format[1024];
    va_list ap;
    _snprintf(format, sizeof(format), "%20s: %08x: %s", find_zone_name(zone),
...
void ownPrintImageAddressing(const vx_imagepatch_addressing_t *addr) {
  if (addr) {
    VX_PRINT(VX_ZONE_IMAGE,
             "addr:%p dim={%u,%u} stride={%d,%d} scale={%u,%u} step={%u,%u}\n",
             addr, addr->dim_x, addr->dim_y, addr->stride_x, addr->stride_y,
