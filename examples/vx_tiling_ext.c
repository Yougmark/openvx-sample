...
static vx_status VX_CALLBACK vxFilterInputValidator(vx_node node,
                                                    vx_uint32 index) {
...
static vx_status VX_CALLBACK vxFilterOutputValidator(vx_node node,
                                                     vx_uint32 index,
                                                     vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 1) {
    vx_parameter param =
        vxGetParameterByIndex(node, 0); /* we reference an input image */
    if (param) {
      vx_image input = 0;
      vxQueryParameter(param, VX_PARAMETER_REF, &input, sizeof(input));
      if (input) {
        ...
        vxSetMetaFormatAttribute(meta, VX_IMAGE_HEIGHT, &height,
                                 sizeof(height));
        vxSetMetaFormatAttribute(meta, VX_IMAGE_FORMAT, &format,
                                 sizeof(format));
...
static vx_status VX_CALLBACK vxAddInputValidator(vx_node node,
                                                 vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if ((index == 0) || (index == 1)) {
    vx_parameter param = vxGetParameterByIndex(node, index);
...
static vx_status VX_CALLBACK vxAddOutputValidator(vx_node node, vx_uint32 index,
                                                  vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 2) {
    vx_parameter param =
...
static vx_status VX_CALLBACK vxAlphaInputValidator(vx_node node,
                                                   vx_uint32 index) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 0) {
    vx_parameter param = vxGetParameterByIndex(node, index);
...
static vx_status VX_CALLBACK vxAlphaOutputValidator(vx_node node,
                                                    vx_uint32 index,
                                                    vx_meta_format meta) {
  vx_status status = VX_ERROR_INVALID_PARAMETERS;
  if (index == 2) {
...
VX_API_ENTRY vx_status VX_API_CALL vxPublishKernels(vx_context context) {
  ...
  for (k = 0; k < dimof(tiling_kernels); k++) {
    vx_kernel kernel = vxAddTilingKernel(
        context, tiling_kernels[k].name, tiling_kernels[k].enumeration,
        tiling_kernels[k].flexible_function, tiling_kernels[k].fast_function,
        tiling_kernels[k].num_params, tiling_kernels[k].input_validator,
        tiling_kernels[k].output_validator);
    if (kernel) {
      vx_uint32 p = 0;
      for (p = 0; p < tiling_kernels[k].num_params; p++) {
        status |= vxAddParameterToKernel(
            kernel, p, tiling_kernels[k].parameters[p].direction,
