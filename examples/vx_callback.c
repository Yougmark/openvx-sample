...
vx_status vx_example_callback(vx_context context, vx_image input,
                              vx_image output) {
  (void)output;
  ...
  if (status == VX_SUCCESS) {
    ...
    vx_array arrays[] = {vxCreateArray(context, VX_TYPE_COORDINATES2D, 1),
                         vxCreateArray(context, VX_TYPE_COORDINATES2D, 1)};
    vx_node nodes[] = {
        vxMinMaxLocNode(graph, input, scalars[0], scalars[1], arrays[0],
                        arrays[1], scalars[2], scalars[3]),
        /// other nodes
    };
