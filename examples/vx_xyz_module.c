 ...
 */
vx_status VX_CALLBACK XYZValidator(vx_node node,
                                   const vx_reference parameters[],
                                   vx_uint32 num, vx_meta_format metas[]) {
  (void)node;

  if (num != (XYZ_PARAM_TEMP + 1)) return VX_ERROR_INVALID_PARAMETERS;

  for (vx_uint32 i = 0u; i < num; i++) {
    if (!parameters[i]) return VX_ERROR_INVALID_REFERENCE;

    switch (i) {
      case XYZ_PARAM_INPUT: {
        vx_df_image df_image = 0;

        if (vxQueryImage((vx_image)parameters[i], VX_IMAGE_FORMAT, &df_image,
                         sizeof(df_image)) != VX_SUCCESS)
          return VX_ERROR_INVALID_PARAMETERS;

        if (df_image != VX_DF_IMAGE_U8) return VX_ERROR_INVALID_VALUE;
      ...
      case XYZ_PARAM_VALUE: {
        ...
        if (vxQueryScalar((vx_scalar)parameters[i], VX_SCALAR_TYPE, &type,
                          sizeof(type)) != VX_SUCCESS)
          return VX_ERROR_INVALID_PARAMETERS;

        if (type != VX_TYPE_INT32 ||
            vxCopyScalar((vx_scalar)parameters[i], &value, VX_READ_ONLY,
      ...
      case XYZ_PARAM_OUTPUT: {
        ...
        if (!metas[i]) return VX_ERROR_INVALID_REFERENCE;

        if (vxQueryImage(input, VX_IMAGE_FORMAT, &format, sizeof(format)) !=
                VX_SUCCESS ||
        ...
        if (vxSetMetaFormatAttribute(metas[i], VX_IMAGE_WIDTH, &width,
                                     sizeof(width)) != VX_SUCCESS ||
            vxSetMetaFormatAttribute(metas[i], VX_IMAGE_HEIGHT, &height,
                                     sizeof(height)) != VX_SUCCESS ||
            vxSetMetaFormatAttribute(metas[i], VX_IMAGE_FORMAT, &format,
                                     sizeof(format)))
          return VX_ERROR_INVALID_VALUE;
      ...
      case XYZ_PARAM_TEMP: {
        ...
        if (vxQueryArray((vx_array)parameters[i], VX_ARRAY_NUMITEMS, &num_items,
                         sizeof(num_items)) != VX_SUCCESS)
          return VX_ERROR_INVALID_PARAMETERS;

        if (num_items < XYZ_TEMP_NUMITEMS) return VX_ERROR_INVALID_DIMENSION;
 ...
 * \ingroup group_example_kernel
 */
vx_status VX_CALLBACK XYZKernel(vx_node node, const vx_reference *parameters,
                                vx_uint32 num) {
  (void)node;
  ...
  if (num == 4) {
    ...
    status |= vxMapImagePatch(input, &rect, 0, &map_id_input, &addr1, &in,
                              VX_READ_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
    status |= vxMapImagePatch(output, &rect, 0, &map_id_output, &addr2, &out,
                              VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST, VX_NOGAP_X);
    status |=
        vxQueryArray(temp, VX_ARRAY_ITEMTYPE, &item_type, sizeof(item_type));
    status |=
        vxQueryArray(temp, VX_ARRAY_NUMITEMS, &num_items, sizeof(num_items));
    status |=
        vxQueryArray(temp, VX_ARRAY_CAPACITY, &capacity, sizeof(capacity));
    status |= vxMapArrayRange(temp, 0, num_items, &map_id_array, &stride, &buf,
 ...
 * \ingroup group_example_kernel
 */
vx_status VX_CALLBACK XYZInitialize(vx_node node,
                                    const vx_reference *parameters,
                                    vx_uint32 num) {
  (void)node;
  (void)parameters;
 ...
 * \ingroup group_example_kernel
 */
vx_status VX_CALLBACK XYZDeinitialize(vx_node node,
                                      const vx_reference *parameters,
                                      vx_uint32 num) {
  (void)node;
  (void)parameters;
 ...
 * point, "vxPublishKernels". \ingroup group_example_kernel
 */
/*VX_API_ENTRY*/ vx_status VX_API_CALL vxPublishKernels(vx_context context) {
  vx_status status = VX_SUCCESS;
  vx_kernel kernel = vxAddUserKernel(
      context, "org.khronos.example.xyz", VX_KERNEL_KHR_XYZ, XYZKernel, 4,
      XYZValidator, XYZInitialize, XYZDeinitialize);
  if (kernel) {
    ...
    status = vxAddParameterToKernel(kernel, 2, VX_OUTPUT, VX_TYPE_IMAGE,
                                    VX_PARAMETER_STATE_REQUIRED);
    if (status != VX_SUCCESS) goto exit;
    status = vxAddParameterToKernel(kernel, 3, VX_OUTPUT, VX_TYPE_ARRAY,
                                    VX_PARAMETER_STATE_REQUIRED);
    if (status != VX_SUCCESS) goto exit;
    status = vxSetKernelAttribute(kernel, VX_KERNEL_LOCAL_DATA_SIZE, &size,
                                  sizeof(size));
    if (status != VX_SUCCESS) goto exit;
 ...
 * \return A \ref vx_status_e enumeration. Returns errors if some or all kernels
 * were not added correctly. \note This follows the function pointer definition
 * of a \ref vx_unpublish_kernels_f and uses the predefined name for the entry
 * point, "vxUnpublishKernels". \ingroup group_example_kernel
 */
