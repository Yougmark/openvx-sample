...
/*!
 * \file vx_super_res.c
 * \brief The example graph used to several images into a super resolution
 * output. \author Tomer Schwartz <tomer@broadcom.com>
 */
...
void userCalculatePerspectiveTransformFromLK(vx_matrix matrix_forward,
  ...
  vxMapArrayRange(old_features, 0, olen, &map_id_old, &old_features_stride,
                  (void **)&old_features_ptr, VX_READ_ONLY, VX_MEMORY_TYPE_HOST,
                  0);
  vxMapArrayRange(new_features, 0, nlen, &map_id_new, &new_features_stride,
                  (void **)&new_features_ptr, VX_READ_ONLY, VX_MEMORY_TYPE_HOST,
                  0);

  /*! \internal do least square algorithm that find perspective transform */
  /*! \see "Computer Vision Algorithm and Application by Richard Szeliski
   * section 6.1.3 */
  for (ind = 0; ind < nlen; ind++) {
    // vx_int32 matrix_index = 2 * ind;

    // vx_keypoint_t old_kp = vxArrayItem(vx_keypoint_t, old_features_ptr, ind,
    // old_features_stride); vx_keypoint_t new_kp = vxArrayItem(vx_keypoint_t,
    // new_features_ptr, ind, new_features_stride);
  ...
  if (vxGetStatus((vx_reference)context) == VX_SUCCESS) {
    vx_image images[] = {
        ...
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8),  // index 1: Get Y channel
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_U8),  // index 2: scale up to high res.
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_U8),  // index 3: back wrap: transform to the
                                        // original Image.
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_U8),  // index 4: guassian blur
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8),  // index 5: scale down
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_S16),  // index 6: Subtract the transformed
                                         // Image with original moved Image
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_S16),  // index 7: Scale Up the delta image.
        vxCreateImage(
            context, width * 2, height * 2,
            VX_DF_IMAGE_S16),  // index 8: Guassian blur the delta Image
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_S16),  // index 9: forward wrap: tranform the
                                         // deltas back to the high res Image
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_U8),  // index 10: accumulate sum?
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8),  // index 11: Get U channel
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_U8),  // index 12: scale up to high res.
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_U8),  // index 13: Get V channel
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_U8),  // index 14: scale up to high res.
        vxCreateImage(context, width, height,
                      VX_DF_IMAGE_UYVY),  // index 15: output image
        vxCreateImage(context, width * 2, height * 2,
                      VX_DF_IMAGE_U8),  // index 16: original y image scaled
        vxCreateImage(
            context, width * 2, height * 2,
            VX_DF_IMAGE_U8),  // index 17: difference image for last calculation
    };
    vx_pyramid pyramid_new =
        vxCreatePyramid(context, 4, 2, width, height, VX_DF_IMAGE_U8);
    ...
    if (vxGetStatus((vx_reference)graphs[0]) == VX_SUCCESS) {
      vxChannelExtractNode(
          graphs[0], images[0], VX_CHANNEL_Y,
          images[1]);  // One iteration of super resolution calculation
      vxScaleImageNode(graphs[0], images[1], images[2],
      ...
      vxScaleImageNode(graphs[0], images[4], images[5],
                       VX_INTERPOLATION_BILINEAR);
      vxSubtractNode(graphs[0], images[5], images[16],
                     VX_CONVERT_POLICY_SATURATE, images[6]);
      vxScaleImageNode(graphs[0], images[6], images[7],
                       VX_INTERPOLATION_BILINEAR);
      vxGaussian3x3Node(graphs[0], images[7], images[8]);
      vxWarpPerspectiveNode(graphs[0], images[8], matrix_backwords, 0,
                            images[9]);
      vxAccumulateWeightedImageNode(graphs[0], images[9], alpha_s, images[10]);
    }
    if (vxGetStatus((vx_reference)graphs[1]) == VX_SUCCESS) {
      vxChannelExtractNode(
          graphs[1], images[0], VX_CHANNEL_Y,
          images[1]);  // One iteration of super resolution calculation
      vxGaussianPyramidNode(graphs[1], images[1], pyramid_new);

      vxOpticalFlowPyrLKNode(graphs[1], pyramid_old, pyramid_new, old_features,
                             old_features, new_features, criteria, epsilon_s,
                             num_iterations_s, use_initial_estimate_s, winSize);
    }
    if (vxGetStatus((vx_reference)graphs[2]) == VX_SUCCESS) {
      vxChannelExtractNode(
          graphs[2], images[0], VX_CHANNEL_Y,
          images[1]);  // One iteration of super resolution calculation

      vxHarrisCornersNode(graphs[2], images[1], sens_thresh_s, min_distance_s,
                          sensitivity_s, gradient_size, block_size,
                          old_features, num_corners);
      vxGaussianPyramidNode(graphs[2], images[1], pyramid_old);
      vxScaleImageNode(graphs[2], images[1], images[16],
                       VX_INTERPOLATION_BILINEAR);
    }
    if (vxGetStatus((vx_reference)graphs[3]) == VX_SUCCESS) {
      vxSubtractNode(graphs[3], images[10], images[16],
                     VX_CONVERT_POLICY_SATURATE, images[17]);
      ...
      vxScaleImageNode(graphs[3], images[11], images[12],
                       VX_INTERPOLATION_BILINEAR);  // upscale the u channel
      vxChannelExtractNode(graphs[3], images[0], VX_CHANNEL_V, images[13]);
      vxScaleImageNode(graphs[3], images[13], images[14],
                       VX_INTERPOLATION_BILINEAR);  // upscale the v channel
      vxChannelCombineNode(graphs[3], images[10], images[12], images[14], 0,
                           images[15]);  // recombine the channels
    }
    ...
    if (status == VX_SUCCESS) {
      ...
      status |= vxuFReadImage(
          context, "c:\\work\\super_res\\superres_1_UYVY.yuv", images[0]);
      ...
      for (image_index = 1; image_index < max_num_images; image_index++) {
        ...
        sprintf(filename, "c:\\work\\super_res\\superres_%d_UYVY.yuv",
                image_index + 1);
        status |= vxuFReadImage(context, filename, images[0]);
        status |= vxProcessGraph(graphs[1]);
        userCalculatePerspectiveTransformFromLK(
            matrix_forward, matrix_backwords, old_features, new_features);
