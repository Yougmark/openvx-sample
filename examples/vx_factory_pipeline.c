...
/*!
 * \brief This will be called after the \ref vxMinMaxLocNode completes and
 * determines the maximum value in an image. \ingroup group_example
 */
vx_action VX_CALLBACK example_maximacallback(vx_node node) {
  ...
  status |=
      vxCopyScalar(maxima, &max_intensity, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
  ...
  if (vxGetStatus((vx_reference)graph) == VX_SUCCESS) {
    ...
    vx_kernel kernels[] = {
        ...
        vxGetKernelByEnum(context,
                          VX_KERNEL_KHR_XYZ),  // some extended user feature.
    };
    ...
    vxSetThresholdAttribute(thresh, VX_THRESHOLD_THRESHOLD_VALUE, &lo_threshold,
                            sizeof(lo_threshold));
