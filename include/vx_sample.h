 ...
 * list is generated which contains the data objects which are singularly
 * referenced as VX_INPUT or VX_OUTPUT or VX_BIDIRECTION (there can be only 1
 * referent in the total graph here too). The parameter list order is first all
 * inputs, then bi-directional, then outputs, by order of which nodes where
 * declared in the child graph. \ingroup group_int_graph
 */
