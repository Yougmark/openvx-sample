 ...
 * \brief The helper is an non-standardized set of convenience constructs for
 * OpenVX. \details These functions use only the OpenVX API in order to
 * implement their functionality. As such structures, objects, defines, typedefs
 * and functions defined herein are not part of the OpenVX standard, and are
...
/*! \brief Contains everything needed to abstractly describe a parameter to a
 * kernel. This is used to declare kernel parameters at compile time. \ingroup
 * group_helper
 */
...
typedef struct _vx_log_entry_t {
  ...
  vx_enum active;
  /*! \brief The message given to the log from OpenVX. This may be an empty
   * string. */
 ...
 * message value.
 * This must point to at least <tt>\ref VX_MAX_LOG_MESSAGE_LEN</tt> bytes of
 * characters. \return Returns the status of the log entry from <tt>\ref
 * vx_status_e</tt>. \ingroup group_helper \note The API returns errors oldest
 * to newest order. When VX_SUCCESS is returned, the log reading is complete.
 */
vx_status vxGetLogEntry(vx_reference ref, char message[VX_MAX_LOG_MESSAGE_LEN]);

/*! \brief This enables the helper library logging feature to take over the
 * error log callback and keep a database of previous log entries. \ingroup
 * group_helper
 */
...
/*! \brief A method to clear out the log for a particular reference, such as a
 * graph. \param [in] ref The reference to remove from the log. \ingroup
 * group_helper
 */
...
/*! \brief This helper is used to easily set the affine matrix to a rotation and
 * scale. \param [in] matrix The handle to the matrix. \param [in] angle The
 * rotation angle in degrees. \param [in] scale The scaling value. Values less
 * than one are enlarging. \param [in] center_x The center pixel in the x
 * direction. \param [in] center_y The center pixel in the y direction. \return
 * Returns a \ref vx_status_e enumeration. \ingroup group_helper
 */
