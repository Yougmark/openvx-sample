...
#define VX_MAX_TENSOR_DIMENSIONS 6
#define Q78_FIXED_POINT_POSITION 8
/*! \def VX_INT_API Used to deliniate APIs which are not intended to be
 * exported. \ingroup group_int_defines
 */
#if defined(__GNUC__)
...
#define VX_TYPE_IS_STRUCT(type) \
  ((type) >= VX_TYPE_RECTANGLE && (type) < VX_TYPE_VENDOR_STRUCT_END)
...
#define VX_TYPE_IS_OBJECT(type) \
  ((type) >= VX_TYPE_REFERENCE && (type) < VX_TYPE_VENDOR_OBJECT_END)
...
typedef struct _vx_event_t {
  vx_bool autoreset;   /*!< Indicates whether the event will auto-reset after
                          signalling */
 ...
 * \retval false_e Indicates that the worker failed to process data or had some
 * other error. \ingroup group_threadpools
 */
...
typedef struct _vx_reference {
  ...
  /*! \brief The number of times the object has been written to (in some
   * portion) */
  vx_uint32 write_count;
  /*! \brief The reference lock which is used to protect access to "in-fly"
   * data. */
  vx_sem_t lock;
  ...
  /* \brief This indicates the original delay slot index when the object belongs
   * to a delay */
  vx_int32 delay_slot_index;
  /*! \brief This indicates that if the object is virtual whether it is
   * accessible at the moment or not */
...
/*! \brief The internal representation of the attributes associated with a
 * run-time parameter. \ingroup group_int_kernel
 */
typedef struct _vx_signature_t {
  ...
  /*! \brief The number of items in both \ref vx_signature_t::directions and
   * \ref vx_signature_t::types. */
...
typedef struct _vx_parameter {
  /*! \brief The internal reference object. */
  vx_reference_t base;
  /*! \brief Index at which this parameter is tracked in both the node
   * references and kernel signatures */
  vx_uint32 index;
  /*! \brief Pointer to the node which this parameter is associated with */
  vx_node node;
  /*! \brief Pointer to the kernel which this parameter is associated with, if
   * retreived from \ref vxGetKernelParameterByIndex.
   */
  vx_kernel kernel;
} vx_parameter_t;
...
typedef struct _vx_kernel {
  ...
  /*! \brief Target Index, back reference for the later nodes to inherit
   * affinity */
  vx_uint32 affinity;
#ifdef OPENVX_KHR_TILING
 ...
 * \param [out] pIndex The pointer to the index of the kernel in the target's
 * list if the kernel is supported (the function will return VX_SUCCESS). \note
 * The target interface function must be exported as "vxTargetSupports" \return
 * A <tt>\ref vx_status_e</tt> enumeration \retval VX_SUCCESS The kernel is
 * supported and the pIndex has been set. \retval VX_ERROR_NOT_SUPPORTED the
 * kernel is not supported as stated. \ingroup group_int_target
 */
typedef vx_status (*vx_target_supports_f)(
    vx_target target, vx_char targetName[VX_MAX_TARGET_NAME],
    vx_char kernelName[VX_MAX_TARGET_NAME], vx_uint32 *pIndex);
 ...
 * \ingroup group_int_target
 */
typedef vx_action (*vx_target_process_f)(vx_target target, vx_node nodes[],
                                         vx_size startIndex, vx_size numNodes);
...
typedef vx_kernel (*vx_target_addkernel_f)(
    vx_target target, const vx_char name[VX_MAX_KERNEL_NAME],
    vx_enum enumeration, vx_kernel_f func_ptr, vx_uint32 num_parameters,
    vx_kernel_validate_f validate, vx_kernel_input_validate_f input,
    vx_kernel_output_validate_f output, vx_kernel_initialize_f initialize,
    vx_kernel_deinitialize_f deinitialize);
...
typedef vx_kernel (*vx_target_addtilingkernel_f)(
    ...
    vx_tiling_kernel_f fast_func_ptr, vx_uint32 num_parameters,
...
typedef struct _vx_target {
  ...
  /*! \brief Used to determine precidence when more than once core supports a
   * kernel */
  vx_uint32 priority;
...
typedef struct _vx_external_t {
  ...
  vx_enum usage;
  /*! \brief The allocated state of the pointer, if true, the framework can free
   * the memory. */
  vx_bool allocated;
...
typedef struct _vx_context {
  ...
  /*! \brief The reference table which contains the handle for later garage
  ...
  vx_bool log_enabled;
  /*! \brief If true the log callback is reentrant and doesn't need to be
   * locked. */
...
/*! \brief A data structure used to track the various costs which could being
 * optimized.
 *
 */
...
typedef struct _vx_node {
  ...
  /*! \brief Enable to change read-only attributes (VX_NODE_LOCAL_DATA_SIZE,
   * VX_NODE_LOCAL_DATA_PTR) for user-kernel initialization callback needs. */
  vx_bool local_data_change_is_enabled;
  /*! \brief Indicates that attributes (VX_NODE_LOCAL_DATA_SIZE,
   * VX_NODE_LOCAL_DATA_PTR) were set by implementation, but not user-kernel
...
typedef struct _vx_graph {
  ...
  vx_bool verified;
  /*! \brief This indicates that the graph has been verified earlier, but
   * invalidated latter and is need for verification again. */
  vx_bool reverify;
...
typedef struct _vx_memory_t {
   ...
   * VX_WRITE_ONLY or VX_READ_AND_WRITE flag parts. Only single writers are
   * permitted.
   */
...
typedef struct _vx_distribution {
  ...
  /*! \brief Memory layout */
  vx_memory_t memory;
  /*! \brief The total number of the values in the active X dimension of the
   * distribution. */
  vx_uint32 range_x;
  /*! \brief The total number of the values in the active Y dimension of the
   * distribution. */
  vx_uint32 range_y;
...
typedef struct _vx_meta_format {
  ...
  vx_enum type;        /*!< \brief The <tt>\ref vx_type_e</tt> or <tt>\ref
                          vx_df_image_e</tt> code */
  ...
  struct dim {
    ...
    struct image {
      ...
      vx_rectangle_t
          delta; /*!< \brief The delta rectangle applied to this image */
    } image;
    ...
    struct pyramid {
      ...
      vx_df_image format; /*!< \brief The <tt>\ref vx_df_image_e</tt> format of
                             the image. */
    ...
    struct distribution {
      ...
      vx_int32 offset; /*!< \brief Indicates the start of the values to use
                          (inclusive)*/
      vx_uint32 range; /*!< \brief Indicates the total number of the consecutive
    ...
    struct tensor {
      ...
      vx_size dimensions[VX_MAX_TENSOR_DIMENSIONS]; /*! \brief Size of all
                                                       dimensions */
...
#if !DISABLE_ICD_COMPATIBILITY
VX_API_ENTRY vx_context VX_API_CALL
vxCreateContextFromPlatform(struct _vx_platform *platform);
#endif
