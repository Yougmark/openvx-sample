...
void raw2header(char *basename, FILE *fin, uint32_t bpc, uint32_t channels,
      ...
      for (y = 0; y < height; y++) {
        ...
        for (x = 0; x < width; x++) {
          ...
          for (c = 0; c < channels; c++) {
            // uint32_t offset = (stride_p * p) + (stride_y * y) + (stride_x *
            // x) + (c);
            uint64_t value = 0;
            size_t elems;
...
int main(int argc, char *argv[]) {
  if (argc > 1) {
    ...
    if (strncmp("pgm", extension, 3) == 0) {
      ...
      if (max == UINT8_MAX) bpc = 1;
      if (max == UINT16_MAX) bpc = 2;
      if (max == UINT32_MAX) bpc = 4;
      printf("shortname=%s\n", shortname);
      ...
      char bits[4] = "1";
      uint32_t bpc = 1;
      sscanf(basename, "%[a-zA-Z0-9]_%ux%u_%[A-Z0-9]_%[0-9-]b", shortname,
             &width, &height, code, bits);
      bpc = abs(atoi(bits));
      if (bpc != 1) bpc /= 8;
      printf("width=%u\n", width);
      printf("height=%u\n", height);
